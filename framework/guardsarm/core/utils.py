...
import guardsarm.core.results as results
        ...
        if match := re.match(regex, pid_file):
            try:
                ...
                if daemon.replace('-', '_') in command:
                    ...
                    print(f"{daemon}: Process {pid} does not belong to {daemon}, removing from {common.GUARDSARM_PATH}/var/run...")

            except (OSError, psutil.NoSuchProcess):
                print(f'{daemon}: Non existent process {pid}, removing from {common.GUARDSARM_PATH}/var/run...')
            finally:
...
def process_array(array: list, search_text: str = None, complementary_search: bool = False,
    ...
    """Process a GuardSarm framework data array.
...
def cut_array(array: list, offset: int = 0, limit: int = common.DATABASE_LIMIT) -> list:
    ...
    if limit is not None:
        if limit > common.MAXIMUM_DATABASE_LIMIT:
            raise GuardSarmError(1405, extra_message=str(limit))
        elif limit == 0:
            raise GuardSarmError(1406)
    ...
    if offset < 0:
        raise GuardSarmError(1400)
    elif limit < 1:
        raise GuardSarmError(1401)
    else:
...
def sort_array(array: list, sort_by: list = None, sort_ascending: bool = True,
    ...
    def check_sort_fields(allowed_sort_fields, sort_by):
        ...
        if not sort_by.issubset(allowed_sort_fields):
            ...
            raise GuardSarmError(1403, extra_remediation='Allowed sort fields: {0}. '
    ...
    if sort_by:  # array should be a dictionary or a Class
        ...
        if type(array) is set or (type(array[0]) is not dict and 'class \'guardsarm' not in str(type(array[0]))):
...
def select_array(array: list, select: list = None, required_fields: set = None,
    ...
    if allowed_select_fields and not select_no_nested.issubset(allowed_select_fields):
        raise GuardSarmError(1724, "{}".format(', '.join(select_no_nested)))
    ...
    for item in array:
        ...
        if array and not allowed_select_fields and not selected_fields:
            raise GuardSarmError(1724, "{}".format(', '.join(select)))
...
def _get_hashing_algorithm(hash_algorithm):
    ...
    if hash_algorithm not in algorithm_list:
        raise GuardSarmError(1723, "Available algorithms are {0}.".format(', '.join(algorithm_list)))
...
def xml_to_dict(root, section_path: list):
    ...
    ----------
    root : Element
        Root element containing one or more guardsarm_config nodes.
    section_path : list
...
def check_agents_allow_higher_versions(new_conf: Element, original_conf: Element):
    ...
    AUTH_HIERARCHY = ['guardsarm_config', 'auth', 'allow_higher_versions']
    REMOTE_HIERARCHY = ['guardsarm_config', 'remote', 'agents', 'allow_higher_versions']
    ...
    if not upload_configuration['agents']['allow_higher_versions']['allow']:
        ...
        if normalize(new_auth) != normalize(original_auth):
            raise GuardSarmError(1129, extra_message='auth > allow_higher_versions')
        ...
        if normalize(new_remote) != normalize(original_remote):
            raise GuardSarmError(1129, extra_message='remote > allow_higher_versions')
...
def check_indexer(new_conf, original_conf):
    ...
    CONFIG_INDEXER_HIERARCHY = ['guardsarm_config', 'indexer']
    upload_configuration = configuration.api_conf['upload_configuration']
    ...
    if not upload_configuration['indexer']['allow']:
        ...
        if normalize(new_indexer) != normalize(original_indexer):
            raise GuardSarmError(1127, extra_message='indexer')


def load_guardsarm_xml(xml_path, data=None):
    if not data:
        with open(xml_path) as f:
            try:
                data = f.read()
            except Exception as e:
                raise GuardSarmError(1113, extra_message=str(e))
...
def get_timeframe_in_seconds(timeframe: str) -> int:
    ...
    if not timeframe.isdigit():
        if 'h' not in timeframe and 'd' not in timeframe and 'm' not in timeframe and 's' not in timeframe:
            raise GuardSarmError(1411, timeframe)
        ...
        for or_clause in or_clauses:
            ...
            for and_clause in and_clauses:
                ...
                try:
                    ...
                    raise GuardSarmError(1407, extra_message=f"Parameter 'q' is not valid: '{and_clause}'")
...
class GuardSarmDBBackend(AbstractDatabaseBackend):
    """
    This class describes a guardsarm db backend that executes database queries.
    """
    ...
    def __init__(self, agent_id=None, query_format='agent', request_slice=500):
        if query_format == 'agent' and not path.exists(path.join(common.WDB_PATH, f"{agent_id}.db")):
            raise GuardSarmError(2007, extra_message=f"There is no database for agent {agent_id}. "
    ...
    def connect_to_db(self):
        return GuardSarmDBConnection(request_slice=self.request_slice)
    ...
    def execute(self, query, request, count=False):
        """Execute SQL query through GuardSarmDB socket."""
        query = self._substitute_params(query, request)
        return self.conn.execute(query=self._render_query(query), count=count)


class GuardSarmDBQuery(object):
    """This class describes a database query for guardsarm."""
    ...
    def __init__(self, offset: int, limit: int, table: str, sort: dict, search: dict, select: list, query: str,
        ...
        """GuardSarm DB Query constructor.
        ...
        # For example, the following regex -> (name!=guardsarm;id>5),group=webserver <- would return 3 different matches:
        #   (name != guardsarm ;
        #    id   > 5      ),
        #    group=webserver
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        isinstance(self.backend, GuardSarmDBBackend) and self.backend.close_connection()
    ...
    def _add_limit_to_query(self):
        if self.limit:
            if self.limit > common.MAXIMUM_DATABASE_LIMIT:
                raise GuardSarmError(1405, extra_message=str(self.limit))
        ...
        if self.sort:
            if self.sort['fields']:
                ...
                if not set(sort_fields).issubset(allowed_sort_fields):
                    raise GuardSarmError(1403, "Allowed sort fields: {}. Fields: {}".format(
    ...
    def _parse_select_filter(self, select_fields):
        ...
        if select_fields:
            ...
            if not select_fields_set or not select_fields_set.issubset(allowed_select_fields):
                raise GuardSarmError(1724, "Allowed select fields: {0}. Fields: {1}". \
    ...
    def _parse_query(self):
        ...
        if not self.query_regex.match(self.q):
            raise GuardSarmError(1407, self.q)
        ...
        for open_level, field, operator, value, close_level, separator in self.query_regex.findall(self.q):
            if field not in allowed_query_fields:
                raise GuardSarmError(1408, "Available fields: {}. Field: {}".format(
                    ', '.join(sorted(allowed_query_fields)), field)
                )
            if operator not in self.query_operators:
                raise GuardSarmError(1409,
    ...
    def _filter_date(self, date_filter, filter_db_name):
        ...
        if date_filter['value'].isdigit() or re.match(r'\d+[dhms]', date_filter['value']):
            ...
            raise GuardSarmError(1412, date_filter['value'])
    ...
    def oversized_run(self) -> dict:
        ...
        final_ids = list()
        resources = list()
        if self.__class__.__name__ == 'GuardSarmDBQueryAgents':
            resource = 'id'
        elif self.__class__.__name__ == 'GuardSarmDBQueryGroups':
            resource = 'name'
        else:
            raise GuardSarmInternalError(1123)
...
class GuardSarmDBQueryGroupBy(GuardSarmDBQuery):
    ...
    def _add_select_to_query(self):
        GuardSarmDBQuery._add_select_to_query(self)
...
def validate_guardsarm_xml(content: str):
    """Validate GuardSarm XML files (guardsarm-manager.conf)
    ...
    try:
        ...
        # Check xml format
        incoming_xml = load_guardsarm_xml(xml_path='', data=final_xml)
        current_xml = load_guardsarm_xml(xml_path=common.OSSEC_CONF)
...
def to_relative_path(full_path: str, prefix: str = common.GUARDSARM_PATH) -> str:
    """Return a relative path from the GuardSarm base directory.
    ...
    full_path : str
        Absolute path.
    prefix : str, opt
        Prefix to strip from the absolute path. Default `common.GUARDSARM_PATH`
...
def get_localtime() -> tzinfo:
    ...
    -------
    timezone : tzinfo
        The localtime timezone. GuardSarm custom localtime has precedence over the
        system one. If none is found, UTC is returned.   """
    if (tz := gettz(common.GUARDSARM_LOCALTIME_PATH)) is not None:
        return tz
