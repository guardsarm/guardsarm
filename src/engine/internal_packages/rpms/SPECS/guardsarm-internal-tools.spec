...
# Installation paths
# ------------------------------
%global prefix  /opt/guardsarm-internal-tools
%global venv    %{prefix}/venv
%global wheelsd %{prefix}/wheels

%description
GuardSarm internal CLI utilities.  
...
#!/bin/bash
exec /opt/guardsarm-internal-tools/venv/bin/python -m ${mod} "\$@"
EOF
...
if [ "${1:-0}" -eq 0 ]; then
  ...
  # Safety guard
  case "$PREFIX" in
    /opt/guardsarm-internal-tools|/opt/guardsarm-internal-tools/*) ;;
