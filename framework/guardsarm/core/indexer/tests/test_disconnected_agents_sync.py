...
with patch("guardsarm.core.common.guardsarm_uid"), patch("guardsarm.core.common.guardsarm_gid"):
    ...
    from guardsarm.core.cluster.master import DisconnectedAgentSyncTasks
    from guardsarm.core.exception import (
        IndexerUnavailableError,
        GuardSarmError,
        GuardSarmIndexerError,
    )
    from guardsarm.core.indexer.indexer import create_indexer
...
@patch("guardsarm.core.indexer.disconnected_agents.core_utils.get_utc_now")
...
@pytest.mark.asyncio
@patch(
    "guardsarm.core.indexer.disconnected_agents.GuardSarmDBQueryAgents",
...
async def test_sync_agent_batch_success(task):
    ...
    task.disconnected_agent_group_sync = AsyncMock(
        return_value=AffectedItemsGuardSarmResult()
    )
...
@patch(
    "guardsarm.core.indexer.disconnected_agents.get_ossec_conf",
...
async def test_disconnected_agent_group_sync_missing_params(task):
    """
    Verify that missing required parameters raise a GuardSarmError.
    """
    with pytest.raises(GuardSarmError):
...
@pytest.mark.asyncio
@patch("guardsarm.core.indexer.disconnected_agents.get_agents_info", return_value={})
...
@pytest.mark.asyncio
@patch(
    "guardsarm.core.indexer.disconnected_agents.get_agents_info", return_value={"001": {}}
)
@patch("guardsarm.core.indexer.disconnected_agents.get_indexer_client")
...
def test_init_without_server_or_logger():
    ...
    """
    with patch(
        "guardsarm.core.indexer.disconnected_agents.get_ossec_conf", side_effect=Exception
    ):
...
@patch.object(DisconnectedAgentSyncTasks, "check_indexer", new_callable=AsyncMock)
@patch(
    "guardsarm.core.indexer.disconnected_agents.asyncio.sleep",
...
@pytest.mark.asyncio
@patch("guardsarm.core.indexer.disconnected_agents.get_indexer_client")
@patch("guardsarm.core.indexer.disconnected_agents.get_ossec_conf")
...
async def test_run_cluster_name_sync_full_flow(
    ...
    mock_conf.return_value = {"cluster": {"name": "guardsarm-new"}}
...
async def test_create_indexer_raises_indexer_unavailable_after_retries():
    ...
    with patch("guardsarm.core.indexer.indexer.Indexer") as mock_indexer_cls:
        mock_instance = AsyncMock()
        mock_instance.connect.side_effect = GuardSarmIndexerError(2200)
        mock_instance.close = AsyncMock()
        mock_indexer_cls.return_value = mock_instance

        with patch("guardsarm.core.indexer.indexer.sleep", new_callable=AsyncMock), \
                patch("guardsarm.core.indexer.indexer.random.random", return_value=0):
...
async def test_create_indexer_raises_on_typeerror():
    """A TypeError in Indexer() constructor must be converted to GuardSarmIndexerError."""
    with patch("guardsarm.core.indexer.indexer.Indexer", side_effect=TypeError("bad arg")):
        with pytest.raises(GuardSarmIndexerError, match="Invalid arguments for Indexer"):
    ...
    with patch.object(task, "check_indexer", new_callable=AsyncMock):
        with patch.object(
            ...
            with patch.object(
                ...
                with patch("guardsarm.core.indexer.disconnected_agents.asyncio.sleep",
                           side_effect=[None, asyncio.CancelledError]):
        ...
        with patch(
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    with patch("guardsarm.core.indexer.disconnected_agents.asyncio.sleep", return_value=None):
                        await task.run_cluster_name_sync()
