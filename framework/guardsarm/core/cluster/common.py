...
from guardsarm.core import common, exception, utils
from guardsarm.core.cluster import cluster
from guardsarm.core.cluster import utils as cluster_utils
from guardsarm.core.indexer.indexer import get_indexer_client
from guardsarm.core.wdb import AsyncGuardSarmDBConnection, GuardSarmDBConnection
from guardsarm.core.wdb_http import get_wdb_http_client

IGNORED_WDB_EXCEPTIONS = ['Cannot execute Global database query; FOREIGN KEY constraint failed']

_ALLOWED_PREFIXES = (
    os.path.join(common.GUARDSARM_PATH, "queue/cluster"),
)

ALLOWED_CALLABLES_PACKAGES = ["guardsarm", "api"]
...
class InBuffer:
    ...
    def __init__(self, total=0):
        ...
        if total > MAX_TOTAL_SIZE:
            raise exception.GuardSarmClusterError(3050, extra_message=str(total))
    ...
    def get_info_from_header(self, header: bytes, header_format: str, header_size: int) -> bytes:
        ...
        if self.total > MAX_TOTAL_SIZE :
            raise exception.GuardSarmClusterError(3050, extra_message=f"Header total size out of range: {self.total}")
...
class SendStringTask:
    ...
    def __init__(self, guardsarm_common, logger):
        """Class constructor.

        Parameters
        ----------
        guardsarm_common : GuardSarmCommon object
            Instance of GuardSarmCommon.
        logger : Logger object
            Logger to use during the reception process.
        """
        self.guardsarm_common = guardsarm_common
...
class ReceiveStringTask:
    ...
    def __init__(self, guardsarm_common, logger, task_id, info_type='agent-info'):
        """Class constructor.
    ...
    def done_callback(self, future=None):
        ...
        Remove string and task_id (if exist) from sync_tasks dict. If task was not cancelled, raise stored exception.
        """
        if self.task_id in self.guardsarm_common.in_str:
            # pop() is used instead of 'del' so an exception is never raised here
            self.guardsarm_common.in_str.pop(self.task_id, None)
        if self.task_id in self.guardsarm_common.sync_tasks:
            del self.guardsarm_common.sync_tasks[self.task_id]
        if not self.task.cancelled():
            task_exc = self.task.exception()
            if task_exc:
...
class ReceiveFileTask:
    ...
    def __init__(self, guardsarm_common, logger, task_id: bytes = b''):
        """Class constructor.
...
class Handler(asyncio.Protocol):
    ...
    def __init__(self, fernet_key: str, cluster_items: Dict, logger: logging.Logger = None, tag: str = "Handler"):
        ...
        self.logger = logging.getLogger('guardsarm') if not logger else logger
        # Logging tag.
        self.tag = tag
    ...
    def msg_build(self, command: bytes, counter: int, data: bytes) -> List[bytearray]:
        ...
        if cmd_len > self.cmd_len - len(InBuffer.divide_flag):
            raise exception.GuardSarmClusterError(3024, extra_message=command)
        ...
        while parsed:
            if self.in_msg.received == self.in_msg.total:
                ...
                try:
                    ...
                    raise exception.GuardSarmClusterError(3025)
    ...
    async def send_request(self, command: bytes, data: bytes) -> Union[exception.GuardSarmClusterError, Any]:
        ...
        try:
            ...
            raise exception.GuardSarmClusterError(3026)
        except Exception as e:
            raise exception.GuardSarmClusterError(3018, extra_message=str(e))
        try:
    ...
    async def get_chunks_in_task_id(self, task_id: bytes, error_command: bytes) -> dict:
        ...
        try:
            ...
            raise exception.GuardSarmClusterError(3035, extra_message=f"it should be under task_id {str(e)}, "
                                                                  f"but it's empty.")
        except ValueError as e:
            with contextlib.suppress(Exception):
                await self.send_request(command=error_command,
                                        data=f'error while trying to load JSON: {str(e)}'.encode())
            raise exception.GuardSarmClusterError(3036, extra_message=str(e))
    ...
    async def update_chunks_wdb(self, data: dict, info_type: str, logger: logging.Logger, error_command: bytes,
        ...
        ----------
        data : dict
            Dict containing command and list of chunks to be sent to guardsarm-manager-db.
        info_type : str
            Information type handled.
        logger : Logger object
            Logger to use.
        ...
        try:
            ...
            raise exception.GuardSarmClusterError(3037, extra_message=str(e))
        ...
        for error in result['error_messages']['chunks']:
            ...
            logger.error(f'GuardSarm-db response for chunk {error[0] + 1}/{len(data["chunks"])} was not "ok": {error[1]}')

        logger.debug(f'{result["updated_chunks"]}/{len(data["chunks"])} chunks updated in guardsarm-manager-db '
    ...
    async def send_file(self, filename: str, task_id: bytes = None) -> int:
        ...
        if not os.path.exists(filename):
            raise exception.GuardSarmClusterError(3034, extra_message=filename)

        sent_size = 0
        filename = filename.encode()
        relative_path = filename.replace(common.GUARDSARM_PATH.encode(), b'')

        try:
            # Tell to the destination node where (inside guardsarm_path) the file has to be written.
            await self.send_request(command=b'new_file', data=relative_path)
        except exception.GuardSarmClusterError as e:
            if e.code != 3020:
                raise e
    ...
    async def forward_dapi_response(self, data: bytes):
        ...
        try:
            ...
            if isinstance(e, exception.GuardSarmException):
                exc = json.dumps(e, cls=GuardSarmJSONEncoder)
            else:
                exc = json.dumps(exception.GuardSarmClusterError(1000, extra_message=str(e)), cls=GuardSarmJSONEncoder)
            with contextlib.suppress(Exception):
            ...
            for command, counter, payload, flag_divided in self.get_messages():
                ...
                if flag_divided == InBuffer.divide_flag:
                    ...
                    if len(self.div_msg_box) >= MAX_CONCURRENT_DIVIDED_MSGS and counter not in self.div_msg_box:
                        raise GuardSarmClusterError(3051, extra_message=str(len(self.div_msg_box)))

                    current_size = len(self.div_msg_box.get(counter, b''))
                    if current_size + len(payload) > MAX_TOTAL_SIZE:
                        raise GuardSarmClusterError(3050, extra_message=str(current_size + len(payload)))
                    try:
    ...
    def dispatch(self, command: bytes, counter: int, payload: bytes) -> None:
        ...
        try:
            ...
            command, payload = b'err', json.dumps(e, cls=GuardSarmJSONEncoder).encode()
        except Exception as e:
            self.logger.error(f"Unhandled error processing request '{command}': {e}", exc_info=True)
            command, payload = b'err', json.dumps(exception.GuardSarmInternalError(1000, extra_message=str(e)),
                                                  cls=GuardSarmJSONEncoder).encode()
    ...
    def process_dapi_error(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        if dapi_client.decode() in self.server.local_server.clients:
            ...
            except exception.GuardSarmClusterError:
                raise exception.GuardSarmClusterError(3025)
        else:
            raise exception.GuardSarmClusterError(3032, extra_message=dapi_client.decode())
    ...
    def receive_file(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        dst = os.path.realpath(os.path.join(common.GUARDSARM_PATH, rel.lstrip("/")))
    ...
    def cancel_task(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        ----------
        data : bytes
            String containing task_id and GuardSarmJSONEncoded object with the exception details.
        ...
        error_json = json.loads(error_details, object_hook=as_guardsarm_object)
        if task_id != b'None':
    ...
    def process_error_from_peer(self, data: bytes) -> bytes:
        ...
        """
        try:
            exc = json.loads(data.decode(), object_hook=as_guardsarm_object)
        except json.JSONDecodeError:
            exc = exception.GuardSarmClusterError(3000, extra_message=data.decode())
    ...
    async def wait_for_file(self, file, task_id):
        ...
        try:
            ...
            if isinstance(e, asyncio.TimeoutError):
                exc = exception.GuardSarmClusterError(3039)
            else:
                exc = exception.GuardSarmClusterError(3040, extra_message=str(e))
            # Notify the sending node to stop its task.
            with contextlib.suppress(Exception):
                await self.send_request(command=b"cancel_task",
                                        data=f"{task_id} {json.dumps(exc, cls=GuardSarmJSONEncoder)}".encode())
            raise exc
...
class GuardSarmCommon:
    """
    Task implementing common methods for both clients and servers that are GuardSarm specific.
    """
    ...
    async def recalculate_group_hash(logger) -> None:
        ...
        try:
            ...
            await AsyncGuardSarmDBConnection().run_wdb_command(command='global recalculate-agent-group-hashes')
        except (exception.GuardSarmInternalError, exception.GuardSarmError) as e:
    ...
    def end_receiving_file(self, task_and_file_names: str, logger_tag: str = '') -> Tuple[bytes, bytes]:
        ...
        safe_path = os.path.realpath(os.path.join(common.GUARDSARM_PATH, filename.lstrip("/")))
        ...
        if not any(os.path.commonpath([safe_path, root]) == root for root in _ALLOWED_PREFIXES):
            ...
            raise exception.GuardSarmClusterError(3027, extra_message=task_id)
    ...
    def error_receiving_file(self, task_id_and_error_details: str, logger_tag: str = '') -> Tuple[bytes, bytes]:
        ...
        task_id_and_error_details : str
             GuardSarmJSONEncoded object with the exception details.
        logger_tag : str
            Logger task to use. If empty, it will use main class logger.
        ...
        error_details_json = json.loads(error_details, object_hook=as_guardsarm_object)
    ...
    async def sync(self, files: Iterable, files_metadata: Dict, metadata_len: int, task_pool=None,
        ...
        try:
            ...
            await self.server.send_request(
                ...
                data=task_id + b' ' + os.path.relpath(compressed_data, common.GUARDSARM_PATH).encode()
            )
        except Exception as e:
            self.logger.error(f"Error sending zip file: {e}")
            if isinstance(e, exception.GuardSarmException):
                exc = json.dumps(e, cls=GuardSarmJSONEncoder).encode()
            else:
                exc = json.dumps(exception.GuardSarmClusterError(1000, extra_message=str(e)), cls=GuardSarmJSONEncoder).encode()
            with contextlib.suppress(Exception):
...
class SyncGuardSarmdb(SyncTask):
    """
    Define methods to send information to the master/worker node (guardsarm-manager-db) through send_string protocol.
    """
    ...
    def __init__(self, manager, logger, data_retriever: Callable, cmd: bytes = b'', get_data_command: str = '',
        ...
        get_data_command : str
            Command to retrieve data from local guardsarm-manager-db.
        set_data_command : str
            Command to set data in master/worker's guardsarm-manager-db.
        logger : Logger object
            Logger to use during synchronization process.
    ...
    async def retrieve_information(self):
        ...
        try:
            # Retrieve information from local guardsarm-manager-db
            ...
            self.logger.error(f"Could not obtain data from guardsarm-manager-db: {e}")
            return []
    ...
    async def sync(self, start_time: float, chunks: List):
        ...
        if chunks:
            ...
            if task_id.startswith(b'Error'):
                raise exception.GuardSarmClusterError(3016, extra_message=f'String with agents information could '
...
async def send_data_to_wdb(data, timeout, info_type='agent-info'):
    """Send chunks of data to GuardSarm-db socket.
    ...
    try:
        with utils.Timeout(timeout):
            if info_type == 'agent-info':
                ...
                wdb_conn = GuardSarmDBConnection()
...
class GuardSarmJSONEncoder(json.JSONEncoder):
    """
    Define special JSON encoder for GuardSarm.
    """
    ...
    def default(self, obj):
        if callable(obj):
            ...
            if hasattr(obj, '__self__'):
                if isinstance(obj.__self__, GuardSarm):
                    attributes['__guardsarm__'] = obj.__self__.to_dict()
            attributes['__type__'] = type(obj).__name__
            return result
        elif isinstance(obj, exception.GuardSarmException):
            result = {'__guardsarm_exception__': {'__class__': obj.__class__.__name__,
                                              '__object__': obj.to_dict()}}
            return result
        elif isinstance(obj, wresults.AbstractGuardSarmResult):
            result = {'__guardsarm_result__': {'__class__': obj.__class__.__name__,
                                           '__object__': obj.encode_json()}
                      }
            return result
        elif isinstance(obj, (datetime.datetime, datetime.date)):
            return {'__guardsarm_datetime__': obj.isoformat()}
...
def as_guardsarm_object(dct: Dict):
    try:
        if '__callable__' in dct:
            encoded_callable = dct['__callable__']
            funcname = encoded_callable['__name__']
            if '__guardsarm__' in encoded_callable:
                # Encoded GuardSarm instance method.
                guardsarm = GuardSarm()
                func = getattr(guardsarm, funcname)
                ...
                if not getattr(underlying_func, '__guardsarm_exposed__', False):
                    raise exception.GuardSarmInternalError(
                        1000,
                ...
                # The decorator sets a specific __guardsarm_exposed__ marker
        ...
        elif '__guardsarm_exception__' in dct:
            guardsarm_exception = dct['__guardsarm_exception__']
            return getattr(exception, guardsarm_exception['__class__']).from_dict(guardsarm_exception['__object__'])
        elif '__guardsarm_result__' in dct:
            guardsarm_result = dct['__guardsarm_result__']
            return getattr(wresults, guardsarm_result['__class__']).decode_json(guardsarm_result['__object__'])
        elif '__guardsarm_datetime__' in dct:
            return datetime.datetime.fromisoformat(dct['__guardsarm_datetime__'])
    ...
    except exception.GuardSarmInternalError:
        # Re-raise GuardSarm internal errors (including our security checks)
        raise
    except (KeyError, AttributeError, TypeError, ValueError):
        raise exception.GuardSarmInternalError(1000,
                                           extra_message=f"GuardSarm object cannot be decoded from JSON {dct}",
                                           cmd_error=True)
class IndexerTaskManager:
    """
    Mixin class for managing the lifecycle of indexer-dependent tasks.

    Supervises a set of asyncio tasks that require the GuardSarm Indexer to be
    ...
    logger is found on the instance, a fallback logger named ``'guardsarm'`` is
    ...
    def __init__(self):
        ...
        self.logger = logging.getLogger('guardsarm')
