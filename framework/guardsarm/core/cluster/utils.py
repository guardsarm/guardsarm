...
from guardsarm.core.exception import GuardSarmError, GuardSarmException, GuardSarmInternalError, GuardSarmHAPHelperError
from guardsarm.core.results import GuardSarmResult
from guardsarm.core.utils import temporary_cache
from guardsarm.core.wlogging import GuardSarmLogger
...
api_operation_lockfile = os.path.join(common.GUARDSARM_PATH, "var", "run", ".api_operation_lock")

HELPER_DEFAULTS = {
    HAPROXY_PORT: 5555,
    HAPROXY_PROTOCOL: 'http',
    HAPROXY_BACKEND: 'guardsarm_reporting',
    ...
    for field in [
        ...
        if helper_config.get(field):
            try:
                ...
                raise GuardSarmError(3004, extra_message=f"HAProxy Helper {field} must be an integer.")
    ...
    for field in [IMBALANCE_TOLERANCE]:
        if helper_config.get(field):
            try:
                ...
                raise GuardSarmError(3004, extra_message=f"HAProxy Helper {field} must be a float.")
    return helper_config
...
def parse_haproxy_helper_config(helper_config: dict) -> dict:
    ...
    if helper_config[HAPROXY_PROTOCOL].lower() == 'https' and type(helper_config[HAPROXY_CERT]) == bool:
        raise GuardSarmHAPHelperError(3042, extra_message='HAProxy certificate file required in the haproxy_cert parameter')

    return helper_config


def read_cluster_config(config_file=common.OSSEC_CONF, from_import=False) -> typing.Dict:
    """Read cluster configuration from guardsarm-manager.conf.

    If some fields are missing in the guardsarm-manager.conf cluster configuration, they are replaced
    with default values.
    ...
    try:
        config_cluster = get_ossec_conf(section='cluster', conf_file=config_file, from_import=from_import)['cluster']
            ...
            if e.code == 1106:
                # If no cluster configuration is present in guardsarm configuration file, return the default configuration.
                return cluster_default_configuration

            raise GuardSarmError(3006, extra_message=e.message)
    except Exception as e:
        raise GuardSarmError(3006, extra_message=str(e))
    ...
    if isinstance(config_cluster['port'], str) and not config_cluster['port'].isdigit():
        raise GuardSarmError(3004, extra_message="Cluster port must be an integer.")

    config_cluster['port'] = int(config_cluster['port'])

    if config_cluster['node_type'] not in {'master', 'worker'}:
        raise GuardSarmError(3004, extra_message=f"Invalid node type {config_cluster['node_type']}. Correct values are master and worker")
...
def get_manager_status(cache=False) -> typing.Dict:
    ...
    try:
        ...
        raise GuardSarmInternalError(1913, extra_message=str(e))

    processes = ['guardsarm-manager-analysisd', 'guardsarm-manager-authd', 'guardsarm-manager-monitord',
                 'guardsarm-manager-remoted', 'guardsarm-manager-clusterd',
                 'guardsarm-manager-modulesd', 'guardsarm-manager-db', 'guardsarm-manager-apid']

    data, pidfile_regex, run_dir = {}, re.compile(r'.+\-(\d+)\.pid$'), os.path.join(common.GUARDSARM_PATH, "var", "run")
...
def get_cluster_status() -> typing.Dict:
    ...
    """
    try:
        cluster_status = {"running": "yes" if get_manager_status()['guardsarm-manager-clusterd'] == 'running' else "no"}
...
def _send_control_command(msg: str) -> None:
    """Send a command to the GuardSarm manager control socket.
    ...
    Raises
    ------
    GuardSarmInternalError(1901)
        If the socket path doesn't exist.
    GuardSarmInternalError(1902)
        If there is a socket connection error.
    GuardSarmInternalError(1014)
    ...
    try:
        ...
        if os.path.exists(socket_path):
            try:
                ...
                raise GuardSarmInternalError(1902)
        else:
            raise GuardSarmInternalError(1901)
        ...
        try:
            ...
            if not response.startswith('ok'):
                raise GuardSarmInternalError(1014, extra_message=response)
        except socket.error as e:
            raise GuardSarmInternalError(1014, extra_message=str(e))
    finally:
...
def manager_restart() -> GuardSarmResult:
    """Restart GuardSarm manager.
    ...
    return GuardSarmResult({'message': 'Restart request sent'})


def manager_reload() -> GuardSarmResult:
    """Reload GuardSarm manager.
    ...
    return GuardSarmResult({'message': 'Reload request sent'})
...
def get_cluster_items():
    ...
    try:
        ...
        with open(os.path.join(common.GUARDSARM_PATH, here, 'cluster.json')) as f:
        ...
        raise GuardSarmError(3005, str(e))
...
def safe_join(base_path: str, *paths: str) -> str:
    ...
    if os.path.commonpath([base, final_path]) != base:
        raise GuardSarmInternalError(3003, extra_message=f"unsafe path '{final_path}'")
...
class ClusterLogger(GuardSarmLogger):
    """
    Define the logger used by guardsarm-manager-clusterd.
    """
    ...
    def setup_logger(self):
        ...
        Set ups cluster logger. In addition to super().setup_logger() this method adds:
            ...
            * Sets log level based on the "debug_level" parameter received from guardsarm-manager-clusterd binary.
        """
        super().setup_logger()
...
def process_spawn_sleep(child):
    ...
    """
    pid = os.getpid()
    pyDaemonModule.create_pid(f'guardsarm-manager-clusterd_child_{child}', pid)
...
async def forward_function(func: callable, f_kwargs: dict = None, request_type: str = 'local_master',
    ...
    Returns
    -------
    Return either a dict or `GuardSarmResult` instance in case the execution did not fail. Return an exception otherwise.
    """
