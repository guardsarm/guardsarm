...
from pubsub.subscriber import GuardSarmGCloudSubscriber
...
def get_wodle_config(credentials_file: str = "credentials.json", project: str = "test_project",
    ...
    """Return a dict containing every parameter supported by GuardSarmGCloudSubscriber. Used to simulate different
...
def test_GuardSarmGCloudSubscriber__init__(mock_client):
    """Test if an instance of GuardSarmGCloudSubscriber is created properly."""
    pubsub = GuardSarmGCloudSubscriber(**get_wodle_config())
...
def test_GuardSarmGCloudSubscriber__init__ko(credentials_file, errcode):
    """Test that the appropriate exceptions are raised when the GuardSarmGCloudSubscriber constructor is called with
    invalid parameters.
    """
    with pytest.raises(GCloudError) as e:
        GuardSarmGCloudSubscriber(**get_wodle_config(credentials_file=os.path.join(data_path, credentials_file)))
    assert e.value.errcode == errcode


@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_GuardSarmGCloudSubscriber_get_subscriber_client(mock_credentials):
    """Test get_subscriber_client attempts to create a client object using the provided credentials file."""
    GuardSarmGCloudSubscriber.get_subscriber_client("credentials.json")
    mock_credentials.assert_called_with("credentials.json")


@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_GuardSarmGCloudSubscriber_get_subscription_path(mock_credentials):
...
@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_GuardSarmGCloudSubscriber_check_permissions(mock_credentials):
...
@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_GuardSarmGCloudSubscriber_check_permissions_ko(mock_credentials, errcode, msg):
    """Test check_permissions raises the expected GCloudError when the subscriber does not have the required
    permissions."""
    pubsub = GuardSarmGCloudSubscriber(**get_wodle_config(credentials_file="credentials"))
    if errcode != 1206:
...
@patch('pubsub.subscriber.GuardSarmGCloudSubscriber.send_msg')
@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_GuardSarmGCloudSubscriber_pull_request(mock_credentials, mock_send_msg, num_messages):
...
@patch('pubsub.subscriber.GuardSarmGCloudSubscriber.send_msg')
@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_GuardSarmGCloudSubscriber_pull_request_ko(mock_credentials, mock_send_msg):
...
@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_GuardSarmGCloudSubscriber_process_messages(mock_credentials):
