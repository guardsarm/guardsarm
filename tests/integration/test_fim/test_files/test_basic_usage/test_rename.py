...
from guardsarm_testing.modules.fim.patterns import EVENT_TYPE_ADDED, EVENT_TYPE_DELETED
...
def test_rename(test_configuration, test_metadata, set_guardsarm_configuration, configure_local_internal_options,
                truncate_monitored_files, folder_to_monitor, daemons_handler, start_monitoring, path_to_edit):
    '''
    description: Check if the 'guardsarm-syscheckd' daemon detects 'added' and 'deleted' events when renaming a
    ...
    input_description: The test cases are contained in external YAML file (cases_rename.yaml) which includes
                       configuration parameters for the 'guardsarm-syscheckd' daemon and testing directories
    ...
    guardsarm_log_monitor.start(generate_callback(EVENT_TYPE_ADDED), timeout=70)
    assert guardsarm_log_monitor.callback_result
    guardsarm_log_monitor.start(generate_callback(EVENT_TYPE_DELETED), timeout=70)
    assert guardsarm_log_monitor.callback_result
