...
from guardsarm.core.cluster import client, cluster, server
from guardsarm.core.cluster import common as c_common
from guardsarm.core.cluster.dapi import dapi
from guardsarm.core.cluster.utils import context_tag
    ...
    def process_request(self, command: bytes, data: bytes):
        ...
        if command == b'dapi':
            ...
            if node_name in self.server.node.clients:
                ...
                raise GuardSarmClusterError(3022)
        else:
            return super().process_request(command, data)
    ...
    def process_request(self, command: bytes, data: bytes):
        ...
        if command == b'dapi':
            if self.server.node.client is None:
                raise GuardSarmClusterError(3023)
            asyncio.create_task(self.log_exceptions(
