# GuardSarm Engine Standalone

## Building the standalone package locally

You can build the GuardSarm Engine Standalone package locally using the `generate_package.sh` script:

```bash
cd guardsarm/src/engine/standalone
...
```bash
cd guardsarm-engine-standalone
./run_engine.sh
```
...
- **data/**: Contains the data files used by the engine in runtime, this directory should
    ...
    - **data/store**: Contains all the assets in catalog in a format that guardsarm-engine can understand (precompiled)
...
|---|---|---|
| `GUARDSARM_STANDALONE_LOG_LEVEL` | `info` | Log verbosity level |
| `GUARDSARM_SERVER_ENABLE_EVENT_PROCESSING` | `false` | Enable/disable event processing |
| `GUARDSARM_SERVER_API_MAX_RESOURCE_PAYLOAD_SIZE` | `50000` | Max payload size (bytes) for API resource requests |
| `GUARDSARM_SERVER_API_MAX_RESOURCE_KVDB_PAYLOAD_SIZE` | `100000` | Max payload size (bytes) for KVDB API requests |
...
|---|---|---|
| `GUARDSARM_STANDALONE_LOG_FILE_PATH` | `/var/log/guardsarm-indexer/guardsarm-engine.log` (production) or `logs/guardsarm-engine.log` (dev/CI) | `<RollingFile fileName="...">` |
| `GUARDSARM_STANDALONE_LOG_ROTATION_HOUR` | `0` (midnight) | `<TimeBasedTriggeringPolicy interval="1" modulate="true"/>` |
| `GUARDSARM_STANDALONE_LOG_ROTATION_MINUTE` | `0` | same as above |
| `GUARDSARM_STANDALONE_LOG_MAX_FILE_SIZE` | `134217728` (128 MB) | `<SizeBasedTriggeringPolicy size="128 MB"/>` |
| `GUARDSARM_STANDALONE_LOG_MAX_FILES` | `7` | `<DefaultRolloverStrategy max="7">` |
| `GUARDSARM_STANDALONE_LOG_MAX_ACCUMULATED_SIZE` | `2147483648` (2 GB) | `<IfAccumulatedFileSize exceeds="2 GB"/>` |

The following variables are custom extensions with no direct Log4j2 equivalent:

| Variable | Default | Description |
|---|---|---|
| `GUARDSARM_STANDALONE_LOG_ROTATION_ENABLED` | `true` | Enable/disable the entire log rotation subsystem |
| `GUARDSARM_STANDALONE_LOG_COMPRESSION_ENABLED` | `true` | Enable gzip compression of rotated files. Compression runs asynchronously; the active log file is never compressed |
| `GUARDSARM_STANDALONE_LOG_COMPRESSION_LEVEL` | `5` | zlib compression level: `0` = store only, `1` = fastest, `9` = maximum compression |
