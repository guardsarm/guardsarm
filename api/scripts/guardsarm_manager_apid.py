#!/var/guardsarm-manager/framework/python/bin/python3
...
def assign_guardsarm_ownership(filepath: str):
    ...
    if not os.path.isfile(filepath):
        f = open(filepath, "w")
        f.close()
    if os.stat(filepath).st_gid != common.guardsarm_gid() or \
        os.stat(filepath).st_uid != common.guardsarm_uid():
        os.chown(filepath, common.guardsarm_uid(), common.guardsarm_gid())
...
def configure_ssl(params):
    ...
    try:
        ...
        if not os.path.exists(api_conf['https']['key']) \
            ...
            logger.info(
                f"Generated private key file in GUARDSARM_PATH/{to_relative_path(api_conf['https']['key'])}")
            generate_self_signed_certificate(private_key, api_conf['https']['cert'])
            logger.info(
                f"Generated certificate file in GUARDSARM_PATH/{to_relative_path(api_conf['https']['cert'])}")

        # Check and assign ownership to guardsarm user for manager.key and manager.crt files
        assign_guardsarm_ownership(api_conf['https']['key'])
        assign_guardsarm_ownership(api_conf['https']['cert'])
        ...
        if exc.errno == 22:
            ...
            msg = f'GuardSarm API SSL ERROR. Please, ensure ' \
                    f'if path to certificates is correct in the configuration ' \
                    f'file GUARDSARM_PATH/{to_relative_path(CONFIG_FILE_PATH)}'
            print(msg)
            logger.error(msg)
            raise exc from exc
...
def start(params: dict):
    """Run the GuardSarm API.

    If another GuardSarm API is running, this function will fail because uvicorn server will
    ...
    app.add_middleware(GuardSarmAccessLoggerMiddleware, MiddlewarePosition.BEFORE_EXCEPTION)
...
def print_version():
    ...
    print('\n{} {} - {}\n\n{}'.format(__guardsarm_name__, __version__, __author__, __licence__))
...
def add_debug2_log_level_and_error():
    """Add a new debug level used by guardsarm api and framework."""
...
if __name__ == '__main__':
    ...
    from guardsarm.core import common, pyDaemonModule, utils
    from guardsarm.rbac.orm import check_database_integrity
    ...
    from api.middlewares import (
        ...
        GuardSarmAccessLoggerMiddleware,
    ...
    try:
        ...
        print(f"Error when trying to start the GuardSarm API. {e}")
        sys.exit(1)
    ...
    try:
        ...
        print(f"Error when trying to start the GuardSarm API. {api_log_error}")
        sys.exit(1)

    # set permission on log files
    for handler in uvicorn_params['log_config']['handlers'].values():
        if 'filename' in handler:
            assign_guardsarm_ownership(handler['filename'])
            os.chmod(handler['filename'], 0o660)  # nosec B103

    # Configure and create the guardsarm-api logger
    ...
    try:
        ...
        print(f'Internal error when trying to start the GuardSarm API. {e}')
        sys.exit(1)
    finally:
