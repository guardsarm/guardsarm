 ...
 * "guardsarm_agent_v<VERSION>_<rest>.wpk". The extracted token is prefixed with 'v'
 * so it can be compared directly with compare_guardsarm_versions().
 *
...
int wm_agent_upgrade_validate_wpk_version(wm_agent_info *agent_info, wm_upgrade_task *task, const char *wpk_repository_config) {
    ...
    if (!strcmp(agent_info->platform, "windows")) {
        ...
        if (compare_guardsarm_versions(task->wpk_version, WM_UPGRADE_NEW_VERSION_STRUCTURE_REPOSITORY, true) >= 0) {
            ...
            snprintf(file_url, OS_SIZE_2048, "guardsarm_agent_%s_macos_%s.wpk",
        ...
        if (compare_guardsarm_versions(task->wpk_version, WM_UPGRADE_NEW_LINUX_VERSION_REPOSITORY, true) >= 0) {
            if (compare_guardsarm_versions(task->wpk_version, WM_UPGRADE_NEW_VERSION_STRUCTURE_REPOSITORY, true) >= 0) {
                ...
                snprintf(file_url, OS_SIZE_2048, "guardsarm_agent_%s_linux_%s.wpk",
                         task->wpk_version, agent_info->architecture);
            }
            ...
            snprintf(file_url, OS_SIZE_2048, "guardsarm_agent_%s_%s_%s.%s_%s.wpk",
            ...
            snprintf(file_url, OS_SIZE_2048, "guardsarm_agent_%s_%s_%s_%s.wpk",
            ...
            if (next_line = strchr(version, '\n'), next_line) {
                *next_line = '\0';
                if (sha1 = strchr(version, ' '), sha1) {
                    *sha1 = '\0';
                    if (compare_guardsarm_versions(task->wpk_version, version, true) == 0) {
