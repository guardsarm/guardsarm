#!/usr/bin/env python3
"""
Integration Tests for GuardSarm Inventory Sync Module
Tests the inventory_sync module using the GuardSarm agent protocol with FlatBuffers.
"""
import json
import os
import pytest
...
from guardsarm_agent_controller import GuardSarmAgent
...
class InventorySyncIntegrationTester:
    def __init__(self, manager_address: str = "127.0.0.1",
        ...
        Args:
            manager_address: GuardSarm manager IP address
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if indices:
                    ...
                    if 'inventory' in indices.lower() or 'guardsarm' in indices.lower():
    ...
    def setup_agent(self, agent_id: Optional[str] = None,
        ...
        """
        Setup and register a GuardSarm agent for testing.
