...
brief: GuardSarm-db exposes a socket to receive requests and provide information. Some commands stream their result
       row by row, pushing each row as a "due {payload}" message with a blocking send bounded by
       WDB_BLOCK_SEND_TIMEOUT_S. If the client stops reading, the send buffer fills up, guardsarm-db blocks on send and
...
# Tests
def test_guardsarm_db_timeout(daemons_handler_module, connect_to_sockets_module, pre_insert_agents):
    '''
    description: Check that guardsarm-db aborts a large streamed response once the block-send timeout is reached
    ...
    tags:
        ...
        - wdb_socket
    '''
    guardsarm_db_send_sleep = 2
    command = 'global get-all-agents context'
    receiver_sockets[0].send(command, size=True)

    # Let guardsarm-db fill the socket send buffer while the client does not read.
    time.sleep(guardsarm_db_send_sleep)
