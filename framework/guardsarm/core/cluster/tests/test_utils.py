...
with patch('guardsarm.core.common.getgrnam'):
    with patch('guardsarm.core.common.getpwnam'):
        with patch('guardsarm.core.common.guardsarm_uid'):
            with patch('guardsarm.core.common.guardsarm_gid'):
                ...
                from guardsarm.core.cluster import utils
                from guardsarm.core.exception import GuardSarmError, GuardSarmException, GuardSarmInternalError, GuardSarmPermissionError, \
                    GuardSarmResourceNotFound, GuardSarmHAPHelperError
...
def test_read_cluster_config():
    ...
    with patch('guardsarm.core.cluster.utils.get_ossec_conf', side_effect=GuardSarmError(1001)):
        with pytest.raises(GuardSarmError, match='.* 3006 .*'):
            utils.read_cluster_config()

    with patch('guardsarm.core.configuration.load_guardsarm_xml', return_value=SystemExit):
        with pytest.raises(SystemExit) as pytest_wrapped_e:
            utils.read_cluster_config(from_import=True)
        assert pytest_wrapped_e.type == SystemExit
        assert pytest_wrapped_e.value.code == 0

    with patch('guardsarm.core.cluster.utils.get_ossec_conf', side_effect=KeyError(1)):
        with pytest.raises(GuardSarmError, match='.* 3006 .*'):
            utils.read_cluster_config()

    with patch('guardsarm.core.cluster.utils.get_ossec_conf', return_value={'cluster': default_cluster_config}):
        ...
        with pytest.raises(GuardSarmError, match='.* 3004 .*'):
            utils.read_cluster_config()
...
def test_parse_haproxy_helper_config_ko(config: dict, exception_type: GuardSarmException, expected_error_code: str):
...
def test_get_manager_status():
    ...
    with patch('guardsarm.core.cluster.utils.glob', return_value=['ossec-0.pid']):
        with patch('re.match', return_value='None'):
            status = utils.get_manager_status()
            for value in status.values():
                assert value == 'failed'

        # with patch('guardsarm.core.cluster.utils.join', return_value='failed') as join_mock:
        with patch('guardsarm.core.cluster.utils.os.path.exists', side_effect=exist_mock):
            status = utils.get_manager_status()
            for value in status.values():
                assert value == 'failed'
...
def test_get_manager_status_ko(mock_stat, exc):
    ...
    with pytest.raises(GuardSarmInternalError, match='.* 1913 .*'):
...
def test_get_cluster_status():
    ...
    GuardSarmInternalError is raised."""
    status = utils.get_cluster_status()
    assert {'running': 'no'} == status

    with patch('guardsarm.core.cluster.utils.get_manager_status', side_effect=GuardSarmInternalError(1913)):
        status = utils.get_cluster_status()
        assert {'running': 'no'} == status
...
def test_manager_restart():
    ...
    with patch('guardsarm.core.cluster.utils.open', side_effect=None):
                    ...
                    with patch('socket.socket.send', side_effect=None):
                        ...
                        with patch('socket.socket.recv', return_value=b'ok\n'):
                            with patch('socket.socket.close', side_effect=None):
                                ...
                                assert GuardSarmResult({'message': 'Restart request sent'}) == status
                    ...
                    with patch('socket.socket.send', side_effect=None):
                        ...
                        with patch('socket.socket.recv', return_value=b'ok\n'):
                            with patch('socket.socket.close', side_effect=None):
                                ...
                                assert GuardSarmResult({'message': 'Reload request sent'}) == result
...
def test_get_cluster_items():
    ...
    with patch("os.path.abspath", side_effect=FileNotFoundError):
        with pytest.raises(GuardSarmException, match=".* 3005 .*"):
    ...
    expected = {
        "files": {
            ...
            "excluded_files": ["guardsarm-manager.conf"],
...
def test_ClusterLogger():
    """Verify that ClusterLogger defines the logger used by guardsarm-manager-clusterd."""
...
@patch('guardsarm.core.cluster.utils.pyDaemonModule.create_pid')
def test_process_spawn_sleep(pyDaemon_create_pid_mock, get_pid_mock):
    ...
    pyDaemon_create_pid_mock.assert_called_once_with(f'guardsarm-manager-clusterd_child_{child}', get_pid_mock.return_value)


@pytest.mark.asyncio
@patch('concurrent.futures.ThreadPoolExecutor')
@patch('guardsarm.core.cluster.dapi.dapi.DistributedAPI')
