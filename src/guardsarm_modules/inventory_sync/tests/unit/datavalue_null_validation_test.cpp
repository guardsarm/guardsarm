...
protected:
    ...
    buildDataValue(bool includeId,
                   bool includeIndex,
                   bool includeData,
                   GuardSarm::SyncSchema::Operation operation = GuardSarm::SyncSchema::Operation_Upsert)
    {
        ...
        if (includeIndex)
        {
            indexOff = builder.CreateString("guardsarm-states-vulnerabilities");
        }
        if (includeData)
        {
            const char json[] = "{}";
        ...
        GuardSarm::SyncSchema::DataValueBuilder dvBuilder(builder);
        dvBuilder.add_seq(1);
        ...
        auto msgOff =
            GuardSarm::SyncSchema::CreateMessage(builder, GuardSarm::SyncSchema::MessageType_DataValue, dvOff.Union());
        GuardSarm::SyncSchema::FinishMessageBuffer(builder, msgOff);
...
TEST_F(DataValueNullValidationTest, ValidDataValue_AllFieldsPresent_PassesVerification)
{
    ...
    EXPECT_TRUE(GuardSarm::SyncSchema::VerifyMessageBuffer(verifier));

    auto message = GuardSarm::SyncSchema::GetMessage(buffer.data());
...
{
    auto buffer = buildDataValue(true, true, false, GuardSarm::SyncSchema::Operation_Upsert);
...
TEST_F(DataValueNullValidationTest, EmptyStringId_DifferentFromNullId)
{
    ...
    auto indexOff = builder.CreateString("guardsarm-states-test");
    const char json[] = "{}";
    ...
    dvBuilder.add_seq(1);
    dvBuilder.add_session(1);
    dvBuilder.add_operation(GuardSarm::SyncSchema::Operation_Upsert);
    dvBuilder.add_id(idOff);
    dvBuilder.add_index(indexOff);
    dvBuilder.add_data(dataOff);

    auto dvOff = dvBuilder.Finish();
    auto msgOff = GuardSarm::SyncSchema::CreateMessage(builder, GuardSarm::SyncSchema::MessageType_DataValue, dvOff.Union());
    GuardSarm::SyncSchema::FinishMessageBuffer(builder, msgOff);

    flatbuffers::Verifier verifier(reinterpret_cast<const uint8_t*>(builder.GetBufferPointer()), builder.GetSize());
    EXPECT_TRUE(GuardSarm::SyncSchema::VerifyMessageBuffer(verifier));

    auto message = GuardSarm::SyncSchema::GetMessage(builder.GetBufferPointer());
    auto data = message->content_as_DataValue();
    ASSERT_NE(data, nullptr);
...
{
    auto buffer = buildDataValue(true, true, false, GuardSarm::SyncSchema::Operation_Delete);
    ...
    EXPECT_EQ(data->operation(), GuardSarm::SyncSchema::Operation_Delete);
