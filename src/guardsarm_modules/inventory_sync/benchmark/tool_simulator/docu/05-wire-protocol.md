...
- The GuardSarm agent code does the same derivation; do not change it.
...
└──────────────────────────────────────────────────────────────┘
   ...
   aes_ciphertext = AES_256_CBC(aes_key, IV, padded_plaintext) ← crypto
                          │
                          v
   padded_plaintext = ('!' × pad_len) || zlib_compressed       ← GuardSarm padding
...
of plain text. The leading `'1'` is the queue byte (a GuardSarm legacy
...
| ---------------------------------------------------------------- | ------------------------------------------------------------- |
| `guardsarm-remoted` logs `Bad message format`                        | Length prefix endianness / extra/missing header bytes         |
| `guardsarm-remoted` logs `Decoding error`                            | AES key/IV mismatch                                           |
| `guardsarm-remoted` logs `inflate error`                             | zlib not applied or wrong padding                             |
| `guardsarm-remoted` logs `Invalid checksum`                          | MD5_hex missing or computed over the wrong slice              |
