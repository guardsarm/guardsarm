...
@patch("guardsarm.core.cluster.client.asyncio.get_running_loop")
...
async def test_localclient_start():
    ...
    with patch("uvloop.Loop.create_unix_connection", side_effect=create_unix_connection) as mock_create_unix_connection:
        ...
        with patch("guardsarm.core.cluster.local_client.common.CLUSTERD_SOCKET", new="path/test"):
            with patch("guardsarm.core.cluster.client.asyncio.get_running_loop", return_value=mocked_loop):
                lc = LocalClient()
                await lc.start()
...
async def test_localclient_start_exceptions(
    ...
    with pytest.raises(GuardSarmInternalError, match=rf".* {expected_error} .*"):
...
async def test_wait_for_response():
    ...
    lc.protocol.send_request.side_effect = [b"None", exception.GuardSarmClusterError(3018)]

    with patch("asyncio.Event.wait", side_effect=asyncio.TimeoutError):
        with pytest.raises(GuardSarmInternalError, match=rf".* 3020 .*"):
...
async def test_localclient_execute():
    ...
    with patch("guardsarm.core.cluster.local_client.LocalClient.start"):
        with patch("guardsarm.core.cluster.local_client.LocalClient.send_api_request", return_value="Test"):
...
async def test_localclient_send_file():
    ...
    with patch("guardsarm.core.cluster.local_client.LocalClient.start"):
        with patch("guardsarm.core.cluster.local_client.LocalClient.send_api_request", return_value=b"guardsarm/test python"):
            lc = LocalClient()
            assert await lc.send_file(path="guardsarm/test", node_name="python") == b"guardsarm/test python"
