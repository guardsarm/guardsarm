...
from guardsarm.core.common import guardsarm_uid, guardsarm_gid, DEFAULT_RBAC_RESOURCES
from guardsarm.core.utils import get_utc_now, safe_move
from guardsarm.rbac.utils import clear_tokens_cache

logger = logging.getLogger("guardsarm-api")

# Max reserved ID value
GUARDSARM_USER_ID = 1
GUARDSARM_WUI_USER_ID = 2
MAX_ID_RESERVED = 99
CLOUD_RESERVED_RANGE = 89

# Dummy hash for constant-time username enumeration protection
_DUMMY_HASH = generate_password_hash("guardsarm-dummy-constant-never-matches-any-real-password")
    ...
    def migrate_data(self, source: str, target: str, from_id: int = None, to_id: int = None):
        ...
        with AuthenticationManager(self.sessions[target]) as auth_manager:
            for user in old_users:
                if user.id in (GUARDSARM_USER_ID, GUARDSARM_WUI_USER_ID):
        ...
        if from_id == GUARDSARM_USER_ID and to_id == GUARDSARM_WUI_USER_ID:
            return
...
def check_database_integrity():
    ...
    def _set_permissions_and_ownership(database: str):
        """Set GuardSarm ownership and permissions.
        ...
        """
        chown(database, guardsarm_uid(), guardsarm_gid())
    ...
    try:
        ...
        if os.path.exists(DB_FILE):
            ...
            if current_version < expected_version:
                ...
                db_manager.migrate_data(source=DB_FILE, target=DB_FILE_TMP, from_id=GUARDSARM_USER_ID,
                                        to_id=GUARDSARM_WUI_USER_ID)
                ...
                safe_move(DB_FILE_TMP, DB_FILE,
                          ownership=(guardsarm_uid(), guardsarm_gid()),
                          permissions=0o640)
        ...
        logger.error("Error retrieving the current GuardSarm RBAC database version. Aborting database integrity check")
        db_manager.close_sessions()
        raise e
