...
This development container provides a complete, ready-to-use environment for developing and testing the GuardSarm Engine. It includes all necessary tools, dependencies, and pre-configured VS Code settings to streamline the development workflow.

## Features

- **Complete Build Environment**: Includes all required dependencies to compile the GuardSarm engine from source (GCC, CMake, Python, Go, Docker CLI, and more)
...
```bash
curl -o download_devContainer.sh https://raw.githubusercontent.com/guardsarm/guardsarm/main/src/engine/tools/devContainer/download_devContainer.sh
...
**Example:**
```bash
./download_devContainer.sh -d ~/guardsarm-engine-dev -b development
```
...
The `scripts/` and `e2e/` directories are **not** included in the download. They are available in the full GuardSarm repository under `src/engine/tools/devContainer/`.


## Development Utilities (scripts/)

> [!NOTE]
> The `scripts/` directory is not downloaded by `download_devContainer.sh`. It is available in the GuardSarm repository at `src/engine/tools/devContainer/scripts/`.
...
### mount_guardsarm_proc.sh
Mounts the `/proc` filesystem inside the GuardSarm installation directory for development and testing purposes.

### toggle_event_dumper.sh
Enables or disables the event dumper functionality in the GuardSarm engine.
...
- `guardsarm_stream_socket.go`: WebSocket streaming utility for engine events

### purge_guardsarm.sh
Located at `tools/purge_guardsarm.sh` (GuardSarm repository root), this script provides a comprehensive cleanup for local GuardSarm manager/agent installations:
- Removes GuardSarm packages via apt-get or yum
- Supports `dnf`, `yum`, `zypper`, and `rpm`-based package removal
- Unmounts proc filesystem if mounted for development
- Stops and removes GuardSarm services
- Cleans up all GuardSarm-related files and directories
- Removes GuardSarm user and group from the system
- Falls back to a full filesystem cleanup when the installation was created from sources instead of packages

## E2E Testing Environment

> [!NOTE]
> The `e2e/` directory is not downloaded by `download_devContainer.sh`. It is available in the GuardSarm repository at `src/engine/tools/devContainer/e2e/`.

The `e2e/` directory provides scripts to deploy a complete GuardSarm ecosystem for end-to-end testing and development within the devContainer.


### init.sh
Initializes the E2E environment by running three steps in order:

1. **Artifact download** — uses the GitHub CLI to find the latest successful build in `guardsarm/guardsarm-indexer` and `guardsarm/guardsarm-dashboard` and downloads the `.deb` packages into `guardsarm-indexer/` and `guardsarm-dashboard/` respectively.
2. **Certificate generation** — downloads `guardsarm-install.sh` from the official GuardSarm 4.x repository, generates a temporary `config.yml`, runs `--generate-config-files` to produce the cert bundle, extracts it into `certs/`, and cleans up all temporary files. If `certs/` already exists the script prompts whether to regenerate.
...
### docker-compose.yml
Orchestrates the E2E environment. Both images are **built locally** from their subdirectories (`./guardsarm-indexer`, `./guardsarm-dashboard`) using the packages downloaded by `init.sh` — they are not pulled from a registry.

**guardsarm-indexer**
- OpenSearch-based search and analytics engine
- Exposed on port `9200` (HTTPS)
- Mounts certificates from `./certs` (read-only)
- Three named volumes: `guardsarm-indexer-data`, `guardsarm-indexer-config`, `guardsarm-indexer-engine`

**guardsarm-dashboard**
- GuardSarm web interface for visualization and management
- Exposed on port `443` (HTTPS)
- Resolves `host.docker.internal` to the host gateway for engine communication
- Depends on `guardsarm-indexer`
...
The `agents/` subdirectory provides a self-contained environment to run GuardSarm agents inside containers connected to a manager running on the devContainer host.

#### agents/init.sh

Downloads the four agent installer packages required by the compose services before the first build:

- `4.x .deb` (Ubuntu) and `4.x .rpm` (CentOS) — from the official GuardSarm 4.x repository
...
### guardsarm_copy_certs.sh
Deploys generated certificates to an existing guardsarm-manager installation:
- Copies SSL/TLS certificates from `e2e/certs/` to `/var/guardsarm-manager/etc/certs/`
- Sets appropriate ownership (`guardsarm-manager:guardsarm-manager`) and permissions (640)
- Maps certificate files to guardsarm-manager expected names:
  - `guardsarm-1-key.pem` → `manager-key.pem`
  - `guardsarm-1.pem` → `manager.pem`
  - `root-ca.pem` → `root-ca.pem`
- Updates `ossec.conf` with indexer configuration

**Important:** Must be executed after installing guardsarm-manager and before starting the service.

**VS Code Task:** Available as "E2E Scripts: Copy guardsarm-manager certs" in the task menu (`Ctrl+Shift+P` → `Tasks: Run Task`)

### purge_guardsarm.sh
Use the repo-level `tools/purge_guardsarm.sh` script before re-running the E2E setup if you need to reset a local GuardSarm installation completely.

**VS Code Task:** Available as "Scripts: Purge GuardSarm installation" in the task menu
...
- **Engine Documentation**: See `guardsarm/src/engine/docs/` for detailed engine architecture and API documentation
