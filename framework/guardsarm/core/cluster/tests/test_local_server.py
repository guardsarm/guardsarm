...
with patch('guardsarm.common.guardsarm_uid'):
    with patch('guardsarm.common.guardsarm_gid'):
        ...
        from guardsarm.core.cluster.local_server import *
...
async def test_LocalServerHandler_connection_made(event_loop):
    ...
    with patch.object(logger, "debug") as logger_debug_mock:
        with patch("guardsarm.core.cluster.local_server.context_tag", ContextVar("tag", default="")) as mock_contextvar:
...
@pytest.mark.asyncio
@patch("guardsarm.core.cluster.local_server.server.AbstractServerHandler.process_request")
...
@pytest.mark.asyncio
@patch("guardsarm.core.cluster.local_server.LocalServerHandler.send_request", return_value='changed')
...
async def test_LocalServerHandlerMaster_process_request(process_request_mock, event_loop):
    ...
    with pytest.raises(GuardSarmClusterError, match=".* 3022 .*"):
...
@pytest.mark.asyncio
@patch("guardsarm.core.cluster.local_server.LocalServerHandler.process_request")
async def test_LocalServerHandlerWorker_process_request(process_request_mock, event_loop):
    ...
    with pytest.raises(GuardSarmClusterError, match=".* 3023 .*"):
