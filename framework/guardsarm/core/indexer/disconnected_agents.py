...
from guardsarm.core.agent import GuardSarmDBQueryAgents, get_agents_info
from guardsarm.core.cluster import master
...
from guardsarm.core.indexer.indexer import IndexerUnavailableError, get_indexer_client
...
class DisconnectedAgentSyncTasks:
    ...
    minimum duration, queries the GuardSarm Indexer to obtain the maximum
    ...
    def __init__(
        ...
        # Use from_import=True to avoid raising during tests when guardsarm configuration file
        # does not contain the indexer section. The config is only used for
        # informational purposes here.
        try:
            guardsarm_config = get_ossec_conf(section="indexer", from_import=True)
        except Exception:
            guardsarm_config = {}
        self.logger.debug(f"GuardSarm config for indexer section: {guardsarm_config}")
    ...
    async def _get_disconnected_agents_filter_by_time(self) -> List[dict]:
        ...
        This method queries the GuardSarm database for agents whose status is not
        ...
        try:
            ...
            self.logger.info(
                f"Retrieved {len(agents)} non-active agents from GuardSarmDB "
            ...
            self.logger.error(
                f"Error retrieving non-connected agents from GuardSarmDB: {e}",
                exc_info=True,
            )
            return []
    ...
    async def _get_max_versions_batch_from_indexer(self, agent_ids: List[str]) -> dict:
        """
        Query the GuardSarm Indexer for maximum document versions of multiple agents.
        ...
        try:
            ...
            if result:
                ...
                raise Exception("Failed query to guardsarm-indexer")
    ...
    async def run_cluster_name_sync(self) -> None:
        ...
        try:
            ...
            if not agent_ids:
                ...
                return
            # Read cluster name from guardsarm-manager.conf
            try:
                conf = get_ossec_conf(section="cluster")
                cluster_name = conf.get("cluster", {}).get("name")
            except Exception as e:
                self.logger.error(f"Failed reading cluster name from guardsarm-manager.conf: {e}")
                return

            if not cluster_name:
                self.logger.warning(
                    "Cluster name not found in guardsarm-manager.conf; aborting sync"
                )
                return
    ...
    async def disconnected_agent_group_sync(
        ...
        """
        result = AffectedItemsGuardSarmResult(
        ...
        for agent_id in agent_list:
            if agent_id not in system_agents:
                result.add_failed_item(id_=agent_id, error=GuardSarmResourceNotFound(1701))
            ...
            for agent_id in valid_agents:
                try:
                    ...
                    result.add_failed_item(
                        id_=agent_id,
                        error=GuardSarmInternalError(1000, extra_message=str(e)),
                    )
    ...
    async def _get_disconnected_agents(self) -> List[str]:
        """
        Retrieve non-connected agents from GuardSarmDB.

        Queries the GuardSarmDB for agents whose status is not `active` and
    ...
    async def _get_cluster_name_from_indexer(
        ...
        Only agents that have an associated guardsarm.cluster.name will be returned.
        ...
        query = {
            ...
            "query": {
                "bool": {
                    "filter": [
                        {"terms": {"guardsarm.agent.id": agent_ids}},
                        {"exists": {"field": "guardsarm.cluster.name"}},
                    ],
                    "must_not": [
                        {"term": {"guardsarm.cluster.name.keyword": cluster_name}}
                    ],
                }
            },
            "aggs": {
                "by_agent": {
                    "terms": {"field": "guardsarm.agent.id", "size": len(agent_ids)},
                    "aggs": {
                        "cluster_name": {
                            "terms": {"field": "guardsarm.cluster.name", "size": 5}
                        }
                    },
                }
            },
