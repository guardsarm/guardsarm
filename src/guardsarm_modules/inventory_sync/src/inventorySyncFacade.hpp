                                               ...
                                               TRocksDBWrapper>>
{
    ...
    void run(const std::vector<char>& dataRaw)
    {
        ...
        auto syncMessage = GuardSarm::SyncSchema::GetMessage(dataRaw.data());

        if (syncMessage->content_type() == GuardSarm::SyncSchema::MessageType_DataValue)
        {
            const auto data = syncMessage->content_as<GuardSarm::SyncSchema::DataValue>();
            if (!data)
            {
        ...
        else if (syncMessage->content_type() == GuardSarm::SyncSchema::MessageType_DataClean)
        {
            const auto dataClean = syncMessage->content_as<GuardSarm::SyncSchema::DataClean>();
            if (!dataClean)
            {
        ...
        else if (syncMessage->content_type() == GuardSarm::SyncSchema::MessageType_DataContext)
        {
            const auto dataContext = syncMessage->content_as<GuardSarm::SyncSchema::DataContext>();
            if (!dataContext)
            {
        ...
        else if (syncMessage->content_type() == GuardSarm::SyncSchema::MessageType_DataBatch)
        {
            const auto dataBatch = syncMessage->content_as<GuardSarm::SyncSchema::DataBatch>();
            ...
            for (const auto* dataValue : *dataBatch->values())
            {
                ...
                else
                {
                    try
                    {
                        ...
                        auto msgOffset = GuardSarm::SyncSchema::CreateMessage(
                            dvBuilder, GuardSarm::SyncSchema::MessageType_DataValue, dvOffset.Union());
        ...
        else if (syncMessage->content_type() == GuardSarm::SyncSchema::MessageType_Start)
        {
            const auto startMsg = syncMessage->content_as<GuardSarm::SyncSchema::Start>();
            if (!startMsg)
            {
            ...
            if (isAgentLocked(agentIdStr, isVDModule))
            {
                logDebug2(LOGGER_DEFAULT_TAG,
                          ...
                          agentIdStr.c_str());
                m_responseDispatcher->sendStartAck(GuardSarm::SyncSchema::Status_Error, agentId, -1, moduleName);
            }
            else if (!m_indexerConnector->isAvailable())
            {
                logDebug2(LOGGER_DEFAULT_TAG, "InventorySyncFacade::start: No available server");
                m_responseDispatcher->sendStartAck(GuardSarm::SyncSchema::Status_Offline, agentId, -1, moduleName);
            }
            else
            {
        ...
        else if (syncMessage->content_type() == GuardSarm::SyncSchema::MessageType_ChecksumModule)
        {
            const auto checksumModule = syncMessage->content_as<GuardSarm::SyncSchema::ChecksumModule>();
            if (!checksumModule)
            {
        ...
        else if (syncMessage->content_type() == GuardSarm::SyncSchema::MessageType_End)
        {
            const auto end = syncMessage->content_as<GuardSarm::SyncSchema::End>();
            if (!end)
            {
    ...
    std::string calculateChecksumOfChecksums(const std::string& index, const std::string& agentId)
    {
        ...
        while (true)
        {
            ...
            searchQuery["query"]["term"]["guardsarm.agent.id"] = agentId;
...
public:
        ...
        m_indexerQueue = std::make_unique<IndexerQueue>(
            [this](const Response& res)
            {
                ...
                try
                {
                    ...
                    if (res.context->mode == GuardSarm::SyncSchema::Mode_MetadataDelta ||
                        res.context->mode == GuardSarm::SyncSchema::Mode_MetadataCheck ||
                        res.context->mode == GuardSarm::SyncSchema::Mode_GroupDelta ||
                        res.context->mode == GuardSarm::SyncSchema::Mode_GroupCheck)
                    {
                        ...
                        m_indexerConnector->registerNotify(
                            [this, ctx = res.context]()
                            {
                                ...
                                m_responseDispatcher->sendEndAck(
                                    GuardSarm::SyncSchema::Status_Ok, ctx->agentId, ctx->sessionId, ctx->moduleName);
                                // Delete Session.
                    ...
                    else if (res.context->mode == GuardSarm::SyncSchema::Mode_GroupDelta)
                    {
                        logDebug2(LOGGER_DEFAULT_TAG,
                    ...
                    else if (res.context->mode == GuardSarm::SyncSchema::Mode_MetadataCheck)
                    {
                        logDebug2(LOGGER_DEFAULT_TAG,
                    ...
                    else if (res.context->mode == GuardSarm::SyncSchema::Mode_GroupCheck)
                    {
                        logDebug2(LOGGER_DEFAULT_TAG,
                    ...
                    else if (res.context->mode == GuardSarm::SyncSchema::Mode_ModuleCheck)
                    {
                        logDebug2(LOGGER_DEFAULT_TAG,
                        ...
                        try
                        {
                            if (res.context->checksumIndex.empty())
                            {
                                logError(LOGGER_DEFAULT_TAG,
                                         ...
                                         res.context->agentId.c_str());
                                m_responseDispatcher->sendEndAck(GuardSarm::SyncSchema::Status_Error,
                                                                 res.context->agentId,
                                                                 res.context->sessionId,
                            ...
                            else
                            {
                                ...
                                if (checksumMatch)
                                {
                                    logInfo(LOGGER_DEFAULT_TAG,
                                            ...
                                            res.context->agentId.c_str());
                                    m_responseDispatcher->sendEndAck(GuardSarm::SyncSchema::Status_Ok,
                                                                     res.context->agentId,
                                                                     res.context->sessionId,
                                                                     res.context->moduleName);
                                }
                                else
                                {
                                    logInfo(LOGGER_DEFAULT_TAG,
                                            ...
                                            res.context->agentId.c_str(),
                                            MAX_RETRIES);
                                    m_responseDispatcher->sendEndAck(GuardSarm::SyncSchema::Status_ChecksumMismatch,
                                                                     res.context->agentId,
                                                                     res.context->sessionId,
                                                                     res.context->moduleName);
                                }
                    ...
                    else
                    {
                        ...
                        for (const auto& [key, value] : m_dataStore->seek(prefix))
                        {
                            ...
                            if (GuardSarm::SyncSchema::VerifyMessageBuffer(verifier))
                            {
                                auto message = GuardSarm::SyncSchema::GetMessage(value.data());
                                ...
                                if (!isInventoryStateIndex(rawIndex))
                                {
                                    logWarn(LOGGER_DEFAULT_TAG,
                                            ...
                                            "index '%.*s' does not belong to guardsarm-states-* family.",
                                            res.context->sessionId,
                                ...
                                const bool isUpsert = data->operation() == GuardSarm::SyncSchema::Operation_Upsert;
                                if (isUpsert)
                                {
                                ...
                                else if (data->operation() != GuardSarm::SyncSchema::Operation_Delete)
                                {
                                ...
                                if (isUpsert)
                                {
                                    ...
                                    // Any field the agent tries to set under guardsarm.* gets clobbered here,
                                    // so the agent cannot impersonate another agent or another cluster.
                                    document["guardsarm"]["agent"]["id"] = res.context->agentId;
                                    document["guardsarm"]["agent"]["name"] = res.context->agentName;
                                    document["guardsarm"]["agent"]["version"] = res.context->agentVersion;
                                    document["guardsarm"]["agent"]["groups"] = res.context->groups;
                                    document["guardsarm"]["agent"]["host"]["architecture"] = res.context->architecture;
                                    document["guardsarm"]["agent"]["host"]["hostname"] = res.context->hostname;
                                    document["guardsarm"]["agent"]["host"]["os"]["name"] = res.context->osname;
                                    document["guardsarm"]["agent"]["host"]["os"]["platform"] = res.context->osplatform;
                                    document["guardsarm"]["agent"]["host"]["os"]["type"] = res.context->ostype;
                                    document["guardsarm"]["agent"]["host"]["os"]["version"] = res.context->osversion;
                                    document["guardsarm"]["cluster"]["name"] =
                        ...
                        if (res.context->option == GuardSarm::SyncSchema::Option_VDFirst ||
                            res.context->option == GuardSarm::SyncSchema::Option_VDSync)
                        {
                            if (VulnerabilityScannerFacade::instance().isInitialized())
                            {
                                ...
                                if (VulnerabilityScannerFacade::instance().isFeedReady())
                                {
                                    ...
                                    const bool skipScan =
                                        feedUpdateInProgress && res.context->option == GuardSarm::SyncSchema::Option_VDSync;
    ...
    std::unordered_set<std::string> getAgentsWithActiveSessionForModule(const std::string& moduleName,
                                                                        GuardSarm::SyncSchema::Option option) const
    {
    ...
    bool waitForAllVDSyncSessions(std::chrono::seconds timeout, uint32_t maxRetries) const
    {
        ...
        for (uint32_t i = 0; i < MAX_OUTER_ITERATIONS; ++i)
        {
            const auto vdSyncAgents =
                getAgentsWithActiveSessionForModule("syscollector_vd", GuardSarm::SyncSchema::Option_VDSync);
                                   ...
                                   uint32_t maxRetries) const
    {
        ...
        for (const auto& [sessionId, session] : m_agentSessions)
        {
            ...
            {
                if (context->option == GuardSarm::SyncSchema::Option_VDFirst)
                {
...
private:
    ...
    void deleteAgent(const std::string& agentId)
    {
        ...
        logInfo(LOGGER_DEFAULT_TAG,
                ...
                agentId.c_str(),
                GUARDSARM_STATES_INDEX_PATTERN);

        try
        {
            // Delete all agent data from guardsarm-states-* indexes using wildcard pattern
            auto lock = m_indexerConnector->scopeLock();
            m_indexerConnector->deleteByQuery(GUARDSARM_STATES_INDEX_PATTERN, agentId);
