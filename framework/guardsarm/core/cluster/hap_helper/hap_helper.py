...
from guardsarm.core.cluster.hap_helper.proxy import Proxy, ProxyAPI, ProxyServerState
from guardsarm.core.cluster.hap_helper.guardsarm import GuardSarmAgent, GuardSarmDAPI
from guardsarm.core.cluster.utils import (
    AGENT_CHUNK_SIZE,
...
from guardsarm.core.exception import GuardSarmException, GuardSarmHAPHelperError

CONNECTION_PORT = 1514


class HAPHelper:
    """Helper to balance GuardSarm agents through cluster calling HAProxy."""
    ...
    def __init__(
        ...
        self.proxy = proxy
        self.guardsarm_dapi = guardsarm_dapi
    ...
    def _get_logger(tag: str) -> logging.Logger:
        ...
        logger = logging.getLogger('guardsarm').getChild('HAPHelper')
    ...
    async def initialize_guardsarm_cluster_configuration(self):
        """Initialize main components of the GuardSarm cluster."""
        if not await self.proxy.exists_backend(self.proxy.guardsarm_backend):
            self.logger.info(f"Could not find GuardSarm backend '{self.proxy.guardsarm_backend}'")
            await self.proxy.add_new_backend(name=self.proxy.guardsarm_backend)
            self.logger.info('Added GuardSarm backend')

        if not await self.proxy.exists_frontend(f'{self.proxy.guardsarm_backend}_front'):
            self.logger.info(f"Could not find GuardSarm frontend '{self.proxy.guardsarm_backend}_front'")
            await self.proxy.add_new_frontend(
                name=f'{self.proxy.guardsarm_backend}_front',
                port=self.proxy.guardsarm_connection_port,
                backend=self.proxy.guardsarm_backend,
            )
            self.logger.info('Added GuardSarm frontend')
    ...
    async def check_node_to_delete(self, node_name: str) -> bool:
        ...
        """
        node_stats = await self.proxy.get_guardsarm_server_stats(server_name=node_name)
    ...
    async def obtain_nodes_to_configure(
        self, guardsarm_cluster_nodes: dict, proxy_backend_servers: dict
    ) -> tuple[list, list]:
        """Returns the nodes able to add and delete.

        Parameters
        ----------
        guardsarm_cluster_nodes : dict
            GuardSarm cluster nodes to check.
        ...
        for node_name, node_address in guardsarm_cluster_nodes.items():
            if node_name not in proxy_backend_servers:
                ...
                add_nodes.append(node_name)
        for node_name in proxy_backend_servers.keys() - guardsarm_cluster_nodes.keys():
            if node_name in self.guardsarm_dapi.excluded_nodes:
    ...
    async def update_agent_connections(self, agent_list: list[str]):
        ...
        for index in range(0, len(agent_list), self.agent_reconnection_chunk_size):
            await self.guardsarm_dapi.reconnect_agents(agent_list[index : index + self.agent_reconnection_chunk_size])
    ...
    async def force_agent_reconnection_to_server(self, chosen_server: str, agents_list: list[dict]):
        ...
        eligible_agents = GuardSarmAgent.get_agents_able_to_reconnect(agents_list=agents_list)
    ...
    async def migrate_old_connections(self, new_servers: list[str], deleted_servers: list[str]):
        ...
        """
        guardsarm_backend_stats = {}
        backend_stats_iteration = 1
        while any([server not in guardsarm_backend_stats for server in new_servers]):
            if backend_stats_iteration > self.UPDATED_BACKEND_STATUS_TIMEOUT:
                self.logger.error(f'Some of the new servers did not go UP: {set(new_servers) - guardsarm_backend_stats}')
                raise GuardSarmHAPHelperError(3041)

            self.logger.debug('Waiting for new servers to go UP')
            await asyncio.sleep(1)
            backend_stats_iteration += 1
            guardsarm_backend_stats = (await self.proxy.get_guardsarm_backend_stats()).keys()

        self.logger.debug('All new servers are UP')
        previous_agent_distribution = await self.guardsarm_dapi.get_agents_node_distribution()
        previous_connection_distribution = await self.proxy.get_guardsarm_backend_server_connections() | {
        ...
        for guardsarm_worker, agents in previous_agent_distribution.items():
            if guardsarm_worker in deleted_servers:
                agents_to_balance += [agent['id'] for agent in agents]
                continue
            try:
                agents_to_balance += [agent['id'] for agent in agents[: unbalanced_connections[guardsarm_worker]]]
                agents = agents[unbalanced_connections[guardsarm_worker] :]
            except KeyError:
                pass

            self.logger.info(f"Migrating {len(agents)} connections from server '{guardsarm_worker}'")
            await self.force_agent_reconnection_to_server(chosen_server=guardsarm_worker, agents_list=agents)

        if agents_to_balance:
            self.logger.info('Balancing exceeding connections after changes on the GuardSarm backend')
    ...
    def check_for_balance(self, current_connections_distribution: dict) -> dict:
        """Checks if the GuardSarm cluster is balanced.
    ...
    async def calculate_agents_to_balance(self, affected_servers: dict) -> dict:
        ...
        for server_name, n_agents in affected_servers.items():
            agent_candidates = await self.guardsarm_dapi.get_agents_belonging_to_node(node_name=server_name, limit=n_agents)
            eligible_agents = GuardSarmAgent.get_agents_able_to_reconnect(agents_list=agent_candidates)
    ...
    async def manage_guardsarm_cluster_nodes(self):
        """Main loop for check balance of GuardSarm cluster."""

        while True:
            context_tag.set(self.tag)
            try:
                await self.backend_servers_state_healthcheck()
                current_guardsarm_cluster = await self.guardsarm_dapi.get_cluster_nodes()
                ...
                nodes_to_add, nodes_to_remove = await self.obtain_nodes_to_configure(
                    current_guardsarm_cluster, current_proxy_backend
                )
                if nodes_to_add or nodes_to_remove:
                    self.logger.info(
                        'Detected changes in GuardSarm cluster nodes. Current cluster: ' f'{current_guardsarm_cluster}'
                    )
                    self.logger.info('Attempting to update proxy backend')

                    for node_to_remove in nodes_to_remove:
                        await self.proxy.remove_guardsarm_manager(manager_name=node_to_remove)

                    for node_to_add in nodes_to_add:
                        await self.proxy.add_guardsarm_manager(
                            manager_name=node_to_add,
                            manager_address=current_guardsarm_cluster[node_to_add],
                ...
                unbalanced_connections = self.check_for_balance(
                    current_connections_distribution=await self.proxy.get_guardsarm_backend_server_connections()
                )
                if not unbalanced_connections:
                    self.logger.debug(
                        'Current backend stats: ' f'{await self.proxy.get_guardsarm_backend_server_connections()}'
                    )
            ...
            except GuardSarmException as handled_exc:
    ...
    async def set_hard_stop_after(self, wait_connection_retry: bool = True, reconnect_agents: bool = True):
        ...
        agents_distribution = await self.guardsarm_dapi.get_agents_node_distribution()
        agents_id = [item['id'] for agents in agents_distribution.values() for item in agents]
        current_cluster = await self.guardsarm_dapi.get_cluster_nodes()
    ...
    async def start(cls):
        ...
        try:
            ...
            proxy = Proxy(
                guardsarm_backend=helper_config[HAPROXY_BACKEND],
                guardsarm_connection_port=connection_port,
                proxy_api=proxy_api,
                tag=tag,
            ...
            if await helper.proxy.check_multiple_frontends(
                port=connection_port, frontend_to_skip=f'{helper.proxy.guardsarm_backend}_front'
            ):
                logger.warning(
