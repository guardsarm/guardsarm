...
class TestBulkIndex:
    ...
    async def test_bulk_index_calls_async_bulk(self, metrics_index):
        ...
        with patch("guardsarm.core.indexer.metrics.async_bulk", new_callable=AsyncMock, return_value=(2, 0)) as mock_bulk:
            await metrics_index.bulk_index(
                index="guardsarm-metrics-agents",
                docs=docs,
                bulk_size=100,
            )
    ...
    async def test_bulk_index_action_shape(self, metrics_index):
        ...
        with patch("guardsarm.core.indexer.metrics.async_bulk", new_callable=AsyncMock, return_value=(2, 0)) as mock_bulk:
            ...
            for i, action in enumerate(actions):
                ...
                assert action["_index"] == "guardsarm-metrics-agents"
    ...
    async def test_bulk_index_passes_chunk_size(self, metrics_index):
        ...
        with patch("guardsarm.core.indexer.metrics.async_bulk", new_callable=AsyncMock, return_value=(1, 0)) as mock_bulk:
            await metrics_index.bulk_index(
                index="guardsarm-metrics-comms",
                docs=docs,
                bulk_size=50,
            )
    ...
    async def test_bulk_index_empty_docs(self, metrics_index):
        ...
        with patch("guardsarm.core.indexer.metrics.async_bulk", new_callable=AsyncMock, return_value=(0, 0)) as mock_bulk:
            await metrics_index.bulk_index(
                index="guardsarm-metrics-agents",
                docs=[],
                bulk_size=100,
    ...
    async def test_bulk_index_different_indices(self, metrics_index):
        ...
        for stream in ["guardsarm-metrics-agents", "guardsarm-metrics-comms"]:
            with patch("guardsarm.core.indexer.metrics.async_bulk", new_callable=AsyncMock, return_value=(1, 0)) as mock_bulk:
                await metrics_index.bulk_index(
                    index=stream,
                    docs=docs,
    ...
    async def test_bulk_index_logs_warning_on_failures(self, metrics_index):
        ...
        with patch("guardsarm.core.indexer.metrics.async_bulk", new_callable=AsyncMock, return_value=(95, 5)):
    ...
    async def test_bulk_index_no_warning_on_success(self, metrics_index):
        ...
        with patch("guardsarm.core.indexer.metrics.async_bulk", new_callable=AsyncMock, return_value=(1, 0)):
            metrics_index._logger = MagicMock()
            await metrics_index.bulk_index(
                index="guardsarm-metrics-agents",
                docs=docs,
