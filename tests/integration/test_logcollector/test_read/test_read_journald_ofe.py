...
from guardsarm_testing.utils.file import write_json_file
...
def test_configuration_location(test_configuration, test_metadata, truncate_monitored_files, configure_local_internal_options,
                                reset_ofe_status, remove_all_localfiles_guardsarm_config, set_guardsarm_configuration, pre_send_journal_logs,
                                daemons_handler, wait_for_logcollector_start):
    '''
    description: Check if the 'guardsarm-logcollector' daemon starts properly when the 'journald' tag is used 
                 ...
                 Finally, the test will verify that the GuardSarm-logcollector read the logs, and the GuardSarm API returns the correct values
                 for the 'localfile' section.

    guardsarm_min_version: 4.9.0
    ...
    parameters:
        ...
        - remove_all_localfiles_guardsarm_config:
            type: fixture
            brief: Remove all 'localfile' sections from the GuardSarm configuration.
        - reset_ofe_status:
            type: fixture
    ...
    assertions:
        - Verify that the GuardSarm component (agent or manager) can start when the 'jouranld' tag is used.
        - Verify that the GuardSarm component (agent or manager) can read the logs from the 'journald' log format and the logs older than the current logcollector start time.

    input_description: A configuration file with journal block settings and the expected log messages.
                       Those include configuration settings for `journal` configuration in 'guardsarm-logcollector'.
