...
#define GM_AWS_TEST_LOGTAG "guardsarm-modulesd:aws-s3"

static gmodule *aws_module;
static OS_XML *lxml;
extern int test_mode;

extern void gm_aws_run_s3(gm_aws_bucket *bucket);

void gm_aws_run_s3(gm_aws_bucket *exec_bucket) {
...
static void gmodule_cleanup(gmodule *module) {
    free( ((gm_aws*) module->data)->buckets->bucket);
    free( ((gm_aws*) module->data)->buckets->aws_profile);
    free( ((gm_aws*) module->data)->buckets->trail_prefix);
    free( ((gm_aws*) module->data)->buckets->type);
    free( ((gm_aws*) module->data)->buckets);
    free(module->data);
    free(module->tag);
    free(module);
}
...
static int setup_module() {
    int ret;
    aws_module = calloc(1, sizeof(gmodule));
    const char *string =
    ...
    ret = gm_aws_read(lxml, nodes, aws_module);
    OS_ClearNode(nodes);
    test_mode = 1;
    return ret;
}
...
static int teardown_test_executions(void **state) {
    gm_aws* module_data = (gm_aws *) *state;
...
static int teardown_test_read(void **state) {
    ...
    gm_aws *module_data = (gm_aws*)test->module->data;
    sched_scan_free(&(module_data->scan_config));
...
void test_interval_execution(void **state) {
    gm_aws* module_data = (gm_aws *)aws_module->data;
    int i = 0;
    ...
    *state = module_data;
    ...
    expect_string_count(__wrap__mterror, tag, GM_AWS_TEST_LOGTAG, TEST_MAX_DATES + 1);
...
void test_fake_tag(void **state) {
    ...
    assert_int_equal(gm_aws_read(&(test->xml), test->nodes, test->module), -1);
...
void test_read_scheduling_monthday_configuration(void **state) {
    ...
    test->nodes = string_to_xml_node(string, &(test->xml));
    assert_int_equal(gm_aws_read(&(test->xml), test->nodes, test->module), 0);
    gm_aws *module_data = (gm_aws*)test->module->data;
