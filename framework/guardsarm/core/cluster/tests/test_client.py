...
with patch('guardsarmsiem.common.guardsarm_uid'):
    with patch('guardsarmsiem.common.guardsarm_gid'):
        ...
        import guardsarm.core.cluster.client as client
...
def test_acm_init():
    ...
    assert abstract_client_manager.logger == logging.getLogger("guardsarm")
...
@patch('guardsarm.core.cluster.client.AbstractClientManager.add_tasks', return_value=[])
    ...
    try:
        ...
        with patch.object(logging.getLogger('guardsarm'), "error") as logger_mock:
            ...
            with patch.object(abstract_client.transport, "close") as close_mock:
                ...
                future.set_result([GuardSarmClusterError(3020)])
                abstract_client.connection_result(future)
                logger_mock.assert_called_once_with(f"Could not connect to master: {str(GuardSarmClusterError(3020))}.")
                ...
                future = loop.create_future()
                future.set_exception(GuardSarmClusterError(3020))
                abstract_client.connection_result(future)
                logger_mock.assert_called_once_with(f"Could not connect to master: {str(GuardSarmClusterError(3020))}.")
                close_mock.assert_called_once()

        # Check second condition
        with patch.object(logging.getLogger('guardsarm'), "info") as logger_mock:
            abstract_client.transport = CloseMock()
            future = loop.create_future()
...
async def test_ac_connection_made():
    ...
    with patch(
        "guardsarm.core.cluster.client.AbstractClient.send_request",
        ...
        # Verify send_request was triggered with the expected hello payload
        g_mock.assert_called_once()
        args, kwargs = g_mock.call_args
        if kwargs:
            assert kwargs["command"] == b"hello"
            assert kwargs["data"] == abstract_client.client_data
        else:
            assert args[0] == b"hello"
            assert args[1] == abstract_client.client_data

        assert abstract_client.connected is True

@patch('guardsarm.core.cluster.client.AbstractClient._cancel_all_tasks')
def test_ac_connection_lost(cancel_tasks_mock):
    ...
    with patch.object(logging.getLogger('guardsarm'), "error") as logger_mock:
        abstract_client.connection_lost(exc=GuardSarmException(1001))
        logger_mock.assert_called_once_with(f"Connection closed due to an unhandled error: {GuardSarmException(1001)}\n",
                                            exc_info=False)
...
def test_ac_process_response():
    ...
    # Test the second condition
    with patch('guardsarm.core.cluster.common.Handler.process_response', return_value=b'ok') as pr_mock:
...
def test_ac_process_request():
    ...
    # Test the fist condition
    with patch('guardsarm.core.cluster.client.AbstractClient.echo_client', return_value=b'ok') as echo_mock:
        assert (abstract_client.process_request(command=b"echo-m", data=b"data") == b'ok')
        echo_mock.assert_called_once_with(b'data')

    # Test the second condition
    with patch('guardsarm.core.cluster.common.Handler.process_request', return_value=b'ok') as pr_mock:
...
@patch('guardsarm.core.cluster.client.AbstractClient.send_request', return_value=b"ok")
async def test_ac_client_echo_ok(send_request_mock, done_mock, asyncio_sleep_mock):
    ...
    # Test try
    with patch.object(logging.getLogger("guardsarm"), "info") as logger_mock:
        with patch('guardsarm.core.cluster.common.Handler.setup_task_logger',
                   return_value=logging.getLogger("guardsarm")) as setup_logger_mock:
            try:
...
@patch.object(FutureMock, "done", return_value=False)
@patch('guardsarm.core.cluster.client.perf_counter', return_value=0)
@patch('guardsarm.core.cluster.client.AbstractClient.send_request', return_value="ok")
...
@freeze_time("2022-01-01")
@patch('guardsarm.core.cluster.client.perf_counter', return_value=0)
@patch('guardsarm.core.cluster.client.AbstractClient.send_file', return_value="ok")
...
@freeze_time("2022-01-01")
@patch('guardsarm.core.cluster.client.perf_counter', return_value=0)
@patch('guardsarm.core.cluster.client.AbstractClient.send_string', return_value="ok")
