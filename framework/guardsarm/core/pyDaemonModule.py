...
API_MAIN_PROCESS = 'guardsarm-manager-apid'
API_LOCAL_REQUEST_PROCESS = 'guardsarm-manager-apid_exec'
API_AUTHENTICATION_PROCESS = 'guardsarm-manager-apid_auth'
...
def create_pid(name: str, pid: int):
    ...
    Raises
    ------
    GuardSarmInternalError(3002)
        Error creating pidfile.
    """
    filename = path.join(common.GUARDSARM_PATH, common.OS_PIDFILE_PATH, f'{name}-{pid}.pid')
    ...
    with open(filename, 'a') as fp:
        try:
            ...
            raise GuardSarmInternalError(3002, str(e))
...
def delete_child_pids(name: str, ppid: int, logger: logging.Logger):
    ...
    logger : logging.Logger
        Logger object.
    """
    filenames = glob.glob(path.join(common.GUARDSARM_PATH, common.OS_PIDFILE_PATH, f'{name}*.pid'))
