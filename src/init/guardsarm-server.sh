...
# or branding.json is absent. Does NOT affect daemon names, paths or the
# machine-readable GUARDSARM_VERSION keys emitted by info().
PRODUCT_NAME="GuardSarm"
if [ -f "${LOCAL}/branding.sh" ]; then
    . "${LOCAL}/branding.sh"
fi
...
add_to_cgroup()
{
    CGROUP_PATH="/sys/fs/cgroup/systemd/system.slice/guardsarm-manager.service/cgroup.procs"

    # Check if cgroup path exists
    if [ ! -f "$CGROUP_PATH" ]; then
        echo "Warning: cgroup path does not exist: $CGROUP_PATH" >&2
    else
        for pidfile in ${DIR}/var/run/guardsarm-manager-*.pid; do
            [ -f "$pidfile" ] || continue
...
get_guardsarm_engine_pid()
{
    local max_ticks=100
    local ticks=0
    local pidfile

    ${DIR}/bin/guardsarm-manager-analysisd

    while [ $ticks -lt $max_ticks ]; do
        pidfile=$(ls ${DIR}/var/run/guardsarm-manager-analysisd-*.pid 2>/dev/null | head -n1)
        if [ -n "$pidfile" ]; then
...
wait_for_guardsarm_engine_ready()
{
    local attempts=0
    local max_attempts=120

    ENGINE_PID=$(get_guardsarm_engine_pid)
    if [ $? -ne 0 ]; then
        echo "Failed to obtain PID for guardsarm-manager-analysisd"
        return 1
    fi
    ...
    while [ $attempts -lt $max_attempts ]; do
        ...
        if ! kill -0 "$ENGINE_PID" 2>/dev/null; then
            echo "guardsarm-manager-analysisd died during event dumper check."
            return 1
        fi
    ...
    echo "guardsarm-manager-analysisd did not respond correctly after $max_attempts attempts."
    kill $ENGINE_PID
    return 1
}
...
start_service()
{
    ...
    if [ $USE_JSON = false ]; then
        echo "Starting $PRODUCT_NAME $VERSION..."
    fi
    ...
    node_type=$(grep '<node_type>' ${DIR}/etc/${GUARDSARM_CONF} | sed 's/<node_type>\(.*\)<\/node_type>/\1/' | tr -d ' ');
    if [ -z $node_type ]; then
        echo "Invalid cluster configuration, check the $DIR/etc/${GUARDSARM_CONF} file."
        unlock;
        exit 1;
    fi
    ...
    for i in ${SDAEMONS}; do
        ...
        if [ X"$i" = "Xguardsarm-manager-apid" ] && [ "$node_type" != "master" ]; then
            continue
        fi

        ## If guardsarm-manager-authd is disabled, don't try to start it.
        if [ X"$i" = "Xguardsarm-manager-authd" ]; then
             start_config="$(grep -n "<auth>" ${DIR}/etc/${GUARDSARM_CONF} | cut -d':' -f 1)"
             end_config="$(grep -n "</auth>" ${DIR}/etc/${GUARDSARM_CONF} | cut -d':' -f 1)"
             if [ -n "${start_config}" ] && [ -n "${end_config}" ]; then
                sed -n "${start_config},${end_config}p" ${DIR}/etc/${GUARDSARM_CONF} | grep "<disabled>yes" >/dev/null 2>&1
                if [ $? = 0 ]; then
                    continue
                fi
             else
        ...
        if [ $? = 0 ]; then
            ...
            if [ ! -z "$LEGACY_SYSTEMD_VERSION" ]; then
                ...
                if [ "$i" = "guardsarm-manager-analysisd" ]; then
                    wait_for_guardsarm_engine_ready
        ...
        for pid in `cat ${DIR}/var/run/${daemon_name}-*.pid 2>/dev/null`; do
            ...
            if [ ! $? = 0 ]; then
                if [ $USE_JSON = false ] && [ "X${_pstatus_quiet}" = "X" ]; then
                    echo "${pfile}: Process ${pid} not used by GuardSarm, removing..."
                fi
...
stop_service()
{
    ...
    if [ $USE_JSON = true ]; then
        ...
        echo "$PRODUCT_NAME $VERSION Stopped"
    fi
}
...
info()
{
    if [ "X${1}" = "X" ]; then
        if [ $USE_JSON = true ]; then
            echo -n '{"error":0,"data":['
            echo -n '{"GUARDSARM_VERSION":"'${VERSION}'"},'
            echo -n '{"GUARDSARM_REVISION":"'${REVISION}'"},'
            echo -n '{"GUARDSARM_TYPE":"'${TYPE}'"}'
            echo -n ']}'
        else
            echo "GUARDSARM_VERSION=\"${VERSION}\""
            echo "GUARDSARM_REVISION=\"${REVISION}\""
            echo "GUARDSARM_TYPE=\"${TYPE}\""
        fi
    else
        case "${1}" in
            -v) echo "${VERSION}" ;;
...
restart)
    ...
    ;;
reload)
    DAEMONS=$(echo $DAEMONS | sed 's/guardsarm-manager-remoted//')
    SDAEMONS=$(echo $DAEMONS | awk '{ for (i=NF; i>1; i--) printf("%s ",$i); print $1; }')
    if is_systemd; then
