...
> **Note:** GuardSarm 5.0 uses JSON format with ECS (Elastic Common Schema) for alerts stored in `/var/ossec/logs/alerts/alerts.json`. All examples in this document are from actual alerts captured in a GuardSarm deployment.
...
```bash
tail -f /var/ossec/logs/alerts/alerts.json | jq 'select(.guardsarm.protocol.location=="rootcheck")'
```
...
```bash
tail -f /var/ossec/logs/alerts/alerts.json | jq 'select(.guardsarm.protocol.location=="rootcheck" and (.event.original | contains("Starting") or contains("Ending") | not))'
```
...
```bash
jq 'select(.guardsarm.protocol.location=="rootcheck" and .guardsarm_rootcheck.anomaly.type=="dev-file")' /var/ossec/logs/alerts/alerts.json
```

**World-writable files:**
```bash
jq 'select(.guardsarm.protocol.location=="rootcheck" and .guardsarm_rootcheck.vulnerability.type=="world-writable")' /var/ossec/logs/alerts/alerts.json
```

**Promiscuous mode (search in message):**
```bash
jq 'select(.guardsarm.protocol.location=="rootcheck" and (.event.original | contains("promiscuous")))' /var/ossec/logs/alerts/alerts.json
```

**Specific agent:**
```bash
jq 'select(.guardsarm.protocol.location=="rootcheck" and .agent.name=="ubuntu22")' /var/ossec/logs/alerts/alerts.json
```
...
```bash
jq -r 'select(.guardsarm.protocol.location=="rootcheck") | .event.original' /var/ossec/logs/alerts/alerts.json
```

**Count alerts by anomaly type:**
```bash
jq -r 'select(.guardsarm.protocol.location=="rootcheck") | .guardsarm_rootcheck.anomaly.type // empty' /var/ossec/logs/alerts/alerts.json | sort | uniq -c
```

**List affected agents:**
```bash
jq -r 'select(.guardsarm.protocol.location=="rootcheck") | .agent.name' /var/ossec/logs/alerts/alerts.json | sort -u
```

**Export alerts to CSV:**
```bash
jq -r 'select(.guardsarm.protocol.location=="rootcheck") | [."@timestamp", .agent.name, .event.original, .guardsarm_rootcheck.anomaly.type // "N/A"] | @csv' /var/ossec/logs/alerts/alerts.json > rootcheck_alerts.csv
```

**Get file paths from detections:**
```bash
jq -r 'select(.guardsarm.protocol.location=="rootcheck" and .file.path?) | .file.path' /var/ossec/logs/alerts/alerts.json
```
...
jq --arg time "$ONE_HOUR_AGO" 'select(.guardsarm.protocol.location=="rootcheck" and ."@timestamp" > $time)' /var/ossec/logs/alerts/alerts.json
```

**Multiple conditions:**
```bash
# Malware category alerts from specific agent
jq 'select(.guardsarm.protocol.location=="rootcheck" and .agent.name=="ubuntu22" and (.event.category | contains(["malware"])))' /var/ossec/logs/alerts/alerts.json
```

**Group by agent and anomaly type:**
```bash
jq -r 'select(.guardsarm.protocol.location=="rootcheck") | "\(.agent.name),\(.guardsarm_rootcheck.anomaly.type // "N/A")"' /var/ossec/logs/alerts/alerts.json | sort | uniq -c
```
...
```bash
jq -r 'select(.guardsarm.protocol.location=="rootcheck") | .agent.name' /var/ossec/logs/alerts/alerts.json | sort | uniq -c | sort -rn
```

**Alerts timeline (by hour):**
```bash
jq -r 'select(.guardsarm.protocol.location=="rootcheck") | ."@timestamp" | split("T")[1] | split(":")[0]' /var/ossec/logs/alerts/alerts.json | sort | uniq -c
```
