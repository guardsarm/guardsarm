...
class TestUpdateAgentClusterName:
    ...
    async def test_update_cluster_name_script_structure(self, max_version_index, indexer_client):
        ...
        # Verify script initializes guardsarm structure
        assert "ctx._source.guardsarm" in source
        assert "ctx._source.guardsarm.cluster" in source

        # Verify script sets cluster name
        assert "ctx._source.guardsarm.cluster.name" in source
    ...
    async def test_update_cluster_name_query_structure(self, max_version_index, indexer_client):
        ...
        assert any(
            "guardsarm.agent.id" in str(clause) for clause in must_clause
        ), "Query should filter by guardsarm.agent.id"
    ...
    async def test_update_cluster_name_logs_debug_info(self, max_version_index, indexer_client):
        ...
        with patch("guardsarm.core.indexer.max_version_components.logging") as mock_logging:
            logger = MagicMock()
