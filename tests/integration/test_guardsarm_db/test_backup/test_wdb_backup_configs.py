...
brief: GuardSarm-db is the daemon in charge of the databases with all the GuardSarm persistent information, exposing a socket
       to receive requests and provide information. GuardSarm-db has the capability to do automatic database backups, based
...
from guardsarm_testing.constants.paths.logs import GUARDSARM_PATH, GUARDSARM_LOG_PATH
from guardsarm_testing.utils.time import validate_interval_format, time_to_seconds
from guardsarm_testing.modules.guardsarm_db import patterns
...
# Configuration
t_config_path = Path(CONFIGURATIONS_FOLDER_PATH, 'configuration_guardsarm_db_backups_conf.yaml')
t_cases_path = Path(TEST_CASES_FOLDER_PATH, 'cases_guardsarm_db_backups_conf.yaml')
...
backups_path = Path(GUARDSARM_PATH, 'backup', 'db')

# Variables
interval = 5
timeout = 15

# The "Created Global database backup" event requires guardsarm_db debug level 1, set in the
# manager internal options file.
internal_options_path = Path(GUARDSARM_PATH, 'etc', 'guardsarm-manager-internal-options.conf')
guardsarm_db_debug_option = 'guardsarm_db.debug'


@pytest.fixture()
def configure_guardsarm_db_debug():
    ...
    for line in lines:
        if line.strip().startswith(f'{guardsarm_db_debug_option}=') and not line.lstrip().startswith('#'):
            updated_lines.append(f'{guardsarm_db_debug_option}=1')
...
def test_wdb_backup_configs(test_configuration, test_metadata, configure_guardsarm_db_debug,
                            set_guardsarm_configuration, truncate_monitored_files, remove_backups):
    '''
    description: Check that given different wdb backup configuration parameters, the expected behavior is achieved.
                 For this, the test gets a series of parameters for the guardsarm_db_backups_conf.yaml file and applies
    ...
    input_description:
        - Test cases are defined in the parameters and metada variables, that will be applied to the the
          guardsarm_db_backup_command.yaml file. The parameters tested are: "enabled", "interval" and "max_files".
    ...
    try:
        ...
        if not validate_interval_format(test_interval):
            guardsarm_log_monitor.start(callback=callbacks.generate_callback(patterns.WRONG_INTERVAL_CALLBACK), timeout=timeout)
            assert guardsarm_log_monitor.callback_result, 'Did not receive expected ' \
                                                    '"Invalid value element for interval..." event'

            return
        elif not isinstance(test_max_files, numbers.Number) or test_max_files==0:
            guardsarm_log_monitor.start(callback=callbacks.generate_callback(patterns.WRONG_MAX_FILES_CALLBACK), timeout=timeout)
            assert guardsarm_log_monitor.callback_result, 'Did not receive expected ' \
    ...
    # Manage if backup generation is enabled - one or more backups expected
        ...
        guardsarm_log_monitor.start(callback=callbacks.generate_callback(patterns.BACKUP_CREATION_CALLBACK),
                                        timeout=timeout, accumulations=int(test_max_files)+1)
        result = guardsarm_log_monitor.callback_result
