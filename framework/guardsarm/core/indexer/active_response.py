...
from guardsarm.core.cluster.utils import ClusterFilter
from guardsarm.core.exception import GuardSarmError, IndexerUnavailableError
from guardsarm.core.indexer.indexer import get_indexer_client
from guardsarm.core.guardsarm_queue import GuardSarmQueue


AR_INDEX = "guardsarm-active-responses*"
AR_SCHEMA = {
...
class ActiveResponse:
    ...
    def target_agents(self, available_agents: List[str]) -> List[str]:
        ...
        """
        location = self.doc_source["guardsarm"]["active_response"]["location"]
        ...
        match location:
            ...
            case "local":
                agent_id = self.doc_source["guardsarm"]["agent"]["id"]
                return [agent_id] if agent_id in available_agents else []

            case "defined-agent":
                agent_id = self.doc_source["guardsarm"]["active_response"]["agent_id"]
                return [agent_id] if agent_id in available_agents else []
...
class ActiveResponseHelpers:
    ...
    async def fetch_active_response_docs(
        ...
        query = {
            ...
            "query": {
                "bool": {
                    "filter": [],
                    "must_not": [{ "regexp": {"guardsarm.agent.version": "v[0-4]\\..*" }}],
                }
            },
        }
    ...
    def build_ar_messages(
        ...
        def format_message(
            ...
            guardsarm = ar_source.get("guardsarm", {}).copy()

            if event:
                msg = {**ar_source, **event}
                event_guardsarm = event.get("guardsarm", {})
                guardsarm = {**event_guardsarm, **guardsarm}

            else:
                msg = dict(ar_source)

            msg["guardsarm"] = guardsarm
...
class ActiveResponseBuilder:
    ...
    def dispatch(self) -> "ActiveResponseBuilder":
        ...
        with GuardSarmQueue(common.AR_SOCKET) as wq:
            for agent_id, msg, bookmark in msgs:
                try:
                    ...
                    wq.send_msg_to_agent(
                        msg=json_msg, agent_id=agent_id, msg_type=GuardSarmQueue.AR_TYPE
                    )
                    msgs_sent += 1
                except GuardSarmError as e:
                    self.logger.error(
...
class ActiveResponseFetchTask:
    ...
    async def active_response_processing(self) -> None:
        ...
        try:
            ...
            self.logger.warning("Cannot connect to GuardSarm Indexer")
