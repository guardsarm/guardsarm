...
The schema according to the GuardSarm Common Schema (WCS) is available in `src/external/indexer-plugins` and is downloaded during the agent build process as part of external dependencies (`make deps`):

- `sca.json`

This schema defines the standardized format for SCA data that is sent to the GuardSarm indexer.
