...
void * w_rotate_log_thread(__attribute__((unused)) void * arg) {
    ...
    // /var/gsmsec/logs/gsmsec.json
    snprintf(path_json, PATH_MAX, "%s", LOGJSONFILE);
#else
    // <guardsarm-home>/logs/<guardsarm log file>
    snprintf(path, PATH_MAX, "%s", LOGFILE);
    // /var/gsmsec/logs/gsmsec.json
    snprintf(path_json, PATH_MAX, "%s", LOGJSONFILE);
#endif
    ...
    while (1) {
        ...
        if (today != tm.tm_mday) {
            sleep(day_wait);
            /* Daily rotation and compression of gsmsec.log/gsmsec.json */
        ...
        if (size_rotate > 0) {
            if (w_stat(path, &buf) == 0) {
                size = buf.st_size;
                /* If log file reachs maximum size, rotate gsmsec.log */
