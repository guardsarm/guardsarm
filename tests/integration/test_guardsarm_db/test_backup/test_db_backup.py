...
from guardsarm_testing.constants.daemons import GUARDSARM_DB_DAEMON
from guardsarm_testing.constants.paths import GUARDSARM_PATH
from guardsarm_testing.constants.paths.sockets import GUARDSARM_DB_SOCKET_PATH
from guardsarm_testing.utils.database import query_wdb
from guardsarm_testing.utils import configuration
from guardsarm_testing.utils.db_queries.global_db import remove_metadata_value
...
# Configurations
t_cases_path = Path(TEST_CASES_FOLDER_PATH, 'cases_guardsarm_db_backup_command.yaml')
t_config_parameters, t_config_metadata, t_case_ids = configuration.get_test_cases_data(t_cases_path)

backups_path = Path(GUARDSARM_PATH, 'backup', 'db')
receiver_sockets_params = [(GUARDSARM_DB_SOCKET_PATH, 'AF_UNIX', 'TCP')]
monitored_sockets_params = [(GUARDSARM_DB_DAEMON, None, True)]
...
def configure_wdb_socket_environment_module():
    """Keep the GuardSarm DB socket lifecycle isolated without stopping unrelated daemons."""
    services.control_service('stop', daemon=GUARDSARM_DB_DAEMON)
    services.wait_expected_daemon_status(target_daemon=GUARDSARM_DB_DAEMON, running_condition=False)
    services.control_service('start', daemon=GUARDSARM_DB_DAEMON, debug_mode=True)
    services.wait_expected_daemon_status(target_daemon=GUARDSARM_DB_DAEMON, running_condition=True)

    yield

    services.control_service('stop', daemon=GUARDSARM_DB_DAEMON)
    services.wait_expected_daemon_status(target_daemon=GUARDSARM_DB_DAEMON, running_condition=False)
    services.control_service('start', daemon=GUARDSARM_DB_DAEMON)
    services.wait_expected_daemon_status(target_daemon=GUARDSARM_DB_DAEMON, running_condition=True)
...
def test_wdb_backup_command(configure_wdb_socket_environment_module, connect_to_sockets_module, remove_backups,
    ...
    '''
    description: Check that every input message using the 'backup' command in guardsarm-manager-db socket generates
                 the proper output to guardsarm-manager-db socket. To do this, it performs a series of queries to the socket with
    ...
    parameters:
        - configure_wdb_socket_environment_module:
            type: fixture
            brief: Configure GuardSarm DB socket environment.
    ...
    input_description:
        - Test cases are defined in the guardsarm_db_backup_command.yaml file. This file contains the amount of backups to
