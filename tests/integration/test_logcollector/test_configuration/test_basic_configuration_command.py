...
brief: The 'guardsarm-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       correctly executed by the logcollector, and the GuardSarm API returns the same values for
...
references:
    ...
    - https://documentation.guardsarmsiem.com/current/user-manual/reference/ossec-conf/localfile.html#command
    - https://documentation.guardsarmsiem.com/current/user-manual/reference/ossec-conf/localfile.html#alias
...
config_path = Path(CONFIGURATIONS_PATH, 'guardsarm_basic_configuration_command.yaml')
...
def test_configuration_command(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
                            set_guardsarm_configuration, daemons_handler):
    '''
    description: Check if the 'guardsarm-logcollector' daemon can monitor commands that use multiple parameters.
                 ...
                 will verify that the GuardSarm API returns the same values for the 'localfile' section that
                 the configured one.
