...
from guardsarm.core import common, configuration
from guardsarm.core.cluster.cluster import get_node
from guardsarm.core.cluster.utils import manager_restart, manager_reload
from guardsarm.core.configuration import get_ossec_conf, write_ossec_conf
from guardsarm.core.exception import GuardSarmError, GuardSarmInternalError
from guardsarm.core.manager import status, get_api_conf, get_guardsarm_logs, \
    get_logs_summary, validate_ossec_conf, GUARDSARM_LOG_FIELDS
from guardsarm.core.results import AffectedItemsGuardSarmResult
from guardsarm.core.utils import process_array, safe_move, validate_guardsarm_xml, full_copy
from guardsarm.rbac.decorators import expose_resources, mask_sensitive_config

node_id = get_node().get('node')


@expose_resources(actions=['cluster:read'], resources=[f'node:id:{node_id}'])
def get_status() -> AffectedItemsGuardSarmResult:
    """Wrapper for status().
    ...
    AffectedItemsGuardSarmResult
        Affected items.
    """
    result = AffectedItemsGuardSarmResult(all_msg=f"Processes status was successfully read"
...
def ossec_log(level: str = None, tag: str = None, offset: int = 0, limit: int = common.DATABASE_LIMIT,
              ...
              q: str = '', select: str = None, distinct: bool = False) -> AffectedItemsGuardSarmResult:
    """Get logs from guardsarm-manager.log.
    ...
    AffectedItemsGuardSarmResult
        Affected items.
    """
    result = AffectedItemsGuardSarmResult(all_msg=f"Logs were successfully read"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
                                      some_msg='Could not read logs in some nodes',
                                      none_msg=f"Could not read logs"
                                               f"{' in specified node' if node_id != 'manager' else ''}"
                                      )
    logs = get_guardsarm_logs()
    ...
    data = process_array(logs, search_text=search_text, search_in_fields=search_in_fields,
                         ...
                         select=select, allowed_select_fields=GUARDSARM_LOG_FIELDS, distinct=distinct)
...
@expose_resources(actions=['cluster:read'], resources=[f'node:id:{node_id}'])
def ossec_log_summary() -> AffectedItemsGuardSarmResult:
    """Summary of guardsarm-manager.log.
    ...
    AffectedItemsGuardSarmResult
        Affected items.
    """
    result = AffectedItemsGuardSarmResult(all_msg=f"Log was successfully summarized"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
...
def get_api_config() -> AffectedItemsGuardSarmResult:
    ...
    """
    result = AffectedItemsGuardSarmResult(**_get_config_default_result_kwargs)
...
def restart() -> AffectedItemsGuardSarmResult:
    ...
    AffectedItemsGuardSarmResult
        Affected items.
    """
    result = AffectedItemsGuardSarmResult(**_restart_default_result_kwargs)
    try:
        manager_restart()
...
def reload() -> AffectedItemsGuardSarmResult:
    ...
    AffectedItemsGuardSarmResult
        Affected items.
    """
    result = AffectedItemsGuardSarmResult(**_reload_default_result_kwargs)
    try:
        manager_reload()
        result.affected_items.append(node_id)
...
def validation() -> AffectedItemsGuardSarmResult:
    ...
    AffectedItemsGuardSarmResult
        Affected items.
    """
    result = AffectedItemsGuardSarmResult(**_validation_default_result_kwargs)
...
@expose_resources(actions=["cluster:read"], resources=[f'node:id:{node_id}'])
def get_config(component: str = None, config: str = None) -> AffectedItemsGuardSarmResult:
    ...
    AffectedItemsGuardSarmResult
        Affected items.
    """
    result = AffectedItemsGuardSarmResult(all_msg=f"Active configuration was successfully read"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
...
def read_ossec_conf(section: str = None, field: str = None, raw: bool = False,
                    distinct: bool = False) -> AffectedItemsGuardSarmResult:
    ...
    AffectedItemsGuardSarmResult
        Affected items.
    """
    result = AffectedItemsGuardSarmResult(all_msg=f"Configuration was successfully read"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
...
@expose_resources(actions=['cluster:read'], resources=[f'node:id:{node_id}'])
def get_basic_info() -> AffectedItemsGuardSarmResult:
    """Wrapper for GuardSarm().to_dict
    ...
    AffectedItemsGuardSarmResult
        Affected items.
    """
    result = AffectedItemsGuardSarmResult(all_msg=f"Basic information was successfully read"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
                                      some_msg='Could not read basic information in some nodes',
    ...
    try:
        result.affected_items.append(GuardSarm().to_dict())
    except GuardSarmError as e:
        result.add_failed_item(id_=node_id, error=e)
    result.total_affected_items = len(result.affected_items)
...
def update_ossec_conf(new_conf: str = None) -> AffectedItemsGuardSarmResult:
    """Replace guardsarm configuration (guardsarm-manager.conf) with the provided configuration.
    ...
    AffectedItemsGuardSarmResult
        Affected items.
    """
    result = AffectedItemsGuardSarmResult(all_msg=f"Configuration was successfully updated"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
