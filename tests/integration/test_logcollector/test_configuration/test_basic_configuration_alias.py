...
brief: The 'guardsarm-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       specified in the 'alias' tag when monitoring a command, and the GuardSarm API returns the same
...
config_path = Path(CONFIGURATIONS_PATH, 'guardsarm_basic_configuration.yaml')
...
def test_configuration_alias(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
    ...
    '''
    description: Check if the 'guardsarm-logcollector' daemon changes a command name in the log messages by
    ...
    input_description: A configuration template (test_basic_configuration_alias) is contained in an external YAML file
                       (guardsarm_basic_configuration.yaml). That template is combined with two test cases defined
                       in the module. Those include configuration settings for the 'guardsarm-logcollector' daemon.
    ...
    guardsarm_log_monitor.start(callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_READING_COMMAND_ALIAS, {
