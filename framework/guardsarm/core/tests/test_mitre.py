...
from guardsarm.tests.util import InitWDBSocketMock

with patch('guardsarm.core.common.guardsarm_uid'):
    with patch('guardsarm.core.common.guardsarm_gid'):
        from guardsarm.core.mitre import *


@patch('guardsarm.core.utils.GuardSarmDBConnection', return_value=InitWDBSocketMock(sql_schema_file='schema_mitre_test.sql'))
def test_GuardSarmDBQueryMitreMetadata(mock_wdb):
    """Verify that the method connects correctly to the database and returns the correct type."""
    db_query = GuardSarmDBQueryMitreMetadata()
    data = db_query.run()

    assert isinstance(db_query, GuardSarmDBQueryMitre) and isinstance(data, dict)


@pytest.mark.parametrize('wdb_query_class', [
    GuardSarmDBQueryMitreGroups,
    GuardSarmDBQueryMitreMitigations,
    GuardSarmDBQueryMitreReferences,
    GuardSarmDBQueryMitreTactics,
    GuardSarmDBQueryMitreTechniques,
    GuardSarmDBQueryMitreSoftware

])
@patch('guardsarm.core.utils.GuardSarmDBConnection', return_value=InitWDBSocketMock(sql_schema_file='schema_mitre_test.sql'))
def test_GuardSarmDBQueryMitre_classes(mock_wdb, wdb_query_class):
...
@patch('guardsarm.core.utils.GuardSarmDBConnection')
