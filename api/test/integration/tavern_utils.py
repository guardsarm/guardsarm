    ...
    for agent_id in agents_list:
        ...
        while tries < 80:
            try:
                ...
                command = f"docker exec env-guardsarm-agent{int(agent_id)}-1 grep agentd /var/ossec/logs/ossec.log"
            ...
            # Check the log indicating agentd started is in the agent's guardsarm log file (after the restart request)
            ...
            raise ProcessLookupError("The guardsarm-agentd daemon was not started after requesting the restart")
...
def check_agent_active_status(agents_list):
    ...
    while tries < 25:
        try:
            # Get active agents
            output = subprocess.check_output(f"docker exec env-guardsarm-master-1 /var/guardsarm-manager/framework/python/bin/python3 "
