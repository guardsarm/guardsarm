#ifndef _BASE_LIBGUARDSARMSHARED_HPP
#define _BASE_LIBGUARDSARMSHARED_HPP
...
/**
 * @brief Namespace for managing the libguardsarmshared dynamic library and its functions.
 *
 * This namespace provides utilities to initialize, manage, and access functions from
 * the libguardsarmshared dynamic library. It includes functionality for library lifecycle
 * management, dynamic function loading, and wrapper functions for common operations.
 */
namespace base::libguardsarmshared
{
 ...
 * It loads the libguardsarmshared dynamic library and prepares it for use.
 */
void init();
...
FuncType getFunction(std::string_view name)
{
    if (!getLibPtr())
    {
        throw std::runtime_error("libguardsarmshared is not initialized.");
    }
...
#endif // _BASE_LIBGUARDSARMSHARED_HPP
