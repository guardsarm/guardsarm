...
```
openssl req -new -nodes -newkey rsa:2048 -keyout wpkcert.key -out wpkcert.csr -subj '/C=US/ST=CA/O=GuardSarm'
```

- `/C=US` is the country.
- `/ST=CA` is the state.
- `/O=GuardSarm` is the organization's name.
...
```
curl -Lo guardsarm-3.0.zip https://github.com/guardsarm/guardsarm/archive/3.0.zip
unzip guardsarm-3.0.zip
```

2. Compile the project:

```
make -C guardsarm-3.0/src TARGET=agent
```
...
In this particular case, the GuardSarm Project's root directory contains the proper `upgrade.sh` file.
...
1. Overwrite the shipped root CA with your certificate. This will prevent your agent from upgrading using WPK packages from GuardSarm.

```
cp /path/to/certificate etc/wpk_root.pem
```

2. Add a new certificate by editing the `gsmsec.conf` file:

```
<active-response>
  <ca_store>/var/guardsarm-manager/etc/wpk_root.pem</ca_store>
...
Get the WPK package into the GuardSarm manager and run:

```
/var/guardsarm-manager/bin/agent_upgrade -a 001 -f path/to/myagent.wpk -x upgrade.sh
```
...
> guardsarm_agent_v3.0.0-beta7_centos_7_x86_64.wpk
...
```
/var/guardsarm-manager/bin/agent_upgrade -a 001 -r https://example.com/repo
```
