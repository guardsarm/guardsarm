...
This document describes the event formats generated by the Syscollector module in GuardSarm. These events use GuardSarm's custom field structure for inventory data.
...
```json
{
    ...
    {
        "browser_name": "chrome",
        "browser_profile_name": "Nico GuardSarm",
...
Syscollector events are processed by the Inventory Harvester and stored directly in GuardSarm Indexer without going through traditional rule processing, since they represent inventory state rather than security events.

## Integration

Access syscollector inventory data through:

- **GuardSarm API**: REST endpoints for querying current inventory state
- **GuardSarm Dashboard**: Built-in inventory visualization
- **Direct Database**: Query GuardSarm Indexer for raw event data
