...
int gm_ms_graph_read(const OS_XML* xml, xml_node** nodes, gmodule* module) {
	...
	gm_ms_graph* ms_graph;

	if (!nodes) {
		merror("Empty configuration found in module '%s'.", GM_MS_GRAPH_CONTEXT.name);
		return OS_CFGERR;
	}

	// Init module
	os_malloc(sizeof(gm_ms_graph), ms_graph);
	ms_graph->enabled = GM_MS_GRAPH_DEFAULT_ENABLED;
	ms_graph->only_future_events = GM_MS_GRAPH_DEFAULT_ONLY_FUTURE_EVENTS;
	ms_graph->curl_max_size = GM_MS_GRAPH_DEFAULT_CURL_MAX_SIZE;
	ms_graph->page_size = GM_MS_GRAPH_ITEM_PER_PAGE;
	ms_graph->time_delay = GM_MS_GRAPH_DEFAULT_DELAY;
	ms_graph->run_on_start = GM_MS_GRAPH_DEFAULT_RUN_ON_START;
	os_strdup(GM_MS_GRAPH_DEFAULT_VERSION, ms_graph->version);

	sched_scan_init(&(ms_graph->scan_config));
	ms_graph->scan_config.interval = GM_DEF_INTERVAL;
	gm_ms_graph_auth *it;
	int tenant = 0;

	os_calloc(1, sizeof(gm_ms_graph_auth*), ms_graph->auth_config);
	os_malloc(sizeof(gm_ms_graph_resource) * 2, ms_graph->resources);
	ms_graph->num_resources = 0;

	module->context = &GM_MS_GRAPH_CONTEXT;
	...
	for (int i = 0; nodes[i]; i++) {
		...
		if (!nodes[i]->element) {
			...
			if (!strcmp(nodes[i]->content, "yes")) {
				...
				merror("Invalid content for tag '%s' at module '%s'.", XML_ENABLED, GM_MS_GRAPH_CONTEXT.name);
				return OS_CFGERR;
			}
			...
			if (!strcmp(nodes[i]->content, "yes")) {
				...
				merror("Invalid content for tag '%s' at module '%s'.", XML_ONLY_FUTURE_EVENTS, GM_MS_GRAPH_CONTEXT.name);
				return OS_CFGERR;
			}
			...
			if (ms_graph->curl_max_size < 1024L) {
				merror("Invalid content for tag '%s' at module '%s'. The minimum value allowed is 1KB.", XML_CURL_MAX_SIZE, GM_MS_GRAPH_CONTEXT.name);
				return OS_CFGERR;
			}
		} else if (!strcmp(nodes[i]->element, XML_TIME_DELAY)) {
            ms_graph->time_delay = w_parse_time(nodes[i]->content);
            if (ms_graph->time_delay < 0) {
                merror("Invalid content for tag '%s' at module '%s'.", XML_TIME_DELAY, GM_MS_GRAPH_CONTEXT.name);
                return OS_CFGERR;
            }
        } else if (!strcmp(nodes[i]->element, XML_PAGE_SIZE)) {
			ms_graph->page_size = atol(nodes[i]->content);
			if (ms_graph->page_size < 1) {
				merror("Invalid content for tag '%s' at module '%s'. The minimum value allowed is 1.", XML_PAGE_SIZE, GM_MS_GRAPH_CONTEXT.name);
				return OS_CFGERR;
			}
			...
			if (!strcmp(nodes[i]->content, "yes")) {
				...
				merror("Invalid content for tag '%s' at module '%s'.", XML_RUN_ON_START, GM_MS_GRAPH_CONTEXT.name);
				return OS_CFGERR;
			}
			...
			if (!strcmp(nodes[i]->content, "v1.0") || !strcmp(nodes[i]->content, "beta")) {
				...
				merror("Invalid content for tag '%s' at module '%s'.", XML_VERSION, GM_MS_GRAPH_CONTEXT.name);
				return OS_CFGERR;
			}
		} else if (!strcmp(nodes[i]->element, XML_API_AUTH)) {
			if (!(children = OS_GetElementsbyNode(xml, nodes[i]))) {
				OS_ClearNode(children);
				merror("Empty content for tag '%s' at module '%s'.", XML_API_AUTH, GM_MS_GRAPH_CONTEXT.name);
				return OS_CFGERR;
			}

			os_realloc(ms_graph->auth_config, sizeof(gm_ms_graph_auth*) * (tenant + 2), ms_graph->auth_config);
			os_calloc(1, sizeof(gm_ms_graph_auth), ms_graph->auth_config[tenant]);
			...
			for (int j = 0; children[j]; j++) {
				if (!strcmp(children[j]->element, XML_CLIENT_ID)) {
					if (strlen(children[j]->content) > 0 ) {
						...
						merror("Empty content for tag '%s' at module '%s'.", XML_CLIENT_ID, GM_MS_GRAPH_CONTEXT.name);
						OS_ClearNode(children);
						return OS_CFGERR;
					}
					...
					if (strlen(children[j]->content) > 0) {
						...
						merror("Empty content for tag '%s' at module '%s'.", XML_TENANT_ID, GM_MS_GRAPH_CONTEXT.name);
						OS_ClearNode(children);
						return OS_CFGERR;
					}
					...
					if (strlen(children[j]->content) > 0) {
						...
						merror("Empty content for tag '%s' at module '%s'.", XML_SECRET_VALUE, GM_MS_GRAPH_CONTEXT.name);
						OS_ClearNode(children);
						return OS_CFGERR;
					}
				} else if (!strcmp(children[j]->element, XML_API_TYPE)) {
                    if (strlen(children[j]->content) == 0) {
						merror("Empty content for tag '%s' at module '%s'.", XML_API_TYPE, GM_MS_GRAPH_CONTEXT.name);
						OS_ClearNode(children);
						return OS_CFGERR;
                    } else if (!strcmp(children[j]->content, "global")) {
                        os_free(it->login_fqdn);
                        os_strdup(GM_MS_GRAPH_GLOBAL_API_LOGIN_FQDN, it->login_fqdn);
                        os_free(it->query_fqdn);
                        os_strdup(GM_MS_GRAPH_GLOBAL_API_QUERY_FQDN, it->query_fqdn);
                    } else if (!strcmp(children[j]->content, "gcc-high")) {
                        os_free(it->login_fqdn);
                        os_strdup(GM_MS_GRAPH_GCC_HIGH_API_LOGIN_FQDN, it->login_fqdn);
                        os_free(it->query_fqdn);
                        os_strdup(GM_MS_GRAPH_GCC_HIGH_API_QUERY_FQDN, it->query_fqdn);
                    } else if (!strcmp(children[j]->content, "dod")) {
                        os_free(it->login_fqdn);
                        os_strdup(GM_MS_GRAPH_DOD_API_LOGIN_FQDN, it->login_fqdn);
                        os_free(it->query_fqdn);
                        os_strdup(GM_MS_GRAPH_DOD_API_QUERY_FQDN, it->query_fqdn);
                    } else {
						merror("Invalid content for tag '%s' at module '%s'.", XML_API_TYPE, GM_MS_GRAPH_CONTEXT.name);
                        OS_ClearNode(children);
                        return OS_CFGERR;
                    }
				} else {
					merror(XML_INVATTR, children[j]->element, GM_MS_GRAPH_CONTEXT.name);
					OS_ClearNode(children);
					return OS_CFGERR;
				}
			}
			...
			if (!(children = OS_GetElementsbyNode(xml, nodes[i]))) {
				merror("Empty content for tag '%s' at module '%s'.", XML_RESOURCE, GM_MS_GRAPH_CONTEXT.name);
				OS_ClearNode(children);
				return OS_CFGERR;
			}
			...
			if (ms_graph->num_resources > 1 && !(ms_graph->num_resources & (ms_graph->num_resources - 1))) {
				os_realloc(ms_graph->resources, (ms_graph->num_resources * 2) * sizeof(gm_ms_graph_resource), ms_graph->resources);
			}
			...
			for (int j = 0; children[j]; j++) {
				if (!strcmp(children[j]->element, XML_RESOURCE_NAME)) {
					if (strlen(children[j]->content) > 0) {
						...
						merror("Empty content for tag '%s' at module '%s'.", XML_RESOURCE_NAME, GM_MS_GRAPH_CONTEXT.name);
						OS_ClearNode(children);
						return OS_CFGERR;
					}
					...
					if (strlen(children[j]->content) > 0) {
						...
						merror("Empty content for tag '%s' at module '%s'.", XML_RESOURCE_RELATIONSHIP, GM_MS_GRAPH_CONTEXT.name);
						OS_ClearNode(children);
						return OS_CFGERR;
					}
				} else {
			...
			merror(XML_INVATTR, nodes[i]->element, GM_MS_GRAPH_CONTEXT.name);
			return OS_CFGERR;
		}
	}

    if (sched_scan_read(&(ms_graph->scan_config), nodes, module->context->name) != 0) {
		merror("Invalid content for tag '%s' at module '%s'.", XML_INTERVAL, GM_MS_GRAPH_CONTEXT.name);
        return OS_INVALID;
    }
