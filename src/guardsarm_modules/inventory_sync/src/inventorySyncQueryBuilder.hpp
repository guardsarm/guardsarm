/*
 * GuardSarm inventory sync - Query builders for indexer operations
...
namespace InventorySyncQueryBuilder
{
                                                   ...
                                                   uint64_t globalVersion)
    {
        ...
        // Build the query: match guardsarm.agent.id AND only update if version <= globalVersion (external_gte behavior)
        updateQuery["query"]["bool"]["must"][0]["term"]["guardsarm.agent.id"] = agentId;
        ...
        std::string script = "ctx._source.guardsarm.agent.id = params.id; "
                             "ctx._source.guardsarm.agent.name = params.name; "
                             "ctx._source.guardsarm.agent.version = params.version; "
                             "if (ctx._source.guardsarm.agent.host == null) { ctx._source.guardsarm.agent.host = [:]; } "
                             "ctx._source.guardsarm.agent.host.architecture = params.architecture; "
                             "ctx._source.guardsarm.agent.host.hostname = params.hostname; "
                             "if (ctx._source.guardsarm.agent.host.os == null) { ctx._source.guardsarm.agent.host.os = [:]; } "
                             "ctx._source.guardsarm.agent.host.os.name = params.osname; "
                             "ctx._source.guardsarm.agent.host.os.platform = params.osplatform; "
                             "ctx._source.guardsarm.agent.host.os.type = params.ostype; "
                             "ctx._source.guardsarm.agent.host.os.version = params.osversion; "
    ...
    buildGroupsUpdateQuery(const std::string& agentId, const std::vector<std::string>& groups, uint64_t globalVersion)
    {
        ...
        std::string script = "ctx._source.guardsarm.agent.groups = params.groups; "
                             "if (ctx._source.state == null) { ctx._source.state = [:]; } "
                             "ctx._source.state.document_version = params.globalVersion; "
                             "ctx._source.state.modified_at = params.timestamp;";
                                                  ...
                                                  const std::string& osversion)
    {
        ...
        // Build the query: match guardsarm.agent.id only
        updateQuery["query"]["bool"]["must"][0]["term"]["guardsarm.agent.id"] = agentId;
        ...
        std::string script =
            ...
            "if (ctx._source.guardsarm.agent.id != params.id) { needsUpdate = true; } "
            "if (ctx._source.guardsarm.agent.name != params.name) { needsUpdate = true; } "
            "if (ctx._source.guardsarm.agent.version != params.version) { needsUpdate = true; } "
            "if (ctx._source.guardsarm.agent?.host?.architecture != params.architecture) { needsUpdate = true; } "
            "if (ctx._source.guardsarm.agent?.host?.hostname != params.hostname) { needsUpdate = true; } "
            "if (ctx._source.guardsarm.agent?.host?.os?.name != params.osname) { needsUpdate = true; } "
            "if (ctx._source.guardsarm.agent?.host?.os?.platform != params.osplatform) { needsUpdate = true; } "
            "if (ctx._source.guardsarm.agent?.host?.os?.type != params.ostype) { needsUpdate = true; } "
            "if (ctx._source.guardsarm.agent?.host?.os?.version != params.osversion) { needsUpdate = true; } "
            "if (!needsUpdate) { ctx.op = 'noop'; } else { "
            "  ctx._source.guardsarm.agent.id = params.id; "
            "  ctx._source.guardsarm.agent.name = params.name; "
            "  ctx._source.guardsarm.agent.version = params.version; "
            "  if (ctx._source.guardsarm.agent.host == null) { ctx._source.guardsarm.agent.host = [:]; } "
            "  ctx._source.guardsarm.agent.host.architecture = params.architecture; "
    ...
    inline nlohmann::json buildGroupsCheckQuery(const std::string& agentId, const std::vector<std::string>& groups)
    {
        ...
        // Note: Does NOT update document_version or timestamp - this is disaster recovery
        std::string script = "boolean needsUpdate = false; "
                             "def currentGroups = ctx._source.guardsarm.agent?.groups; "
                                           ...
                                           const std::string& searchAfter = "")
    {
        ...
        must.push_back({{"term", {{"guardsarm.agent.id", agentId}}}});
    ...
    ///   for a given agent from the guardsarm-states-vulnerabilities index.
    ///
    ///   The query:
    ///   - Filters by guardsarm.agent.id
    ...
    buildContextGetQuery(const std::string& agentId, std::size_t size, const std::string& searchAfter = "")
    {
        nlohmann::json query = {{"_source", nlohmann::json::array({"vulnerability.id", "vulnerability.score"})},
                                {"query", {{"term", {{"guardsarm.agent.id", agentId}}}}},
                                {"size", size},
                                {"sort", {{{"_id", {{"order", "asc"}}}}}}};
