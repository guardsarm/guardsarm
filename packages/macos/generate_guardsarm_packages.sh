#!/bin/bash
set -e
# Program to build and package OSX guardsarm-agent
...
ARCH="intel64"
GUARDSARM_SOURCE_REPOSITORY="https://github.com/guardsarm/guardsarm"
SERVICE_PATH="/Library/LaunchDaemons/com.guardsarm.agent.plist"
STARTUP_PATH="/Library/StartupItems/GUARDSARM/StartupParameters.plist"
LAUNCHER_SCRIPT_PATH="/Library/StartupItems/GUARDSARM/GuardSarm-launcher"
STARTUP_SCRIPT_PATH="/Library/StartupItems/GUARDSARM/GUARDSARM"
INSTALLATION_PATH="/Library/gsmsec"    # Installation path.
...
function clean_and_exit() {
    ...
    if [ -z "$BRANCH_TAG" ]; then
        make -C $GUARDSARM_PATH/src clean clean-deps
    fi
...
function sign_binaries() {
    if [ ! -z "${KEYCHAIN}" ] && [ ! -z "${CERT_APPLICATION_ID}" ] ; then
        ...
        # Sign every single binary in GuardSarm's installation. This also includes library files.
...
function prepare_building_folder() {
    ...
    version="$1"
    pkg_final_name="$2"
    build_info_file="${GUARDSARM_PACKAGES_PATH}/specs/build-info.json"
    preinstall_script="${GUARDSARM_PACKAGES_PATH}/package_files/preinstall.sh"
    postinstall_script="${GUARDSARM_PACKAGES_PATH}/package_files/postinstall.sh"
    packaged_directory=$CURRENT_PATH/guardsarm-agent/payload

    if [ -d "$CURRENT_PATH/guardsarm-agent" ]; then

        echo "\nThe guardsarm agent building directory is present on this machine."
        echo "Removing it from the system."

        rm -rf $CURRENT_PATH/guardsarm-agent
    fi

    munkipkg --create --json $CURRENT_PATH/guardsarm-agent

    cp -f $build_info_file $CURRENT_PATH/guardsarm-agent/

    sed -i '' "s|VERSION|$version|g" $CURRENT_PATH/guardsarm-agent/$(basename $build_info_file)
    sed -i '' "s|PACKAGE_NAME|$pkg_final_name|g" $CURRENT_PATH/guardsarm-agent/$(basename $build_info_file)

    cp $preinstall_script $CURRENT_PATH/guardsarm-agent/scripts/preinstall
    cp $postinstall_script $CURRENT_PATH/guardsarm-agent/scripts/postinstall

    sed -i '' "s|PACKAGE_ARCH|$ARCH|g" $CURRENT_PATH/guardsarm-agent/scripts/preinstall
...
function build_package() {
    ...
    if [ -n "$BRANCH_TAG" ]; then
        ...
        GUARDSARM_PATH="${SOURCES_DIRECTORY}/guardsarm"
        git clone --depth=1 -b ${BRANCH_TAG} ${GUARDSARM_SOURCE_REPOSITORY} "${GUARDSARM_PATH}"
    else
        GUARDSARM_PATH="${CURRENT_PATH}/../.."
    fi
    short_commit_hash="$(cd "${GUARDSARM_PATH}" && git rev-parse --short=7 HEAD)"

    export CONFIG="${GUARDSARM_PATH}/etc/preloaded-vars.conf"
    GUARDSARM_PACKAGES_PATH="${GUARDSARM_PATH}/packages/macos"
    ENTITLEMENTS_PATH="${GUARDSARM_PACKAGES_PATH}/entitlements.plist"

    VERSION="$(awk -F'"' '/"version"[ \t]*:/ {print $4}' $GUARDSARM_PATH/VERSION.json)"

    # Define output package name
    if [ $IS_STAGE == "no" ]; then
        pkg_name="guardsarm-agent_${VERSION}-${REVISION}_${ARCH}_${short_commit_hash}"
    else
        pkg_name="guardsarm-agent-${VERSION}-${REVISION}.${ARCH}"
    fi

    if [ -d "${INSTALLATION_PATH}" ]; then

        echo "\nThe guardsarm agent is already installed on this machine."
        echo "Removing it from the system."

        ${CURRENT_PATH}/uninstall.sh
    fi

    ${GUARDSARM_PACKAGES_PATH}/package_files/build.sh "${INSTALLATION_PATH}" "${GUARDSARM_PATH}" ${JOBS} ${DEBUG} ${MAKE_COMPILATION}
    ...
    # create package
    if munkipkg $CURRENT_PATH/guardsarm-agent ; then
        echo "The guardsarm agent package for macOS has been successfully built."
        mv $CURRENT_PATH/guardsarm-agent/build/* $DESTINATION/
        symbols_pkg_name="guardsarm-agent-debug-symbols-${VERSION}-${REVISION}.${ARCH}-macos"
        cp -R "${GUARDSARM_PATH}/src/symbols"  "${DESTINATION}"
