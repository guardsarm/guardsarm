...
void test_set_auditd_config_audit3_plugin_created(void **state) {
    ...
    const char *audit3_socket = "/etc/audit/plugins.d/af_guardsarm.conf";
...
void test_set_auditd_config_audit2_plugin_created(void **state) {
    ...
    // Plugin already created
    const char *audit2_socket = "/etc/audisp/plugins.d/af_guardsarm.conf";
...
void test_set_auditd_config_audit_plugin_tampered_configuration(void **state) {
    ...
    expect_string(__wrap__minfo, formatted_msg, "(6024): Generating Auditd socket configuration file: 'tmp/af_guardsarm.conf'");
    ...
    expect_string(__wrap_wfopen, path, "tmp/af_guardsarm.conf");
    ...
    // Move plugin
    expect_string(__wrap_OS_MoveFile, src, "tmp/af_guardsarm.conf");
    expect_string(__wrap_OS_MoveFile, dst, audit3_socket);
    will_return(__wrap_OS_MoveFile, 0);

    expect_string(__wrap__minfo, formatted_msg, "(6025): Audit plugin configuration (/etc/audit/plugins.d/af_guardsarm.conf) was modified. Restarting Auditd service.");
...
void test_set_auditd_config_audit_plugin_not_created_fopen_error(void **state) {
    ...
    expect_string(__wrap__merror, formatted_msg, "(1103): Could not open file 'tmp/af_guardsarm.conf' due to [(0)-(Success)].");
...
void test_set_auditd_config_audit_plugin_not_created_fclose_error(void **state) {
    ...
    expect_string(__wrap__merror, formatted_msg, "(1140): Could not close file 'tmp/af_guardsarm.conf' due to [(0)-(Success)].");
...
void test_set_auditd_config_audit_plugin_not_created_recreate_hardlink_error(void **state) {
    ...
    expect_string(__wrap__merror, formatted_msg, "Failed to move 'tmp/af_guardsarm.conf' to '/etc/audit/plugins.d/af_guardsarm.conf'");
...
void test_audit_read_events_select_success_recv_error_audit_connection_closed(void **state) {
    ...
    expect_string(__wrap_SendMSG, message, "guardsarm: Audit: Connection closed");
...
void test_audit_read_events_select_success_recv_success(void **state) {
    ...
    char * buffer = " \
        ...
        type=SYSCALL msg=audit(1571914029.306:3004255): arch=c000003e syscall=263 success=yes exit=0 a0=ffffff9c a1=55c5f8170490 a2=0 a3=7ff365c5eca0 items=2 ppid=3211 pid=44082 auid=4294967295 uid=0 gid=0 euid=0 suid=0 fsuid=0 egid=0 sgid=0 fsgid=0 tty=pts3 ses=5 comm=\"test\" exe=\"74657374C3B1\" key=\"guardsarm_fim\"\n\
...
void test_audit_create_rules_file(void **state) {
    ...
    expect_string(__wrap_fprintf, formatted_msg, "-w /test0 -p wa -k guardsarm_fim\n");
    will_return(__wrap_fprintf, 0);

    expect_string(__wrap__mdebug2, formatted_msg, "(6365): Added directory '/test1' to audit rules file.");

    expect_any(__wrap_fprintf, __stream);
    expect_string(__wrap_fprintf, formatted_msg, "-w /test1 -p wa -k guardsarm_fim\n");
    will_return(__wrap_fprintf, 0);
