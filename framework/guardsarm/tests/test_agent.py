...
with patch('guardsarm.core.common.guardsarm_uid'):
    with patch('guardsarm.core.common.guardsarm_gid'):
        ...
        from guardsarm.agent import add_agent, assign_agents_to_group, create_group, delete_agents, delete_groups, \
        ...
        from guardsarm.core.agent import Agent
        from guardsarm import GuardSarmError, GuardSarmException, GuardSarmInternalError
        ...
        from guardsarm.core.exception import GuardSarmResourceNotFound
        from guardsarm.core.wdb_http import AgentsSummary
...
@patch('guardsarm.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
...
def test_agent_get_distinct_agents(socket_mock, send_mock, fields, expected_items):
    ...
    assert isinstance(distinct, AffectedItemsGuardSarmResult), 'The returned object is not an "AffectedItemsGuardSarmResult".'
...
def test_agent_sort_order(socket_mock, send_mock, fields, order, expected_items):
    ...
    assert isinstance(sorted_agents, AffectedItemsGuardSarmResult), 'The returned object is not an ' \
...
def test_agent_get_agents_summary_status(socket_mock, send_mock):
    ...
    assert isinstance(summary, GuardSarmResult), 'The returned object is not an "GuardSarmResult" instance.'
...
def test_agent_get_agents_summary_os(connect_mock, send_mock):
    ...
    assert isinstance(summary, AffectedItemsGuardSarmResult), 'The returned object is not an "GuardSarmResult" instance.'
...
@patch('guardsarm.core.agent.Agent.reconnect')
@patch('guardsarm.agent.get_agents_info', return_value=short_agent_list)
@patch('guardsarm.core.wdb.GuardSarmDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect_agents(socket_mock, send_mock, agents_info_mock, reconnect_mock, agent_list, expected_items,
    ...
    assert isinstance(result, AffectedItemsGuardSarmResult), 'The returned object is not an "AffectedItemsGuardSarmResult".'
...
@patch('guardsarm.agent.send_restart_command')
@patch('guardsarm.agent.get_agents_info', return_value=set(short_agent_list))
@patch('guardsarm.core.wdb_http.GuardSarmDBHTTPClient._post', side_effect=send_msg_to_wdb_http_post_restartinfo)
@patch('socket.socket.connect')
...
@patch('guardsarm.agent.send_reload_command')
@patch('guardsarm.agent.get_agents_info', return_value=set(short_agent_list))
@patch('guardsarm.core.wdb_http.GuardSarmDBHTTPClient._post', side_effect=send_msg_to_wdb_http_post_restartinfo)
@patch('socket.socket.connect')
...
@patch('guardsarm.agent.get_agents')
@patch('guardsarm.agent.get_groups')
@patch('guardsarm.core.wdb.GuardSarmDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@pytest.mark.parametrize('group, q, expected_q', [
    ('default', '(name~guardsarm,status~active)', 'group=default;((name~guardsarm,status~active))'),
    ('default', 'name~guardsarm,status~active', 'group=default;(name~guardsarm,status~active)')
])
@patch('guardsarm.agent.get_agents')
@patch('guardsarm.agent.get_groups')
...
@pytest.mark.parametrize('agent_list, filters, q, error_code, expected_items', [
    ...
    (full_agent_list, {'status': 'all', 'older_than': '1s', 'version': 'GuardSarm v4.2.0'}, None, 1731, ['002']),
    ...
    (['001', '002'], {'status': 'all', 'older_than': '1s'}, None, GuardSarmError(1726), None),
])
@patch('guardsarm.agent.Agent.remove')
@patch('guardsarm.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('guardsarm.core.wdb.GuardSarmDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_delete_agents(socket_mock, send_mock, mock_remove, agent_list, filters, q, error_code, expected_items):
    ...
    """
    if not isinstance(error_code, GuardSarmException):
...
@patch('guardsarm.core.agent.get_manager_status', return_value={'guardsarm-manager-authd': 'running'})
...
@patch('guardsarm.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('guardsarm.core.common.SHARED_PATH', new=test_shared_path)
@patch('guardsarm.core.wdb.GuardSarmDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@pytest.mark.parametrize('shared_path, group_list, group_exists, side_effect, expected_exception', [
    (test_shared_path, ['none'], False, None, GuardSarmResourceNotFound(1710)),
    ('invalid-path', ['default'], True, None, GuardSarmError(1006)),
    (test_shared_path, ['default'], True, GuardSarmError(1405), GuardSarmError(1405)),
    (test_shared_path, ['default'], True, GuardSarmException(1400), GuardSarmInternalError(1727))
])
@patch('guardsarm.agent.process_array')
@patch('guardsarm.core.agent.Agent.group_exists')
def test_agent_get_group_files_exceptions(mock_group_exists, mock_process_array, shared_path, group_list, group_exists,
    ...
    """
    with patch('guardsarm.core.common.SHARED_PATH', new=shared_path):
        ...
        except (GuardSarmError, GuardSarmInternalError) as e:
...
@patch('guardsarm.core.common.SHARED_PATH', new=test_shared_path)
@patch('guardsarm.core.common.guardsarm_gid', return_value=getgrnam('root'))
@patch('guardsarm.core.common.guardsarm_uid', return_value=getpwnam('root'))
@patch('guardsarm.agent.chown_r')
def test_create_group(chown_mock, uid_mock, gid_mock, group_id):
    ...
    try:
        ...
        assert isinstance(result, GuardSarmResult), 'The returned object is not an "GuardSarmResult" instance.'
...
@pytest.mark.parametrize('group_id, exception, exception_code', [
    ('default', GuardSarmError, 1711),
    ('group-1', GuardSarmError, 1711),
    ('invalid!', GuardSarmError, 1722),
    ('delete-me', GuardSarmInternalError, 1005),
    ('agent-template.conf', GuardSarmError, 1713)
])
@patch('guardsarm.core.common.SHARED_PATH', new=test_shared_path)
def test_create_group_exceptions(group_id, exception, exception_code):
    ...
    exception_code : int
        Expected error code for the GuardSarm Exception object raised by `get_group_files` with the given parameters.
    """
    try:
...
@patch('guardsarm.agent.get_groups')
@patch('guardsarm.agent.Agent.delete_single_group')
...
@pytest.mark.parametrize('group_list, expected_errors', [
    (['none-1'], [GuardSarmResourceNotFound(1710)]),
    (['default'], [GuardSarmError(1712)]),
    (['none-1', 'none-2'], [GuardSarmResourceNotFound(1710)]),
    (['default', 'none-1'], [GuardSarmError(1712), GuardSarmResourceNotFound(1710)]),
])
@patch('guardsarm.agent.get_groups')
def test_agent_delete_groups_other_exceptions(mock_get_groups, group_list, expected_errors):
    ...
    group_list : List of str
        List of groups to be deleted.
    expected_errors : list of GuardSarmError
        List of expected GuardSarmError to be raised by delete_groups if a group is not valid. An exception will be returned
...
@patch('guardsarm.agent.Agent.add_group_to_agent')
@patch('guardsarm.core.wdb.GuardSarmDBConnection._send', side_effect=send_msg_to_wdb)
...
@pytest.mark.parametrize('group_list, agent_list, expected_error, catch_exception', [
    (['none-1'], ['001'], GuardSarmResourceNotFound(1710), True),
    (['group-1'], ['100'], GuardSarmResourceNotFound(1701), False),
])
@patch('guardsarm.agent.Agent.group_exists')
@patch('guardsarm.agent.Agent.add_group_to_agent')
@patch('guardsarm.core.wdb.GuardSarmDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
async def test_agent_assign_agents_to_group_exceptions(socket_mock, send_mock, mock_add_group, mock_group_exists, group_list,
    ...
    catch_exception : bool
        ...
        `AffectedItemsGuardSarmResult` containing the exceptions in its 'failed_items'.
    """
    ...
    except GuardSarmException as ex:
...
@patch('guardsarm.core.agent.Agent.unset_single_group_agent')
@patch('guardsarm.agent.get_groups')
@patch('guardsarm.agent.get_agents_info')
...
@pytest.mark.parametrize('group_id, agent_id, expected_error', [
    ('any-group', '100', GuardSarmResourceNotFound(1701)),
    ('group-1', '005', GuardSarmResourceNotFound(1710)),
])
@patch('guardsarm.agent.get_agents_info', return_value=short_agent_list)
@patch('guardsarm.agent.get_groups', side_effect={'default'})
async def test_agent_remove_agent_from_group_exceptions(group_mock, agents_info_mock, group_id, agent_id, expected_error):
    ...
    expected_error : GuardSarmError
        The GuardSarmError object expected to be raised by remove_agent_from_group with the given parameters.
    """
    try:
    ...
    except (GuardSarmError, GuardSarmResourceNotFound) as error:
...
@patch('guardsarm.core.agent.Agent.unset_single_group_agent')
@patch('guardsarm.agent.get_agents_info', return_value=short_agent_list)
@patch('guardsarm.agent.get_groups', return_value={'group-1'})
...
@pytest.mark.parametrize('group_list, agent_list, expected_error, catch_exception', [
    (['any-group'], ['100'], GuardSarmResourceNotFound(1701), True),
    (['any-group'], ['005'], GuardSarmResourceNotFound(1710), False),
])
@patch('guardsarm.core.agent.Agent.unset_single_group_agent')
@patch('guardsarm.agent.get_agents_info', return_value=short_agent_list)
@patch('guardsarm.agent.get_groups', return_value={'group-1'})
async def test_agent_remove_agent_from_groups_exceptions(mock_get_groups, mock_get_agents, mock_unset, group_list, agent_list,
    ...
    try:
        ...
        assert not catch_exception, \
            'An "GuardSarmError" exception was expected but was not raised.'
        # Check Typing
        assert isinstance(result, AffectedItemsGuardSarmResult), 'The returned object is not an "AffectedItemsGuardSarmResult".'
        assert isinstance(result.failed_items, dict), \
        ...
        assert catch_exception, \
            'No exception should be raised at this point. An AffectedItemsGuardSarmResult object with at least one ' \
...
@patch('guardsarm.agent.get_agents_info', return_value=set(full_agent_list))
@patch('guardsarm.agent.GuardSarm')
@patch('guardsarm.core.wdb.GuardSarmDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_outdated_agents(socket_mock, send_mock, mock_guardsarm, mock_get_agents_info):
    ...
    mock_guardsarm.return_value.to_dict.return_value = {'version': 'v5.0.0'}

    outdated_agents = ['001', '002', '005']
    result = get_outdated_agents(agent_list=short_agent_list)
    # Check typing
    assert isinstance(result, AffectedItemsGuardSarmResult)
    assert isinstance(result.affected_items, list)
    # Check affected items
...
@pytest.mark.parametrize('agent_set, expected_errors_and_items, result_from_socket, filters, raise_error', [
    ...
    (
            ...
            {'error': 1,
             'data': [{'error': 18,
                       'message': 'Error from socket indicating GuardSarmInternalError',
                       'agent': 1}
                      ],
             'message': 'Error'},
            None,
...
def test_agent_upgrade_agents(mock_socket, mock_wdb, mock_client_keys, agent_set, expected_errors_and_items,
    ...
    """
    with patch('guardsarm.core.agent.core_upgrade_agents') as core_upgrade_agents_mock:
        core_upgrade_agents_mock.return_value = result_from_socket

        if raise_error:
            # Upgrade expecting a GuardSarm Exception
            for error in expected_errors_and_items.keys():
                if int(error) in ERROR_CODES_UPGRADE_SOCKET_BAD_REQUEST:
                    with pytest.raises(GuardSarmError, match=f".* {error} .*"):
                    ...
                    with pytest.raises(GuardSarmInternalError, match=f".* {error} .*"):
                        upgrade_agents(agent_list=list(agent_set), filters=filters)
        else:
...
def test_agent_get_upgrade_result(mock_socket, mock_wdb, mock_client_keys, agent_set, expected_errors_and_items,
    ...
    with patch('guardsarm.core.agent.core_upgrade_agents') as core_upgrade_agents_mock:
        ...
        if raise_error:
            # Get upgrade result expecting a GuardSarm Exception
            for error in expected_errors_and_items.keys():
                with pytest.raises(GuardSarmInternalError, match=f".* {error} .*"):
...
def test_agent_get_agent_config(mock_exists, socket_mock, send_mock, guardsarm_socket_mock, agent_list, component, configuration):
    ...
    """
    guardsarm_socket_mock.return_value.receive.return_value = b'ok {"test": "conf"}'
...
def test_agent_get_agent_config_exceptions(socket_mock, send_mock, agent_list):
    ...
    except GuardSarmError as error:
        assert error == GuardSarmError(1740)
...
@patch('guardsarm.core.common.SHARED_PATH', new=test_shared_path)
@patch('guardsarm.core.configuration.upload_group_configuration')
...
@patch('guardsarm.core.common.SHARED_PATH', new=test_shared_path)
@patch('guardsarm.agent.get_distinct_agents')
@patch('guardsarm.agent.get_agent_groups')
@patch('guardsarm.agent.get_agents_summary_status')
@patch('guardsarm.agent.get_agents')
@patch('guardsarm.core.wdb.GuardSarmDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@patch('guardsarm.agent.get_agents_info', return_value=['test', 'test2'])
@patch('guardsarm.core.wdb.GuardSarmDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_get_agents_big_env(mock_conn, mock_send, mock_get_agents, insert_agents_db, agent_list, params, expected_ids):
    ...
    with patch('guardsarm.agent.get_agents_info', return_value=set(agent_ids_format(range(1, 100000)))):
...
@patch('guardsarm.core.agent.Agent.get_agent_groups', new_callable=AsyncMock)
@patch('guardsarm.core.agent.Agent.group_exists', return_value=False)
...
async def test_unset_single_group_agent_ko(agent_basic_mock, group_exists_mock, get_groups_mock, agent_id, group_id,
    ...
    expected_exc: int
        Expected GuardSarmException code error.
    """
    get_groups_mock.return_value = ['default']
    with pytest.raises(GuardSarmException, match=f".* {expected_exc} .*"):
...
def test_check_uninstall_permission():
    ...
    expected = GuardSarmResult({'message': 'User has permission to uninstall agents'})
