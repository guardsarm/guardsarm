...
TestMetadataInjection                        – @timestamp, guardsarm.cluster.node, guardsarm.cluster.name in every document
TestRunMetricsSnapshot                       – frequency=0 early-exit; frequency<600 clamped to 600
TestDisconnectionTimeOmission                – guardsarm.agent.disconnected_at absent when disconnection_time is 0
...
# opensearchpy must be mocked before any guardsarm.core.indexer module is imported
# because guardsarm.core.indexer.indexer imports AsyncOpenSearch at the top level.
...
mocked_modules = {
    ...
    "guardsarm.core.agent": MagicMock(),
    "guardsarm.core.common": MagicMock(),
    "guardsarm.core.configuration": MagicMock(),
    "guardsarm.core.stats": MagicMock(),
    "guardsarm.core.utils": MagicMock(),
    "guardsarm.core.InputValidator": MagicMock(),
    "guardsarm.core.cluster": MagicMock(),
    "guardsarm.core.cluster.utils": MagicMock(),
    "guardsarm.core.cluster.dapi": MagicMock(),
    "guardsarm.core.cluster.dapi.dapi": MagicMock(),
    "guardsarm.core.exception": MagicMock(),
    "guardsarm.core.guardsarm_queue": MagicMock(),
    "guardsarm.core.guardsarm_socket": MagicMock(),
    "guardsarm.core.wdb": MagicMock(),
    "guardsarm.core.wdb_http": MagicMock(),
    "guardsarm.rbac": MagicMock(),
    "guardsarm.rbac.utils": MagicMock(),
    "guardsarm.stats": MagicMock(),
}

with patch.dict(sys.modules, mocked_modules):
    import guardsarm.core.indexer.metrics_snapshot as _metrics_snapshot_module
    from guardsarm.core.indexer.metrics_snapshot import (
        MetricsSnapshotTasks,
        _build_jsonschema_properties,
        _opensearch_template_to_jsonschema,
    )
    import guardsarm.core.indexer.metrics as _metrics_module
    from guardsarm.core.indexer.metrics import MetricsIndex

import guardsarm.core.indexer as _indexer_pkg

_indexer_pkg.metrics_snapshot = _metrics_snapshot_module
_indexer_pkg.metrics = _metrics_module

sys.modules.setdefault("guardsarm.core.indexer.metrics", _metrics_module)
sys.modules.setdefault("guardsarm.core.indexer.metrics_snapshot", _metrics_snapshot_module)
...
SAMPLE_TEMPLATE = {
    "index_patterns": ["guardsarm-metrics-agents*"],
    "priority": 1,
    "data_stream": {},
    "template": {
        "settings": {},
...
def _make_dapi_result(items):
    """Return a mock AffectedItemsGuardSarmResult with the given affected_items list."""
    result = MagicMock()
...
@pytest.mark.asyncio
@patch("guardsarm.core.indexer.metrics_snapshot.GuardSarmDBQueryAgents")
async def test_collect_agents(mock_guardsarm_db_query_agents):
    ...
    mock_guardsarm_db_query_agents.return_value = mock_query_instance
    ...
    mock_guardsarm_db_query_agents.assert_called_once_with(limit=None)

    assert len(result) == 2
    for agent_doc in result:
        assert agent_doc["@timestamp"] == TEST_TIMESTAMP
        assert agent_doc["guardsarm"]["cluster"]["node"] == "node01"
        assert agent_doc["guardsarm"]["cluster"]["name"] == "guardsarm"
        assert "id" in agent_doc["guardsarm"]["agent"]
        assert "name" in agent_doc["guardsarm"]["agent"]
...
EXPECTED_COMMS_FIELDS = {
    ...
    "guardsarm.cluster.node",
    "guardsarm.schema.version",
}
...
class TestCollectCommsAllNodes:
    ...
    async def test_single_master_node_injects_metadata(self):
        ...
        with patch(
            "guardsarm.core.indexer.metrics_snapshot.get_daemons_stats",
        ...
        assert doc["guardsarm"]["cluster"]["node"] == "node01"
        assert doc["guardsarm"]["cluster"]["name"] == "guardsarm"
    ...
    async def test_worker_node_injects_its_own_node_name(self):
        """Worker node documents carry their own node name in guardsarm.cluster.node."""
        ...
        with (
            ...
            patch(
                "guardsarm.core.indexer.metrics_snapshot.DistributedAPI",
        ...
        assert len(docs) == 2
        node_names = {doc["guardsarm"]["cluster"]["node"] for doc in docs}
    ...
    async def test_node_failure_is_logged_and_skipped(self):
        ...
        assert len(docs) == 1
        assert docs[0]["guardsarm"]["cluster"]["node"] == "node02"
    ...
    async def test_dapi_called_for_worker_not_master(self):
        ...
        mock_local.assert_called_once_with(daemons_list=["guardsarm-manager-remoted"])
        MockDAPI.assert_called_once()
        call_kwargs = MockDAPI.call_args.kwargs
        assert call_kwargs["f_kwargs"]["daemons_list"] == ["guardsarm-manager-remoted"]
...
# Representative raw agent document as returned by GuardSarmDBQueryAgents.run()["items"].
AGENT_DOC_FULL = {
    "id": "001",
    "name": "ubuntu-agent",
    "ip": "192.168.1.10",
...
EXPECTED_AGENT_FIELDS = {
    ...
    "guardsarm.agent.host.ip",
    "guardsarm.agent.register.ip",
    "guardsarm.agent.status",
    "guardsarm.agent.status_code",
    "guardsarm.agent.registered_at",
    "guardsarm.agent.last_seen",
    "guardsarm.agent.disconnected_at",
    "guardsarm.agent.config.group.synced",
    "guardsarm.agent.config.group.hash.md5",
    "guardsarm.agent.host.architecture",
    "guardsarm.agent.host.os.name",
    "guardsarm.agent.host.os.version",
    "guardsarm.agent.host.os.platform",
    "guardsarm.cluster.name",
    "guardsarm.cluster.node",
    "guardsarm.schema.version",
}
...
class TestAgentFieldMapping:
    ...
    @pytest.mark.asyncio
    @patch("guardsarm.core.indexer.metrics_snapshot.GuardSarmDBQueryAgents")
    async def test_all_fields_present(self, mock_guardsarm_db_query_agents):
        """Every normalized field key exists in the output document."""
        mock_guardsarm_db_query_agents.return_value.run.return_value = {
    ...
    @pytest.mark.asyncio
    @patch("guardsarm.core.indexer.metrics_snapshot.GuardSarmDBQueryAgents")
    async def test_field_types(self, mock_guardsarm_db_query_agents):
        ...
        doc = docs[0]
        assert isinstance(doc["guardsarm"]["agent"]["id"], str)
        assert isinstance(doc["guardsarm"]["agent"]["name"], str)
        assert isinstance(doc["guardsarm"]["agent"]["status"], str)
        assert isinstance(doc["guardsarm"]["agent"]["config"]["group"]["synced"], bool)
        assert isinstance(doc["guardsarm"]["agent"]["groups"], list)
        assert isinstance(doc["@timestamp"], str)
        assert isinstance(doc["guardsarm"]["cluster"]["node"], str)
        assert isinstance(doc["guardsarm"]["cluster"]["name"], str)
        assert isinstance(doc["guardsarm"]["schema"]["version"], str)

    @pytest.mark.asyncio
    @patch("guardsarm.core.indexer.metrics_snapshot.GuardSarmDBQueryAgents")
    async def test_multiple_agents_all_have_expected_fields(
        self, mock_guardsarm_db_query_agents
    ):
        ...
        for doc in docs:
            ...
            assert not missing, (
                ...
                f"{doc.get('guardsarm', {}).get('agent', {}).get('id')}: {missing}"
            )

    @pytest.mark.asyncio
    @patch("guardsarm.core.indexer.metrics_snapshot.GuardSarmDBQueryAgents")
    async def test_register_ip_any_converted_to_cidr(self, mock_guardsarm_db_query_agents):
        ...
        assert docs[0]["guardsarm"]["agent"]["register"]["ip"] == "0.0.0.0"

    @pytest.mark.asyncio
    @patch("guardsarm.core.indexer.metrics_snapshot.GuardSarmDBQueryAgents")
    async def test_register_ip_real_value_preserved(self, mock_guardsarm_db_query_agents):
        ...
        assert docs[0]["guardsarm"]["agent"]["register"]["ip"] == "10.0.1.5"

    @pytest.mark.asyncio
    @patch("guardsarm.core.indexer.metrics_snapshot.GuardSarmDBQueryAgents")
    async def test_register_ip_empty_field_omitted(self, mock_guardsarm_db_query_agents):
        ...
        agent = docs[0].get("guardsarm", {}).get("agent", {})
    ...
    async def test_group_config_status_synced_maps_to_true(
        ...
        assert docs[0]["guardsarm"]["agent"]["config"]["group"]["synced"] is True
    ...
    async def test_group_config_status_not_synced_maps_to_false(
        ...
        assert docs[0]["guardsarm"]["agent"]["config"]["group"]["synced"] is False

    @pytest.mark.asyncio
    @patch("guardsarm.core.indexer.metrics_snapshot.GuardSarmDBQueryAgents")
    async def test_redundant_raw_fields_dropped(self, mock_guardsarm_db_query_agents):
        ...
        doc = docs[0]
        # Top-level keys must only be "@timestamp" and "guardsarm" — no raw fields
        for dropped in (
            "manager",
            "node_name",
            "id",
...
class TestMetadataInjection:
    """TC-2 – @timestamp, guardsarm.cluster.node, guardsarm.cluster.name in every document."""
    ...
    async def test_agent_docs_have_all_metadata_fields(
        ...
        for doc in docs:
            ...
            assert "guardsarm" in doc
            assert "node" in doc["guardsarm"]["cluster"]
            assert "name" in doc["guardsarm"]["cluster"]
...
class TestRunMetricsSnapshot:
    ...
    async def test_frequency_zero_returns_immediately(self):
        ...
        with (
            ...
            patch(
                "guardsarm.core.indexer.metrics_snapshot.asyncio.sleep",
    ...
    async def test_frequency_zero_no_side_effects(self):
        ...
        with (
            ...
            patch(
                "guardsarm.core.indexer.metrics_snapshot.get_indexer_client",
                new_callable=AsyncMock,
            ) as mock_indexer,
        ):
            await tasks.run_metrics_snapshot()
...
class TestDisconnectionTimeOmission:
    """TC-5 – guardsarm.agent.disconnected_at is absent when disconnection_time is 0."""

    @pytest.mark.asyncio
    @patch("guardsarm.core.indexer.metrics_snapshot.GuardSarmDBQueryAgents")
    async def test_disconnection_time_zero_is_absent(self, mock_guardsarm_db_query_agents):
        """TC-5: GuardSarmDBQueryAgents strips disconnection_time=0; normalized field must not appear."""
        mock_guardsarm_db_query_agents.return_value.run.return_value = {
    ...
    async def test_nonzero_disconnection_time_is_preserved(
        ...
        """A non-zero disconnection_time maps to guardsarm.agent.disconnected_at."""
        mock_guardsarm_db_query_agents.return_value.run.return_value = {
            "items": [
                {
                    "id": "002",
        ...
        assert docs[0]["guardsarm"]["agent"]["disconnected_at"] == 19345809
    ...
    async def test_mixed_agents_disconnection_time_handled_per_agent(
        ...
        active_doc = next(
            d for d in docs if d.get("guardsarm", {}).get("agent", {}).get("id") == "001"
        )
        disconnected_doc = next(
            d for d in docs if d.get("guardsarm", {}).get("agent", {}).get("id") == "002"
        )

        assert "disconnected_at" not in active_doc.get("guardsarm", {}).get("agent", {})
        assert disconnected_doc["guardsarm"]["agent"]["disconnected_at"] == 5000
...
class TestBulkActionShape:
    ...
    async def test_bulk_actions_have_op_type_create(self):
        ...
        docs = [
            ...
            {
                "guardsarm": {"agent": {"id": "002"}},
                "@timestamp": TIMESTAMP,
            },
            {
                "guardsarm": {"agent": {"id": "003"}},
                "@timestamp": TIMESTAMP,
            },
        ]
        ...
        with patch(
            "guardsarm.core.indexer.metrics.async_bulk",
            new_callable=AsyncMock,
            side_effect=_capture_bulk,
        ):
            await metrics_index.bulk_index("guardsarm-metrics-agents", docs, bulk_size=100)
    ...
    async def test_bulk_actions_index_name_matches_target(self):
        ...
        assert captured_actions[0]["_index"] == "guardsarm-metrics-agents"
    ...
    async def test_bulk_actions_source_contains_original_document(self):
        ...
        docs = [
            {"guardsarm": {"agent": {"id": "001"}}, "@timestamp": TIMESTAMP},
            {"guardsarm": {"agent": {"id": "002"}}, "@timestamp": TIMESTAMP},
        ]
        captured_actions = []
...
class TestInit:
    ...
    def test_no_setup_task_logger_uses_logging_getLogger(self):
        ...
        server.configuration = {"node_name": "node01", "name": "guardsarm"}

        with patch("guardsarm.core.indexer.metrics_snapshot.logging") as mock_logging:
            tasks = MetricsSnapshotTasks(server=server, cluster_items=CLUSTER_ITEMS)

        mock_logging.getLogger.assert_called_once_with("guardsarm")
...
class TestCollectAndIndex:
    ...
    async def test_collects_agents_and_comms_then_bulk_indexes_both(self):
        ...
        agent_docs = [{"guardsarm": {"agent": {"id": "001"}}, "@timestamp": TIMESTAMP}]
    ...
    async def test_bulk_index_called_for_agents_index(self):
        """bulk_index is called with 'guardsarm-metrics-agents' and the normalized agent docs."""
        agent_docs = [{"guardsarm": {"agent": {"id": "001"}}}]
        ...
        mock_indexer.metrics.bulk_index.assert_any_await(
            "guardsarm-metrics-agents", agent_docs, tasks.bulk_size
        )

    @pytest.mark.asyncio
    async def test_bulk_index_called_for_comms_index(self):
        """bulk_index is called with 'guardsarm-metrics-comms' and the normalized comms docs."""
        ...
        mock_indexer.metrics.bulk_index.assert_any_await(
            "guardsarm-metrics-comms", comms_docs, tasks.bulk_size
        )
...
class TestBuildJsonschemaProperties:
    ...
    def test_nested_properties_produce_nested_schema(self):
        ...
        result = _build_jsonschema_properties(props)
        assert "guardsarm" in result
        assert result["guardsarm"]["type"] == "object"
        assert "agent" in result["guardsarm"]["properties"]
        assert "id" in result["guardsarm"]["properties"]["agent"]["properties"]
        assert result["guardsarm"]["properties"]["agent"]["properties"]["id"] == {
            "anyOf": [
                {"type": "string"},
                {"type": "array", "items": {"type": "string"}},
                {"type": "null"},
    ...
    def test_mixed_flat_and_nested(self):
        ...
        assert "guardsarm" in result
        assert "cluster" in result["guardsarm"]["properties"]
        assert "name" in result["guardsarm"]["properties"]["cluster"]["properties"]
...
class TestOpensearchTemplateToJsonschema:
    ...
    def test_basic_conversion(self):
        """keyword fields under guardsarm.agent produce correct nested JSON Schema."""
        schema = _opensearch_template_to_jsonschema(SAMPLE_TEMPLATE)
        assert schema["type"] == "object"
        assert "guardsarm" in schema["properties"]
        agent_props = schema["properties"]["guardsarm"]["properties"]["agent"]["properties"]
...
class TestLoadSchema:
    ...
    def test_missing_file_returns_none_and_logs_warning(self):
        ...
        with patch(
            "guardsarm.core.indexer.metrics_snapshot.os.path.isfile", return_value=False
        ):
    ...
    def test_valid_schema_file_is_loaded_and_returned(self):
        ...
        with (
            patch(
                "guardsarm.core.indexer.metrics_snapshot.os.path.isfile", return_value=True
            ),
        ...
        assert "guardsarm" in result["properties"]
    ...
    def test_template_without_mappings_returns_none_and_logs_warning(self):
        ...
        tasks = _make_tasks()
        bad_template = json.dumps({"index_patterns": ["guardsarm-metrics-agents*"]})
...
class TestValidateDocuments:
    ...
    def _make_flat_schema(self):
        ...
        return {
            ...
            "properties": {
                ...
                "guardsarm_cluster_node": {"type": "string"},
            },
    ...
    def test_valid_document_is_kept(self):
        ...
        docs = [{"id": "001", "@timestamp": TIMESTAMP, "guardsarm_cluster_node": "n"}]

        result = tasks._validate_documents(docs, schema, "guardsarm-metrics-agents")
    ...
    def test_invalid_document_is_filtered_and_logged(self):
        ...
        tasks = _make_tasks()
        schema = self._make_flat_schema()
        valid_doc = {"id": "001", "@timestamp": TIMESTAMP, "guardsarm_cluster_node": "n"}
        invalid_doc = {
            "id": "002",
            "@timestamp": TIMESTAMP,
            "guardsarm_cluster_node": "n",
            "unknown_field": "value",
        }

        result = tasks._validate_documents(
            [valid_doc, invalid_doc], schema, "guardsarm-metrics-agents"
        )
    ...
    def test_type_mismatch_triggers_validation_error(self):
        ...
        tasks = _make_tasks()
        schema = self._make_flat_schema()
        bad_doc = {"id": 123, "@timestamp": TIMESTAMP, "guardsarm_cluster_node": "n"}

        result = tasks._validate_documents([bad_doc], schema, "guardsarm-metrics-agents")
    ...
    def test_empty_document_list_returns_empty_list(self):
        ...
        result = tasks._validate_documents([], schema, "guardsarm-metrics-agents")
    ...
    def test_multiple_invalid_documents_each_logged(self):
        ...
        schema = self._make_flat_schema()
        docs = [
            {"id": 1, "@timestamp": TIMESTAMP, "guardsarm_cluster_node": "n"},
            {"id": 2, "@timestamp": TIMESTAMP, "guardsarm_cluster_node": "n"},
        ]
...
class TestCollectAndIndexWithValidation:
    ...
    async def test_with_schema_validates_agent_docs(self):
        ...
        mock_validate.assert_called_once_with(
            agent_docs, fake_schema, "guardsarm-metrics-agents"
        )
    ...
    async def test_with_schema_validates_comms_docs(self):
        ...
        mock_validate.assert_called_once_with(
            comms_docs, fake_schema, "guardsarm-metrics-comms"
        )
    ...
    async def test_invalid_docs_are_not_indexed(self):
        ...
        mock_indexer.metrics.bulk_index.assert_any_await(
            "guardsarm-metrics-agents", valid_only, tasks.bulk_size
        )
...
class TestNormalizeAgentDocNoEmptyHash:
    ...
    async def test_config_hash_absent_when_configsum_missing(
        ...
        """guardsarm.agent.config.hash is absent from the output when configSum is not in the raw doc."""
        ...
        agent_config = docs[0].get("guardsarm", {}).get("agent", {}).get("config", {})
        assert "hash" not in agent_config, (
            "guardsarm.agent.config.hash should be absent when configSum is not in the raw doc; "
    ...
    async def test_config_hash_present_when_configsum_provided(
        ...
        """guardsarm.agent.config.hash.md5 is present and correct when configSum IS supplied."""
        mock_guardsarm_db_query_agents.return_value.run.return_value = {
        ...
        agent_config = docs[0]["guardsarm"]["agent"]["config"]
...
class TestClusterNameFallback:
    """guardsarm.cluster.name must be omitted when 'name' is absent from config."""

    @pytest.mark.asyncio
    async def test_collect_comms_cluster_name_is_none_when_missing(self):
        """_collect_comms_all_nodes omits guardsarm.cluster.name when 'name' is absent."""
        server = MagicMock()
        ...
        with patch("guardsarm.core.indexer.metrics_snapshot.get_daemons_stats") as mock_stats:
            mock_stats.return_value = MagicMock(affected_items=[{"name": "guardsarm-manager-remoted"}])
            docs = await tasks._collect_comms_all_nodes(TIMESTAMP)

        assert docs, "Expected at least one document"
        for doc in docs:
            cluster = doc.get("guardsarm", {}).get("cluster", {})
...
ENGINE_DUMP = {
    "status": "OK",
    "name": "guardsarm-manager-analysisd",
...
class TestNormalizeNormalizationDoc:
    ...
    def test_global_metric_space_name_is_null(self):
        """Global metrics have guardsarm.space.name == None (null in JSON)."""
        metric = {"name": "router.queue.size", "type": "pull", "enabled": True, "value": 0}
        doc = MetricsSnapshotTasks._normalize_normalization_doc(
            metric, None, TIMESTAMP, "guardsarm", "node01"
        )
        assert doc["guardsarm"]["space"]["name"] is None

    def test_space_metric_space_name_is_set(self):
        """Space metrics have guardsarm.space.name set to the space name."""
        metric = {"name": "events.unclassified", "type": "counter", "enabled": True, "value": 264}
        doc = MetricsSnapshotTasks._normalize_normalization_doc(
            metric, "standard", TIMESTAMP, "guardsarm", "node01"
        )
        assert doc["guardsarm"]["space"]["name"] == "standard"
    ...
    def test_cluster_fields_match_arguments(self):
        """guardsarm.cluster.name and node are sourced from the arguments."""
        metric = {"name": "router.eps.1m", "type": "pull", "enabled": True, "value": 5}
        doc = MetricsSnapshotTasks._normalize_normalization_doc(
            metric, None, TIMESTAMP, "my-cluster", "my-node"
        )
        assert doc["guardsarm"]["cluster"]["name"] == "my-cluster"
        assert doc["guardsarm"]["cluster"]["node"] == "my-node"
    ...
    def test_event_module_and_kind_are_correct(self):
        ...
        assert doc["event"]["module"] == "guardsarm-manager-analysisd"
...
class TestCollectNormalizationAllNodes:
    ...
    async def test_single_master_generates_one_doc_per_metric(self):
        ...
        with patch(
            "guardsarm.core.indexer.metrics_snapshot.get_engine_metrics",
            return_value=local_result,
        ):
    ...
    async def test_global_metrics_have_null_space_name(self):
        """Documents from global[] have guardsarm.space.name == None."""
        tasks = _make_tasks(server=_make_server(node_name="node01"))
        local_result = _make_dapi_result([dict(ENGINE_DUMP)])
    ...
    async def test_space_metrics_have_space_name_set(self):
        ...
        space_docs = [d for d in docs if d["guardsarm"]["space"]["name"] == "standard"]
        assert len(space_docs) == 2

    @pytest.mark.asyncio
    async def test_metadata_injected_into_every_document(self):
        """@timestamp, guardsarm.cluster.name and guardsarm.cluster.node appear in every doc."""
        tasks = _make_tasks(server=_make_server(node_name="node01"))
        local_result = _make_dapi_result([dict(ENGINE_DUMP)])
    ...
    async def test_node_failure_is_logged_and_skipped(self):
        ...
        assert all(d["guardsarm"]["cluster"]["node"] == "node02" for doc in docs for d in [doc])
        tasks.logger.exception.assert_called_once()

    @pytest.mark.asyncio
    async def test_space_name_none_when_name_key_missing(self):
        """A space without a 'name' key produces guardsarm.space.name == None."""
        dump_no_space_name = {
            **ENGINE_DUMP,
            "global": [],
            "spaces": [
        ...
        assert len(docs) == 1
        assert docs[0]["guardsarm"]["space"]["name"] is None
