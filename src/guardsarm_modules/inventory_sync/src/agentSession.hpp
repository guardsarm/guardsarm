 ...
 *        guardsarm-states-* family and have a non-empty suffix.
 *
...
{
    constexpr std::string_view kPrefix = "guardsarm-states-";
...
public:
    explicit AgentSessionImpl(const uint64_t sessionId,
                              GuardSarm::SyncSchema::Start const* data,
                              TStore& store,
...

    {
        ...
        // Extract indices. Only keep entries that belong to the guardsarm-states-* family
                ...
                if (index)
                {
                    ...
                    if (!isInventoryStateIndex(entry))
                    {
                        logWarn(LOGGER_DEFAULT_TAG,
                                "Start: ignoring index '%s' (does not belong to guardsarm-states-* family) for "
                                "agent '%.*s' (session %llu).",
                                entry.c_str(),
        ...
        // so size can be 0
        if (data->size() == 0 && data->mode() != GuardSarm::SyncSchema::Mode_MetadataDelta &&
            data->mode() != GuardSarm::SyncSchema::Mode_MetadataCheck &&
            data->mode() != GuardSarm::SyncSchema::Mode_GroupDelta && data->mode() != GuardSarm::SyncSchema::Mode_GroupCheck &&
            data->mode() != GuardSarm::SyncSchema::Mode_ModuleCheck)
        {
            responseDispatcher.sendStartAck(GuardSarm::SyncSchema::Status_Error, agentId, sessionId, moduleName);
        ...
        responseDispatcher.sendStartAck(
            GuardSarm::SyncSchema::Status_Ok, m_context->agentId, m_context->sessionId, m_context->moduleName);
    }
     ...
     */
    void handleData(GuardSarm::SyncSchema::DataValue const* data, const uint8_t* dataRaw, size_t dataSize)
    {
        if (data == nullptr)
        {
     ...
     */
    void handleChecksumModule(GuardSarm::SyncSchema::ChecksumModule const* data)
    {
        if (data == nullptr)
        {
        ...
        if (data->index())
        {
            ...
            {
                logWarn(LOGGER_DEFAULT_TAG,
                        "ChecksumModule: ignoring index '%s' (does not belong to guardsarm-states-* family) for "
                        "session %llu.",
                        candidate.c_str(),
                        m_context->sessionId);
            }
     ...
     * @param dataSize Size of the raw payload.
     */
    void handleDataContext(GuardSarm::SyncSchema::DataContext const* data, const uint8_t* dataRaw, size_t dataSize)
    {
        if (data == nullptr)
        {
     ...
     */
    void handleDataClean(GuardSarm::SyncSchema::DataClean const* data)
    {
        if (data == nullptr)
        {
        ...
        if (data->index())
        {
            ...
            {
                logWarn(LOGGER_DEFAULT_TAG,
                        "DataClean: ignoring index '%s' (does not belong to guardsarm-states-* family) for "
                        "session %llu, seq %llu.",
                        index.c_str(),
                        m_context->sessionId,
                        seq);
    ...
    void handleEnd(const TResponseDispatcher& responseDispatcher)
    {
        ...
        if (m_endEnqueued)
        {
            ...
            responseDispatcher.sendEndAck(
                GuardSarm::SyncSchema::Status_Processing, m_context->agentId, m_context->sessionId, m_context->moduleName);
            return;
        }
