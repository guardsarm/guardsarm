...
from guardsarm.core.common import GUARDSARM_LOCALTIME_PATH
...
with patch('guardsarm.core.common.guardsarm_uid'):
    with patch('guardsarm.core.common.guardsarm_gid'):
        from guardsarm import GuardSarmException
        from guardsarm.core.agent import GuardSarmDBQueryAgents
        from guardsarm.core import utils, exception
        from guardsarm.core.common import GUARDSARM_PATH
...
@patch('guardsarm.core.utils.common.MAXIMUM_DATABASE_LIMIT', new=10)
...
def test_cut_array_ko(limit, offset, expected_exception):
    ...
    """
    with pytest.raises(exception.GuardSarmException, match=f'.* {expected_exception} .*'):
...
@patch('guardsarm.core.utils.len', return_value=1)
@patch('guardsarm.core.utils.cut_array')
@patch('guardsarm.core.utils.select_array', return_value=ANY)
@patch('guardsarm.core.utils.filter_array_by_query', return_value=ANY)
@patch('guardsarm.core.utils.search_array', return_value=ANY)
@patch('guardsarm.core.utils.sort_array', return_value=ANY)
...
@patch('guardsarm.core.utils.chmod')
...
@patch('guardsarm.core.utils.chown')
...
@patch('guardsarm.core.utils.chown')
@patch('guardsarm.core.utils.chmod')
@patch('guardsarm.core.utils.utime')
...
def test_safe_move_exception(mock_utime, mock_chmod, mock_chown):
    ...
    with TemporaryDirectory() as tmpdirname:
        tmp_file = NamedTemporaryFile(dir=tmpdirname, delete=False)
        target_file = os.path.join(tmpdirname, 'target')
        with patch('guardsarm.core.utils.rename', side_effect=OSError(1)):
...
@patch('guardsarm.core.utils.chmod')
@patch('guardsarm.core.utils.mkdir')
@patch('guardsarm.core.utils.curdir', new='var')
def test_mkdir_with_mode(mock_mkdir, mock_chmod, dir_name, path_exists):
    """Test mkdir_with_mode function."""
    with patch('guardsarm.core.utils.path.exists', return_value=path_exists):
...
@patch('guardsarm.core.utils.mkdir', side_effect=OSError)
def test_mkdir_with_mode_ko(mock_mkdir, dir_name, exists):
    """Test mkdir_with_mode function errors work."""
    with patch('guardsarm.core.utils.path.exists', return_value=exists):
        with pytest.raises(OSError):
            utils.mkdir_with_mode(dir_name)


@patch('guardsarm.core.utils.open')
@patch('guardsarm.core.utils.iter', return_value=['1', '2'])
def test_md5(mock_iter, mock_open):
    """Test md5 function."""
    with patch('guardsarm.core.utils.hashlib.md5') as md:
...
def test_blake2b(mock_iter, mock_open):
    """Test md5 function."""
    with patch('guardsarm.core.utils.hashlib.blake2b') as blake2b_mock:
...
def test_protected_get_hashing_algorithm_ko():
    ...
    with pytest.raises(exception.GuardSarmException, match=".* 1723 .*"):
...
def test_get_hash(mock_open):
    ...
    with patch('guardsarm.core.utils.iter', return_value=['1', '2']):
        with patch('guardsarm.core.utils.hashlib.new') as md:
            md.return_value.update.side_effect = None
    ...
    with patch('guardsarm.core.utils.iter', return_value=[]):
...
@patch('guardsarm.core.utils.compile', return_value='Something')
def test_basic_load_guardsarm_xml(mock_compile):
    """Test basic load_guardsarm_xml functionality."""
    with patch('guardsarm.core.utils.open') as f:
        f.return_value.__enter__.return_value = StringIO(test_xml)
        result = utils.load_guardsarm_xml('test_file')

        assert isinstance(result, Element)


def test_load_guardsarm_xml():
    """Test load_guardsarm_xml function."""
    ...
    for rule_file in os.listdir(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data/test_load_guardsarm_xml')):
        path = os.path.join(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data/test_load_guardsarm_xml'),
                            rule_file)
        original = parse(path).getroot()
        result = utils.load_guardsarm_xml(path)
...
def test_load_guardsarm_xml_ko(expected_exception):
    """Test load_guardsarm_xml fails gracefully when reading an invalid utf-8 character sequence"""
    file_path = os.path.join(test_data_path, 'test_load_guardsarm_xml_ko/invalid_utf8.xml')
    with pytest.raises(GuardSarmException, match=f'.* {expected_exception} .*'):
        utils.load_guardsarm_xml(file_path)

@pytest.mark.parametrize('version1, version2', [
    ('GuardSarm v3.5.0', 'GuardSarm v3.5.2'),
    ('GuardSarm v3.6.1', 'GuardSarm v3.6.3'),
    ('GuardSarm v3.7.2', 'GuardSarm v3.8.0'),
    ('GuardSarm v3.8.0', 'GuardSarm v3.8.1'),
    ('GuardSarm v3.9.0', 'GuardSarm v3.9.2'),
    ('GuardSarm v3.9.10', 'GuardSarm v3.9.14'),
    ('GuardSarm v3.10.1', 'GuardSarm v3.10.10'),
    ('GuardSarm v4.10.10', 'GuardSarm v4.11.0'),
    ('GuardSarm v5.1.15', 'GuardSarm v5.2.0'),
    ('v3.6.0', 'v3.6.1'),
    ('v3.9.1', 'v3.9.2'),
    ('v4.0.0', 'v4.0.1'),
    ('3.6.0', '3.6.1'),
...
def test_version_ok(version1, version2):
    """Test GuardSarmVersion class."""
    current_version = utils.GuardSarmVersion(version1)
    new_version = utils.GuardSarmVersion(version2)
...
@pytest.mark.parametrize('version1, version2', [
    ('v3.6.0', 'v.3.6.1'),
    ('GuardSarm v4', 'GuardSarm v5'),
    ('GuardSarm v3.9', 'GuardSarm v3.10'),
    ('ABC v3.10.1', 'ABC v3.10.12'),
    ('GuardSarmv3.9.0', 'GuardSarmv3.9.2'),
    ('3.9', '3.10'),
    ('3.9.0', '3.10'),
    ('3.10', '4.2'),
    ('3', '3.9.1')
...
def test_version_ko(version1, version2):
    """Test GuardSarmVersion class."""
    try:
        utils.GuardSarmVersion(version1)
        utils.GuardSarmVersion(version2)
    except ValueError:
        return


@pytest.mark.parametrize('version1, version2', [
    ('GuardSarm v3.10.10', 'GuardSarm v3.10.10'),
    ('GuardSarm v5.1.15', 'GuardSarm v5.1.15'),
    ('v3.6.0', 'v3.6.0'),
    ('v3.9.2', 'v3.9.2')
])
...
def test_GuardSarmVersion__str__():
    """Test GuardSarmVersion.__str__ function."""
    version = utils.GuardSarmVersion('GuardSarm v3.10.0-alpha4')

    assert isinstance(version.__str__(), str)


@pytest.mark.parametrize('version1, version2', [
    ('GuardSarm v3.5.2', 'GuardSarm v4.0.0'),
    ('GuardSarm v3.10.0-alpha', 'GuardSarm v3.10.0'),
    ('GuardSarm v3.10.0-alpha4', 'GuardSarm v3.10.0-beta4'),
    ('GuardSarm v3.10.0-alpha3', 'GuardSarm v3.10.0-alpha4'),
])
def test_GuardSarmVersion__ge__(version1, version2):
    """Test GuardSarmVersion.__ge__ function."""
    current_version = utils.GuardSarmVersion(version1)
    new_version = utils.GuardSarmVersion(version2)
...
def test_failed_test_get_timeframe_in_seconds():
    ...
    with pytest.raises(exception.GuardSarmException, match=".* 1411 .*"):
...
@patch('guardsarm.core.utils.path.exists', return_value=True)
@patch('glob.glob', return_value=True)
@patch('guardsarm.core.utils.GuardSarmDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('guardsarm.core.utils.common.MAXIMUM_DATABASE_LIMIT', new=10)
def test_GuardSarmDBQuery_protected_clean_filter(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                             query_filter, expected_query_filter, expected_wef):
    """Test GuardSarmDBQuery._clean_filter function."""
    query = utils.GuardSarmDBQuery(offset=0, limit=500, table='agent', sort=None,
                               search=None, select=None, filters=None,
                               fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
                               backend=utils.GuardSarmDBBackend(agent_id=1),
...
@patch('socket.socket.connect')
@patch('guardsarm.core.utils.common.MAXIMUM_DATABASE_LIMIT', new=10)
def test_GuardSarmDBQuery_protected_add_limit_to_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                                   limit, error, expected_exception):
    """Test GuardSarmDBQuery._add_limit_to_query function."""
    query = utils.GuardSarmDBQuery(offset=0, limit=limit, table='agent', sort=None,
                               search=None, select=None, filters=None,
                               fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
                               backend=utils.GuardSarmDBBackend(agent_id=1),
...
@patch('guardsarm.core.utils.GuardSarmDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardSarmDBQuery_protected_sort_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Tests GuardSarmDBQuery._sort_query function works"""

    query = utils.GuardSarmDBQuery(offset=0, limit=1, table='agent',
                               sort={'order': 'asc'}, search=None, select=None,
                               filters=None, fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
                               backend=utils.GuardSarmDBBackend(agent_id=1), count=5,
                               get_data=None)
...
@patch('guardsarm.core.utils.GuardSarmDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardSarmDBQuery_protected_add_sort_to_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                                  sort, expected_exception):
    """Test GuardSarmDBQuery._add_sort_to_query function."""
    fields = {'1': 'one', '2': 'two', '3': 'three', '4': 'four'}
    query = utils.GuardSarmDBQuery(offset=0, limit=1, table='agent', sort=sort,
                               search=None, select=None, filters=None,
                               fields=fields,
...
@patch('guardsarm.core.utils.GuardSarmDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardSarmDBQuery_protected_add_search_to_query(mock_socket_conn, mock_conn_db, mock_glob,
                                                    mock_exists):
    """Test GuardSarmDBQuery._add_search_to_query function."""
    query = utils.GuardSarmDBQuery(offset=0, limit=1, table='agent', sort=None,
...
@patch('glob.glob', return_value=True)
@patch('socket.socket.connect')
def test_GuardSarmDBQuery_protected_parse_select_filter(mock_socket_conn, mock_glob, mock_conn_db, mock_exists,
                                                    selector_fields, error, expected_exception):
    """Test GuardSarmDBQuery._parse_select_filter function."""
    query = utils.GuardSarmDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None, filters=None,
                               fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
...
@patch('guardsarm.core.utils.GuardSarmDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('guardsarm.core.utils.GuardSarmDBQuery._parse_select_filter')
def test_GuardSarmDBQuery_protected_add_select_to_query(mock_parse, mock_socket_conn, mock_conn_db, mock_glob,
                                                    mock_exists):
    """Test GuardSarmDBQuery._add_select_to_query function."""
    query = utils.GuardSarmDBQuery(offset=0, limit=1, table='agent',
                               sort={'order': 'asc'}, search=None, select=None,
                               filters=None, fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
...
@patch('guardsarm.core.utils.path.exists', return_value=True)
@patch('guardsarm.core.utils.GuardSarmDBBackend.connect_to_db')
def test_GuardSarmDBQuery_protected_parse_query_regex(mock_backend_connect, mock_exists, q, expected_query_filters):
    """Test GuardSarmDBQuery._parse_query function."""
    query = utils.GuardSarmDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None, filters=None,
...
@patch('guardsarm.core.utils.GuardSarmDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardSarmDBQuery_protected_parse_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
...
@patch('guardsarm.core.utils.GuardSarmDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardSarmDBQuery_protected_parse_legacy_filters(mock_socket_conn, mock_conn_db, mock_glob,
                                                     mock_exists, filter_):
    """Test GuardSarmDBQuery._parse_legacy_filters function."""
    query = utils.GuardSarmDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None, filters=filter_,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
                               backend=utils.GuardSarmDBBackend(agent_id=0),
                               count=5, get_data=None)
...
@patch('guardsarm.core.utils.GuardSarmDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('guardsarm.core.utils.GuardSarmDBQuery._parse_legacy_filters')
@patch('guardsarm.core.utils.GuardSarmDBQuery._parse_query')
def test_GuardSarmDBQuery_parse_filters(mock_query, mock_filter, mock_socket_conn, mock_conn_db, mock_glob,
                                    mock_exists, filter, q):
    """Test GuardSarmDBQuery._parse_filters function."""
    query = utils.GuardSarmDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search={"negation": True, "value": "1"}, select=None,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=q,
...
@patch('guardsarm.core.utils.GuardSarmDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('guardsarm.core.utils.GuardSarmDBQuery._filter_status')
@patch('guardsarm.core.utils.GuardSarmDBQuery._filter_date')
def test_GuardSarmDBQuery_protected_process_filter(mock_date, mock_status, mock_socket_conn, mock_conn_db,
                                               mock_glob, mock_exists, field_name, field_filter, q_filter):
    """Tests GuardSarmDBQuery._process_filter."""
    query = utils.GuardSarmDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04', 'status': 'active'},
                               default_sort_field=None, query=None,
                               backend=utils.GuardSarmDBBackend(agent_id=0), count=5,
...
@patch('guardsarm.core.utils.GuardSarmDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('guardsarm.core.utils.GuardSarmDBQuery._process_filter')
def test_GuardSarmDBQuery_protected_add_filters_to_query(mock_process, mock_socket_conn,
                                                     mock_conn_db, mock_glob, mock_exists):
    """Test GuardSarmDBQuery._add_filters_to_query function."""
    query = utils.GuardSarmDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=['os.name'],
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query='os.name=ubuntu',
                               backend=utils.GuardSarmDBBackend(agent_id=0), distinct=True,
...
@patch('guardsarm.core.utils.GuardSarmDBBackend.connect_to_db')
@patch('guardsarm.core.utils.path.exists', return_value=True)
def test_GuardSarmDBQuery_protected_add_filters_to_query_final_query(mock_conn_db, mock_file_exists,
                                                                 filters, expected_query):
    """Test GuardSarmDBQuery._add_filters_to_query final query."""
    query = utils.GuardSarmDBQuery(offset=0, limit=1, table='agent', sort=None, search=None, select=None,
                               fields={'id': 'id', 'status': 'status', 'group': 'group'},
                               default_sort_field=None, query='', backend=utils.GuardSarmDBBackend(agent_id=0),
                               count=5, get_data=None)
...
@patch('guardsarm.core.utils.GuardSarmDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardSarmDBQuery_protected_get_total_items(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Test GuardSarmDBQuery._get_total_items function."""
    query = utils.GuardSarmDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
@patch('guardsarm.core.utils.GuardSarmDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardSarmDBQuery_protected_get_total_items_mitre(mock_socket_conn, mock_conn_db, mock_glob,
                                                      mock_exists):
    query = utils.GuardSarmDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
                               backend=utils.GuardSarmDBBackend(agent_id=0, query_format='mitre'), count=5,
                               get_data=None, date_fields=['date1', 'date2'])
...
@patch('guardsarm.core.utils.GuardSarmDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardSarmDBQuery_substitute_params(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Test utils.GuardSarmDBQuery._get_total_items function."""
    query = utils.GuardSarmDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
@patch('guardsarm.core.utils.GuardSarmDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardSarmDBQuery_protected_format_data_into_dictionary(mock_socket_conn, mock_conn_db, mock_glob,
                                                            mock_exists):
    """Test utils.GuardSarmDBQuery._format_data_into_dictionary."""
    query = utils.GuardSarmDBQuery(offset=0, limit=1, table='agent', sort=None,
...
@patch('guardsarm.core.utils.GuardSarmDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardSarmDBQuery_protected_filter_status(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Test utils.GuardSarmDBQuery._filter_status function."""
    query = utils.GuardSarmDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'fields': set(['os.name'])},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
@patch('guardsarm.core.utils.GuardSarmDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardSarmDBQuery_protected_filter_date(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                            date_filter, filter_db_name, time, error):
    """Test utils.GuardSarmDBQuery._filter_date function."""
    query = utils.GuardSarmDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'fields': set(['os.name'])},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
    ...
    with patch('guardsarm.core.utils.get_timeframe_in_seconds', return_value=time):
        if error:
            with pytest.raises(exception.GuardSarmException, match=".* 1412 .*"):
...
@patch('socket.socket.connect')
def test_GuardSarmDBQuery_general_run(mock_socket_conn, execute_value, expected_result):
    """Test utils.GuardSarmDBQuery.general_run function."""
    with patch('guardsarm.core.utils.GuardSarmDBBackend.execute', return_value=execute_value):
        query = GuardSarmDBQueryAgents(offset=0, limit=None, sort=None, search=None, select={'id'},
...
@patch('socket.socket.connect')
def test_GuardSarmDBQuery_oversized_run(mock_socket_conn, execute_value, rbac_ids, negate,
                                    final_rbac_ids, expected_result):
    """Test utils.GuardSarmDBQuery.oversized_run function."""
    with patch('guardsarm.core.utils.GuardSarmDBBackend.execute', side_effect=[execute_value, final_rbac_ids]):
        query = GuardSarmDBQueryAgents(offset=0, limit=None, sort=None, search=None, select={'id'},
...
@patch('guardsarm.core.utils.GuardSarmDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('guardsarm.core.utils.GuardSarmDBQuery._default_query')
def test_GuardSarmDBQuery_reset(mock_query, mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Test utils.GuardSarmDBQuery.reset function."""
    query = utils.GuardSarmDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'os.name'},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
@patch('guardsarm.core.utils.GuardSarmDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardSarmDBQuery_protected_default_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Test utils.GuardSarmDBQuery._default_query function."""
    query = utils.GuardSarmDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'fields': set(['os.name'])},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
...
@patch('guardsarm.core.utils.GuardSarmDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardSarmDBQuery_protected_default_count_query(mock_socket_conn, mock_conn_db, mock_glob,
                                                    mock_exists):
    """Test utils.GuardSarmDBQuery._default_count_query function."""
    query = utils.GuardSarmDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'fields': set(['os.name'])},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
@patch('guardsarm.core.utils.GuardSarmDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardSarmDBQuery_protected_pass_filter(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                            value):
    """Test utils.GuardSarmDBQuery._pass_filter function."""
    query = utils.GuardSarmDBQuery(offset=0, limit=1, table='agent', sort=None,
...
@patch('guardsarm.core.utils.GuardSarmDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardSarmDBQueryGroupBy__init__(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Tests utils.GuardSarmDBQueryGroupBy.__init__ function works"""
    utils.GuardSarmDBQueryGroupBy(filter_fields=None, offset=0, limit=1, table='agent',
                              ...
                              backend=utils.GuardSarmDBBackend(agent_id=0), get_data=None,
...
@patch('guardsarm.core.utils.GuardSarmDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('guardsarm.core.utils.GuardSarmDBQuery._get_total_items')
def test_GuardSarmDBQueryGroupBy_protected_get_total_items(mock_total, mock_socket_conn, mock_conn_db,
                                                       mock_glob, mock_exists):
    """Test utils.GuardSarmDBQueryGroupBy._get_total_items function."""
    query = utils.GuardSarmDBQueryGroupBy(filter_fields={'fields': ['name']}, offset=0,
...
@patch('guardsarm.core.utils.GuardSarmDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('guardsarm.core.utils.GuardSarmDBQuery._add_select_to_query')
@patch('guardsarm.core.utils.GuardSarmDBQuery._parse_select_filter')
def test_GuardSarmDBQueryGroupBy_protected_add_select_to_query(mock_parse, mock_add, mock_socket_conn,
                                                           mock_conn_db, mock_glob, mock_exists):
    """Test utils.GuardSarmDBQueryGroupBy._add_select_to_query function."""
    query = utils.GuardSarmDBQueryGroupBy(filter_fields={'fields': ['name']}, offset=0,
                                      limit=1, table='agent', sort=None, search=None,
                                      select={'name'}, filters=None,
                                      fields={'name': '`group`'}, query=None,
...
def test_filter_array_by_query(q, return_length):
    ...
    if return_length == -1:
        with pytest.raises(exception.GuardSarmError, match='.* 1407 .*'):
...
def test_select_array(select, required_fields, expected_result):
    ...
    except utils.GuardSarmError as e:
        assert e.code == 1724


@patch('guardsarm.core.utils.check_agents_allow_higher_versions')
@patch('guardsarm.core.utils.check_indexer')
@patch('guardsarm.core.manager.common.GUARDSARM_PATH', new=test_files_path)
def test_validate_guardsarm_xml(mock_check_indexer, mock_agents_versions):
    """Test validate_guardsarm_xml method works and methods inside are called with expected parameters"""
    ...
    with patch('builtins.open', m):
        utils.validate_guardsarm_xml(xml_file)
...
def test_validate_guardsarm_xml_ko(effect, expected_exception):
    """Tests validate_guardsarm_xml function works when open function raises an exception.
    Parameters
    ----------
    effect : Exception
    ...
    with patch('guardsarm.core.utils.load_guardsarm_xml', side_effect=effect):
        with pytest.raises(GuardSarmException, match=f'.* {expected_exception} .*'):
            utils.validate_guardsarm_xml(input_file)


def test_to_relative_path():
    """Test to_relative_path function."""
    path = 'etc/guardsarm-manager.conf'
    assert utils.to_relative_path(os.path.join(GUARDSARM_PATH, path)) == path
...
@patch('guardsarm.core.utils.copy2', new=copyfile)
...
@patch('guardsarm.core.utils.get_localtime')
...
@pytest.mark.parametrize(
    'guardsarm_localtime, system_localtime',
    [
        pytest.param(None, sentinel.system_tz, id='system_fallback'),
        pytest.param(sentinel.guardsarm_tz, sentinel.system_tz, id='guardsarm'),
        pytest.param(None, None, id='utc_fallback'),
    ]
)
@patch('guardsarm.core.utils.gettz')
def test_get_localtime(mock_gettz, guardsarm_localtime, system_localtime):
    """Test if the result is the expected timezone."""

    mock_gettz.side_effect = [guardsarm_localtime, system_localtime]

    timezone = utils.get_localtime()

    calls = [call(GUARDSARM_LOCALTIME_PATH)]

    if guardsarm_localtime is None:
        calls.append(call())
    ...
    assert timezone is (
        guardsarm_localtime
        if guardsarm_localtime is not None
        else system_localtime or timezone.utc
    )

@pytest.mark.parametrize("new_conf, original_conf, agents_conf, should_raise", [
    ("<guardsarm_config><auth></auth></guardsarm_config>",
     "<guardsarm_config><auth><allow_higher_versions>yes</allow_higher_versions></auth></guardsarm_config>",
     {'allow_higher_versions': {'allow': False}},
     True),

    ("<guardsarm_config><remote></remote></guardsarm_config>",
     "<guardsarm_config><remote><agents><allow_higher_versions>yes</allow_higher_versions></agents></remote></guardsarm_config>",
     {'allow_higher_versions': {'allow': False}},
     True),
    ...
    ("<guardsarm_config><remote><agents><allow_higher_versions>yes</allow_higher_versions></agents></remote></guardsarm_config>",
     "<guardsarm_config><remote><agents><allow_higher_versions>no</allow_higher_versions></agents></remote></guardsarm_config>",
     {'allow_higher_versions': {'allow': True}},
     False),
])
def test_check_agents_allow_higher_versions(new_conf, original_conf, agents_conf, should_raise):
    """Check if guardsarm-manager.conf agents versions are protected by the API."""
    api_conf = utils.configuration.api_conf.copy()
    api_conf['upload_configuration']['agents'].update(agents_conf)

    xml_new_conf = utils.load_guardsarm_xml(None, new_conf)
    xml_original_conf = utils.load_guardsarm_xml(None, original_conf)

    with patch('guardsarm.core.utils.configuration.api_conf', new=api_conf):
        if should_raise:
            with pytest.raises(exception.GuardSarmError, match=".* 1129 .*"):
...
@pytest.mark.parametrize("new_conf, original_conf, indexer_changed", [
    (
        "<guardsarm_config><indexer></indexer></guardsarm_config>",
        "<guardsarm_config><indexer></indexer></guardsarm_config>",
        False,
    ),
    (
        "<guardsarm_config><indexer><hosts><host>https://0.0.0.0:9200/</host></hosts></indexer></guardsarm_config>",
        "<guardsarm_config><indexer><hosts><host>https://127.0.0.1:9200/</host></hosts></indexer></guardsarm_config>",
        True,
    ),
    (
        "<guardsarm_config><indexer><ssl><key>/var/guardsarm-manager/etc/certs/server-key.pem</key></ssl>" \
        "</indexer></guardsarm_config>",
        "<guardsarm_config><indexer></indexer></guardsarm_config>",
        True,
    ),
    (
        "<guardsarm_config><indexer><ssl><key>/var/guardsarm-manager/etc/certs/server-key.pem</key></ssl>" \
        "</indexer></guardsarm_config>",
        "<guardsarm_config><indexer><ssl><key>server-key.pem</key></ssl></indexer></guardsarm_config>",
        True,
    ),
    (
        "<guardsarm_config><auth><disabled>no</disabled></auth></guardsarm_config>",
        "<guardsarm_config><auth><disabled>yes</disabled></auth></guardsarm_config>",
        False,
    ),
    (
        "<guardsarm_config><indexer><enabled>no</enabled></indexer></guardsarm_config>"
        "<guardsarm_config><integration><name>custom-test-ampersand</name><hook_url>https://localhost?querystring1=1&querystring2=2</hook_url><alert_format>json</alert_format></integration></guardsarm_config>",
        "<guardsarm_config><indexer><enabled>no</enabled></indexer></guardsarm_config>",
        False,
    ),
    (
...
def test_check_indexer(new_conf, original_conf, indexer_changed, indexer_allowed):
    """Check if the guardsarm-manager.conf indexer section is protected by the API.

    Parameters
    ----------
    new_conf : str
        New guardsarm-manager.conf to be uploaded.
    original_conf : str
        Original guardsarm-manager.conf.
    ...
    with patch('guardsarm.core.utils.configuration.api_conf', new=api_conf):
        ...
        if indexer_allowed:
            ...
            with pytest.raises(exception.GuardSarmError, match=".* 1127 .*"):
                utils.check_indexer(xml_new_conf, xml_original_conf)
