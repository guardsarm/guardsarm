...
from guardsarm.core.agent import Agent, get_agents_info, get_rbac_filters, GuardSarmDBQueryAgents
from guardsarm.core.cluster.cluster import get_node
from guardsarm.core.engine_http import EngineHTTPClient
from guardsarm.core.exception import GuardSarmException
from guardsarm.core.results import AffectedItemsGuardSarmResult
from guardsarm.core.stats import get_daemons_stats_socket
...
def get_daemons_stats_agents(daemons_list: list = None, agent_list: list = None):
    ...
    if agent_list:
        if 'all' not in agent_list:
            ...
            with GuardSarmDBQueryAgents(limit=None, select=["id", "status"], **rbac_filters) as db_query:
                data = db_query.run()
            ...
            [result.add_failed_item(id_=agent, error=exception.GuardSarmResourceNotFound(1701)) for agent in
             not_found_agents]

            # Add non-active agents to failed_items
            non_active_agents = [agent['id'] for agent in data['items'] if agent['status'] != 'active']
            [result.add_failed_item(id_=agent, error=exception.GuardSarmError(1707)) for agent in non_active_agents]
...
def get_daemons_stats(daemons_list: list = None) -> AffectedItemsGuardSarmResult:
...
@expose_resources(actions=['cluster:read'], resources=[f'node:id:{node_id}'])
def get_engine_metrics() -> AffectedItemsGuardSarmResult:
...
def get_agents_component_stats_json(agent_list: list = None, component: str = None) -> AffectedItemsGuardSarmResult:
    ...
    AffectedItemsGuardSarmResult
        Component stats.
    """
    result = AffectedItemsGuardSarmResult(all_msg='Statistical information for each agent was successfully read',
    ...
    for agent_id in agent_list:
        try:
            if agent_id not in system_agents:
                raise exception.GuardSarmResourceNotFound(1701)
