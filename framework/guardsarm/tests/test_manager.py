...
with patch('guardsarm.core.common.guardsarm_uid'):
    with patch('guardsarm.core.common.guardsarm_gid'):
        ...
        from guardsarm.manager import *
        from guardsarm.core.manager import LoggingFormat
        from guardsarm.core.tests.test_manager import get_logs
        from guardsarm import GuardSarmInternalError, GuardSarmError
...
def mock_guardsarm_path():
    with patch('guardsarm.core.common.GUARDSARM_PATH', new=test_data_path):
        yield
...
manager_status = {'guardsarm-manager-analysisd': 'running', 'guardsarm-manager-authd': 'running',
 'guardsarm-manager-monitord': 'running', 'guardsarm-manager-remoted': 'running',
 'guardsarm-manager-clusterd': 'running', 'guardsarm-manager-modulesd': 'running',
 'guardsarm-manager-db': 'running', 'guardsarm-manager-apid': 'running'}

@patch('guardsarm.core.manager.status', return_value=manager_status)
def test_get_status(mock_status):
    """Tests get_status() function works"""
    result = get_status()

    # Assert there are no errors and type returned
    assert isinstance(result, AffectedItemsGuardSarmResult), 'No expected result type'
...
@pytest.mark.parametrize('tag, level, total_items, sort_by, sort_ascending', [
    ...
    ('guardsarm-manager-modulesd:database', None, 2, None, None),
    ('guardsarm-manager-modulesd:aws-s3', None, 5, None, None),
...
@patch("guardsarm.core.manager.get_guardsarm_active_logging_format", return_value=LoggingFormat.plain)
@patch("guardsarm.core.manager.exists", return_value=True)
def test_ossec_log(mock_exists, mock_active_logging_format, tag, level, total_items, sort_by, sort_ascending):
    """Test reading guardsarm-manager.log file contents.
    ...
    tag : str
        Filters by log category (i.e. guardsarm-manager-remoted).
    total_items : int
    ...
    """
    with patch('guardsarm.core.manager.tail') as tail_patch:
...
def test_ossec_log_summary(mock_exists, mock_active_logging_format):
    ...
    expected_result = {
        'guardsarm-manager-modulesd:aws-s3': {'all': 5, 'info': 2, 'error': 1, 'critical': 0, 'warning': 2, 'debug': 0},
        'guardsarm-manager-modulesd:database': {'all': 2, 'info': 0, 'error': 0, 'critical': 0, 'warning': 0, 'debug': 2}
    }

    logs = get_logs().splitlines()
    with patch('guardsarm.core.manager.tail', return_value=logs):
...
@patch('socket.socket')
@patch('guardsarm.core.cluster.utils.fcntl')
@patch('guardsarm.core.cluster.utils.open')
...
def test_restart_ko_socket(mock_exists, mock_fcntl, mock_open):
    ...
    with pytest.raises(GuardSarmInternalError, match='.* 1901 .*'):
        restart()

    # Socket error
    with patch("os.path.exists", return_value=True):
        with patch('socket.socket', side_effect=socket.error):
            with pytest.raises(GuardSarmInternalError, match='.* 1902 .*'):
                restart()

        with patch('socket.socket.connect'):
            with patch('socket.socket.send', side_effect=socket.error):
                with pytest.raises(GuardSarmInternalError, match='.* 1014 .*'):
                    restart()
...
def test_reload_ko_socket(mock_exists, mock_fcntl, mock_open):
    ...
    Unlike restart(), reload() catches GuardSarmInternalError internally, so socket
    errors are surfaced as failed_items rather than raised exceptions.
    """
    # Socket path not exists -> GuardSarmInternalError(1901) caught, added to failed_items
    result = reload()
    ...
    # Socket connection error -> GuardSarmInternalError(1902) caught, added to failed_items
    with patch('os.path.exists', return_value=True):
        with patch('socket.socket', side_effect=socket.error):
            result = reload()
            assert result.render()['data']['total_failed_items'] == 1
        ...
        # Send error -> GuardSarmInternalError(1014) caught, added to failed_items
        with patch('socket.socket.connect'):
            with patch('socket.socket.send', side_effect=socket.error):
                result = reload()
                assert result.render()['data']['total_failed_items'] == 1
...
@pytest.mark.parametrize('error_flag, error_msg', [
    (0, ""),
    (1, "2019/02/27 11:30:07 guardsarm-manager-clusterd: ERROR: [Cluster] [Main] Error 3004 - Error in cluster configuration: "
        "Unspecified key"),
    (1, "2019/02/27 11:30:24 guardsarm-manager-authd: ERROR: (1230): Invalid element in the configuration: "
        "'use_source_i'.\n2019/02/27 11:30:24 guardsarm-manager-authd: ERROR: (1202): Configuration error at "
        "'/var/guardsarm-manage/etc/guardsarm-manager.conf'.")
])
@patch('guardsarm.manager.validate_ossec_conf')
def test_validation(mock_validate_ossec_conf, error_flag, error_msg):
    ...
    if error_flag == 0:
        ...
        mock_validate_ossec_conf.side_effect = GuardSarmError(1908, extra_message=error_msg)
...
@pytest.mark.parametrize('exception', [
    GuardSarmInternalError(1020),  # File not found
    GuardSarmError(1113),  # XML validation error
    GuardSarmError(1908)  # General validation error
])
@patch('guardsarm.manager.validate_ossec_conf')
def test_validation_ko(mock_validate, exception):
    mock_validate.side_effect = exception

    if isinstance(exception, GuardSarmInternalError):
        with pytest.raises(GuardSarmInternalError, match='.* 1020 .*'):
            validation()
    else:
        result = validation()
...
@patch('guardsarm.core.configuration.get_active_configuration')
...
@patch('guardsarm.rbac.decorators._has_update_permissions', return_value=False)
...
@patch('guardsarm.rbac.decorators._has_update_permissions', return_value=True)
@patch('builtins.open', new_callable=mock_open, read_data=_OSSEC_CONF_WITH_CLUSTER_KEY)
...
@patch('guardsarm.manager.safe_move')
@patch('guardsarm.manager.remove')
@patch('guardsarm.manager.exists', return_value=True)
@patch('guardsarm.manager.full_copy')
@patch('guardsarm.manager.validate_guardsarm_xml')
@patch('guardsarm.manager.write_ossec_conf')
@patch('guardsarm.manager.validate_ossec_conf', return_value={'status': 'OK'})
