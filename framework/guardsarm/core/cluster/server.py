...
class AbstractServerHandler(c_common.Handler):
    ...
    def hello(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        if not re.fullmatch(r'[A-Za-z0-9_-]+', node_name):
            raise exception.GuardSarmClusterError(3060, extra_message=node_name)
        elif node_name in self.server.clients:
            raise exception.GuardSarmClusterError(3028, extra_message=node_name)
        elif node_name == self.server.configuration['node_name']:
            raise exception.GuardSarmClusterError(3029)
        else:
            self.name = node_name
...
class AbstractServer:
    ...
    def __init__(self, performance_test: int, concurrency_test: int, configuration: Dict, cluster_items: Dict,
        ...
        configuration : dict
            guardsarm-manager.conf cluster configuration.
        cluster_items : dict
    ...
    def get_connected_nodes(self, filter_node: str = None, offset: int = 0, limit: int = common.DATABASE_LIMIT,
        ...
        if select is None:
            ...
            if not set(select).issubset(default_fields):
                raise exception.GuardSarmError(1724, extra_message=', '.join(set(select) - default_fields),
                                           extra_remediation=', '.join(default_fields))

        if filter_type != 'all' and filter_type not in {'worker', 'master'}:
            raise exception.GuardSarmError(1728)
        ...
        if filter_node is not None:
            ...
            if not filter_node.issubset(set(itertools.chain(self.clients.keys(), [self.configuration['node_name']]))):
                raise exception.GuardSarmResourceNotFound(1730)
