...
static const char* const XML_TAG_GUARDSARM_CONFIG = GUARDSARMCONFIG;
...
char* get_indexer_cnf(const char* cnf_file, char* err_buf, size_t err_buf_size)
{
    ...
    // Search for guardsarm_config element
    for (int i = 0; root_nodes[i] != NULL; i++)
    {
        ...
        // Skip non-guardsarm_config elements
        if (strcmp(current_root->element, XML_TAG_GUARDSARM_CONFIG) != 0)
        {
            continue;
        }
        ...
        // Search for indexer element within guardsarm_config
    ...
    snprintf(err_buf, err_buf_size, "Could not find <%s> element in configuration file %s", GUARDSARMCONFIG, cnf_file);
    XML_NODE nodes_to_clear[] = {root_nodes};
    cleanup_xml_resources(&xml, nodes_to_clear, 1);
    return NULL;
}
