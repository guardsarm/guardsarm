...
from guardsarm.core.exception import GuardSarmInternalError, GuardSarmError

DATE_FORMAT = re.compile(r'\d{4}\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2}')


class AsyncGuardSarmDBConnection:
    """
    ...
    async def _send(self, msg, raw=False):
        """Format and send message to guardsarm-manager-db socket without blocking event loop.

        Parameters
        ----------
        msg : str
            Message to be sent to guardsarm-manager-db.
        raw : bool
            If `True`, the status message from guardsarm-manager-db is included in the response.

        Returns
        -------
        str, list
            Result for the request sent to guardsarm-manager-db.
        """
        try:
            ...
            try:
                ...
                raise GuardSarmInternalError(2010, extra_message=e)

            if raw:
                return data
            elif data[0] == "err":
                raise GuardSarmError(2003, data[1])
            else:
                return json.loads(data[1], object_hook=GuardSarmDBConnection.json_decoder)
        except (FileNotFoundError, ConnectionError) as e:
            with contextlib.suppress(Exception):
                await self.open_connection()
            raise GuardSarmInternalError(2005, extra_message=e)
    ...
    async def run_wdb_command(self, command):
        ...
        ----------
        command : str
            Command to be executed inside guardsarm-manager-db
        ...
        if len(result) > 1:
            if result[0] == 'err':
                raise GuardSarmInternalError(2007, extra_message=result[1])

        else:
            if result[0] != 'ok':
                raise GuardSarmInternalError(2007)

        return result


class GuardSarmDBConnection:
    """
    ...
    def __init__(self, request_slice=500):
        ...
        ----------
        request_slice : int
            Maximum number of items to request from guardsarm-manager-db on the first call.
        """
        self.socket_path = common.WDB_SOCKET
        ...
        try:
            ...
            raise GuardSarmInternalError(2005, e)
    ...
    def __query_input_validation(self, query: str):
        ...
        for check, error_text in input_val_errors:
            if not check:
                raise GuardSarmError(2004, error_text)
    ...
    def _send(self, msg: str, raw: bool = False) -> dict:
        ...
        Raises
        ------
        GuardSarmInternalError(2009)
            Pagination error. Response from guardsarm-manager-db was over the maximum socket buffer size.
        GuardSarmError(2003)
        ...
        if data_size >= MAX_SOCKET_BUFFER_SIZE:
            raise GuardSarmInternalError(2009)
        ...
        if data[0] == "err":
            ...
            return GuardSarmDBConnection.loads(data[1])
    ...
    def loads(string: str) -> dict:
        ...
        ----------
        string : str
            String response from `guardsarm-manager-db`. It must be a dumped JSON.
        ...
        dict
            JSON object.
        """
        data = json.loads(string, object_hook=GuardSarmDBConnection.json_decoder)
        if '"(null)"' in string:
    ...
    def send(self, query: str, raw: bool = True) -> Union[str, dict]:
        ...
        ----------
        query : str
            Query to be executed in guardsarm-manager-db.
        raw : bool
    ...
    def execute(self, query, count=False, delete=False, update=False):
        ...
        def send_request_to_wdb(query_lower, step, off, response):
            try:
                ...
                response.extend(GuardSarmDBConnection.loads(request_response))
        ...
        if delete:
            ...
            if regex.match(query_lower) is None:
                raise GuardSarmError(2004, "Delete query is wrong")
        ...
        if update:
            regex = re.compile(r"\w+ \d+? sql update ([\w\d,*_ ]+) set value = '([\w\d,*_ ]+)' where key (=|like)?"
                               r" '([a-z0-9,*_%\- ]+)'")
            if regex.match(query_lower) is None:
                raise GuardSarmError(2004, "Update query is wrong")
            return self._send(query_lower)
        ...
        if not re.search(r'.?select count\([\w \*]+\)( as [^,]+)? from', query_without_where):
            ...
            try:
                ...
                raise GuardSarmError(2006, str(e))
            except (GuardSarmError, GuardSarmInternalError) as e:
                raise e
            except Exception as e:
                raise GuardSarmInternalError(2007, str(e))
