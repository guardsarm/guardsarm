...
import guardsarm.core.manager
import guardsarm.core.results as wresults
from guardsarm import agent
from guardsarm.cluster import get_node_wrapper, get_nodes_info
from guardsarm.core import common, exception
from guardsarm.core.cluster import local_client, common as c_common
from guardsarm.core.exception import GuardSarmException, GuardSarmClusterError, GuardSarmError
from guardsarm.core.pyDaemonModule import spawn_process_pool_worker, API_AUTHENTICATION_PROCESS
from guardsarm.core.guardsarm_socket import guardsarm_sendsync


authentication_funcs = {'check_token', 'check_user_master', 'get_permissions', 'get_security_conf'}

node_info = guardsarm.core.cluster.cluster.get_node()
pools = common.mp_pools.get()
if node_info['type'] == 'master':
    # Suppress exception when the user running GuardSarm cannot access /dev/shm.
...
class DistributedAPI:
    ...
    def __init__(self, f: Callable, logger: logging.getLogger, f_kwargs: Dict = None, node: c_common.Handler = None,
        ...
        self.cluster_items = guardsarm.core.cluster.utils.get_cluster_items() if node is None else node.cluster_items
        self.debug = debug
        ...
        if not basic_services:
            self.basic_services = ('guardsarm-manager-modulesd', 'guardsarm-manager-analysisd',
                                   'guardsarm-manager-db', 'guardsarm-manager-remoted')
        else:
    ...
    def debug_log(self, message):
        ...
        message : str
            Full log message.
        """
        if self.logger.name == 'guardsarm-api':
            self.logger.debug2(message)
        else:
            self.logger.debug(message)

    async def distribute_function(self) -> [Dict, exception.GuardSarmException]:
        """
        Distribute an API call.

        Returns
        -------
        dict or GuardSarmException
            Dictionary with API response or GuardSarmException in case of error.
        """
        try:
            ...
            try:
                response = json.loads(response, object_hook=c_common.as_guardsarm_object) \
                    if isinstance(response, str) else response
            except json.decoder.JSONDecodeError:
                response = {'message': response}

            return response if isinstance(response, (wresults.AbstractGuardSarmResult, exception.GuardSarmException)) \
                else wresults.GuardSarmResult(response)

        except json.decoder.JSONDecodeError:
            e = exception.GuardSarmInternalError(3036)
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            self.logger.error(f"{e.message}")
            return e
        except exception.GuardSarmInternalError as e:
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            self.logger.error(f"{e.message}", exc_info=not isinstance(e, exception.GuardSarmClusterError))
            return e
        except exception.GuardSarmError as e:
            e.dapi_errors = self.get_error_info(e)
            return e
            ...
            return exception.GuardSarmInternalError(1000,
                                                dapi_errors=self.get_error_info(e))

    def check_guardsarm_status(self):
        """
        There are some services that are required for guardsarm to correctly process API requests. If any of those services
        is not running, the API must raise an exception indicating that:
            ...
            * There's an error in any of those services that must be addressed before using the API if any service is
              in failed status.
            * GuardSarm must be started before using the API is the services are stopped.

        The basic services guardsarm needs to be running are: guardsarm-manager-modulesd, guardsarm-manager-remoted,
        guardsarm-manager-analysisd
        and guardsarm-manager-db
        """
        if self.f == guardsarm.core.manager.status:
            return

        status = guardsarm.core.manager.status()
        ...
        if not_ready_daemons:
            ...
            raise exception.GuardSarmInternalError(1017, extra_message=extra_info)
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    raise exception.GuardSarmInternalError(3021)
                except OperationalError as exc:
                    raise exception.GuardSarmInternalError(2008, extra_message=str(exc.orig))
                except process.BrokenProcessPool:
                    raise exception.GuardSarmInternalError(901)
            except json.decoder.JSONDecodeError:
                raise exception.GuardSarmInternalError(3036)
            except process.BrokenProcessPool:
                raise exception.GuardSarmInternalError(900)
            ...
            # GuardSarmClusterError
            self.logger.error(f"{e.message}",
                              exc_info=e.code not in {3021, 3036, 1913, 1017} and not isinstance(e,
                                                                                           exception.GuardSarmClusterError))
            if self.debug:
                raise
            return json.dumps(e, cls=c_common.GuardSarmJSONEncoder)
        except (exception.GuardSarmError, exception.GuardSarmResourceNotFound) as e:
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            return json.dumps(e, cls=c_common.GuardSarmJSONEncoder)
            ...
            if self.debug:
                raise
            return json.dumps(exception.GuardSarmInternalError(1000,
                                                           dapi_errors=self.get_error_info(e)),
                              cls=c_common.GuardSarmJSONEncoder)
    ...
    def get_error_info(self, e) -> Dict:
        ...
        except exception.GuardSarmException as rbac_exception:
        ...
        error_message = e.message if isinstance(e, exception.GuardSarmException) else exception.GENERIC_ERROR_MSG
        result = {node: {'error': error_message}
                  }

        # Give log path only in case of GuardSarmInternalError
        if isinstance(e, exception.GuardSarmInternalError):
            log_filename = None
            for h in self.logger.handlers or self.logger.parent.handlers:
                if hasattr(h, 'baseFilename'):
                    log_filename = os.path.join('GUARDSARM_HOME', os.path.relpath(h.baseFilename, start=common.GUARDSARM_PATH))
    ...
    async def send_tmp_file(self, node_name=None):
        ...
        # a temporary file in /var/guardsarm-manager/tmp which needs to be sent to the master before forwarding the request
        tmp_file = self.f_kwargs.get('tmp_file', '')
        if not tmp_file:
            raise exception.GuardSarmClusterError(3034, extra_message='tmp_file parameter is required')
        ...
        if not resolved_path.startswith(allowed_tmp_dir + os.sep) and resolved_path != allowed_tmp_dir:
            raise exception.GuardSarmClusterError(3034,
        ...
        res = json.loads(await client.send_file(resolved_path, node_name),
                         object_hook=c_common.as_guardsarm_object)
    ...
    async def execute_remote_request(self) -> Dict:
        ...
        node_response = await client.execute(command=b'dapi',
                                             data=json.dumps(self.to_dict(),
                                                             cls=c_common.GuardSarmJSONEncoder).encode())
        return json.loads(node_response,
                          object_hook=c_common.as_guardsarm_object)

    async def forward_request(self) -> [wresults.AbstractGuardSarmResult, exception.GuardSarmException]:
        ...
        Returns
        -------
        wresults.AbstractGuardSarmResult or exception.GuardSarmException
        """

        async def forward(node_name: Tuple) -> [wresults.AbstractGuardSarmResult, exception.GuardSarmException]:
            ...
            if node_name == self.node_info['node']:
                ...
                try:
                    ...
                    if e.code == 3022:
                        ...
                        raise e
                # Convert a non existing node into a GuardSarmError exception
                if isinstance(result, GuardSarmClusterError) and result.code == 3022:
                    common.rbac.set(self.rbac_permissions)
                    try:
                    ...
                    dikt['ids'] = {node_name}
                    result = GuardSarmError.from_dict(dikt)

            return result if isinstance(result, (wresults.AbstractGuardSarmResult, exception.GuardSarmException)) \
                else wresults.GuardSarmResult(result)
        ...
        if 'node_id' in self.f_kwargs or 'node_list' in self.f_kwargs:
            ...
            allowed_nodes = wresults.AffectedItemsGuardSarmResult()
        ...
        if allowed_nodes.total_affected_items > 1:
            ...
            if isinstance(response, wresults.AbstractGuardSarmResult):
        ...
        # It might be a GuardSarmError after reducing
        if isinstance(response, wresults.AffectedItemsGuardSarmResult):
...
class GuardSarmRequestQueue:
    """Represents a queue of GuardSarm requests"""
...
class APIRequestQueue(GuardSarmRequestQueue):
    """
    ...
    def __init__(self, server):
        ...
        self.logger = logging.getLogger('guardsarm').getChild('dapi')
        self.logger.addFilter(guardsarm.core.cluster.utils.ClusterFilter(tag='Cluster', subtag='D API'))
    ...
    async def run(self):
        while True:
            ...
            try:
                request = json.loads(request, object_hook=c_common.as_guardsarm_object)
                ...
                task_id = await node.send_string(json.dumps(result, cls=c_common.GuardSarmJSONEncoder).encode())
            except Exception as e:
...
class SendSyncRequestQueue(GuardSarmRequestQueue):
    """
    ...
    def __init__(self, server):
        super().__init__(server)
        self.logger = logging.getLogger('guardsarm').getChild('sendsync')
        self.logger.addFilter(guardsarm.core.cluster.utils.ClusterFilter(tag='Cluster', subtag='SendSync'))
    ...
    async def run(self):
        while True:
            ...
            try:
                ...
                result = await guardsarm_sendsync(**request)
