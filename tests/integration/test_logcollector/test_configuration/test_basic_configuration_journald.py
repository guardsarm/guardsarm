...
brief: The 'guardsarm-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       the 'localfile' with 'journal' as 'log_format' is set in the configuration, and the GuardSarm API returns the 
...
def test_configuration_location(test_configuration, test_metadata, truncate_monitored_files, configure_local_internal_options,
    ...
    '''
    description: Check if the 'guardsarm-logcollector' daemon starts properly when the 'journald' tag is used as log_format.
                 For this purpose, the test will configure the logcollector to monitor a 'journald'.
                 Finally, the test will verify that the GuardSarm component is started by checking its process, and the GuardSarm API returns the correct values
    ...
    assertions:
        - Verify that the GuardSarm component (agent or manager) can start when the 'jouranld' tag is used as log_format.
        - Verify that the GuardSarm API returns the correct values for the 'localfile' section.
