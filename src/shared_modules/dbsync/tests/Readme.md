# Unit Tests
## Index
1. [Compile GuardSarm](#compile-guardsarm)
...
Navigate into `guardsarm/src/dbsync/` and run the following commands:
```
mkdir build
cd build
cmake -DEXTERNAL_LIB=~/path/to/guardsarm/src/external/ -DCMAKE_BUILD_TYPE=Debug -DUNIT_TEST=ON ..
cmake --build .
```
