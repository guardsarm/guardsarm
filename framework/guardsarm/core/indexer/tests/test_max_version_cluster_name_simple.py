...
class TestUpdateAgentClusterName:
    ...
    async def test_script_contains_cluster_name_field(
        ...
        """Verify script updates guardsarm.cluster.name field."""
        mock_indexer_client.update_by_query.return_value = {"updated": 1}
        ...
        assert "guardsarm.cluster.name" in script_source
        assert "ctx._source.guardsarm.cluster.name" in script_source
...
class TestUpdateAgentClusterNameScriptContent:
    ...
    @pytest.mark.asyncio
    async def test_script_initializes_guardsarm_structure(
        self, max_version_index, mock_indexer_client
    ):
        """Verify script safely initializes guardsarm structure."""
        mock_indexer_client.update_by_query.return_value = {"updated": 1}
        ...
        # Should initialize guardsarm if null
        assert "ctx._source.guardsarm == null" in script_source or "guardsarm" in script_source
