...
- Generating load against the manager's event ingestion path
  (`guardsarm-engine`, formerly `analysisd`) without spinning up real
  agents.
...
the stack (MD5 + zlib + GuardSarm pad + PKCS#7 + AES + length prefix) is the
...
extracts as `/guardsarm/protocol/queue` (ASCII 49 = `'1'`). It is hard-coded;
...
guardsarm_padded    :  ('!' × N) || zlib_compressed     (N = 8 - len%8, or 8 if already aligned)
