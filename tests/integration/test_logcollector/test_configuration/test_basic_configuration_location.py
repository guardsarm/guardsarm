...
brief: The 'guardsarm-logcollector' daemon monitors configured files and commands for new log messages.
       Specifically, these tests will check if the GuardSarm component (agent or manager) starts when
       the 'location' tag is set in the configuration, and the GuardSarm API returns the same values for
...
# Configuration
default_config_path = Path(CONFIGURATIONS_PATH, 'guardsarm_basic_configuration_log_format_location.yaml')
...
def test_configuration_location(test_configuration, test_metadata, truncate_monitored_files, configure_local_internal_options,
                                set_guardsarm_configuration, daemons_handler, wait_for_logcollector_start):
    '''
    description: Check if the 'guardsarm-logcollector' daemon starts properly when the 'location' tag is used.
                 ...
                 GuardSarm component is started by checking its process, and the GuardSarm API returns the same
    ...
    assertions:
        - Verify that the GuardSarm component (agent or manager) can start when the 'location' tag is used.
        - Verify that the GuardSarm API returns the same value for the 'localfile' section as the configured one.
    ...
    expected_output:
        - Boolean values to indicate the state of the GuardSarm component.
    ...
    if sys.platform == WINDOWS:
        if test_metadata['location'] == 'invalidchannel':
            ...
            guardsarm_log_monitor.start(timeout=LOG_COLLECTOR_GLOBAL_TIMEOUT, callback=callback)
            assert (guardsarm_log_monitor.callback_result != None), patterns.ERROR_EVENTCHANNEL
    else:
