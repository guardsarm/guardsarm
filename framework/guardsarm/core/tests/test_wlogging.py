...
with patch('guardsarm.core.common.guardsarm_uid'):
    with patch('guardsarm.core.common.guardsarm_gid'):
        from guardsarm.core import wlogging
...
def test_sizebasedfilerotatinghandler_compute_log_directory(mock_mkdir):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        with patch("guardsarm.core.wlogging.os.path.exists", new=_mock_exists):
...
@patch('guardsarm.core.wlogging.SizeBasedFileRotatingHandler')
@patch('guardsarm.core.wlogging.TimeBasedFileRotatingHandler')
def test_guardsarm_logger_setup_logger(mock_time_handler, mock_size_handler, mock_add_handler, mock_add_level_name,
                                   max_size):
    """Test if method setup_logger of GuardSarmLogger setups the logger attribute properly.
    ...
    # To bypass the checking of the existence of a valid GuardSarm install
    # Check time handler
    with patch('os.path.join', return_value=tmp_dir.name):
        w_logger = wlogging.GuardSarmLogger(foreground_mode=True, log_path=tmp_dir.name,
...
@patch.object(wlogging.GuardSarmLogger, 'log_path', create=True, new_callable=PropertyMock)
@patch.object(wlogging.GuardSarmLogger, 'tag', create=True, new_callable=PropertyMock)
@patch.object(wlogging.GuardSarmLogger, 'logger', create=True, new_callable=PropertyMock)
@patch.object(wlogging.GuardSarmLogger, 'foreground_mode', create=True, new_callable=PropertyMock)
@patch.object(wlogging.GuardSarmLogger, 'debug_level', create=True, new_callable=PropertyMock)
@patch.object(wlogging.GuardSarmLogger, 'logger_name', create=True, new_callable=PropertyMock)
@patch.object(wlogging.GuardSarmLogger, 'custom_formatter', create=True, new_callable=PropertyMock)
@patch.object(wlogging.GuardSarmLogger, 'max_size', create=True, new_callable=PropertyMock)
@patch('logging.Formatter')
def test_guardsarm_logger__init__(mock_lformatter, mock_max_size, mock_formatter, mock_logger_name, mock_debug_level,
                              mock_foreground_mode, mock_logger, mock_tag, mock_log_path):
    """Test if GuardSarmLogger __init__ method initialize all attributes properly."""
    # To bypass the checking of the existence of a valid GuardSarm install
    with patch('os.path.join'):
        wlogging.GuardSarmLogger(foreground_mode=mock_foreground_mode, log_path=mock_log_path, tag=mock_tag,
...
@patch('guardsarm.core.wlogging.SizeBasedFileRotatingHandler')
@patch('guardsarm.core.wlogging.TimeBasedFileRotatingHandler')
def test_guardsarm_logger_getattr(mock_time_handler, mock_size_handler, attribute, expected_exception, expected_value):
    """Test if GuardSarmLogger __getattr__ method works properly."""
    tmp_dir = tempfile.TemporaryDirectory()
    # To bypass the checking of the existence of a valid GuardSarm install
    with patch('os.path.join'):
        w_logger = wlogging.GuardSarmLogger(foreground_mode=True, log_path=tmp_dir.name, tag='%(test)s %(test)s: %(test)s',
