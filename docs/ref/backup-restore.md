...
This guide provides instructions for backing up and restoring GuardSarm manager and agent data. Regular backups are essential for disaster recovery and should be performed before major operations such as upgrades or configuration changes.
...
The following components should be included in your GuardSarm manager backup strategy:

#### Essential Data

- **Configuration files**: `/var/guardsarm-manager/etc/`
  - `guardsarm-manager.conf` - Main configuration file
  - `guardsarm-manager-internal-options.conf` - Internal configuration overrides

- **Agent keys**: `/var/guardsarm-manager/etc/client.keys`
  - Contains encryption keys for registered agents
  - Critical for agent communication

- **SSL/TLS certificates**: `/var/guardsarm-manager/etc/certs/`
  - Manager certificates and keys
  - Root CA certificates

- **Global database**: `/var/guardsarm-manager/var/db/global.db`
  - Agent information (registration, metadata)
  - Agent group assignments
  - Group membership data

- **Agent groups**: `/var/guardsarm-manager/etc/shared/`
  - Group-specific configurations and files
  - Shared files distributed to agents in each group

#### Optional Data

- **Logs**: `/var/guardsarm-manager/logs/`
...
# Stop the GuardSarm manager
sudo systemctl stop guardsarm-manager

# Backup essential directories
sudo tar -czf $BACKUP_DIR/guardsarm-manager-backup.tar.gz \
    -C /var/guardsarm-manager \
    etc/ \
    var/db/global.db

# Start the GuardSarm manager
...
```bash
sudo tar -czf guardsarm-manager-config-$(date +%Y%m%d).tar.gz -C /var/guardsarm-manager etc/
```

**Agent keys only:**

```bash
sudo cp /var/guardsarm-manager/etc/client.keys guardsarm-client-keys-$(date +%Y%m%d).backup
```

**Global database only:**

```bash
sudo sqlite3 /var/guardsarm-manager/var/db/global.db ".backup 'guardsarm-global-db-$(date +%Y%m%d).db'"
```
...
tar -tzf $BACKUP_DIR/guardsarm-etc.tar.gz > /dev/null && echo "Configuration backup verified" || echo "Backup verification failed"
...
tar -tzf $BACKUP_DIR/guardsarm-etc.tar.gz | head -20
```
...
```bash
#!/bin/bash
# /usr/local/bin/guardsarm-manager-backup.sh

BACKUP_BASE="/backup/guardsarm-manager"
RETENTION_DAYS=30
...
# Perform backup
tar -czf $BACKUP_DIR/guardsarm-etc.tar.gz -C /var/guardsarm-manager etc/
sqlite3 /var/guardsarm-manager/var/db/global.db ".backup '$BACKUP_DIR/db/global.db'"

# Verify backup
if tar -tzf $BACKUP_DIR/guardsarm-etc.tar.gz > /dev/null 2>&1 && \
   sqlite3 $BACKUP_DIR/db/global.db "PRAGMA integrity_check" > /dev/null 2>&1; then
    echo "$(date): Manager backup completed successfully to $BACKUP_DIR" >> /var/log/guardsarm-backup.log

    # Remove old backups
    find $BACKUP_BASE -type d -name "backup-*" -mtime +$RETENTION_DAYS -exec rm -rf {} \;
else
    echo "$(date): Manager backup FAILED - verification error" >> /var/log/guardsarm-backup.log
    exit 1
fi
```
...
0 2 * * * /usr/local/bin/guardsarm-manager-backup.sh
```
...
2. Ensure compatible GuardSarm version
...
**Step 1: Stop the GuardSarm manager**
...
```bash
sudo mv /var/guardsarm-manager/etc /var/guardsarm-manager/etc.old.$(date +%Y%m%d)
sudo mv /var/guardsarm-manager/var/db/global.db /var/guardsarm-manager/var/db/global.db.old.$(date +%Y%m%d)
```
...
sudo tail -f /var/guardsarm-manager/logs/guardsarm-manager.log
```
...
```bash
sudo systemctl stop guardsarm-manager
sudo tar -xzf guardsarm-manager-config-YYYYMMDD.tar.gz -C /var/guardsarm-manager
sudo chown -R guardsarm-manager:guardsarm-manager /var/guardsarm-manager/etc
sudo systemctl start guardsarm-manager
```
...
```bash
sudo systemctl stop guardsarm-manager
sudo cp guardsarm-client-keys-YYYYMMDD.backup /var/guardsarm-manager/etc/client.keys
sudo chown guardsarm-manager:guardsarm-manager /var/guardsarm-manager/etc/client.keys
sudo chmod 640 /var/guardsarm-manager/etc/client.keys
sudo systemctl start guardsarm-manager
```
...
```bash
sudo systemctl stop guardsarm-manager
sudo cp guardsarm-global-db-YYYYMMDD.db /var/guardsarm-manager/var/db/global.db
sudo chown guardsarm-manager:guardsarm-manager /var/guardsarm-manager/var/db/global.db
sudo chmod 640 /var/guardsarm-manager/var/db/global.db
sudo systemctl start guardsarm-manager
```
...
sudo tar -czf $BACKUP_DIR/guardsarm-worker-config.tar.gz -C /var/guardsarm-manager/etc guardsarm-manager.conf guardsarm-manager-internal-options.conf
```
...
2. Verify cluster configuration in `/var/guardsarm-manager/etc/guardsarm-manager.conf`
3. Start the manager service
4. Verify cluster status: `sudo /var/guardsarm-manager/bin/cluster_control -l`
...
The following components should be included in your GuardSarm agent backup strategy:
...
```bash
# Create backup directory with timestamp
BACKUP_DIR="/backup/guardsarm-agent-$(date +%Y%m%d-%H%M%S)"
sudo mkdir -p $BACKUP_DIR
...
```powershell
# Create backup directory with timestamp
$BackupDir = "C:\backup\guardsarm-agent-$(Get-Date -Format 'yyyyMMdd-HHmmss')"
...
sudo sqlite3 /var/guardsarm-manager/var/db/global.db "SELECT id, name FROM agent"
...
sudo systemctl stop guardsarm-manager
sudo rm /var/guardsarm-manager/var/db/global.db
sudo cp $BACKUP_DIR/db/global.db /var/guardsarm-manager/var/db/global.db
sudo chown guardsarm-manager:guardsarm-manager /var/guardsarm-manager/var/db/global.db
sudo systemctl start guardsarm-manager
```
