...
through `guardsarm-manager-remoted`; executions are recorded in the
`guardsarm-active-responses` data stream.
...
`guardsarm-manager-clusterd` (port 1516).
...
`guardsarm-manager-analysisd`): decoding, optional enrichment (GeoIP/ASN and IOC),
...
(`guardsarm-manager-authd`, port 1515), obtains its key, and declares its agent
group.

**Event** — Normalized record produced by the Engine from raw input. Decoded
events are indexed under `guardsarm-events-v5-<category>` (or
`guardsarm-events-v5-unclassified` when only the root decoder accepted them),
regardless of whether they also produce findings.

**Finding** — Detection result produced by the Engine when a rule matches; the
5.0 replacement for 4.x alerts. Indexed under `guardsarm-findings-v5-*` with rule
metadata in `guardsarm.rule.*`.
...
GuardSarm Indexer, replacing the 4.x Filebeat sidecar.

**Internal options** — Low-level tuning keys read from
`etc/guardsarm-manager-internal-options.conf` (manager) or
...
**Server API** — RESTful management API (`guardsarm-manager-apid`, port 55000) with
JWT authentication and RBAC.

**Space** — Engine namespace that scopes content (decoders, rules, outputs);
events carry the space name in `guardsarm.space.name`.

**Syscollector** — Agent module that collects system inventory and feeds the
`guardsarm-states-inventory-*` indices and Vulnerability Detection.

**Vulnerability Detection** — Module that correlates the Syscollector inventory
against CVE content delivered by the GuardSarm CTI service (the 4.x offline feed is
gone).

**WCS (GuardSarm Common Schema)** — Field naming convention (aligned with ECS) used
across 5.0 indices and event payloads, e.g. `source.ip`, `user.name`,
`guardsarm.rule.level`.
