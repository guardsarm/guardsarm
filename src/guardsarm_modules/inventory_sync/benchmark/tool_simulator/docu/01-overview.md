...
of GuardSarm agents talking to a real `guardsarm-manager`. Each simulated agent:

1. Enrols itself against `guardsarm-authd` on port `1515` (TLS, no client cert).
2. Opens a TCP socket to `guardsarm-remoted` on port `1514`.
...
+----------+-----------+
    ...
    1) cleanup_agents.sh  (delete old bench-* agents via API)
           |
           v
    2) monitor.py &       (sample guardsarm-manager-modulesd CPU/RSS/FDs/disk)
           |
           v
    3) ===== sender =====
