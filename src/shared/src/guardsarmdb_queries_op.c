...
int wdb_insert_agent(int id,
    ...
    if (!data_in) {
        mdebug1("Error creating data JSON for GuardSarm DB.");
        return OS_INVALID;
    }
...
int wdb_update_agent_status_code(int id, agent_status_code_t status_code, const char *version, const char *sync_status, int *sock) {
    ...
    cJSON_AddNumberToObject(data_in, "status_code", status_code);
    if (version != NULL) {
        char guardsarm_version[OS_SIZE_128 + 1] = "";
        // FROZEN (GuardSarm rebrand): __guardsarm_name MUST stay "GuardSarm" here. This is
        // the machine-readable agent `version` field persisted in guardsarm-db and
        // surfaced via the API; rebranding it would rewrite stored records and
        // diverge mixed fleets. Do NOT use PRODUCT_NAME. See branding.h.
        snprintf(guardsarm_version, OS_SIZE_128, "%s %s", __guardsarm_name, version);
        cJSON_AddStringToObject(data_in, "version", guardsarm_version);
    }
    cJSON_AddStringToObject(data_in, "sync_status", sync_status);
    data_in_str = cJSON_PrintUnformatted(data_in);
...
rb_tree* wdb_get_all_agents_rbtree(int *sock) {
    ...
    if (status == WDBC_ERROR) {
        merror("Error querying GuardSarm DB to get agent's IDs.");
        rbtree_destroy(tree);
        tree = NULL;
    }
...
int wdb_find_agent(const char *name, const char *ip, int *sock) {
    ...
    if (!root) {
        merror("Error querying GuardSarm DB for agent ID.");
        return OS_INVALID;
    }
...
cJSON* wdb_get_agent_info(int id, int *sock) {
    ...
    if (!root) {
        merror("Error querying GuardSarm DB to get the agent's %d information.", id);
        return NULL;
    }
...
char* wdb_get_agent_name(int id, int *sock) {
    ...
    if (!root) {
        merror("Error querying GuardSarm DB to get the agent's %d name.", id);
        return NULL;
    }
...
char* wdb_get_agent_group(int id, int *sock) {
    ...
    if (!root) {
        merror("Error querying GuardSarm DB to get the agent's %d group.", id);
        return NULL;
    }
...
int wdb_find_group(const char *name, int *sock) {
    ...
    if (!root) {
        merror("Error querying GuardSarm DB to get the agent group id.");
        return OS_INVALID;
    }
...
int wdb_update_groups(const char *dirname, int *sock) {
    ...
    if (!root) {
        merror("Error querying GuardSarm DB to update groups.");
        if (!sock) {
            wdbc_close(&aux_sock);
        }
        return OS_INVALID;
...
cJSON* wdb_get_distinct_agent_groups(int *sock) {
    ...
    if (status == WDBC_ERROR) {
        merror("Error querying GuardSarm DB to get agent's groups.");
        cJSON_Delete(root);
        root = NULL;
    }
