...
references:
    ...
    - https://documentation.guardsarmsiem.com/current/user-manual/reference/ossec-conf/syscheck.html#diff
...
from guardsarm_testing.constants.platforms import MACOS, WINDOWS
...
from guardsarm_testing.modules.fim.patterns import EVENT_TYPE_MODIFIED, EVENT_TYPE_ADDED, ERROR_MSG_FIM_EVENT_NOT_DETECTED, \
                                               EVENT_TYPE_DELETED, EVENT_TYPE_REPORT_CHANGES, ERROR_MSG_REPORT_CHANGES_EVENT_NOT_DETECTED
from guardsarm_testing.modules.fim.utils import make_diff_file_path, get_fim_event_data
from guardsarm_testing.tools.monitors.file_monitor import FileMonitor
from guardsarm_testing.utils.file import write_file_write, delete_files_in_folder, truncate_file
...
def test_reports_file_and_nodiff(test_configuration, test_metadata, configure_local_internal_options,
    ...
    '''
    description: Check if the 'guardsarm-syscheckd' daemon reports the file changes (or truncates if required)
    ...
    guardsarm_log_monitor.start(generate_callback(EVENT_TYPE_ADDED), timeout=30)
    ...
    assert 'No content changes were found for this file.' in str(guardsarm_log_monitor.callback_result[0]), 'Wrong content_changes field'
    ...
    guardsarm_log_monitor.start(generate_callback(EVENT_TYPE_MODIFIED), timeout=20)
    ...
    assert get_fim_event_data(guardsarm_log_monitor.callback_result)['file']['mode'] == test_metadata.get('fim_mode')
