...
#   ./run_benchmark.sh --scenario scenarios/base_windows_sca.json --label sca-run1
...
# NOTE: start_ack_timeout / end_ack_timeout / end_ack_processing_timeout /
# post_data_delay are scenario-only (set them in the scenario JSON, either
# per-step or via "defaults"). They are intentionally NOT plumbed here.
KEEPALIVE_INTERVAL=""
CLEANUP_AFTER=false
COMPARE_MODE=false
COMPARE_DIRS=()
CHART_FORMAT="png"
MANAGER_LOG="/var/guardsarm-manager/logs/guardsarm-manager.log"
...
Remote manager (SSH) — activated when --manager is not 127.0.0.1:
      ...
      --indexer-port N    Local guardsarm-indexer port to reverse-tunnel so the
...
REMOTE_MONITOR_DIR="/tmp/guardsarm_bench_monitor"
SSH_SOCKET="/tmp/guardsarm_bench_ssh_$$"
...
if $REMOTE_MODE; then
    ...
    if "${SSH_PREFIX[@]}" ssh "${SSH_OPTS[@]}" "root@$MANAGER" \
            "/opt/guardsarm-monitor-venv/bin/python3 -c 'import psutil'" 2>/dev/null; then
        echo "  psutil: OK (venv)"
        ...
        echo "    curl -fsSL https://raw.githubusercontent.com/guardsarm/guardsarm/<branch>/src/engine/tools/devContainer/scripts/setup_monitor.sh | sudo bash"
        exit 1
    fi

    echo "Checking guardsarm-manager status on remote..."
    REMOTE_STATUS=$("${SSH_PREFIX[@]}" ssh "${SSH_OPTS[@]}" "root@$MANAGER" \
        "/var/guardsarm-manager/bin/guardsarm-manager-control status" 2>&1) || true
    if [[ -z "$REMOTE_STATUS" ]] || echo "$REMOTE_STATUS" | grep -q "not running"; then
        echo "Error: guardsarm-manager is not fully running on $MANAGER"
        echo "$REMOTE_STATUS"
        exit 1
    fi
    ...
    echo "  -L 55000:localhost:55000 (GuardSarm API)"
...
MONITOR_MODULESD_CSV="$MONITOR_DIR/guardsarm-manager-modulesd.csv"
...
cat > "$RESULTS_DIR/params.json" <<PARAMS
{
    ...
    "manager": "$MANAGER",
    "port": $PORT,
    "process": "guardsarm-manager-modulesd",
...
if $REMOTE_MODE; then
    ...
    if ssh -S "$SSH_SOCKET" "root@$MANAGER" \
            "test -x /opt/guardsarm-monitor-venv/bin/python3" 2>/dev/null; then
        REMOTE_PYTHON="/opt/guardsarm-monitor-venv/bin/python3"
        echo "  Using remote venv: /opt/guardsarm-monitor-venv"
    fi
    ...
    if ! kill -0 "$MONITOR_BG_PID" 2>/dev/null; then
        ...
        echo "  Make sure the guardsarm-manager daemons are running"
        exit 1
    fi
