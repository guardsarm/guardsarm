...
def test_queue_size_syslog(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
                            set_guardsarm_configuration, restart_guardsarm_expect_error, get_real_configuration):

    '''
    description: Check that when 'guardsarm-manager-remoted' sets a valid queue size. For this purpose, it uses the configuration
