...
GUARDSARM_VERSION = "4.5.0"

TEST_GUARDSARM_PATH = "/var/ossec"
TEST_DATABASE = "test"
...
def get_guardsarm_integration_parameters(service_name: str = TEST_SERVICE_NAME, profile: str = TEST_AWS_PROFILE,
    ...
    """Return a dict containing every parameter supported by GuardSarmIntegration. Used to simulate different ossec.conf
    configurations.
...
def get_guardsarm_aws_database_parameters(service_name: str = TEST_SERVICE_NAME, profile: str = TEST_AWS_PROFILE,
                                      db_name: str = TEST_DATABASE, access_key: str = None, secret_key: str = None,
                                      iam_role_arn: str = None, region: str = None, discard_field: str = None,
                                      discard_regex: str = None, sts_endpoint: str = None, service_endpoint: str = None,
                                      iam_role_duration: str = None, external_id: str = None):
    """Return a dict containing every parameter supported by GuardSarmAWSDatabase.
...
def get_mocked_guardsarm_integration(**kwargs):
    with patch('guardsarm_integration.GuardSarmIntegration.get_client'), \
            patch('guardsarm_integration.sqlite3.connect'), \
            patch('guardsarm_integration.utils.find_guardsarm_path', return_value=TEST_GUARDSARM_PATH), \
            patch('guardsarm_integration.utils.get_guardsarm_version', return_value=GUARDSARM_VERSION):
        return guardsarm_integration.GuardSarmIntegration(**get_guardsarm_integration_parameters(**kwargs))


def get_mocked_guardsarm_aws_database(**kwargs):
    with patch('guardsarm_integration.GuardSarmAWSDatabase.check_metadata_version'), \
            patch('guardsarm_integration.GuardSarmIntegration.get_client'), \
            patch('guardsarm_integration.sqlite3.connect'), \
            patch('guardsarm_integration.utils.find_guardsarm_path', return_value=TEST_GUARDSARM_PATH), \
            patch('guardsarm_integration.utils.get_guardsarm_version', return_value=GUARDSARM_VERSION):
        return guardsarm_integration.GuardSarmAWSDatabase(**get_guardsarm_aws_database_parameters(**kwargs))
