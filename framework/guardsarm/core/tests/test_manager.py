...
with patch('guardsarm.core.common.guardsarm_uid'):
    with patch('guardsarm.core.common.guardsarm_gid'):
        from guardsarm.core.manager import *
...
@patch('guardsarm.core.cluster.utils.glob')
...
def test_get_guardsarm_log_fields():
    """Test get_guardsarm_log_fields() method returns a tuple"""
    result = get_guardsarm_log_fields('2020/07/14 06:10:40 rootcheck: INFO: Ending rootcheck scan.')
    assert isinstance(result, tuple), 'The result is not a tuple'
    assert result[0] == datetime(2020, 7, 14, 6, 10, 40, tzinfo=timezone.utc)
    assert result[1] == 'guardsarm-rootcheck'
    assert result[2] == 'info'
    assert result[3] == ' Ending rootcheck scan.'


def test_get_guardsarm_log_fields_ko():
    """Test get_guardsarm_log_fields() method returns None when nothing matches """
    result = get_guardsarm_log_fields('DEBUG')
    assert not result
...
def test_get_guardsarm_logs(log_format):
    """Test get_guardsarm_logs() method returns result with expected information"""
    logs = get_logs(json_log=log_format == LoggingFormat.json).splitlines()

    with patch("guardsarm.core.manager.get_guardsarm_active_logging_format", return_value=log_format):
        with pytest.raises(GuardSarmInternalError, match=".*1020.*"):
            get_guardsarm_logs()

        with patch('guardsarm.core.manager.exists', return_value=True):
            with patch('guardsarm.core.manager.tail', return_value=logs):
                result = get_guardsarm_logs()
...
def test_get_logs_summary(mock_exists, mock_active_logging_format):
    ...
    with patch('guardsarm.core.manager.tail', return_value=logs):
        ...
        assert result['guardsarm-manager-modulesd:database'] == {'all': 2, 'info': 0, 'error': 0, 'critical': 0, 'warning': 0,
                                                     'debug': 2}


@patch('guardsarm.core.manager.exists', return_value=True)
@patch('guardsarm.core.manager.load_guardsarm_xml')
...
@patch('guardsarm.core.manager.load_guardsarm_xml')
@patch("guardsarm.core.manager.exists")
def test_validation_ko(mock_exists, mock_load_xml):
    ...
    mock_load_xml.side_effect = GuardSarmError(1113, 'Invalid XML syntax')
    ...
    with pytest.raises(GuardSarmError, match='.* 1908 .*'):
        validate_ossec_conf()



@patch('guardsarm.core.manager.configuration.api_conf', new={'max_upload_size': 0})
