...
from guardsarm.core.exception import GuardSarmIndexerError, IndexerUnavailableError
from guardsarm.core.indexer.credential_manager import KeystoreClient
from guardsarm.core.indexer.max_version_components import MaxVersionIndex
from guardsarm.core.indexer.metrics import MetricsIndex

MAX_RETRIES = 3
BACKOFF_TIMEOUT = 60

logger = getLogger("guardsarm")


def resolve_guardsarm_path(path: str) -> str:
    """Resolve GuardSarm-relative paths from the manager installation directory."""
    return path if isabs(path) else join(common.GUARDSARM_PATH, path)


class Indexer:
    """
    Interface to connect with GuardSarm Indexer.

    This class handles the asynchronous connection to the GuardSarm Indexer
    (OpenSearch) nodes, managing authentication and SSL configuration.

    Parameters
    ----------
    hosts : List[str]
        List of hostnames or IP addresses of the GuardSarm Indexer nodes.
    ports : List[int]
    ...
    async def connect(self) -> None:
        """
        Establish a connection to the GuardSarm Indexer and verify its status.
        ...
        try:
            ...
            raise GuardSarmIndexerError(2200, None, e.error)
        except ssl.SSLError as e:
            raise GuardSarmIndexerError(2200, None, e.reason)
    ...
    async def close(self) -> None:
        """
        Close the GuardSarm Indexer client session asynchronously.
        """
    ...
    async def search(self, *args, **kwargs):
        """
        Perform a search query against the GuardSarm Indexer.
    ...
    async def mget(self, *args, **kwargs):
        """
        Perform a multi-get query against the GuardSarm Indexer.
    ...
    async def healthcheck(self) -> bool:
        """Check the health of the GuardSarm Indexer connection.
        ...
        `GuardSarmIndexerError` is raised with the appropriate error code and
        message.
        ...
        try:
            ...
            raise GuardSarmIndexerError(2200, extra_message=f"Failed to create indexer client: {e}")
...
async def create_indexer(retries: int = MAX_RETRIES, backoff: int = BACKOFF_TIMEOUT, **kwargs) -> Indexer:
    ...
    **kwargs : dict
    ...
    try:
        ...
        raise GuardSarmIndexerError(2201, extra_message=f"Invalid arguments for Indexer: Error:{e}") from e

    for attempt in range(retries + 1):
        try:
            await indexer.connect()
            return indexer
        except GuardSarmIndexerError as e:
            if attempt == retries:
                await indexer.close()
                logger.warning(
...
async def get_indexer_client() -> AsyncIterator[Indexer]:
    ...
    This utility fetches configuration from the GuardSarm OSSEC config and
    ...
    """
    try:
        guardsarm_config = get_ossec_conf(section="indexer")
        if not guardsarm_config:
            raise IndexerUnavailableError(
                code=2200, extra_message="Missing indexer configuration in GuardSarm config"
            )
    except Exception as e:
        raise IndexerUnavailableError(
            code=2200, extra_message=f"Failed to parse GuardSarm configuration: {e}"
        )

    indexer_section = guardsarm_config.get("indexer", {})
    ...
    client_cert = resolve_guardsarm_path(ssl_config["certificate"][0])
    client_key = resolve_guardsarm_path(ssl_config["key"][0])
    ca_certs = resolve_guardsarm_path(ssl_config["certificate_authorities"][0]["ca"][0])
