...
2. Provide a **Name** for the sender (e.g., `guardsarm-security-smtp-sender`).
3. Enter the **Outbound email address** (this replaces [`global.email_from`](#email-alerts-mapping) in GuardSarm 4.x). For example, `noreply@guardsarm.local`.
4. Specify the **Host** (this replaces [`global.smtp_server`](#email-alerts-mapping) in GuardSarm 4.x) - e.g., `mailpit` - and the **Port** - e.g., `1025`.
...
6. If you selected SSL/TLS or STARTTLS, authenticate the sender account by storing its credentials in the OpenSearch keystore. On **each indexer node**, run the following commands via the terminal (replace `<sender_name>` with the name you entered in step 2):
   ...
   ```
   /usr/share/guardsarm-indexer/bin/opensearch-keystore add opensearch.notifications.core.email.<sender_name>.username
   /usr/share/guardsarm-indexer/bin/opensearch-keystore add opensearch.notifications.core.email.<sender_name>.password
   ```

7. After adding credentials to all nodes, go to the GuardSarm dashboard **DevTools** and call the reload API to apply the changes without restarting OpenSearch:
...
> **GuardSarm 4.x note:** SMTP authentication was not supported in GuardSarm 4.x - users typically relayed through a local Postfix or similar MTA. In GuardSarm 5.0, the GuardSarm dashboard **Notifications** plugin supports SMTP authentication directly via the OpenSearch keystore. Certificate-based SMTP authentication (e.g., Postfix with TLS client certificates) is not supported by the GuardSarm dashboard **Notifications** plugin in the current release.
...
3. Enter a **Name** for the group (e.g., `guardsarm-security-recipients`).
...
5. In the **Emails** field, type or select one or more email addresses (e.g., `guardsarm-security-recipients@test.com`).
...
2. Enter a **Name** for the channel (e.g., `guardsarm-security-mail-channel`).
3. (Optional) Provide a **Description** clarifying the purpose of this channel (e.g., `Email channel used to deliver GuardSarm security alerts, findings, and incident notifications.`)
4. Under **Configurations**, select **Email** as the **Channel type**.
5. Choose your **Sender type** (e.g., **SMTP sender**).
6. In the **SMTP sender** field, select the sender you created in [Step 1](#1-creating-an-email-sender) (e.g., `guardsarm-security-smtp-sender`).
7. Under **Default recipients**, select the recipient group you created in [Step 2](#2-creating-an-email-recipient-group) (e.g., `guardsarm-security-recipients`).
...
In GuardSarm 4.x, alert severity was expressed as a numeric `rule.level` (0–15), and the `<level>` filter in `<email_alerts>` selected a threshold. In GuardSarm 5.0, the monitor **query** determines which documents match - there is no separate severity filter on the trigger. The trigger severity level (1–Highest to 5–Lowest) sets the action severity, not the data filter.
...
Create a **monitor** in the GuardSarm dashboard **Alerting** plugin according to your needs (for example, a **per-document monitor** to trigger on individual matching documents). Define a query that selects the documents you want to alert on and set the checking schedule. The image below shows an example configured for SSH root login events.
...
2. Verify the alert appears in the GuardSarm dashboard **Alerting** plugin under the **Alerts** tab.
...
- **Index pattern**: `guardsarm-findings-v5-system-activity`
- **Query**: `guardsarm.rule.title` is `SSH root login via password authentication`
...
1. Navigate to the **Reporting** plugin in the GuardSarm dashboard.
