...
from guardsarm.core.cluster import client, cluster, common as c_common
from guardsarm.core.cluster.common import IndexerTaskManager
from guardsarm.core.cluster.dapi import dapi
from guardsarm.core.cluster.utils import log_subprocess_execution, safe_join
...
from guardsarm.core.utils import safe_move, get_utc_now
...
class ReceiveAgentGroupsTask(c_common.ReceiveStringTask):
    ...
    def set_up_coro(self) -> Callable:
        ...
        return self.guardsarm_common.recv_agent_groups_periodic_information
    ...
    def done_callback(self, future=None):
        ...
        self.guardsarm_common.sync_agent_groups_free = True
...
class ReceiveEntireAgentGroupsTask(c_common.ReceiveStringTask):
    ...
    def set_up_coro(self) -> Callable:
        """Set up the function to be called when the worker sends its Agent groups."""
        return self.guardsarm_common.recv_agent_groups_entire_information
...
class ReceiveIntegrityTask(c_common.ReceiveFileTask):
    ...
    def set_up_coro(self) -> Callable:
        ...
        return self.guardsarm_common.process_files_from_master
    ...
    def done_callback(self, future=None):
        ...
        future : asyncio.Future object
            Synchronization process result.
        """
        self.guardsarm_common.check_integrity_free = True
        super().done_callback(future)


class WorkerHandler(client.AbstractClient, c_common.GuardSarmCommon):
    """
    ...
    def connection_result(self, future_result):
        ...
        if self.connected:
            ...
            worker_tmp_files = os.path.join(common.GUARDSARM_PATH, 'queue', 'cluster', self.name)
    ...
    def get_manager(self):
        """Get the Worker object that created this WorkerHandler. Used in the class GuardSarmCommon.
    ...
    def error_receiving_integrity(self, data: str) -> Tuple[bytes, bytes]:
        ...
        ----------
        data : str
            Task ID and error formatted as GuardSarmJSONEncoder.
        ...
        while True:
            ...
            # If exception is raised during sync process, notify the master so it removes the file if received.
                ...
                if isinstance(e, exception.GuardSarmException):
                    exc = json.dumps(e, cls=c_common.GuardSarmJSONEncoder)
                else:
                    exc = json.dumps(exception.GuardSarmClusterError(1000, extra_message=str(e)),
    ...
    async def sync_agent_info(self):
        ...
        A JSON object with the information of all local agents is retrieved from local guardsarm-manager-db socket
        and sent to the master's guardsarm-manager-db.
        """
        logger = self.task_loggers["Agent-info sync"]
        agent_info = c_common.SyncGuardSarmdb(manager=self, logger=logger, data_retriever=None, cmd=b'syn_a_w_m',
                ...
                if self.connected:
                    if await agent_info.request_permission():
                        ...
                        if agents_sync is None:
                            raise GuardSarmException(code=2017)
    ...
    async def sync_extra_valid(self, extra_valid: Dict):
        ...
        # If exception is raised during sync process, notify the master, so it removes the file if received.
            ...
            if isinstance(e, exception.GuardSarmException):
                ...
                exc = json.dumps(exception.GuardSarmClusterError(1000, extra_message=str(e)), cls=c_common.GuardSarmJSONEncoder)
            with contextlib.suppress(Exception):
                await self.send_request(command=b'syn_i_w_m_r', data=f"None {exc}".encode())
    ...
    async def process_files_from_master(self, name: str, file_received: asyncio.Event):
        ...
        if isinstance(received_filename, Exception):
            exc_info = json.dumps(exception.GuardSarmClusterError(
                1000, extra_message=str(self.sync_tasks[name].filename)), cls=c_common.GuardSarmJSONEncoder)
            with contextlib.suppress(Exception):
    ...
    def update_master_files_in_worker(ko_files: Dict, zip_path: str, cluster_items: Dict):
        ...
        def overwrite_or_create_files(filename_: str, data_: Dict):
            ...
            """
            full_filename_path = safe_join(common.GUARDSARM_PATH, filename)
            ...
            if data_['merged']:  # worker nodes can only receive agent-groups files
                ...
                # and then move each one to the destination directory (<guardsarm_path>/filename).
                # The TYPE string used in the 'unmerge_info' function is a placeholder. It corresponds to the
                # directory inside '{guardsarm_path}/queue/' path.
                for name, content, _ in cluster.unmerge_info('TYPE', zip_path, filename_):
                    full_unmerged_name = safe_join(common.GUARDSARM_PATH, name)
                ...
                # Move the file from zipdir (directory containing unzipped files) to <guardsarm_path>/filename.
            ...
            # Remove local files marked as extra.
                ...
                for file_to_remove in files:
                    try:
                        ...
                        file_path = safe_join(common.GUARDSARM_PATH, file_to_remove)
                        try:
                            os.remove(file_path)
                        except OSError as e:
        ...
        for directory in directories_to_check:
            try:
                full_path = safe_join(common.GUARDSARM_PATH, directory)
