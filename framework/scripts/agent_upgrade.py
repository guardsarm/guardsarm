...
# Set framework path
path.append(dirname(argv[0]) + '/../framework')  # It is necessary to import GuardSarm package

# Import framework
try:
    import guardsarm.agent
    from guardsarm.agent import upgrade_agents, get_upgrade_result, get_agents
...
def get_script_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("-v", "--version", type=str, help="Version to upgrade. [Default: latest GuardSarm version]")
...
def list_outdated():
    ...
    agents = guardsarm.agent.get_outdated_agents()
...
async def main():
    ...
    except GuardSarmError as guardsarm_err:
        print(f"Error {guardsarm_err.code}: {guardsarm_err.message}")
        if args.debug:
            raise
