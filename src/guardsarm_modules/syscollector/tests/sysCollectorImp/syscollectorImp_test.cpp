...
#define EXPECT_CALL_SERVICES_JSON R"([{"service_id":"guardsarm-agent","service_name":"GuardSarm Agent","service_description":"Monitors system activity","service_state":"running","service_sub_state":"subState","service_start_type":"auto","service_type":"type","process_pid":1234,"service_exit_code":0,"service_win32_exit_code":0,"process_executable":"/usr/bin/guardsarm-agent","service_address":"/lib/systemd/system/guardsarm-agent.service","user_name":"root","service_enabled":"enabled","service_following":"following","service_object_path":"objectPath","service_target_ephemeral_id":0,"service_target_type":"jobType","service_target_address":"jobPath","file_path":"sourcePath"}])"
...
{
    R"({"collector":"dbsync_services","data":{"error":{"log":{"file":{"path":null}}},"event":{"changed_fields":[],"type":"created"},"file":{"path":"sourcePath"},"log":{"file":{"path":null}},"process":{"args":null,"executable":"/usr/bin/guardsarm-agent","group":{"name":null},"pid":1234,"root_directory":null,"user":{"name":null},"working_directory":null},"service":{"address":"/lib/systemd/system/guardsarm-agent.service","description":"Monitors system activity","enabled":"enabled","exit_code":0,"following":"following","frequency":null,"id":"guardsarm-agent","inetd_compatibility":null,"name":"GuardSarm Agent","object_path":"objectPath","restart":null,"start_type":"auto","starts":{"on_mount":null,"on_not_empty_directory":null,"on_path_modified":null},"state":"running","sub_state":"subState","target":{"address":"jobPath","ephemeral_id":"0","type":"jobType"},"type":"type","win32_exit_code":0}},"module":"inventory"})"
};
...
void SyscollectorImpTest::SetUp()
{
    ...
    mockValidators["guardsarm-states-inventory-hardware"] = m_mockValidator;
    mockValidators["guardsarm-states-inventory-system"] = m_mockValidator;
    mockValidators["guardsarm-states-inventory-interfaces"] = m_mockValidator;
    mockValidators["guardsarm-states-inventory-protocols"] = m_mockValidator;
    mockValidators["guardsarm-states-inventory-networks"] = m_mockValidator;
    mockValidators["guardsarm-states-inventory-ports"] = m_mockValidator;
    mockValidators["guardsarm-states-inventory-packages"] = m_mockValidator;
    mockValidators["guardsarm-states-inventory-processes"] = m_mockValidator;
    mockValidators["guardsarm-states-inventory-hotfixes"] = m_mockValidator;
    mockValidators["guardsarm-states-inventory-groups"] = m_mockValidator;
    mockValidators["guardsarm-states-inventory-users"] = m_mockValidator;
    mockValidators["guardsarm-states-inventory-services"] = m_mockValidator;
    mockValidators["guardsarm-states-inventory-browser-extensions"] = m_mockValidator;
...
{
    R"({"checksum":{"hash":{"sha1":"daa615e783788aec35ae17eeed912ace3910f209"}},"error":{"log":{"file":{"path":null}}},"file":{"path":"sourcePath"},"log":{"file":{"path":null}},"process":{"args":null,"executable":"/usr/bin/guardsarm-agent","group":{"name":null},"pid":1234,"root_directory":null,"user":{"name":null},"working_directory":null},"service":{"address":"/lib/systemd/system/guardsarm-agent.service","description":"Monitors system activity","enabled":"enabled","exit_code":0,"following":"following","frequency":null,"id":"guardsarm-agent","inetd_compatibility":null,"name":"GuardSarm Agent","object_path":"objectPath","restart":null,"start_type":"auto","starts":{"on_mount":null,"on_not_empty_directory":null,"on_path_modified":null},"state":"running","sub_state":"subState","target":{"address":"jobPath","ephemeral_id":"0","type":"jobType"},"type":"type","win32_exit_code":0}})"
};
...
TEST_F(SyscollectorImpTest, schemaValidationAcceptsValidDataAfterCorrections)
{
    ...
    EXPECT_CALL(wrapperPersist, callbackMock(testing::_, testing::_, testing::Eq("guardsarm-states-inventory-hardware"), testing::_, testing::_)).Times(1);
...
TEST_F(SyscollectorImpTest, schemaValidationWithCorrectedDataTypes)
{
    ...
    std::function<void(const std::string&, Operation_t, const std::string&, const std::string&, uint64_t)> callbackDataPersist
    {
        [&wrapperPersist](const std::string & id, Operation_t operation, const std::string & index, const std::string & data, uint64_t version)
        {
            ...
            if (index == "guardsarm-states-inventory-hardware")
            {
            ...
            else if (index == "guardsarm-states-inventory-ports")
            {
    ...
    EXPECT_CALL(wrapperPersist, callbackMock(testing::_, testing::_, testing::Eq("guardsarm-states-inventory-hardware"), testing::_, testing::_)).Times(1);
    EXPECT_CALL(wrapperPersist, callbackMock(testing::_, testing::_, testing::Eq("guardsarm-states-inventory-ports"), testing::_, testing::_)).Times(1);
...
TEST_F(SyscollectorImpTest, schemaValidationRejectsInvalidDataWithMock)
{
    ...
    EXPECT_CALL(wrapperPersist, callbackMock(testing::_, testing::_, testing::Eq("guardsarm-states-inventory-hardware"), testing::_, testing::_)).Times(0);
    ...
    .WillRepeatedly(testing::Return("guardsarm-states-inventory-hardware"));

    // Inject mock validator into factory
    std::map<std::string, std::shared_ptr<SchemaValidator::ISchemaValidatorEngine>> mockValidators;
    mockValidators["guardsarm-states-inventory-hardware"] = mockValidator;
...
TEST_F(SyscollectorImpTest, schemaValidationDiscardsWhenValidatorNotFound)
{
    ...
    for (const auto& msg : *loggedMessages)
    {
        if (msg.find("No schema validator found for index: guardsarm-states-inventory-hardware") != std::string::npos)
        {
...
TEST_F(SyscollectorImpTest, DocumentLimits_ValidInput_UnlimitedPackages)
{
    ...
    EXPECT_TRUE(logCapture->contains(LOG_DEBUG, "Document limit set to unlimited for index 'guardsarm-states-inventory-packages'"))
...
TEST_F(SyscollectorImpTest, DocumentLimits_EndToEnd_Summary)
{
    ...
    EXPECT_TRUE(logCapture->contains(LOG_DEBUG, "Document limit set to unlimited for index 'guardsarm-states-inventory-processes'"))
