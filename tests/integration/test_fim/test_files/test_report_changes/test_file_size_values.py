...
references:
    ...
    - https://documentation.guardsarmsiem.com/current/user-manual/reference/ossec-conf/syscheck.html#file-size
...
from guardsarm_testing.modules.fim.configuration import (
    SYSCHECK_DEBUG,
    FILE_MAX_SIZE,
    RT_DELAY,
)
...
def test_file_size_values(
    ...
    assert "test_string" in content_changes or "More changes..." in content_changes, (
    ...
    guardsarm_log_monitor.start(callback=generate_callback(DIFF_FOLDER_DELETED), timeout=30)
    ...
    guardsarm_log_monitor.start(
        callback=generate_callback(FILE_SIZE_LIMIT_REACHED), timeout=60
    )
