...
import guardsarm.core.exception as wexception
from guardsarm.core import utils
from guardsarm.core.common import DATABASE_LIMIT

current_module = sys.modules[__name__]


class AbstractGuardSarmResult(collections.abc.MutableMapping):
    """
    ...
    def __init__(self, dct: Union[dict, object]):
        ...
        Raises
        ------
        wexception.GuardSarmInternalError(1000)
            If dct is of a wrong type.
        """
        if isinstance(dct, dict):
            self.dikt = dct
        elif isinstance(dct, AbstractGuardSarmResult):
            self.dikt = dct.dikt
        else:
            raise wexception.GuardSarmInternalError(1000, extra_message=f"dct param must be a dict or "
                                                                    f"an AbstractGuardSarmResult subclass, "
                                                                    f"not a {type(dct)}")
    ...
    def __or__(self, other):
        """ | operator used to merge two AbstractGuardSarmResult objects. When merged with a GuardSarmException, the result is
        always a GuardSarmException

        Parameters
        ----------
        other : AbstractGuardSarmResult or wexception.GuardSarmException

        Raises
        ------
        GuardSarmInternalError(1000)
            GuardSarm internal error when two object could not be merged.

        Returns
        -------
        AbstractGuardSarmResult or wexception.GuardSarmException
            Resultant object.
        """
        if isinstance(other, wexception.GuardSarmException):
            return other
        elif not isinstance(other, (dict, AbstractGuardSarmResult)):
            raise wexception.GuardSarmInternalError(1000, extra_message=f"Cannot be merged with {type(other)} object")
    ...
    def _merge_dict(self, self_field: dict, other_field: dict, key: str = None) -> dict:
        ...
        AbstractGuardSarmResult subclass. This method may be redefined in subclasses.
    ...
    def limit(self, limit: int = DATABASE_LIMIT, offset: int = 0):
        """Should take the first `limit` results starting from `offset`. To be redefined in GuardSarmResult subclasses.

        Parameters
        ----------
        limit : int
            Default: the value specified in guardsarm.core.common.DATABASE_LIMIT
        offset : int
            Default: 0.

        Returns
        -------
        AbstractGuardSarmResult
            Filtered AbstractGuardSarmResult
        """
        result = deepcopy(self)
    ...
    def sort(self, fields: list = None, order: str = 'asc'):
        """Sort according to `fields` in order `order`. To be redefined in GuardSarmResult subclasses.
        ...
        -------
        AbstractGuardSarmResult
            Sorted AbstractGuardSarmResult
        """
...
class GuardSarmResult(AbstractGuardSarmResult):
    """
...
class AffectedItemsGuardSarmResult(AbstractGuardSarmResult):
    ...
    def add_failed_item(self, id_: str = None, error: wexception.GuardSarmException = None):
        ...
        error : wexception.GuardSarmException
    ...
    def add_failed_items_from(self, other):
        ...
        Parameters
        ----------
        other : AffectedItemsGuardSarmResult
        ...
        wexception.GuardSarmInternalError(1000)
            If dct is of a wrong type.
        """
        if not isinstance(other, AffectedItemsGuardSarmResult):
            raise wexception.GuardSarmInternalError(1000,
    ...
    def __or__(self, other):
        """Merge a AffectedItemsGuardSarmResult with self.

        Parameters
        ----------
        other : AffectedItemsGuardSarmResult
            AffectedItemsGuardSarmResult object to merge.
        ...
        if isinstance(other, wexception.GuardSarmError):
            if len(other.ids) > 0:
                ...
                return other
        elif isinstance(result, wexception.GuardSarmException):
            return result
        elif not isinstance(other, AffectedItemsGuardSarmResult):
            raise wexception.GuardSarmInternalError(1000, extra_message=f"Cannot be merged with {type(other)} object")
    ...
    def to_dict(self) -> dict:
        """Return the AffectedItemsGuardSarmResult as a dict.
    ...
    def decode_json(cls, obj: dict):
        ...
        -------
        AbstractGuardSarmResult
            Instance of AbstractGuardSarmResult.
        """
        ALLOWED_TYPES = {'int', 'float', 'str', 'bool'}
        sort_casting = obj.get('sort_casting', ['int'])
        if not isinstance(sort_casting, list):
            raise wexception.GuardSarmInternalError(1000, extra_message="sort_casting must be a list")

        for type_name in sort_casting:
            if not isinstance(type_name, str):
                raise wexception.GuardSarmInternalError(1000, extra_message=f"sort_casting type must be a string, got {type(type_name).__name__}")
            if type_name not in ALLOWED_TYPES:
                raise wexception.GuardSarmInternalError(1000, extra_message=f"Invalid sort_casting type '{type_name}'. "
    ...
    def render(self) -> dict:
        """Render AffectedItemsGuardSarmResult object.

        Returns
        -------
        dict
            Return AffectedItemsGuardSarmResult as a dictionary with fields data, message and error.
        """
...
def merge(*iterables, criteria: Union[tuple, list] = None, ascending: Union[tuple, list] = None,
    ...
    for type_ in types:
        if type_ not in ALLOWED_CASTERS:
            raise wexception.GuardSarmInternalError(1000, extra_message=f"Invalid sort_casting type '{type_}'. "
