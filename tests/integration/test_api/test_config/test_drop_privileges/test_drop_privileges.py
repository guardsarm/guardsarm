...
brief: These tests will check if the 'drop_privileges' setting of the API is working properly.
       This setting allows the user who starts the 'guardsarm-manager-apid' daemon to be different from
       the 'root' user. The GuardSarm API is an open source 'RESTful' API that allows for interaction
...
references:
    ...
    - https://documentation.guardsarmsiem.com/current/user-manual/api/configuration.html#drop-privileges
...
from guardsarm_testing.constants.paths.api import GUARDSARM_API_SCRIPT
from guardsarm_testing.utils.configuration import get_test_cases_data, load_configuration_template
from guardsarm_testing.utils.services import search_process_by_command
...
def test_drop_privileges(test_configuration, test_metadata, add_configuration, truncate_monitored_files,
    ...
    description: Check if 'drop_privileges' affects the user of the API process. In this test, the 'PID' of the API
                 process is obtained. After that, it gets the user ('root' or 'guardsarm') and checks if it matches the
    ...
    test_phases:
        ...
        - test:
            - Search guardsarm-manager-apid process and verify that it is present
    ...
    assertions:
        - Verify that when 'drop_privileges' is enabled the user who has started the 'guardsarm-manager-apid' daemon is 'guardsarm'.
        - Verify that when 'drop_privileges' is disabled the user who has started the 'guardsarm-manager-apid' daemon is 'root'.
    ...
    expected_output:
        - PID of the 'guardsarm-manager-apid' process.
        - guardsarm (if drop_privileges is enabled)
        - root (if drop_privileges is disabled)
    """
    expected_user = test_metadata['expected_user']

    # Get guardsarm-apid process info
    api_process = search_process_by_command(GUARDSARM_API_SCRIPT)
    assert api_process is not None, f"The process {GUARDSARM_API_SCRIPT} could not be found"
