 ...
 *  }]
 * */
cJSON* gm_agent_upgrade_send_tasks_information(const cJSON *message_object) __attribute__((nonnull));
 ...
 *
 * */
int gm_agent_upgrade_task_module_callback(cJSON *data_array, const cJSON* task_module_request, cJSON* (*success_callback)(int *error, cJSON* input_json), void (*error_callback)(int agent_id, int free)) __attribute__((nonnull(1,2)));
