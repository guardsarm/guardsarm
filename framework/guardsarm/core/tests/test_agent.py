...
with patch('guardsarm.core.common.guardsarm_uid'):
    with patch('guardsarm.core.common.guardsarm_gid'):
        from guardsarm.core.agent import *
        from guardsarm.core.exception import GuardSarmException
        from api.util import remove_nones_to_dict
        from guardsarm.rbac.utils import RESOURCES_CACHE
...
# list with GuardSarm packages availables with their hash
...
@patch("guardsarm.core.agent.GuardSarmDBBackend")
@patch('guardsarm.core.wdb.GuardSarmDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_GuardSarmDBQueryAgents__init__(socket_mock, send_mock, backend_mock, value):
    """Tests if method __init__ of GuardSarmDBQueryAgents works properly.
    ...
    if value:
        GuardSarmDBQueryAgents()
        backend_mock.assert_called_once()
    else:
        with pytest.raises(GuardSarmException, match=".* 2005 .*"):
            GuardSarmDBQueryAgents()


@patch('socket.socket.connect')
def test_GuardSarmDBQueryAgents_filter_date(mock_socket_conn):
    """Tests _filter_date of GuardSarmDBQueryAgents returns expected query"""
    query_agent = GuardSarmDBQueryAgents()
...
@patch('socket.socket.connect')
def test_GuardSarmDBQueryAgents_sort_query(mock_socket_conn, field, expected_query):
    """Tests _sort_query of GuardSarmDBQueryAgents returns expected result
    ...
    """
    query_agent = GuardSarmDBQueryAgents(sort={'order': 'asc'})
...
@patch('socket.socket.connect')
def test_GuardSarmDBQueryAgents_add_search_to_query(mock_socket_conn):
    """Tests _add_search_to_query of GuardSarmDBQueryAgents returns expected query"""
    query_agent = GuardSarmDBQueryAgents(search={'value': 'test', 'negation': True})
...
@patch('socket.socket.connect')
def test_GuardSarmDBQueryAgents_format_data_into_dictionary(mock_socket_conn, data):
    """Tests _format_data_into_dictionary of GuardSarmDBQueryAgents returns expected data"""
    query_agent = GuardSarmDBQueryAgents(offset=0, limit=1, sort=None,
...
@patch('socket.socket.connect')
def test_GuardSarmDBQueryAgents_parse_legacy_filters(mock_socket_conn):
    """Tests _parse_legacy_filters of GuardSarmDBQueryAgents returns expected query"""
    query_agent = GuardSarmDBQueryAgents(filters={'older_than': 'test'})
...
@patch('socket.socket.connect')
def test_GuardSarmDBQueryAgents_process_filter(mock_socket_conn, field_name, field_filter, q_filter):
    """Tests _process_filter of GuardSarmDBQueryAgents returns expected query
...
@patch('guardsarm.core.wdb.GuardSarmDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_GuardSarmDBQueryGroup__init__(socket_mock, send_mock, backend_mock, value):
    """Test if method __init__ of GuardSarmDBQueryGroup works properly.
...
@patch('guardsarm.core.wdb.GuardSarmDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_GuardSarmDBQueryGroup_filters(socket_mock, send_mock, filters):
    """Test if parameter filters of GuardSarmDBQueryGroup works properly.
        ...
        """
    query_group = GuardSarmDBQueryGroup(filters=filters)
...
@patch('socket.socket.connect')
def test_GuardSarmDBQueryGroupByAgents__init__(mock_socket_conn):
    """Tests if method __init__ of GuardSarmDBQueryGroupByAgents works properly."""
    query_group = GuardSarmDBQueryGroupByAgents(filter_fields=['name', 'os.name'], offset=0, limit=1, sort={'order': 'asc'},
...
@patch('socket.socket.connect')
def test_GuardSarmDBQueryGroupByAgents_format_data_into_dictionary(mock_socket_conn):
    """Tests if method _format_data_into_dictionary of GuardSarmDBQueryGroupByAgents works properly."""
    query_group = GuardSarmDBQueryGroupByAgents(filter_fields=['name', 'os.name'], offset=0, limit=1, sort={'order': 'asc'},
                                            search={'value': 'test', 'negation': True},
                                            select={'os.name'}, query=None, count=5, get_data=None)

    query_group.filter_fields = {'fields': set(query_group.filter_fields)}
    query_group._data = [{'count': 1, 'name': 'guardsarm-master'},
                         {'count': 1, 'name': 'guardsarm-agent1'}]
...
@patch('guardsarm.core.wdb.GuardSarmDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_GuardSarmDBQueryGroupByAgents(mock_socket_conn, send_mock, filter_fields, expected_response):
    """Tests if GuardSarmDBQueryGroupByAgents works properly."""
    query_group = GuardSarmDBQueryGroupByAgents(filter_fields=filter_fields, offset=0, limit=None, sort=None,
...
@patch('guardsarm.core.wdb.GuardSarmDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_GuardSarmDBQueryGroup__add_sort_to_query(mock_socket_conn, send_mock):
    """Tests if _add_sort_to_query method of GuardSarmDBQueryGroup works properly"""
    query_group = GuardSarmDBQueryGroup()
...
@patch('guardsarm.core.agent.Agent._add')
...
def test_agent_load_info_from_db_ko(socket_mock, send_mock):
    ...
    with pytest.raises(GuardSarmResourceNotFound, match='.* 1701 .*'):
...
@patch('guardsarm.core.agent.GuardSarmQueue.send_msg_to_agent')
@patch('guardsarm.core.wdb.GuardSarmDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect(socket_mock, send_mock, mock_send_msg):
    ...
    agent.reconnect(GuardSarmQueue(common.AR_SOCKET))

    # Assert send_msg method is called with correct params
    mock_send_msg.assert_called_with(GuardSarmQueue.HC_FORCE_RECONNECT, agent_id)


@patch('guardsarm.core.agent.GuardSarmQueue')
@patch('guardsarm.core.wdb.GuardSarmDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect_ko(socket_mock, send_mock, mock_queue):
    ...
    with pytest.raises(GuardSarmError, match='.* 1707 .*'):
        agent = Agent('003')
        agent.reconnect(mock_queue)


@patch('guardsarm.core.agent.Agent._remove_authd', return_value='Agent was successfully deleted')
def test_agent_remove(mock_remove_authd):
    """Tests if method remove() works as expected."""

    with patch('guardsarm.core.agent.get_manager_status', return_value={'guardsarm-manager-authd': 'running'}):
        agent = Agent('001')
        result = agent.remove()
...
def test_agent_remove_ko(mock_remove_authd):
    ...
    with pytest.raises(GuardSarmError, match='.* 1726 .*'):
        agent = Agent('001')
        agent.remove()


@patch('guardsarm.core.agent.GuardSarmSocketJSON')
def test_agent_remove_authd(mock_guardsarm_socket):
    """Tests if method remove_authd() works as expected"""
    agent = Agent('001')
    agent._remove_authd(purge=True)
    mock_guardsarm_socket.return_value.send.assert_called_once_with(
        {"function": "remove", "arguments": {"id": str(1).zfill(3), "purge": True}})
    mock_guardsarm_socket.return_value.receive.assert_called_once()
    mock_guardsarm_socket.return_value.close.assert_called_once()
...
@patch('guardsarm.core.agent.Agent._add_authd')
...
@patch('guardsarm.core.agent.get_manager_status', return_value={'guardsarm-manager-authd': 'stopped'})
def test_agent_add_ko(mock_maganer_status):
    """Test if _add() method raises expected exception."""
    agent = Agent('001')

    with pytest.raises(GuardSarmError, match='.* 1706 .*'):
...
@patch('guardsarm.core.agent.GuardSarmSocketJSON')
def test_agent_add_authd(mock_guardsarm_socket, name, ip, id, key, force):
    ...
    mock_guardsarm_socket.return_value.send.assert_called_once_with(socket_msg)


@pytest.mark.parametrize("mocked_exception, expected_exception", [
    (None, None),
    (GuardSarmError(9008, cmd_error=True), ".* 1705 .*"),
    (GuardSarmError(9007, cmd_error=True), ".* 1706 .*"),
    (GuardSarmError(9012, cmd_error=True), ".* 1708 .*"),
    (GuardSarmError(9000, cmd_error=True), ".* None")
])
@patch('guardsarm.core.agent.GuardSarmSocketJSON')
def test_agent_add_authd_ko(mock_guardsarm_socket, mocked_exception, expected_exception):
    """Tests if method _add_authd() raises expected exception"""
    agent = Agent('001')

    if not mocked_exception:
        with pytest.raises(GuardSarmError, match=".* 1709 .*"):
            agent._add_authd('test_add', '192.168.0.1', '2', 'adsiojew')
    else:
        mock_guardsarm_socket.return_value.receive.side_effect = mocked_exception
        with pytest.raises(GuardSarmError, match=expected_exception):
            agent._add_authd('test_add', '192.168.0.1')


@patch('guardsarm.core.agent.rmtree')
@patch('guardsarm.core.agent.path.exists', return_value=True)
@patch('guardsarm.core.common.SHARED_PATH', new=os.path.join(test_data_path, 'etc', 'shared'))
...
@patch('guardsarm.core.agent.Agent.get_agent_groups', new_callable=AsyncMock)
@patch('guardsarm.core.agent.Agent.set_agent_group_relationship')
...
@patch('guardsarm.core.agent.Agent.get_agent_groups', return_value=['default'])
async def test_agent_add_group_to_agent_ko(agent_groups_mock):
    ...
    with patch('guardsarm.core.agent.Agent.get_agent_groups', side_effect=GuardSarmError(2003)):
        with pytest.raises(GuardSarmInternalError, match='.* 2007 .*'):
...
def test_agent_group_exists_ko():
    ...
    with pytest.raises(GuardSarmError, match='.* 1722 .*'):
...
@patch('guardsarm.core.agent.GuardSarmDBConnection.send')
@patch('socket.socket.connect')
...
def test_agent_set_agent_group_relationship_ko(socket_connect_mock):
    ...
    with pytest.raises(GuardSarmInternalError, match='.* 2005 .*'):
...
@patch('guardsarm.core.agent.Agent.set_agent_group_relationship')
@patch('guardsarm.core.agent.Agent.group_exists', return_value=True)
@patch('guardsarm.core.agent.Agent.get_basic_information')
async def test_agent_unset_single_group_agent(agent_info_mock, group_exists_mock, set_agent_group_mock, agent_id, group_id,
    ...
    """
    with patch('guardsarm.core.agent.Agent.get_agent_groups', return_value=previous_groups):
...
async def test_agent_unset_single_group_agent_ko(socket_mock, agent_information_mock):
    ...
    with patch('guardsarm.core.agent.Agent.group_exists', return_value=False):
        with pytest.raises(GuardSarmResourceNotFound, match='.* 1710 .*'):
            await Agent.unset_single_group_agent('002', 'test_group')

    # Agent does not belong to group
    with patch('guardsarm.core.agent.Agent.get_agent_groups', return_value=['new_group', 'new_group2']):
        # Group_id is not within group_list
        with pytest.raises(GuardSarmError, match='.* 1734 .*'):
            await Agent.unset_single_group_agent('002', 'test_group', force=True)

    # Group ID is 'default' and it is the last only one remaining
    with patch('guardsarm.core.agent.Agent.get_agent_groups', return_value=['default']):
        # Agent file does not exists
        with pytest.raises(GuardSarmError, match='.* 1745 .*'):
            await Agent.unset_single_group_agent('002', 'default', force=True)


@patch('guardsarm.core.guardsarm_socket.GuardSarmSocket')
@patch('guardsarm.core.wdb.GuardSarmDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_config(socket_mock, send_mock, mock_guardsarm_socket):
    """Test getconfig method returns expected message."""
    agent = Agent('001')
    mock_guardsarm_socket.return_value.receive.return_value = b'ok {"test": "conf"}'
    result = agent.get_config('com', 'active-response', 'GuardSarm v4.0.0')
...
@patch('guardsarm.core.wdb.GuardSarmDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_config_ko(socket_mock, send_mock, mock_guardsarm_socket):
    """Test getconfig method raises expected exceptions."""
    # Invalid component
    agent = Agent('003')
    with pytest.raises(GuardSarmError, match=".* 1101 .*"):
        agent.get_config('invalid_component', 'active-response', 'GuardSarm v4.0.0')

    # Component or config is none
    agent = Agent('003')
    with pytest.raises(GuardSarmError, match=".* 1307 .*"):
        agent.get_config('com', None, 'GuardSarm v4.0.0')
        agent.get_config(None, 'active-response', 'GuardSarm v4.0.0')

    # Agent GuardSarm version is lower than ACTIVE_CONFIG_VERSION
    agent = Agent('002')
    with pytest.raises(GuardSarmInternalError, match=".* 1735 .*"):
        agent.get_config('com', 'active-response', 'GuardSarm v3.6.0')


@patch('guardsarm.core.guardsarm_socket.GuardSarmSocket')
@patch('guardsarm.core.wdb.GuardSarmDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_stats(socket_mock, send_mock, mock_guardsarm_socket):
    """Test get_stats method returns expected message."""
    agent = Agent('001')
    mock_guardsarm_socket.return_value.receive.return_value = b'{"error":0, "data":{"global":{}, "interval":{}}}'
...
@patch('guardsarm.core.wdb.GuardSarmDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_stats_ko(socket_mock, send_mock, mock_guardsarm_socket):
    """Test get_stats method raises expected exception when the agent's version is lower than required."""
    agent = Agent('002')
    with pytest.raises(GuardSarmInternalError, match=r'\b1735\b'):
        agent.get_stats('logcollector')


@pytest.mark.parametrize('agent_id, mock_response, should_raise', [
    ('001', b"ok ",                False),  # GuardSarm v4.2.0 - socket succeeds
    ('002', b"ok ",                False),  # GuardSarm v4.0.0 - socket succeeds
    ('003', b"ok ",                False),  # GuardSarm v4.2.1 - socket succeeds
    ('004', b"err Restart failed", True),   # GuardSarm v3.13.2 - socket returns error
    ('010', b"ok ",                False),  # v5.0.0 - socket succeeds
])
@patch('guardsarm.core.agent.GuardSarmSocket')
def test_send_restart_command(mock_socket_cls, agent_id, mock_response, should_raise):
    ...
    should_raise : bool
        Whether the function should raise GuardSarmInternalError.
    """
    ...
    if should_raise:
        with pytest.raises(GuardSarmInternalError):
...
@pytest.mark.parametrize('agent_id, mock_response, should_raise', [
    ('001', b"ok ",                False),  # GuardSarm v4.2.0 agent - socket succeeds
    ('002', b"ok ",                False),  # GuardSarm v4.0.0 agent - socket succeeds
    ('003', b"ok ",                False),  # GuardSarm v4.2.1 agent - socket succeeds
    ('004', b"err Reload failed",  True),   # GuardSarm v3.13.2 agent - socket returns error
...
def test_get_agents_info():
    ...
    with patch('guardsarm.core.agent.open', mock_open(read_data=client_keys)) as m:
...
def test_get_groups():
    ...
    with patch('guardsarm.core.common.SHARED_PATH', new=shared):
        try:
...
def test_expand_group(socket_mock, group, wdb_response, expected_agents):
    ...
    wdb_response: list
        Mock return values for the `GuardSarmDBConnection.send` method.
    expected_agents : set
    ...
    with patch('guardsarm.core.wdb.GuardSarmDBConnection.send', side_effect=wdb_response):
...
@patch('guardsarm.core.agent.core_upgrade_agents')
