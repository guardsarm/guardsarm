...
with patch('guardsarm.common.guardsarm_uid'):
    with patch('guardsarm.common.guardsarm_gid'):
        ...
        import guardsarm.core.cluster.common as cluster_common
        import guardsarm.core.results as wresults
        from guardsarm.core import common
        from guardsarm.core.wdb import AsyncGuardSarmDBConnection
...
guardsarm_common = cluster_common.GuardSarmCommon()
...
def test_inbuffer_init_ko():
    ...
    with pytest.raises(exception.GuardSarmClusterError, match=r'.* 3050 .*'):
...
def test_inbuffer_get_info_from_header_ko(unpack_mock):
    ...
    with pytest.raises(exception.GuardSarmClusterError,
...
@patch("guardsarm.core.cluster.common.SendStringTask.set_up_coro")
def test_sst_init(setup_coro_mock, create_task_mock):
    ...
    with patch.object(TaskMock, "add_done_callback") as done_callback_mock:
        sst_task = cluster_common.SendStringTask(guardsarm_common=cluster_common.GuardSarmCommon(), logger='')
        assert sst_task.logger == ''
        assert sst_task.task in sst_task.tasks_hard_reference
        assert isinstance(sst_task.guardsarm_common, cluster_common.GuardSarmCommon)
...
def test_sst_done_callback(setup_coro_mock, create_task_mock):
    ...
    create_task_mock.return_value = TaskMock()
    guardsarm_common_mock = GuardSarmCommon()

    with patch.object(TaskMock, "add_done_callback"):
        logger = logging.getLogger('guardsarm')
        with patch.object(logger, "error") as logger_mock:
            sst_task = cluster_common.SendStringTask(guardsarm_common=guardsarm_common_mock, logger=logger)
            assert sst_task.task in sst_task.tasks_hard_reference
...
@patch("asyncio.create_task")
@patch("guardsarm.core.cluster.common.ReceiveStringTask.set_up_coro")
def test_rst_init(setup_coro_mock, create_task_mock):
    ...
    with patch.object(TaskMock, "add_done_callback") as done_callback_mock:
        string_task = cluster_common.ReceiveStringTask(guardsarm_common=cluster_common.GuardSarmCommon(), logger='',
                                                       info_type='testing', task_id=b"010")
        assert isinstance(string_task.guardsarm_common, cluster_common.GuardSarmCommon)
        setup_coro_mock.assert_called_once()
        done_callback_mock.assert_called_once()
...
async def test_rst_str_method(logger_mock, event_loop):
    ...
    with patch('guardsarm.core.cluster.common.ReceiveStringTask.set_up_coro', side_effect = return_coro) as setup_coro_mock:
        string_task = cluster_common.ReceiveStringTask(cluster_common.GuardSarmCommon(), '', b"task", logger_mock)
...
@patch('logging.Logger')
@patch('guardsarm.core.cluster.common.GuardSarmCommon')
def test_rst_set_up_coro_ko(guardsarm_common_mock, logger_mock):
    """Test if the exception is being properly raised when an Exception takes place."""

    with pytest.raises(NotImplementedError):
        cluster_common.ReceiveStringTask(guardsarm_common_mock, logger_mock, b"task")
...
def test_rst_done_callback(setup_coro_mock, create_task_mock):
    ...
    with patch.object(TaskMock, "add_done_callback"):
        logger = logging.getLogger('guardsarm')
        with patch.object(logger, "error") as logger_mock:
            string_task = cluster_common.ReceiveStringTask(guardsarm_common=guardsarm_common_mock, logger=logger,
                                                           info_type='agent-groups', task_id=b"010")
            string_task.done_callback()
            assert string_task.guardsarm_common.in_str == {b"011": b"123456789"}
            assert string_task.guardsarm_common.sync_tasks == {b"011": b"123456789"}
...
@patch('asyncio.Event')
@patch("asyncio.create_task")
@patch.object(logging.getLogger("guardsarm"), "error")
@patch("guardsarm.core.cluster.common.ReceiveFileTask.set_up_coro")
@patch("guardsarm.core.cluster.common.uuid4", return_value="e6c45993-b0ae-438f-8914-3b5175b2bbfd")
def test_rft_init(uuid_mock, setup_coro_mock, logger_mock, create_task_mock, event_mock):
    ...
    with patch.object(TaskMock, "add_done_callback") as done_callback_mock:
        file_task = cluster_common.ReceiveFileTask(guardsarm_common=cluster_common.GuardSarmCommon(), logger=logger_mock,
                                                   task_id=b"010")
        assert isinstance(file_task.guardsarm_common, cluster_common.GuardSarmCommon)
        setup_coro_mock.assert_called_once()
        done_callback_mock.assert_called_once()
        ...
        string_task = cluster_common.ReceiveFileTask(guardsarm_common=cluster_common.GuardSarmCommon(), logger=logger_mock,
                                                     task_id="")
...
@patch('logging.Logger')
@patch('guardsarm.core.cluster.common.GuardSarmCommon')
def test_rft_set_up_coro(guardsarm_common_mock, logger_mock):
    """Test if the exception is being properly raised when an Exception takes place."""

    with pytest.raises(NotImplementedError):
        cluster_common.ReceiveFileTask(guardsarm_common_mock, logger_mock, b"task")
...
def test_rft_done_callback(set_up_coro_mock, create_task_mock, event_mock):
    ...
    with patch.object(TaskMock, "add_done_callback"):
        with patch.object(logging.getLogger('guardsarm'), "error") as logger_mock:
            file_task = cluster_common.ReceiveFileTask(guardsarm_common=guardsarm_common_mock,
                                                       logger=logging.getLogger('guardsarm'), task_id=b"010")
            file_task.done_callback()
            assert file_task.guardsarm_common.sync_tasks == {"011": b"123456789"}
            logger_mock.assert_called_once_with(Exception, exc_info=False)
...
def test_handler_init():
    ...
    with patch('guardsarm.core.cluster.utils.context_tag', ContextVar('', default="")) as cv:
        ...
        assert handler.logger == logging.getLogger("guardsarm")
...
def test_handler_msg_build_ko():
    ...
    with pytest.raises(exception.GuardSarmClusterError, match=r'.* 3024 .*'):
...
def test_handler_get_messages_ok():
    ...
    with patch('cryptography.fernet.Fernet.decrypt', return_value="decrypted payload"):
        with patch('guardsarm.core.cluster.common.Handler.msg_parse', return_value=True) as handler_mock:
...
@patch('guardsarm.core.cluster.common.Handler.msg_parse', return_value=True)
...
def test_handler_get_messages_ko(decrypt_mock, msg_parse_mock):
    ...
    with pytest.raises(exception.GuardSarmClusterError, match=r'.* 3025 .*'):
        handler.in_msg.total = 0
        list(handler.get_messages())


@pytest.mark.asyncio
@patch('guardsarm.core.cluster.common.Handler.push')
@patch('guardsarm.core.cluster.common.Handler.next_counter', return_value=30)
@patch('guardsarm.core.cluster.common.Handler.msg_build', return_value=["some", "messages"])
async def test_handler_send_request_ok(msg_build_mock, next_counter_mock, push_mock):
    ...
    with patch('guardsarm.core.cluster.common.Response.read', return_value="some value") as read_mock:
    ...
    with patch('guardsarm.core.cluster.common.Response.read', side_effect=delay):
        with pytest.raises(exception.GuardSarmClusterError, match=r'\b3020\b'):
...
async def test_handler_send_request_ko():
    ...
    with patch('guardsarm.core.cluster.common.Handler.msg_build', side_effect=MemoryError):
        with pytest.raises(exception.GuardSarmClusterError, match=r'.* 3026 .*'):
            await handler.send_request(b'some bytes', b'some data')

    with pytest.raises(exception.GuardSarmClusterError, match=r'.* 3018 .*'):
        await handler.send_request(b'some bytes', b'some data')
...
def test_handler_data_received_max_concurrent_divided_msgs():
    ...
    with patch.object(handler, 'get_messages', return_value=[(b'cmd', new_counter, b'payload', cluster_common.InBuffer.divide_flag)]):
        with pytest.raises(cluster_common.GuardSarmClusterError, match=r'.* 3051 .*'):
...
@pytest.mark.asyncio
@patch('guardsarm.core.cluster.common.Handler.send_request')
async def test_handler_get_chunks_in_task_id(send_request_mock):
    ...
    # Test KeyError
    with pytest.raises(exception.GuardSarmClusterError, match='.* 3035 .*'):
    ...
    with pytest.raises(exception.GuardSarmClusterError, match='.* 3036 .*'):
...
async def test_handler_update_chunks_wdb(send_request_mock):
    ...
    with patch('guardsarm.core.cluster.common.send_data_to_wdb', new_callable=AsyncMock) as send_data_to_wdb_mock:
        with patch.object(LoggerMock, "debug") as logger_debug_mock:
            with patch.object(LoggerMock, "debug2") as logger_debug2_mock:
                with patch.object(LoggerMock, "error") as logger_error_mock:
                    ...
                    logger_debug_mock.assert_has_calls([call('2/5 chunks updated in guardsarm-manager-db in 6.000s.')])
                    ...
                    logger_error_mock.assert_has_calls([call('other1'), call('other2'),
                                                        call('GuardSarm-db response for chunk 1/5 was not "ok": 0'),
                                                        call('GuardSarm-db response for chunk 2/5 was not "ok": 1')])

    # Test Exception
    send_request_mock.reset_mock()
    error_message = 'error'
    with patch('guardsarm.core.cluster.common.send_data_to_wdb', side_effect=Exception(error_message)):
        with pytest.raises(exception.GuardSarmClusterError,
...
@patch('guardsarm.core.cluster.common.Handler.send_request', return_value=b'some data')
...
async def test_handler_send_file_ko():
    ...
    with pytest.raises(exception.GuardSarmClusterError, match=r'.* 3034 .*'):
...
async def test_handler_send_string():
    ...
    with patch('guardsarm.core.cluster.common.Handler.send_request', return_value=b"some data"):
        assert (await handler.send_string(b"something") == b"some data")

    with patch('guardsarm.core.cluster.common.Handler.send_request', side_effect=exception.GuardSarmClusterError(3020)):
        with patch.object(logging.getLogger("guardsarm"), "error") as logger_mock:
            assert exception.GuardSarmClusterError(3020).message.encode() in await handler.send_string(b"something")
...
async def test_handler_forward_dapi_response_ok():
    ...
    with patch('guardsarm.core.cluster.common.Handler.get_manager', return_value=mock_manager):
...
async def test_handler_forward_dapi_response_ko():
    ...
    with patch('guardsarm.core.cluster.common.Handler.get_manager', return_value=mock_manager):
        ...
        with patch.object(mock_manager.local_server, "send_string",
                          side_effect=exception.GuardSarmException(1001)) as send_string_mock:
            with patch.object(logging.getLogger('guardsarm'), "error") as logger_mock:
                with patch('guardsarm.core.cluster.common.Handler.send_request', return_value="some value"):
                    with patch('json.dumps', return_value="some value"):
                        ...
                        logger_mock.assert_called_once_with(f"Error sending API response to local client: "
                                                            f"{exception.GuardSarmException(1001)}")
...
@pytest.mark.parametrize('exception, expected_error', [
    (exception.GuardSarmException(1001), exception.GuardSarmException(1001)),
    (exception.GuardSarmClusterError(3020), None),
    (Exception('Test'), exception.GuardSarmClusterError(1000, extra_message='Test'))
])
async def test_handler_forward_sendsync_response_ko(exception, expected_error):
    ...
    with patch('guardsarm.core.cluster.common.Handler.get_manager', return_value=mock_manager):
        ...
        with patch.object(mock_manager.local_server, "send_request", side_effect=exception):
            with patch.object(logging.getLogger('guardsarm'), "error") as logger_mock:
                with patch('guardsarm.core.cluster.common.Handler.send_request',
                    ...
                    if expected_error:
                        ...
                        send_request_mock.assert_called_once_with(
                            b'sendsync_err',
                            json.dumps(expected_error, cls=cluster_common.GuardSarmJSONEncoder).encode()
                        )
                    else:
...
def test_handler_data_received_ok():
    ...
    # Test first if
    with patch('guardsarm.core.cluster.common.Handler.get_messages', return_value=[(b"bytes1", 123, b"bytes2", b"d")]):
        # Test try
    ...
    with patch('guardsarm.core.cluster.common.Handler.get_messages', return_value=[(b"bytes1", 123, b"bytes2", b"bytes3")]):
        with patch('cryptography.fernet.Fernet.decrypt', return_value="decrypted payload"):
            ...
            with patch('guardsarm.core.cluster.common.Handler.dispatch') as dispatch_mock:
                handler.data_received(b"message")
...
@patch('guardsarm.core.cluster.common.Handler.msg_build', return_value=["msg"])
@patch('guardsarm.core.cluster.common.Handler.push')
@patch('guardsarm.core.cluster.common.Handler.process_request', return_value=(b"command", b"payload"))
def test_handler_dispatch(process_request_mock, push_mock, msg_build_mock):
    ...
    process_request_mock.side_effect = exception.GuardSarmException(1001)
    with patch.object(logging.getLogger('guardsarm'), "error") as logger_mock:
        handler.dispatch(None, 123, b"payload")
        logger_mock.assert_called_with(f"Internal error processing request '{None}': {exception.GuardSarmException(1001)}")
...
def test_handler_process_request():
    ...
    with patch('guardsarm.core.cluster.common.Handler.echo') as echo_mock:
        handler.process_request(b"echo", b"data")
        echo_mock.assert_called_once_with(b"data")
    with patch('guardsarm.core.cluster.common.Handler.receive_file') as receive_file_mock:
        handler.process_request(b"new_file", b"data")
        receive_file_mock.assert_called_once_with(b"data")
    with patch('guardsarm.core.cluster.common.Handler.receive_str') as receive_str_mock:
        handler.process_request(b"new_str", b"data")
        receive_str_mock.assert_called_once_with(b"data")
    with patch('guardsarm.core.cluster.common.Handler.update_file') as update_file_mock:
        handler.process_request(b"file_upd", b"data")
        update_file_mock.assert_called_once_with(b"data")
    with patch('guardsarm.core.cluster.common.Handler.str_upd') as str_upd_mock:
        handler.process_request(b"str_upd", b"data")
        str_upd_mock.assert_called_once_with(b"data")
    with patch('guardsarm.core.cluster.common.Handler.process_error_str') as process_error_str_mock:
        handler.process_request(b"err_str", b"data")
        process_error_str_mock.assert_called_once_with(b"data")
    with patch('guardsarm.core.cluster.common.Handler.end_file') as end_file_mock:
        handler.process_request(b"file_end", b"data")
        end_file_mock.assert_called_once_with(b"data")
    with patch('guardsarm.core.cluster.common.Handler.cancel_task') as cancel_task_mock:
        handler.process_request(b"cancel_task", b"data")
        cancel_task_mock.assert_called_once_with(b"data")
    with patch('guardsarm.core.cluster.common.Handler.process_dapi_error') as process_dapi_err_mock:
    ...
    with patch('guardsarm.core.cluster.common.Handler.process_unknown_cmd') as process_unknown_cmd_mock:
...
def test_handler_process_response():
    ...
    with patch('guardsarm.core.cluster.common.Handler.process_error_from_peer', return_value=b"payload"):
...
def test_handler_receive_file_rejects_invalid_and_disallowed_paths():
    ...
    assert handler.receive_file(b"/etc/guardsarm-manager.conf") == (b"err", b"Write path not allowed")
...
def test_handler_str_upd():
    ...
    with patch('guardsarm.core.cluster.common.InBuffer.receive_data'):
...
def test_handler_process_dapi_error_ko():
    ...
    handler = cluster_common.Handler(fernet_key, cluster_items)
    with pytest.raises(exception.GuardSarmClusterError, match=r".* 3032 .*"):
        handler.server = ManagerMock()
        handler.process_dapi_error(data=b"data 2")

    with pytest.raises(exception.GuardSarmClusterError, match=r".* 3025 .*"):
        with patch("asyncio.create_task", side_effect=exception.GuardSarmClusterError(1001)):
            handler.server = ManagerMock()
...
def test_handler_process_error_from_peer():
    ...
    assert isinstance(handler.process_error_from_peer(b"data to decode"), exception.GuardSarmClusterError)
...
def test_handler_setup_task_logger():
    ...
    with patch.object(logging.getLogger("guardsarm"), "getChild", return_value=TaskLoggerMock()) as get_child_mock:
...
async def test_handler_wait_for_file_ko(send_request_mock):
    """Check if expected exception is raised.
        Condition 1: when event.wait() exceeds the timeout, GuardSarmClusterError 3039 is raised
        Condition 2: when any other exception occurs, GuardSarmClusterError 3040  is raised"""
    ...
    with pytest.raises(exception.GuardSarmClusterError, match=r'.* 3039 .*'):
    ...
    with pytest.raises(exception.GuardSarmClusterError, match=r".* 3040 .*"):
...
# Test 'GuardSarmCommon' class methods

def test_guardsarm_common_init():
    """Test the '__init__' method correct functioning."""

    guardsarm_common_test = cluster_common.GuardSarmCommon()
    assert guardsarm_common_test.sync_tasks == {}


@pytest.mark.asyncio
@patch("guardsarm.core.cluster.common.AsyncGuardSarmDBConnection", return_value=AsyncMock())
async def test_guardsarm_common_recalculate_group_hash(asyncguardsarmdbconnection_mock):
    class LoggerMock:
        """Auxiliary class."""
    ...
    logger = LoggerMock()
    await guardsarm_common.recalculate_group_hash(logger)
    assert logger._debug == ['Recalculating agent-group hash.']


@pytest.mark.asyncio
@patch("guardsarm.core.cluster.common.AsyncGuardSarmDBConnection")
async def test_guardsarm_common_recalculate_group_hash_ko(asyncguardsarmdbconnection_mock):
    class LoggerMock:
        """Auxiliary class."""
    ...
    logger = LoggerMock()
    asyncguardsarmdbconnection_mock.side_effect = [exception.GuardSarmInternalError(2007), exception.GuardSarmError(2003)]

    await guardsarm_common.recalculate_group_hash(logger)
    assert logger._debug == ['Recalculating agent-group hash.']
    assert logger._warning == ['Error 2007 executing recalculate agent-group hash command: '
                               'Error retrieving data from GuardSarm DB']

    logger = LoggerMock()
    await guardsarm_common.recalculate_group_hash(logger)
    assert logger._debug == ['Recalculating agent-group hash.']
    assert logger._warning == ['Error 2003 executing recalculate agent-group hash command: Error in guardsarmdb request']

def test_guardsarm_common_get_logger():
    """Check if a Logger object is properly returned."""

    with pytest.raises(NotImplementedError):
        guardsarm_common.get_logger()


def test_guardsarm_common_setup_send_info():
    ...
    with patch('guardsarm.core.cluster.common.GuardSarmCommon.get_logger'):
        first_output, second_output = guardsarm_common.setup_send_info(mock_object)
        assert first_output == b'ok'
        assert isinstance(second_output, bytes)


def test_guardsarm_common_setup_receive_file():
    ...
    with patch('guardsarm.core.cluster.common.GuardSarmCommon.get_logger'):
        first_output, second_output = guardsarm_common.setup_receive_file(mock_object)
        assert first_output == b'ok'
        assert isinstance(second_output, bytes)
        assert MyTaskMock().task_id in guardsarm_common.sync_tasks
        assert isinstance(guardsarm_common.sync_tasks[MyTaskMock().task_id], MyTaskMock)


@patch('guardsarm.core.cluster.common.GuardSarmCommon')
@patch('logging.Logger')
def test_guardsarm_common_end_receiving_file_ok(logger_mock, guardsarm_common_mock):
    """Check if the full path to the received file is properly stored and availability is notified."""

    with patch('guardsarm.core.cluster.common.ReceiveFileTask.set_up_coro'):
        with patch('asyncio.create_task'):
            file_task = cluster_common.ReceiveFileTask(guardsarm_common_mock, logger_mock, b"task")

    with patch('guardsarm.core.cluster.common.GuardSarmCommon.get_logger'):
        guardsarm_common.sync_tasks = {'task_ID': file_task}
        # Use a path within queue/cluster to pass validation
        assert guardsarm_common.end_receiving_file("task_ID queue/cluster/filepath") == (b'ok', b'File correctly received')
        assert isinstance(guardsarm_common.sync_tasks["task_ID"], cluster_common.ReceiveFileTask)


def test_guardsarm_common_end_receiving_file_ko():
    """Test the 'end_receiving_file' correct functioning in a failure scenario."""

    with patch('guardsarm.core.cluster.common.GuardSarmCommon.get_logger'):
        with pytest.raises(exception.GuardSarmClusterError, match=r'.* 3027 .*'):
            guardsarm_common.end_receiving_file("not_task_ID queue/cluster/filepath")


@patch('json.loads')
def test_guardsarm_common_error_receiving_file_ok(json_loads_mock):
    """Check how error are handled by peer in the sent file process."""

    with patch('os.path.exists', return_value=True):
        with patch('os.remove'):
            # Test first condition and its nested condition
            assert guardsarm_common.error_receiving_file("task_ID error_details") == (b'ok', b'Error received')

    # Test second condition
    with patch('guardsarm.core.cluster.common.GuardSarmCommon.get_logger'):
        assert guardsarm_common.error_receiving_file("not_task_ID error_details") == (b'ok', b'Error received')


def test_guardsarm_common_error_receiving_file_ko():
...
def test_guardsarm_common_end_receiving_file_invalid_absolute_path():
    """Test that end_receiving_file rejects absolute paths."""

    with patch('guardsarm.core.cluster.common.GuardSarmCommon.get_logger'):
        with pytest.raises(exception.GuardSarmClusterError, match=r'.* 3027 .*'):
            guardsarm_common.end_receiving_file("not_task_ID /etc/passwd")


def test_guardsarm_common_end_receiving_file_invalid_relative_path():
...
def test_guardsarm_common_end_receiving_file_invalid_nested_path():
    """Test that end_receiving_file validates nested path components."""

    with patch('guardsarm.core.cluster.common.GuardSarmCommon.get_logger'):
        with pytest.raises(exception.GuardSarmClusterError, match=r'.* 3027 .*'):
            guardsarm_common.end_receiving_file("not_task_ID ../../../tmp/evil.sh")


@patch('os.path.exists', return_value=False)
def test_guardsarm_common_end_receiving_file_path_validation_with_valid_task(path_exists_mock):
    ...
    with patch('guardsarm.core.cluster.common.ReceiveFileTask.set_up_coro'):
        with patch('asyncio.create_task'):
            with patch('guardsarm.core.cluster.common.GuardSarmCommon.get_logger'):
                file_task = cluster_common.ReceiveFileTask(guardsarm_common, MagicMock(), b"task")

    guardsarm_common.sync_tasks = {'valid_task_id': file_task}

    with patch('guardsarm.core.cluster.common.GuardSarmCommon.get_logger'):
        with pytest.raises(exception.GuardSarmClusterError, match=r'.* 3027 .*'):
            guardsarm_common.end_receiving_file("valid_task_id /etc/passwd")


@patch('os.remove')
@patch('os.path.exists', return_value=True)
def test_guardsarm_common_end_receiving_file_no_deletion_on_invalid_task(path_exists_mock, os_remove_mock):
    ...
    with patch('guardsarm.core.cluster.common.GuardSarmCommon.get_logger'):
        ...
        with pytest.raises(exception.GuardSarmClusterError, match=r'.* 3027 .*'):
            guardsarm_common.end_receiving_file("non_existent_task /var/guardsarm-manager/api/configuration/security/rbac.db")

        assert os_remove_mock.call_count == 0, "File deletion should not occur when task_id is not found"


def test_guardsarm_common_get_node():
    """Check if it is possible to obtain basic information about the node."""

    class MockClass(cluster_common.GuardSarmCommon, cluster_common.Handler, abc.ABC):
        def __init__(self):
            super().__init__()
    ...
    with patch('guardsarm.core.cluster.common.Handler.get_manager', return_value=mock_manager) as manager_mock:
        mock_class.get_node()
        manager_mock.assert_called_once()


# Test SyncGuardSarmdb class

def test_sync_guardsarm_db_init():
    """Test the '__init__' method from the SyncGuardSarmdb class."""

    sync_guardsarm_db = cluster_common.SyncGuardSarmdb(
        manager=cluster_common.Handler(fernet_key, cluster_items), logger=logging.getLogger("guardsarm"), cmd=b"cmd",
        data_retriever=None, get_data_command="get_command", set_data_command="set_command", pivot_key=None)

    assert sync_guardsarm_db.get_data_command == "get_command"
    assert sync_guardsarm_db.set_data_command == "set_command"
    assert sync_guardsarm_db.data_retriever is None


@pytest.mark.asyncio
@patch("guardsarm.core.wdb.socket.socket")
async def test_sync_guardsarm_db_retrieve_information(socket_mock):
    ...
    wdb_conn = AsyncGuardSarmDBConnection()
    logger = logging.getLogger("guardsarm")
    handler = cluster_common.Handler(fernet_key, cluster_items)
    sync_object = cluster_common.SyncGuardSarmdb(manager=handler, logger=logger, cmd=b'syn_a_w_m',
    ...
    # data_retriever returns a JSON that does not follow the pivoting scheme
    with patch.object(sync_object, 'data_retriever', side_effect=exception.GuardSarmException(1000)):
        with patch.object(sync_object.logger, 'error') as logger_error_mock:
            ...
            logger_error_mock.assert_called_with(
                'Could not obtain data from guardsarm-manager-db: Error 1000 - GuardSarm Internal Error')


@patch('guardsarm.core.wdb_http.GuardSarmDBHTTPClient')
async def test_sync_guardsarm_db_retrieve_agents_information(wdb_http_client_mock):
...
async def test_sync_guardsarm_db_retrieve_agents_information_ko():
    ...
    with patch('guardsarm.core.wdb_http.GuardSarmDBHTTPClient', side_effect=exception.GuardSarmException(1000)):
        with patch.object(sync_object.logger, 'error') as logger_error_mock:
...
@patch('guardsarm.core.cluster.common.time.perf_counter', return_value=0)
async def test_sync_guardsarm_db_sync_ok(perf_counter_mock, json_dumps_mock):
    ...
    # Test try and if
    with patch.object(sync_guardsarm_db.logger, "debug") as logger_debug_mock:
        with patch("guardsarm.core.cluster.common.Handler.send_string", return_value=b"OK") as send_string_mock:
            with patch("guardsarm.core.cluster.common.Handler.send_request",
                       side_effect=None) as send_request_mock:
                assert await sync_guardsarm_db.sync(start_time=10, chunks=['a', 'b']) is True
    ...
    # Test else
    with patch.object(sync_guardsarm_db.logger, "info") as logger_info_mock:
        assert await sync_guardsarm_db.sync(start_time=-10, chunks=[]) is True
        logger_info_mock.assert_called_once_with("Finished in 10.000s. Updated 0 chunks.")

    # Test except
    with patch("guardsarm.core.cluster.common.Handler.send_string", return_value=b'Error 1'):
        sync_guardsarm_db.server = cluster_common.Handler(fernet_key, cluster_items)
        with pytest.raises(exception.GuardSarmClusterError, match=r".* 3016 .*"):
            await sync_guardsarm_db.sync(start_time=10, chunks=['a'])
...
def test_end_sending_agent_information(perf_counter_mock, json_loads_mock):
    ...
    with patch('guardsarm.core.cluster.common.utils.get_utc_now', side_effect=get_utc_now_mock):
...
@patch("guardsarm.core.cluster.common.GuardSarmDBConnection")
async def test_send_data_to_wdb(GuardSarmDBConnection_mock):
    """Check if the data chunks are being properly forward to the GuardSarm-db socket."""

    class MockGuardSarmDBConnection:
        """Auxiliary class."""
    ...
    GuardSarmDBConnection_mock.return_value = MockGuardSarmDBConnection()
    ...
    GuardSarmDBConnection_mock.return_value.exceptions += 1
    ...
    with patch('guardsarm.core.cluster.master.utils.Timeout', side_effect=Exception):
        result = await cluster_common.send_data_to_wdb(data={'chunks': chunks, 'set_data_command': ''},
                                                 timeout=15, info_type='agent-groups')
...
@patch('guardsarm.core.common.os.chmod')
@patch('guardsarm.core.common.os.chown')
@patch('guardsarm.core.common.guardsarm_gid', return_value=0)
def test_guardsarm_json_encoder_default(mock_gid, mock_chown, mock_chmod):
    """Test if a special JSON encoder is defined for GuardSarm."""

    guardsarm_encoder = cluster_common.GuardSarmJSONEncoder()

    # Test first condition
    assert isinstance(guardsarm_encoder.default(object), dict)
    assert guardsarm_encoder.default(object) == {'__callable__': {'__name__': 'object', '__module__': 'builtins',
                                                              '__qualname__': 'object', '__type__': 'type'}}

    class GuardSarmMock(json.JSONEncoder):
        __self__ = GuardSarm()
        a = 0
        b = 0
    ...
    guardsarm_encoder.default(GuardSarmMock)

    with patch('builtins.callable', return_value=False):
        # Test second condition
        assert isinstance(guardsarm_encoder.default(exception.GuardSarmException(3009)), dict)
        assert guardsarm_encoder.default(exception.GuardSarmException(3009)) == \
               {'__guardsarm_exception__': {'__class__': 'GuardSarmException',
                                        '__object__': {'type': 'about:blank', 'title': 'GuardSarmException', 'code': 3009,
                                                       'extra_message': None, 'extra_remediation': None,
                                                       'cmd_error': False, 'dapi_errors': {}}}}

        # Test third condition
        abstract_guardsarm_result = wresults.AffectedItemsGuardSarmResult()

        assert isinstance(guardsarm_encoder.default(abstract_guardsarm_result), dict)
        assert guardsarm_encoder.default(abstract_guardsarm_result) == \
               {'__guardsarm_result__': {'__class__': 'AffectedItemsGuardSarmResult',
        ...
        assert isinstance(guardsarm_encoder.default(date), dict)
        assert guardsarm_encoder.default(date) == {'__guardsarm_datetime__': '2021-10-15T00:00:00'}

        # Test fifth condition
        exc = ValueError('test')
        assert isinstance(guardsarm_encoder.default(exc), dict)
        assert guardsarm_encoder.default(exc) == {'__unhandled_exc__': {'__class__': 'ValueError',
                                                                    '__args__': ('test',)}}

        # Test simple return
        with pytest.raises(TypeError):
            guardsarm_encoder.default({"key": "value"})


@patch('guardsarm.core.common.os.chmod')
@patch('guardsarm.core.common.os.chown')
@patch('guardsarm.core.common.guardsarm_gid', return_value=0)
def test_as_guardsarm_object_ok(mock_gid, mock_chown, mock_chmod):
    """Test the different outputs taking into account the input values."""

    # Test the first condition - GuardSarm methods without @dapi_allower must be blocked
    with pytest.raises(exception.GuardSarmInternalError) as err:
        cluster_common.as_guardsarm_object({"__callable__": {"__name__": "to_dict", "__guardsarm__": "version"}})
    assert "is not exposed with @dapi_allower decorator" in str(err.value)

    # Test the first condition and nested else
    with pytest.raises(exception.GuardSarmInternalError) as err:
        cluster_common.as_guardsarm_object({
            "__callable__": {
                "__name__": "join",
    ...
    with pytest.raises(exception.GuardSarmInternalError) as err:
        cluster_common.as_guardsarm_object({"__callable__": {"__name__": "__name__", "__qualname__": "value",
    ...
    with pytest.raises(exception.GuardSarmInternalError) as err:
        cluster_common.as_guardsarm_object({"__callable__": {"__name__": "check_user",
    ...
    with pytest.raises(exception.GuardSarmInternalError) as err:
        cluster_common.as_guardsarm_object({"__callable__": {"__name__": "get_node",
                                                         "__module__": "guardsarm.core.cluster.cluster",
                                                         "__qualname__": "get_node",
                                                         "__type__": "function"}})
    assert "is not exposed with @dapi_allower decorator" in str(err.value)

    # Test that functions with @dapi_allower decorator are allowed
    func =  cluster_common.as_guardsarm_object({"__callable__": {"__name__": "read_config_wrapper",
                                                             "__module__": "guardsarm.cluster",
                                                             "__qualname__": "read_config_wrapper",
                                                             "__type__": "function"}})
    assert callable(func)
    assert func.__module__ == "guardsarm.cluster"
    assert func.__name__ == "read_config_wrapper"

    # Test the second condition
    assert isinstance(cluster_common.as_guardsarm_object(
        {"__guardsarm_exception__": {"__class__": "GuardSarmException", "__object__": {"code": 1001}}}),
        exception.GuardSarmException)

    # Test the third condition
    with patch('guardsarm.core.results.AbstractGuardSarmResult.decode_json', return_value=wresults.AbstractGuardSarmResult):
        assert isinstance(cluster_common.as_guardsarm_object(
            {"__guardsarm_result__": {"__class__": "AbstractGuardSarmResult", "__object__": {"code": 1001}}}), abc.ABCMeta)

    # Test the fourth condition
    assert isinstance(cluster_common.as_guardsarm_object({"__guardsarm_datetime__": "2021-10-14"}), datetime)
    assert cluster_common.as_guardsarm_object({"__guardsarm_datetime__": "2021-10-14"}) == datetime(2021, 10, 14)

    # Test the fifth condition
    result = cluster_common.as_guardsarm_object({'__unhandled_exc__': {'__class__': 'ValueError',
                                                                   '__args__': ('test',)}})
    assert result == {"ValueError": ["test"]}

    result = cluster_common.as_guardsarm_object({'__unhandled_exc__': {'__class__': 'exit',
                                                                   '__args__': []}})
    assert result == {"exit": []}

    # No condition fulfilled
    assert isinstance(cluster_common.as_guardsarm_object({"__guardsarm_datetime_bad__": "2021-10-14"}), dict)
    assert cluster_common.as_guardsarm_object({"__guardsarm_datetime_bad__": "2021-10-14"}) == \
           {"__guardsarm_datetime_bad__": "2021-10-14"}


@patch('guardsarm.core.common.os.chmod')
@patch('guardsarm.core.common.os.chown')
@patch('guardsarm.core.common.guardsarm_gid', return_value=0)
def test_as_guardsarm_object_ko(mock_chmod, mock_chown, mock_gid):
    """Test if the exceptions are correctly raised."""

    with pytest.raises(exception.GuardSarmInternalError, match=r'.* 1000 .*'):
        cluster_common.as_guardsarm_object({"__callable__": {"__name__": "value", "__guardsarm__": "value"}})


def get_handler():
    """Return a Handler object. This is an auxiliary method."""
    return cluster_common.Handler(fernet_key=fernet_key, cluster_items=cluster_items, logger=logging.getLogger("guardsarm"))
...
def test_sync_task_init():
    ...
    sync_task = cluster_common.SyncTask(b"cmd", logging.getLogger("guardsarm"), get_handler())

    assert sync_task.cmd == b"cmd"
    assert sync_task.logger == logging.getLogger("guardsarm")
    assert isinstance(sync_task.server, cluster_common.Handler)


@pytest.mark.asyncio
@patch('guardsarm.core.cluster.common.Handler.send_request', side_effect=Exception())
async def test_sync_task_request_permission(send_request_mock):
    ...
    with patch.object(logging.getLogger("guardsarm"), "debug") as logger_mock:
        # Test second condition
...
@patch("guardsarm.core.cluster.cluster.compress_files", return_value=("files/path/", {}))
@patch("guardsarm.core.cluster.utils.log_subprocess_execution")
async def test_sync_files_sync_ok(log_subprocess_mock, compress_files_mock, unlink_mock, relpath_mock, json_dumps_mock):
    ...
    class WorkerMock:
        ...
        async def send_request(self, command, data):
            ...
            if command == b"cmd" and data == b"" and self.count == 1:
                raise exception.GuardSarmClusterError(3020, extra_message=command.decode())
                ...
                raise exception.GuardSarmClusterError(3016, extra_message=command.decode())
    ...
    sync_files = cluster_common.SyncFiles(b"cmd", logging.getLogger("guardsarm"), worker_mock)
    ...
    with patch.object(logging.getLogger("guardsarm"), "error") as logger_mock:
        ...
        json_dumps_mock.assert_called_once_with(
            exception.GuardSarmClusterError(code=3020, extra_message="cmd"),
            cls=cluster_common.GuardSarmJSONEncoder)
    ...
    with patch.object(WorkerMock, "send_file", return_value=1000) as send_file_mock:
        ...
        with patch.object(logging.getLogger("guardsarm"), "debug") as logger_debug_mock:
            with patch.object(logging.getLogger("guardsarm"), "error") as logger_error_mock:
                await sync_files.sync(files_to_sync, files_metadata, 1, task_pool=None)
                ...
                relpath_mock.assert_called_once_with('files/path/', common.GUARDSARM_PATH)
                ...
                log_subprocess_mock.assert_called()
                logger_error_mock.assert_called_once_with(
                    f"Error sending zip file: {exception.GuardSarmException(3016, 'cmd_e')}")
...
async def test_sync_files_sync_ko(send_request_mock):
    ...
    sync_files = cluster_common.SyncFiles(b"cmd", logging.getLogger("guardsarm"), handler)
    ...
    with patch('os.unlink', side_effect=FileNotFoundError):
        ...
        with patch("guardsarm.core.cluster.cluster.compress_files", return_value=(compressed_data, {})):
            send_request_mock.side_effect = None
            logger = logging.getLogger('guardsarm')
            with patch.object(logger, "error") as logger_mock:
                await sync_files.sync(files_to_sync, files_metadata, 1, task_pool=None)
