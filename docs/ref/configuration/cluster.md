...
This guide covers the configuration and deployment requirements for the GuardSarm manager cluster.

## Overview

The GuardSarm cluster allows multiple manager nodes to work together for high availability and load distribution. The cluster uses a dedicated internal protocol (DAPI) over port 1516 for node-to-node communication.

Cluster configuration is defined in `/var/guardsarm-manager/etc/guardsarm-manager.conf` under the `<cluster>` section.
...
```bash
sed -n 's|.*<key>\(.*\)</key>.*|\1|p' /var/guardsarm-manager/etc/guardsarm-manager.conf
```

Then set that value on each worker node and restart the manager:

```bash
sudo sed -i 's|<key>.*</key>|<key>PASTE_MASTER_KEY_HERE</key>|' /var/guardsarm-manager/etc/guardsarm-manager.conf
...
> **Note for users migrating from GuardSarm 4.x:** The `<disabled>` option present in 4.x cluster configuration is no longer processed in 5.0. Cluster enable/disable is controlled at the service level — to disable the cluster, stop or disable the `guardsarm-manager-clusterd` service. If `<disabled>` is present in the configuration file it is silently ignored.
...
After editing `/var/guardsarm-manager/etc/guardsarm-manager.conf` on each node, restart the manager service:
...
```bash
/var/guardsarm-manager/bin/cluster_control -l
```
...
- Restrict file permissions on `guardsarm-manager.conf`:
  ```bash
  chown root:guardsarm-manager /var/guardsarm-manager/etc/guardsarm-manager.conf
  chmod 640 /var/guardsarm-manager/etc/guardsarm-manager.conf
  ```
...
```bash
grep -A 10 '<cluster>' /var/guardsarm-manager/etc/guardsarm-manager.conf | grep '<key>'
```
...
Review cluster logs:
```bash
tail -100 /var/guardsarm-manager/logs/cluster.log
```
