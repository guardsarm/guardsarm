...
def _make_client() -> EngineHTTPClient:
    with patch('guardsarm.core.common.ANALYSISD_SOCKET', '/var/guardsarm-manager/queue/sockets/analysis'):
...
def test_get_metrics_dump_http_error():
    ...
    with pytest.raises(GuardSarmError) as exc_info:
...
def test_get_metrics_dump_invalid_json():
    ...
    with pytest.raises(GuardSarmInternalError) as exc_info:
        client.get_metrics_dump()
...
def test_engine_http_client_init_error():
    """Test that the client raises GuardSarmInternalError(2018) if httpx instantiation fails."""
    with patch('guardsarm.core.common.ANALYSISD_SOCKET', '/var/guardsarm-manager/queue/sockets/analysis'):
