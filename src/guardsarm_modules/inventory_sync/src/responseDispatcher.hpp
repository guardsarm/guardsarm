...
public:
    ...
    void sendStartAck(const GuardSarm::SyncSchema::Status status,
                      ...
                      std::string_view moduleName) const
    {
        ...
        auto startAckOffset = GuardSarm::SyncSchema::CreateStartAck(responseMessage.builder, status, sessionId);

        auto messageOffset = GuardSarm::SyncSchema::CreateMessage(
            responseMessage.builder, GuardSarm::SyncSchema::MessageType_StartAck, startAckOffset.Union());
    ...
    void sendEndAck(const GuardSarm::SyncSchema::Status status,
                    std::string_view agentId,
                    const uint64_t sessionId,
                    std::string_view moduleName) const
    {
        ...
        responseMessage.agentId = agentId;
        responseMessage.moduleName = moduleName;
        auto startAckOffset = GuardSarm::SyncSchema::CreateEndAck(responseMessage.builder, status, sessionId);

        auto messageOffset = GuardSarm::SyncSchema::CreateMessage(
            responseMessage.builder, GuardSarm::SyncSchema::MessageType_EndAck, startAckOffset.Union());
                           ...
                           const std::vector<std::pair<uint64_t, uint64_t>>& ranges) const
    {
        ...
        responseMessage.agentId = agentId;
        responseMessage.moduleName = moduleName;
        std::vector<flatbuffers::Offset<GuardSarm::SyncSchema::Pair>> convertedRanges;

        for (const auto& [first, second] : ranges)
        {
            auto offset = GuardSarm::SyncSchema::CreatePair(responseMessage.builder, first, second);
            convertedRanges.push_back(offset);
        }

        auto endOffset = GuardSarm::SyncSchema::CreateReqRetDirect(responseMessage.builder, &convertedRanges, sessionId);
        auto messageOffset = GuardSarm::SyncSchema::CreateMessage(
            responseMessage.builder, GuardSarm::SyncSchema::MessageType_ReqRet, endOffset.Union());
        responseMessage.builder.Finish(messageOffset);
