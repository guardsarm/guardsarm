...
from guardsarm.tests.test_security import db_setup  # noqa
...
def test_revoke_tokens(db_setup):
    """Checks that the return value of revoke_tokens is a GuardSarmResult.
    ...
    db_setup: callable
        This function creates the rbac.db file.
    """
    with patch('guardsarm.core.security.change_keypair', side_effect=None):
        security, GuardSarmResult, _ = db_setup
        mock_current_user = ContextVar('current_user', default='guardsarm')
        with patch("guardsarm.core.common.current_user", new=mock_current_user):
            result = security.revoke_current_user_tokens()
            assert isinstance(result, GuardSarmResult)
...
def test_invalid_roles_tokens(db_setup, role_list, expected_roles):
    ...
    expected_roles : set
        Expected roles.
    """
    with patch('guardsarm.core.security.TokenManager.add_user_roles_rules') as TM_mock:
...
@patch('guardsarm.core.security.TokenManager.add_user_roles_rules')
...
@patch("guardsarm.core.security.revoke_tokens")
@patch("guardsarm.core.security.check_database_integrity")
@patch("guardsarm.core.security.os.remove")
