...
from guardsarm.core.agent import GuardSarmDBQueryAgents, GuardSarmDBQueryGroupByAgents, Agent, \
    GuardSarmDBQueryGroup, create_upgrade_tasks, get_agents_info, get_groups, get_rbac_filters, send_restart_command, \
    send_reload_command, \
...
from guardsarm.core.exception import GuardSarmError, GuardSarmInternalError, GuardSarmException, GuardSarmResourceNotFound
from guardsarm.core.results import GuardSarmResult, AffectedItemsGuardSarmResult
from guardsarm.core.utils import GuardSarmVersion, chmod_r, chown_r, get_hash, mkdir_with_mode, process_array, clear_temporary_caches, \
    full_copy
...
# The rest of codes generated by the socket will be raised as GuardSarm internal errors (500)
...
def get_distinct_agents(agent_list: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None,
                        search: dict = None, fields: list = None, q: str = None) -> AffectedItemsGuardSarmResult:
    ...
    result = AffectedItemsGuardSarmResult(all_msg='All selected agents information was returned',
    ...
    if agent_list:
        ...
        with GuardSarmDBQueryGroupByAgents(filter_fields=fields, select=fields, offset=offset, limit=limit, sort=sort,
...
@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"], post_proc_func=None)
async def get_agents_summary(agent_list: list[str] = None) -> GuardSarmResult:
    ...
    return GuardSarmResult({'data': agents_summary.to_dict()})


@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"], post_proc_func=None)
def get_agents_summary_status(agent_list: list[str] = None) -> GuardSarmResult:
    ...
    if agent_list:
        ...
        with GuardSarmDBQueryGroupByAgents(
    ...
    return GuardSarmResult({'data': {'connection': connection, 'configuration': sync_configuration}})


@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"], post_proc_func=None)
def get_agents_summary_os(agent_list: list[str] = None) -> AffectedItemsGuardSarmResult:
    ...
    AffectedItemsGuardSarmResult
        Affected items.
    """
    result = AffectedItemsGuardSarmResult(none_msg='Could not get the operative system of the agents',
...
def reconnect_agents(agent_list: Union[list, str] = None) -> AffectedItemsGuardSarmResult:
    ...
    AffectedItemsGuardSarmResult
        Affected items.
    """
    result = AffectedItemsGuardSarmResult(all_msg='Force reconnect command was sent to all agents',
...
async def restart_agents(agent_list: list = None) -> AffectedItemsGuardSarmResult:
    ...
    AffectedItemsGuardSarmResult
        Affected items.
    """
    result = AffectedItemsGuardSarmResult(all_msg='Restart command was sent to all agents',
    ...
    if agent_list:
        ...
        for agent_id in agent_list:
            ...
            if agent_id not in active_agents:
                result.add_failed_item(id_=agent_id, error=GuardSarmError(1707))
                continue

            version = active_agents[agent_id]
            if not version or GuardSarmVersion(version) < GuardSarmVersion('v5.0.0'):
                result.add_failed_item(id_=agent_id, error=GuardSarmError(1761))
                continue
...
@expose_resources(actions=['cluster:read'], resources=[f'node:id:{node_id}'],
                  ...
                  post_proc_func=async_list_handler)
async def restart_agents_by_node(agent_list: list = None) -> AffectedItemsGuardSarmResult:
...
@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"],
                  post_proc_kwargs={'exclude_codes': [1701, 1703, 1707], 'force': True},
                  post_proc_func=async_list_handler)
async def restart_agents_by_group(agent_list: list = None) -> AffectedItemsGuardSarmResult:
...
@expose_resources(actions=["agent:reload"], resources=["agent:id:{agent_list}"],
                  post_proc_kwargs={'exclude_codes': [1701, 1703, 1707]},
                  post_proc_func=async_list_handler)
async def reload_agents(agent_list: list = None) -> AffectedItemsGuardSarmResult:
    ...
    AffectedItemsGuardSarmResult
        Affected items.
    """
    result = AffectedItemsGuardSarmResult(all_msg='Reload command was sent to all agents',
...
@expose_resources(actions=['cluster:read', 'agent:reload'], resources=[f'node:id:{node_id}', 'agent:id:{agent_list}'],
                  post_proc_kwargs={'exclude_codes': [1701, 1703, 1707], 'force': True},
                  post_proc_func=async_list_handler)
async def reload_agents_by_node(agent_list: list = None) -> AffectedItemsGuardSarmResult:
...
@expose_resources(actions=["agent:reload"], resources=["agent:id:{agent_list}"],
                  post_proc_kwargs={'exclude_codes': [1701, 1703, 1707], 'force': True},
                  post_proc_func=async_list_handler)
async def reload_agents_by_group(agent_list: list = None) -> AffectedItemsGuardSarmResult:
...
def get_agents(agent_list: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None,
    ...
    if agent_list:
        ...
        with GuardSarmDBQueryAgents(offset=offset, limit=limit, sort=sort, search=search, select=select,
                                query=q, **rbac_filters, distinct=distinct) as db_query:
            data = db_query.run()

        if sort and 'version' in sort['fields']:
            data["items"].sort(key=lambda item: GuardSarmVersion(item.get("version"), treat_as_empty=[None, 'N/A']),
...
@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"],
                  post_proc_kwargs={'exclude_codes': [1701]})
def get_agents_keys(agent_list: list = None) -> AffectedItemsGuardSarmResult:
    ...
    AffectedItemsGuardSarmResult
        Affected items.
    """
    result = AffectedItemsGuardSarmResult(all_msg='Obtained keys for all selected agents',
...
def delete_agents(agent_list: list = None, purge: bool = False, filters: dict = None,
                  q: str = None) -> AffectedItemsGuardSarmResult:
    ...
    """
    result = AffectedItemsGuardSarmResult(all_msg='All selected agents were deleted',
    ...
    if agent_list:
        ...
        with GuardSarmDBQueryAgents(limit=None, select=["id"], query=q, **rbac_filters) as db_query:
            data = db_query.run()
        ...
        list(map(lambda ag: result.add_failed_item(id_=ag, error=GuardSarmResourceNotFound(1701)), not_found_agents))

        # Add non eligible agents to failed_items
        non_eligible_agents = agent_list - not_found_agents - can_purge_agents
        list(map(lambda ag: result.add_failed_item(id_=ag, error=GuardSarmError(
            1731,
...
def add_agent(name: str = None, agent_id: str = None, key: str = None, ip: str = 'any',
              force: dict = None) -> GuardSarmResult:
    ...
    if len(name) > common.AGENT_NAME_LEN_LIMIT:
        raise GuardSarmError(1738)

    new_agent = Agent(name=name, ip=ip, id=agent_id, key=key, force=force)

    return GuardSarmResult({'data': {'id': new_agent.id, 'key': new_agent.key}})
...
def get_agent_groups(group_list: list = None, offset: int = 0, limit: int = None, sort_by: list = None,
    ...
    result = AffectedItemsGuardSarmResult(all_msg='All selected groups information was returned',
    ...
    if group_list:
        ...
        for invalid_group in set(group_list) - system_groups:
            result.add_failed_item(id_=invalid_group, error=GuardSarmResourceNotFound(1710))

        rbac_filters = get_rbac_filters(system_resources=system_groups, permitted_resources=group_list)

        with GuardSarmDBQueryGroup(**rbac_filters, limit=None) as group_query:
...
def get_group_files(group_list: list = None, offset: int = 0, limit: int = None, search_text: str = None,
                    ...
                    select: str = None, distinct: bool = False) -> GuardSarmResult:
    ...
    -------
    GuardSarmResult
        GuardSarmResult object with the groups files.
    """
    ...
    result = AffectedItemsGuardSarmResult(all_msg='All selected groups files were returned',
                                      some_msg='Some groups files were not returned',
                                      none_msg='No groups files were returned'
                                      )
    if group_id:
        if not Agent.group_exists(group_id):
            result.add_failed_item(id_=group_id, error=GuardSarmResourceNotFound(1710))
            return result
        group_path = path.join(common.SHARED_PATH, group_id)

    if not path.exists(group_path):
        result.add_failed_item(id_=group_path, error=GuardSarmError(1006))
        return result
    ...
    try:
        ...
        raise GuardSarmInternalError(1727, extra_message=str(e))

    return result


@expose_resources(actions=["group:create"], resources=["*:*:*"], post_proc_func=None)
def create_group(group_id: str) -> GuardSarmResult:
    """Creates a group.
    ...
    -------
    GuardSarmResult
        GuardSarmResult object with a operation message.
    """
    ...
    if group_id.lower() == "default" or path.exists(group_path):
        if not path.isfile(group_path):
            raise GuardSarmError(1711, extra_message=group_id)
        else:
            raise GuardSarmError(1713, extra_message=group_id)
    ...
    try:
        ...
        chown_r(group_path, common.guardsarm_uid(), common.guardsarm_gid())
        chmod_r(group_path, 0o660)
        chmod(group_path, 0o700)
        msg = f"Group '{group_id}' created."
    except Exception as e:
        raise GuardSarmInternalError(1005, extra_message=str(e))

    return GuardSarmResult({'message': msg})


@expose_resources(actions=["group:delete"], resources=["group:id:{group_list}"],
                  post_proc_kwargs={'exclude_codes': [1710, 1712]})
def delete_groups(group_list: list = None) -> AffectedItemsGuardSarmResult:
    ...
    AffectedItemsGuardSarmResult
        Affected items.
    """
    result = AffectedItemsGuardSarmResult(all_msg='All selected groups were deleted',
...
async def assign_agents_to_group(group_list: list = None, agent_list: list = None, replace: bool = False,
                           replace_list: list = None) -> AffectedItemsGuardSarmResult:
    ...
    """
    group_id = group_list[0]
    result = AffectedItemsGuardSarmResult(all_msg=f'All selected agents were assigned to {group_id}'
...
@expose_resources(actions=["agent:modify_group"], resources=['agent:id:{agent_list}'],
                  post_proc_func=async_list_handler)
async def remove_agent_from_group(group_list: list = None, agent_list: list = None) -> GuardSarmResult:
    ...
    return GuardSarmResult({'message': message})
...
async def remove_agent_from_groups(agent_list: list = None, group_list: list = None) -> AffectedItemsGuardSarmResult:
    ...
    """
    agent_id = agent_list[0]
    result = AffectedItemsGuardSarmResult(all_msg='Specified agent was removed from returned groups',
...
async def remove_agents_from_group(agent_list: list = None, group_list: list = None) -> AffectedItemsGuardSarmResult:
    ...
    """
    group_id = group_list[0]
    result = AffectedItemsGuardSarmResult(all_msg=f'All selected agents were removed from group {group_id}',
...
def get_outdated_agents(agent_list: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None,
                        search: dict = None, select: str = None, q: str = None) -> AffectedItemsGuardSarmResult:
...
def upgrade_agents(agent_list: list = None, wpk_repo: str = None, version: str = None, force: bool = False,
                   ...
                   filters: dict = None, q: str = None) -> AffectedItemsGuardSarmResult:
    ...
    """
    result = AffectedItemsGuardSarmResult(all_msg='All upgrade tasks were created',
    ...
    if agent_list:
        ...
        with GuardSarmDBQueryAgents(limit=None, select=["id", "status"], query=q, **rbac_filters) as db_query:
            data = db_query.run()
        ...
        not_found_agents = agent_list - system_agents
        [result.add_failed_item(id_=agent, error=GuardSarmResourceNotFound(1701)) for agent in not_found_agents]

        # Add non active agents to failed_items
        non_active_agents = [agent['id'] for agent in data['items'] if agent['status'] != 'active']
        [result.add_failed_item(id_=agent, error=GuardSarmError(1707)) for agent in non_active_agents]
        non_active_agents = set(non_active_agents)

        # Add non eligible agents to failed_items
        non_eligible_agents = agent_list - not_found_agents - non_active_agents - filtered_agents
        [result.add_failed_item(id_=ag, error=GuardSarmError(
            1731,
            extra_message="some of the requirements are not met -> {}".format(
                ', '.join(f"{key}: {value}" for key, value in filters.items() if key != 'rbac_ids') +
                (f', q: {q}' if q else '')
        ...
        for agent_result_chunk in tasks_results:
            for agent_result in agent_result_chunk['data']:
                ...
                # Upgrade error for specific agents
                    ...
                    error = GuardSarmError(error_code, cmd_error=True, extra_message=agent_result['message'])
                ...
                # Upgrade error for all agents, bad request
                    ...
                    raise GuardSarmError(error_code, cmd_error=True, extra_message=agent_result['message'])

                # Upgrade error for all agents, internal server error
                else:
                    raise GuardSarmInternalError(error_code, cmd_error=True, extra_message=agent_result['message'])
...
def get_upgrade_result(agent_list: list = None, filters: dict = None, q: str = None) -> AffectedItemsGuardSarmResult:
    ...
    AffectedItemsGuardSarmResult
        Upgrade results.
    """
    result = AffectedItemsGuardSarmResult(all_msg='All upgrade tasks were returned',
        ...
        for task_result_chunk in task_results:
            for task_result in task_result_chunk['data']:
                ...
                # Upgrade error for specific agents (no task in DB)
                    ...
                    error = GuardSarmError(error_code, cmd_error=True, extra_message=task_result['message'])
                    result.add_failed_item(id_=str(task_result['agent']).zfill(3), error=error)

                # Upgrade error for all agents, internal server error
                else:
                    raise GuardSarmInternalError(error_code, cmd_error=True, extra_message=task_result['message'])
...
@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"], post_proc_func=None)
def get_agent_config(agent_list: list = None, component: str = None, config: str = None) -> GuardSarmResult:
    ...
    if my_agent.status != "active":
        raise GuardSarmError(1740)

    return GuardSarmResult(
...
def get_file_conf(group_list: list = None, type_conf: str = None, raw: bool = False,
                  filename: str = None) -> GuardSarmResult:
    ...
    -------
    GuardSarmResult
        GuardSarmResult object with the configuration.
    """
    ...
    return GuardSarmResult({'data': configuration.get_file_conf(filename, group_id=group_id, type_conf=type_conf,
                                                            raw=raw)})


@expose_resources(actions=["group:read"], resources=["group:id:{group_list}"], post_proc_func=None)
def get_agent_conf(group_list: list = None, filename: str = 'agent.conf', offset: int = 0,
                   limit: int = common.DATABASE_LIMIT) -> GuardSarmResult:
...
def upload_group_file(group_list: list = None, file_data: str = None, file_name: str = 'agent.conf') -> GuardSarmResult:
    """Update a group file.
    ...
    -------
    GuardSarmResult
        GuardSarmResult object with the confirmation message.
    """
    # We access unique group_id from list, this may change if and when we decide to add option to update files for
    # a list of groups
    group_id = group_list[0]

    return GuardSarmResult({'message': configuration.upload_group_file(group_id, file_data, file_name=file_name)})


def get_full_overview() -> GuardSarmResult:
    """Get information about agents.

    Returns
    -------
    GuardSarmResult
        GuardSarmResult object with information about agents.
    """
    q = ''
    ...
    return GuardSarmResult({'data': result})


@expose_resources(actions=["agent:uninstall"], resources=["*:*:*"], post_proc_func=None)
def check_uninstall_permission() -> GuardSarmResult:
    ...
    -------
    GuardSarmResult
        GuardSarmResult object containing a message indicating if the
        user has the permission to uninstall agents.
    """
    return GuardSarmResult({'message': 'User has permission to uninstall agents'})
