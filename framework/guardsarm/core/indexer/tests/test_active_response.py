...
from guardsarm.core.indexer.active_response import (
    ActiveResponse,
...
class TestActiveResponseBookmarkFile:
    ...
    @patch("guardsarm.core.indexer.active_response.os.path.exists", return_value=False)
    ...
    @patch("guardsarm.core.indexer.active_response.os.path.exists", return_value=True)
    @patch(
        "builtins.open",
    ...
    @patch("guardsarm.core.indexer.active_response.os.fsync")
...
class TestActiveResponseHelpers:
    ...
    class TestGetActiveAgents:
        ...
        @patch("guardsarm.core.indexer.active_response.GuardSarmDBQueryAgents")
        ...
        @patch("guardsarm.core.indexer.active_response.GuardSarmDBQueryAgents")
        @patch("guardsarm.core.indexer.active_response.ActiveResponseHelpers.logger")
        def test_error(self, mock_logger, mock_db):
            from guardsarm.core.exception import GuardSarmError

            mock_db.side_effect = GuardSarmError(1)
    ...
    class TestFetchActiveResponseDocs:
        ...
        @pytest.mark.asyncio
        @patch("guardsarm.core.indexer.active_response.get_indexer_client")
        ...
        async def test_validation_filters_invalid_docs(self, mock_client):
            ...
            with patch(
                "guardsarm.core.indexer.active_response.jsonschema.validate"
            ) as mock_validate:
    ...
    class TestBuildArMessages:
        ...
        def test_with_event(self):
            ar = ActiveResponse(
                doc_source={"guardsarm": {"a": 1}, "x": 1},
                bookmark=ActiveResponseBookmark(),
                event={"guardsarm": {"b": 2}, "y": 2},
            )
            ...
            assert msg["x"] == 1
            assert msg["y"] == 2
            assert msg["guardsarm"]["a"] == 1
            assert msg["guardsarm"]["b"] == 2

        def test_without_event(self):
            ar = ActiveResponse(
                doc_source={"guardsarm": {"a": 1}},
                bookmark=ActiveResponseBookmark(),
            )
            ...
            assert result[0][1]["guardsarm"]["a"] == 1

        def test_no_targets(self):
            ar = ActiveResponse(
                doc_source={"guardsarm": {}},
                bookmark=ActiveResponseBookmark(),
            )
...
class TestActiveResponseBuilder:
    ...
    class TestFetchArs:
        ...
        @pytest.mark.asyncio
        @patch(
            "guardsarm.core.indexer.active_response.ActiveResponseHelpers.fetch_active_response_docs"
        )
    ...
    class TestEnrich:
        ...
        @pytest.mark.asyncio
        @patch(
            "guardsarm.core.indexer.active_response.ActiveResponseHelpers.get_events_by_ar"
        )
    ...
    class TestKeepOnlyActiveAgents:
        ...
        @patch(
            "guardsarm.core.indexer.active_response.ActiveResponseHelpers.is_valid_agent"
        )
    ...
    class TestDispatch:
        ...
        @patch("guardsarm.core.indexer.active_response.GuardSarmQueue")
        @patch(
            "guardsarm.core.indexer.active_response.ActiveResponseHelpers.build_ar_messages"
        )
        ...
        def test_dispatch_handles_error(self, mock_build, mock_queue):
            ...
            mock_wq = MagicMock()
            mock_wq.send_msg_to_agent.side_effect = GuardSarmError(1)
...
class TestActiveResponseFetchTask:
    ...
    class TestActiveResponseProcessing:
        ...
        @pytest.mark.asyncio
        @patch("guardsarm.core.indexer.active_response.ActiveResponseBuilder")
        ...
        async def test_handles_exception(self, mock_builder_cls):
            from guardsarm.core.exception import IndexerUnavailableError
            ...
            task.logger.warning.assert_called_once_with("Cannot connect to GuardSarm Indexer")
    ...
    class TestRun:
        ...
        @pytest.mark.asyncio
        @patch(
            "guardsarm.core.indexer.active_response.asyncio.sleep", new_callable=AsyncMock
        )
