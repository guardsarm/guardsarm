/**
 * GuardSarm Inventory Sync - ResponseDispatcher Unit tests
...
TEST_F(ResponseDispatcherTest, SendStartAck)
{
    ...
    EXPECT_CALL(*mockQueue, push(_))
        .WillOnce(Invoke(
            ...
            {
                auto msg = flatbuffers::GetRoot<GuardSarm::SyncSchema::Message>(responseMsg.builder.GetBufferPointer());
                ASSERT_EQ(msg->content_type(), GuardSarm::SyncSchema::MessageType_StartAck);

                auto startAck = msg->content_as_StartAck();
                ASSERT_NE(startAck, nullptr);
                EXPECT_EQ(startAck->status(), GuardSarm::SyncSchema::Status_Ok);
                EXPECT_EQ(startAck->session(), 12345);
                // Note: StartAck schema doesn't include module field
            }));

    dispatcher.sendStartAck(GuardSarm::SyncSchema::Status_Ok, agentId, sessionId, moduleName);
}
...
TEST_F(ResponseDispatcherTest, SendEndAck)
{
    ...
    EXPECT_CALL(*mockQueue, push(_))
        .WillOnce(Invoke(
            ...
            {
                auto msg = flatbuffers::GetRoot<GuardSarm::SyncSchema::Message>(responseMsg.builder.GetBufferPointer());
                ASSERT_EQ(msg->content_type(), GuardSarm::SyncSchema::MessageType_EndAck);

                auto endAck = msg->content_as_EndAck();
                ASSERT_NE(endAck, nullptr);
                EXPECT_EQ(endAck->status(), GuardSarm::SyncSchema::Status_Error);
                EXPECT_EQ(endAck->session(), 54321);
                // Note: EndAck schema doesn't include module field
            }));

    dispatcher.sendEndAck(GuardSarm::SyncSchema::Status_Error, agentId, sessionId, moduleName);
}
...
TEST_F(ResponseDispatcherTest, SendEndMissingSeq)
{
    ...
    EXPECT_CALL(*mockQueue, push(_))
        .WillOnce(Invoke(
            ...
            {
                auto msg = flatbuffers::GetRoot<GuardSarm::SyncSchema::Message>(responseMsg.builder.GetBufferPointer());
                ASSERT_EQ(msg->content_type(), GuardSarm::SyncSchema::MessageType_ReqRet);
