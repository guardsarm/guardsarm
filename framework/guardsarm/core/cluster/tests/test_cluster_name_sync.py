...
with patch("guardsarm.core.common.guardsarm_uid", "guardsarm"), patch(
    "guardsarm.core.common.guardsarm_gid", "guardsarm"
):
...
class TestRunClusterNameSync:
    ...
    @pytest.mark.asyncio
    @patch("guardsarm.core.cluster.master.asyncio.sleep", new_callable=AsyncMock)
    ...
    async def test_cluster_name_sync_success(
        ...
        mock_get_ossec_conf.return_value = {"cluster": {"name": "guardsarm-cluster"}}
    ...
    async def test_cluster_name_sync_no_agents_need_update(
        ...
        task._get_cluster_name_from_indexer = AsyncMock(
            return_value={"001": "guardsarm-cluster"}
        )
