...
The `agent_info` module exposes a C-style API for integration with `guardsarm-manager-modulesd` and uses a C++ implementation for its core logic. It also relies on a query-based API to coordinate with other modules.
...
These functions are used by `guardsarm-manager-modulesd` to provide the necessary callbacks for logging, reporting, and inter-module communication.
...
These functions integrate `agent_info` with the Agent Sync Protocol managed by `guardsarm-manager-modulesd`.

#### `agent_info_init_sync_protocol()`
Initializes the synchronization protocol handle. This function is called by `guardsarm-manager-modulesd` before `agent_info_start` to provide the necessary message queue functions.
