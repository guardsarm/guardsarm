...
with patch('guardsarm.core.common.guardsarm_uid'):
    with patch('guardsarm.core.common.guardsarm_gid'):
        from guardsarm.core.results import AffectedItemsGuardSarmResult, merge
...
class TestSortCastingValidation:
    ...
    def test_decode_json_rejects_malicious_types(self, malicious_type):
        ...
        with pytest.raises(GuardSarmException, match=f"Invalid sort_casting type '{malicious_type}'"):
            AffectedItemsGuardSarmResult.decode_json(malicious_obj)
    ...
    def test_decode_json_accepts_valid_types(self, valid_type):
        ...
        result = AffectedItemsGuardSarmResult.decode_json(valid_obj)
    ...
    def test_decode_json_rejects_non_string_types(self):
        ...
        with pytest.raises(GuardSarmException, match="sort_casting type must be a string"):
            AffectedItemsGuardSarmResult.decode_json(invalid_obj)
    ...
    def test_decode_json_rejects_non_list_sort_casting(self):
        ...
        with pytest.raises(GuardSarmException, match="sort_casting must be a list"):
            AffectedItemsGuardSarmResult.decode_json(invalid_obj)
    ...
    def test_rce_payload_blocked_in_decode_json(self):
        ...
        with pytest.raises(GuardSarmException, match="Invalid sort_casting type 'exec'"):
            AffectedItemsGuardSarmResult.decode_json(rce_payload)
    ...
    def test_merge_integration_with_valid_types(self):
        ...
        result1 = AffectedItemsGuardSarmResult(
            affected_items=[{'id': '003', 'name': 'agent3'}],
            sort_fields=['id'],
            sort_casting=['int'],
            sort_ascending=[True]
        )
        result2 = AffectedItemsGuardSarmResult(
