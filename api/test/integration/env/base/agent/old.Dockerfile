...
RUN curl -s https://packages.guardsarm.com/key/GPG-KEY-GUARDSARM | apt-key add - && \
    echo "deb https://packages.guardsarm.com/4.x/apt/ stable main" | tee /etc/apt/sources.list.d/guardsarm.list && \
