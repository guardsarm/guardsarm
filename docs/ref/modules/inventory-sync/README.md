...
The **Inventory Sync** module is the manager-side state synchronization service for agent state data. It receives FlatBuffer messages from agents over the Router topic `inventory-states`, stores the session payload temporarily in RocksDB, indexes the resulting state documents into `guardsarm-states-*`, and acknowledges completion back to the agent.
...
- **SCA** state documents in `guardsarm-states-sca`.
...
Sessions that enable vulnerability processing can also lead to downstream writes to `guardsarm-states-vulnerabilities`, but those documents are produced by the Vulnerability Scanner, not directly by Inventory Sync.
