...
// Fixed IV used by GuardSarm remoted for AES-256-CBC frames. See
// docu/05-wire-protocol.md §3.
var IV = []byte("FEDCBA0987654321")

// Routing prefix common to every GuardSarm agent message. Matches
...
// guardsarmPad prepends '!' bytes to align the input to a multiple of 8 bytes.
// When the input is already aligned, 8 '!' bytes are prepended anyway —
// this quirk matches benchmark_sender.py _guardsarm_pad.
func guardsarmPad(data []byte) []byte {
	extra := len(data) % 8
	padCount := 8 - extra
...
func encodeFrame(aesKey []byte, agentID string, payload []byte) ([]byte, error) {
	...
	padded := guardsarmPad(compressed)
...
//
//	"#AES:" || AES_CBC(pkcs7_pad(guardsarm_pad(zlib(MD5_hex(msg) || msg))))
...
func DecodeFrame(aesKey, frame []byte) ([]byte, error) {
	...
	// Unpad PKCS#7 first, then strip the leading '!' guardsarm padding.
	pt = pkcs7Unpad(pt)
	i := 0
	for i < len(pt) && pt[i] == '!' {
		i++
