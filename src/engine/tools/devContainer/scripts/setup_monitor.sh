...
#   curl -fsSL https://raw.githubusercontent.com/guardsarm/guardsarm/<branch>/src/engine/tools/devContainer/scripts/setup_monitor.sh | bash
#
# To also install charting dependencies (matplotlib, pandas, numpy):
#   INSTALL_CHARTS=1 bash setup_monitor.sh
#
# Default venv location: /opt/guardsarm-monitor-venv
...
# ---------------------------------------------------------------------------
VENV_DIR="${VENV_DIR:-/opt/guardsarm-monitor-venv}"
...
# ---------------------------------------------------------------------------
write_wrapper() {
    local wrapper="/usr/local/bin/guardsarm-monitor"
    ...
    cat > "${wrapper}" <<WRAPPER
#!/bin/bash
# Auto-generated by setup_monitor.sh — runs monitor.py inside the guardsarm-monitor venv.
exec "${VENV_DIR}/bin/python3" "${monitor_py}" "\$@"
WRAPPER
    chmod +x "${wrapper}"
    log "  Run: guardsarm-monitor --help"
}
...
main() {
    log "============================================================"
    log "  GuardSarm Monitor – Dependency Installer"
    log "  OS:         ${OS_NAME}"
    log "  Venv:       ${VENV_DIR}"
    ...
    if [[ -f /usr/local/bin/guardsarm-monitor ]]; then
        log "  Or via wrapper:"
        log "    guardsarm-monitor --output-dir ./out -s 1.0"
    fi
    if [[ "${INSTALL_CHARTS}" != "1" ]]; then
        log ""
