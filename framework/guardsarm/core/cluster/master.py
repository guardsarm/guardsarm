...
from guardsarm.core import cluster as metadata, common, exception, utils
from guardsarm.core.agent import Agent
from guardsarm.core.cluster import server, cluster, common as c_common
from guardsarm.core.cluster.common import IndexerTaskManager
from guardsarm.core.cluster.dapi import dapi
from guardsarm.core.cluster.utils import context_tag, log_subprocess_execution, safe_join
from guardsarm.core.common import DECIMALS_DATE_FORMAT
from guardsarm.core.configuration import get_ossec_conf
from guardsarm.core.indexer.metrics_snapshot import MetricsSnapshotTasks
from guardsarm.core.utils import get_utc_now
from guardsarm.core.wdb import AsyncGuardSarmDBConnection
from guardsarm.core.indexer.active_response import ActiveResponseFetchTask
from guardsarm.core.indexer.disconnected_agents import DisconnectedAgentSyncTasks
...
class ReceiveIntegrityTask(c_common.ReceiveFileTask):
    ...
    def set_up_coro(self) -> Callable:
        ...
        return self.guardsarm_common.integrity_check
    ...
    def done_callback(self, future=None):
        ...
        # if not self.guardsarm_common.extra_valid_requested:
        #     self.guardsarm_common.sync_integrity_free[0] = True
...
class ReceiveExtraValidTask(c_common.ReceiveFileTask):
    ...
    def set_up_coro(self) -> Callable:
        ...
        return self.guardsarm_common.sync_extra_valid
    ...
    def done_callback(self, future=None):
        ...
        """
        super().done_callback(future)
        self.guardsarm_common.extra_valid_requested = False
        self.guardsarm_common.sync_integrity_free[0] = True
...
class ReceiveAgentInfoTask(c_common.ReceiveStringTask):
    ...
    def set_up_coro(self) -> Callable:
        ...
        return self.guardsarm_common.sync_guardsarm_db_info
    ...
    def done_callback(self, future=None):
        ...
        """
        super().done_callback(future)
        self.guardsarm_common.sync_agent_info_free = True
...
class SendEntireAgentGroupsTask(c_common.SendStringTask):
    ...
    def set_up_coro(self) -> Callable:
        ...
        return self.guardsarm_common.send_entire_agent_groups_information


class MasterHandler(server.AbstractServerHandler, c_common.GuardSarmCommon):
    """
    ...
    async def execute(self, command: bytes, data: bytes, wait_for_complete: bool = False) -> Dict:
        ...
        if command == b'dapi_fwd':
            ...
            if client in self.server.clients:
                ...
                raise exception.GuardSarmClusterError(3022, extra_message=client)
        ...
        if command == b'dapi' or command == b'dapi_fwd':
            try:
                ...
                raise exception.GuardSarmClusterError(3021)
        # Otherwise, immediately return the result obtained before.
        else:
            status, request_result = result
            if status != b'ok':
                raise exception.GuardSarmClusterError(3022, extra_message=request_result.decode())
    ...
    def hello(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        ----------
        data : bytes
            Node name, cluster name, node type and guardsarm version all separated by spaces.
        ...
        if self.cluster_name != self.server.configuration['name']:
            raise exception.GuardSarmClusterError(3030)
        elif self.version != metadata.__version__:
            raise exception.GuardSarmClusterError(3031)

        # Create directory where zips and other files coming from or going to the worker will be managed.
        worker_dir = os.path.join(common.GUARDSARM_PATH, 'queue', 'cluster', self.name)
        ...
        # SyncGuardSarmdb instance to send agent-groups data to the worker.
        wdb_conn = AsyncGuardSarmDBConnection()
        self.agent_groups = c_common.SyncGuardSarmdb(manager=self, logger=self.task_loggers['Agent-groups send'],
    ...
    def get_manager(self) -> server.AbstractServer:
        """Get the Master object that created this MasterHandler. Used in the class GuardSarmCommon.
    ...
    def process_dapi_res(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        if req_id in self.server.pending_api_requests:
            ...
            raise exception.GuardSarmClusterError(3032, extra_message=req_id)
    ...
    async def sync_guardsarm_db_info(self, task_id: bytes, info_type: str):
        """Create a process to send to the local guardsarm-manager-db the chunks of data received from a worker.
    ...
    async def send_entire_agent_groups_information(self):
        ...
        sync_object = c_common.SyncGuardSarmdb(manager=self, logger=logger, cmd=b'syn_g_m_w_c',
                                           data_retriever=AsyncGuardSarmDBConnection().run_wdb_command,
    ...
    async def sync_worker_files(self, task_id: str, received_file: asyncio.Event, logger):
        ...
        try:
            ...
            raise exception.GuardSarmClusterError(3038, extra_message=str(e))
        finally:
            shutil.rmtree(decompressed_files_path)
        ...
        try:
            with utils.Timeout(timeout):
                for file_path, data in files_metadata.items():
                    full_path = safe_join(common.GUARDSARM_PATH, file_path)
                    item_key = data['cluster_item_key']

                    if item_key not in cluster_items['files']:
                        raise exception.GuardSarmClusterError(3022, extra_message=f"Invalid cluster_item_key: {item_key}")

                    # Only valid client.keys is the local one (master).
                    if os.path.basename(file_path) == 'client.keys':
                        raise exception.GuardSarmClusterError(3007)
                    ...
                    if data['merged']:
                        for unmerged_file_path, file_data, file_time in cluster.unmerge_info(
                            ...
                            try:
                                # Destination path.
                                full_unmerged_name = safe_join(common.GUARDSARM_PATH, unmerged_file_path)
                                expected_base = safe_join(common.GUARDSARM_PATH, item_key)
                                if not os.path.commonpath([full_unmerged_name, expected_base]).startswith(expected_base):
                                    raise exception.GuardSarmClusterError(3022,
                                ...
                                tmp_unmerged_path = safe_join(common.GUARDSARM_PATH, 'queue', 'cluster', worker_name,
                                ...
                                utils.safe_move(tmp_unmerged_path, full_unmerged_name,
                                                ownership=(common.guardsarm_uid(), common.guardsarm_gid()),
                    ...
                    # If the file is not 'merged' type, move it directly to the destination path.
                        ...
                        try:
                            ...
                            utils.safe_move(zip_path, full_path, ownership=(common.guardsarm_uid(), common.guardsarm_gid()),
                                            permissions=cluster_items['files'][item_key]['permissions'])
                        except TimeoutError as e:
                            raise e
                        except Exception as e:
...
class Master(server.AbstractServer):
    ...
    def __init__(self, **kwargs):
        ...
        # Handle exception when the user running GuardSarm cannot access /dev/shm
        except (FileNotFoundError, PermissionError):
            self.logger.warning(
                "In order to take advantage of GuardSarm 4.3.0 cluster improvements, the directory '/dev/shm' must be "
                "accessible by the 'guardsarm-manager' user. Check that this file has permissions to be accessed by all users. "
                "Changing the file permissions to 777 will solve this issue.")
            self.logger.warning(
                "The GuardSarm cluster will be run without the improvements added in GuardSarm 4.3.0 and higher versions.")
            self.task_pool = None
        ...
        if _indexer_conf:
            ...
            self.logger.warning(
                "Indexer is not configured in guardsarm-manager.conf or it is unavailable; "
    ...
    async def agent_groups_update(self):
        ...
        wdb_conn = AsyncGuardSarmDBConnection()
        sync_object = c_common.SyncGuardSarmdb(manager=self, logger=logger, cmd=b'syn_g_m_w',
