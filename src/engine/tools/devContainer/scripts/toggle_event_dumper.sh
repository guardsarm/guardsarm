#!/usr/bin/env bash
# Script to enable, disable, or check the status of the GuardSarm event-dumper via the local analysisd socket.
#
# Usage:
#   ./toggle_event_dumper.sh {enable|disable|status}
#
# Requirements:
#   - Must be run with sufficient permissions to access $ANALYSISD_SOCKET (usually as root or guardsarm user).
#   - Requires 'curl' installed with support for --unix-socket.
#
# Example:
#   sudo ./toggle_event_dumper.sh enable

ANALYSISD_SOCKET="/var/guardsarm-manager/queue/sockets/analysis"
