...
from guardsarm import common
from guardsarm.core.exception import GuardSarmException, GuardSarmInternalError

SOCKET_COMMUNICATION_PROTOCOL_VERSION = 1


class GuardSarmSocket:
    MAX_SIZE = 65536
    ...
    def _connect(self):
        try:
            ...
            raise GuardSarmInternalError(1013, extra_message=os.path.basename(self.path))
        except ConnectionRefusedError:
            raise GuardSarmInternalError(1121, extra_message=f"Socket '{os.path.basename(self.path)}' cannot receive "
                                                         "connections")
        except Exception as e:
            raise GuardSarmException(1013, str(e))
    ...
    def send(self, msg_bytes, header_format="<I"):
        if not isinstance(msg_bytes, bytes):
            raise GuardSarmException(1105, "Type must be bytes")

        try:
            sent = self.s.send(pack(header_format, len(msg_bytes)) + msg_bytes)
            if sent == 0:
                raise GuardSarmException(1014, "Number of sent bytes is 0")
            return sent
        except Exception as e:
            raise GuardSarmException(1014, str(e))
...
class GuardSarmSocketJSON(GuardSarmSocket):
    MAX_SIZE = 65536

    def __init__(self, path):
        GuardSarmSocket.__init__(self, path)

    def send(self, msg, header_format="<I"):
        return GuardSarmSocket.send(self, msg_bytes=dumps(msg).encode(), header_format=header_format)

    def receive(self, header_format="<I", header_size=4, raw=False):
        response = loads(GuardSarmSocket.receive(self, header_format=header_format, header_size=header_size).decode())
        if not raw:
            if 'error' in response.keys():
                if response['error'] != 0:
                    raise GuardSarmException(response['error'], response['message'], cmd_error=True)
...
daemons = {
    ...
    "guardsarm-manager-db": {"protocol": "TCP", "path": common.WDB_SOCKET, "header_format": "<I", "size": 4},
    "remoted": {"protocol": "TCP", "path": common.REMOTED_SOCKET, "header_format": "<I", "size": 4}
}


async def guardsarm_sendsync(daemon_name: str = None, message: str = None) -> dict:
    ...
    """
    try:
        sock = GuardSarmSocket(daemons[daemon_name]['path'])
        ...
        raise GuardSarmInternalError(1014, extra_message=e)

    return data


def create_guardsarm_socket_message(origin=None, command=None, parameters=None):
