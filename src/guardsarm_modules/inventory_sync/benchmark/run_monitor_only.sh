...
# (the load comes from real GuardSarm agents or another external source).
#
# Workflow:
...
#   3. Start your 4 real GuardSarm agents (or restart them after clearing their
...
#   2. setup_monitor.sh venv (/opt/guardsarm-monitor-venv)
#   3. System python3 that already has psutil
#   4. Known alternative paths
if [[ -n "${VIRTUAL_ENV:-}" ]]; then
    PYTHON="$VIRTUAL_ENV/bin/python3"
elif [[ -x "/opt/guardsarm-monitor-venv/bin/python3" ]]; then
    PYTHON="/opt/guardsarm-monitor-venv/bin/python3"
...
MERGE_PROCESS_NAME="${PROCESS_NAME:-guardsarm-manager-modulesd}"
...
if [[ -n "$PROCESS_NAME" ]]; then
    MONITOR_ARGS=(--exe "/var/guardsarm-manager/bin/$PROCESS_NAME" "${MONITOR_ARGS[@]}")
fi
