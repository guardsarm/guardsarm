...
`guardsarm-manager-db` is the persistent database daemon for the GuardSarm Manager. It manages SQLite databases for agent registration, group assignments, task tracking, and MITRE framework data.

Source: `src/guardsarm_db/`

For backup configuration see [GuardSarm-DB Configuration](../../configuration/guardsarm-db.md).
...
Socket: `/var/guardsarm-manager/queue/db/wdb` (Unix stream)
...
> **Note on {id}.db (4.x legacy):** In GuardSarm 4.x, each agent had a dedicated SQLite database at `queue/db/{agent_id}.db` storing per-agent inventory data (FIM events, packages, processes, network interfaces). In GuardSarm 5.0 this data is shipped directly to OpenSearch indices via the Indexer Connector (e.g. `guardsarm-states-fim-files`, `guardsarm-states-inventory-packages`). The per-agent SQLite databases are no longer created or used; existing files from a 4.x installation can be removed after migration.
