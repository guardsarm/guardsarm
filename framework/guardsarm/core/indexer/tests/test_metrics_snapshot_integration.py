...
AGENT_DOCS = [
    {
        ...
        "guardsarm.agent.name": "ubuntu-agent",
        "guardsarm.agent.host.ip": "10.0.0.1",
        "guardsarm.agent.status": "active",
        "guardsarm.agent.version": "GuardSarm v5.0.0",
        "guardsarm.agent.groups": ["default"],
        "guardsarm.agent.config.group.hash.md5": "def456",
        "guardsarm.agent.registered_at": "2026-01-01T00:00:00Z",
        "guardsarm.agent.last_seen": "2026-03-19T10:00:00Z",
        "guardsarm.agent.disconnected_at": 0,
        "guardsarm.agent.register.ip": "0.0.0.0/0",
        "guardsarm.agent.config.group.synced": True,
        "guardsarm.agent.status_code": 0,
        "guardsarm.agent.host.os.name": "Ubuntu",
        "guardsarm.agent.host.os.version": "22.04",
        "guardsarm.agent.host.os.platform": "ubuntu",
        "guardsarm.agent.host.architecture": "x86_64",
    }
]
...
class TestBulkIndexingIntegration:
    ...
    async def os_client(self):
        ...
        try:
            await client.indices.delete(index="guardsarm-metrics-agents-test", ignore_unavailable=True)
            await client.indices.delete(index="guardsarm-metrics-comms-test", ignore_unavailable=True)
        except Exception:
            pass
        finally:
            await client.close()
    ...
    async def test_agents_documents_indexed_successfully(self, os_client):
        ...
        index = "guardsarm-metrics-agents-test"
        docs_with_meta = [
            {
                "@timestamp": TIMESTAMP,
                "guardsarm.cluster.node": "master-node",
                "guardsarm.cluster.name": "guardsarm-cluster",
                "guardsarm.schema.version": "1",
                **doc,
            }
    ...
    async def test_agents_documents_have_timestamp(self, os_client):
        """Documents indexed into guardsarm-metrics-agents contain a valid @timestamp."""
        from opensearchpy.helpers import async_bulk

        index = "guardsarm-metrics-agents-test"
        doc = {"@timestamp": TIMESTAMP, "guardsarm.agent.id": "001", "guardsarm.agent.name": "test-agent"}
        ...
        result = await os_client.search(
            index=index,
            body={"query": {"term": {"guardsarm.agent.id.keyword": "001"}}},
        )
    ...
    async def test_comms_documents_indexed_successfully(self, os_client):
        ...
        index = "guardsarm-metrics-comms-test"
        docs_with_meta = [
            {
                "@timestamp": TIMESTAMP,
                "guardsarm.cluster.node": "master-node",
    ...
    async def test_comms_documents_have_expected_fields(self, os_client):
        """Documents indexed into guardsarm-metrics-comms contain all expected fields."""
        from opensearchpy.helpers import async_bulk
    ...
    async def test_timestamp_format_is_iso8601(self, os_client):
        ...
        for index in ["guardsarm-metrics-agents-test", "guardsarm-metrics-comms-test"]:
...
class TestRegressionDaemonStats:
    ...
    async def test_get_daemons_stats_still_callable(self):
        ...
        with patch("guardsarm.core.indexer.metrics_snapshot.get_daemons_stats", mock_get_daemons_stats):
            tasks = _make_tasks()
...
class TestRegressionAgentsEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_guardsarm_db_query_agents_called_with_no_limit(self):
        """GuardSarmDBQueryAgents is always called with limit=None — same as the agents endpoint."""
        with patch("guardsarm.core.indexer.metrics_snapshot.GuardSarmDBQueryAgents") as MockQuery:
    ...
    async def test_collect_agents_adds_metadata_fields(self):
        ...
        assert doc["guardsarm"]["agent"]["id"] == original_agent["id"]
        assert doc["guardsarm"]["agent"]["name"] == original_agent["name"]
        assert doc["guardsarm"]["agent"]["status"] == original_agent["status"]

        assert "@timestamp" in doc
        assert doc["guardsarm"]["cluster"]["node"] == "master-node"
        assert doc["guardsarm"]["cluster"]["name"] == "guardsarm-cluster"
        assert doc["guardsarm"]["schema"]["version"] == "1"
