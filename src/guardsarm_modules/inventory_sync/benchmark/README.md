...
└── benchmark/
    ...
    ├── indexer_control.sh       # Convenience wrapper for guardsarm-indexer
...
```bash
cd src/guardsarm_modules/inventory_sync/benchmark
...
GuardSarm manager daemons known by `monitor.py`; pass `--process guardsarm-manager-modulesd`
...
| `monitor/` | `monitor.py` | Process and disk CSVs, including `guardsarm-manager-modulesd.csv` and `disk_usage.csv`. |
...
```bash
git apply src/guardsarm_modules/inventory_sync/benchmark/patches/dbsync_metrics/0001-dbsync-log-metrics.patch
```
...
```json
{
  "metadata": {
    ...
    "mode": "ModuleDelta",
    "option": "Sync",
    "indices": ["guardsarm-states-fim-files"]
  },
  "items": [
    {
      "seq": 0,
      "operation": "Upsert",
      "id": "...",
      "index": "guardsarm-states-fim-files",
      "data": { }
    }
  ]
}
...
`indexer_control.sh` is a small wrapper around the local `guardsarm-indexer`
...
or sudo privileges, just like calling `service guardsarm-indexer ...` directly.
...
python3 result_summary.py \
  ...
  --monitor results_my_run/monitor/guardsarm-manager-modulesd.csv \
