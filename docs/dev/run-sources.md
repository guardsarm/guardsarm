...
This guide describes how to install and run GuardSarm components built from source code.
...
USER_DIR="/var/guardsarm-manager" \
USER_ENABLE_AUTHD="y" \
USER_UPDATE="y" \
USER_AUTO_START="n" \
./install.sh
...
```bash
/var/guardsarm-manager/bin/guardsarm-manager-control status
```
...
```batch
guardsarm-installer-build-msi.bat
```
...
```batch
guardsarm-agent-*.msi /q GUARDSARM_MANAGER="10.0.0.2"
```
...
Start the GuardSarm service on Windows:
...
```bash
/var/guardsarm-manager/bin/guardsarm-manager-control restart
```
...
```bash
/var/guardsarm-manager/bin/guardsarm-manager-control stop
```
...
Logs are located in `/var/guardsarm-manager/logs/`:

- `guardsarm-manager.log` - Main GuardSarm log
- `guardsarm-manager.json` - Structured GuardSarm log
- Individual component logs in `/var/guardsarm-manager/logs/`

To monitor logs in real-time:

```bash
tail -f /var/guardsarm-manager/logs/guardsarm-manager.log
```
