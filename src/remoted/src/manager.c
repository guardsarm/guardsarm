 ...
 * @param wdb_sock GuardSarm-DB socket.
    ...
    *cleaned_msg = clean;
            ...
            if (agent_info = cJSON_Parse(strchr(clean, '{')), agent_info) {
                ...
                if (version = cJSON_GetObjectItem(agent_info, "version"), cJSON_IsString(version)) {
                    ...
                    if (!logr.allow_higher_versions &&
                        compare_guardsarm_versions(__guardsarm_version, version->valuestring, false) < 0) {
    ...
    if (*is_shutdown == 0) {
        if (manager_module_limits_enabled &&
            agent_version[0] != '\0' &&
            compare_guardsarm_versions(agent_version, MIN_VERSION_MODULE_LIMITS, true) >= 0) {
