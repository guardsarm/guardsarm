...
from guardsarm.core.utils import clean_pid_files
...
def set_logging(foreground_mode=False, debug_mode=0) -> GuardSarmLogger:
    """Set cluster logger.
...
def print_version():
    """Print GuardSarm metadata."""
    from guardsarm.core.cluster import __author__, __licence__, __version__, __guardsarm_name__
    print(f"\n{__guardsarm_name__} {__version__} - {__author__}\n\n{__licence__}")
...
def exit_handler(signum, frame):
    ...
    pyDaemonModule.delete_child_pids('guardsarm-manager-clusterd', cluster_pid, main_logger)

    # Remove cluster's pidfile
    pyDaemonModule.delete_pid('guardsarm-manager-clusterd', cluster_pid)
...
# Master main
#
async def master_main(args: argparse.Namespace, cluster_config: dict, cluster_items: dict, logger: GuardSarmLogger):
    ...
    logger : GuardSarmLogger
        Cluster logger.
    """
    from guardsarm.core.cluster import local_server, master
    from guardsarm.core.cluster.hap_helper.hap_helper import HAPHelper
...
# Worker main
#
async def worker_main(args: argparse.Namespace, cluster_config: dict, cluster_items: dict, logger: GuardSarmLogger):
    ...
    from guardsarm.core.cluster import local_server, worker
...
def main():
    """Main function of the guardsarm-manager-clusterd script in charge of starting the cluster process."""
    import guardsarm.core.cluster.cluster

    # Set correct permissions on cluster.log file
    if os.path.exists(f'{common.GUARDSARM_PATH}/logs/cluster.log'):
        os.chown(f'{common.GUARDSARM_PATH}/logs/cluster.log', common.guardsarm_uid(), common.guardsarm_gid())
        os.chmod(f'{common.GUARDSARM_PATH}/logs/cluster.log', 0o660)  # nosec B103
    ...
    try:
        guardsarm.core.cluster.cluster.check_cluster_config(cluster_configuration)
    except Exception as e:
        main_logger.error(e)
        sys.exit(1)
    ...
    guardsarm.core.cluster.cluster.clean_up()

    # Check for unused PID files
    clean_pid_files('guardsarm-manager-clusterd')

    # Foreground/Daemon
    if not args.foreground:
        pyDaemonModule.pyDaemon()

    # Drop privileges to guardsarm
    if not args.root:
        os.setgid(common.guardsarm_gid())
        os.setuid(common.guardsarm_uid())

    pid = os.getpid()
    pyDaemonModule.create_pid('guardsarm-manager-clusterd', pid)
    if args.foreground:
    ...
    try:
        ...
        pyDaemonModule.delete_child_pids('guardsarm-manager-clusterd', pid, main_logger)
        pyDaemonModule.delete_pid('guardsarm-manager-clusterd', pid)


if __name__ == '__main__':
    import guardsarm.core.cluster.utils as cluster_utils
    from guardsarm.core import common, configuration, pyDaemonModule
    ...
    # Set logger
    try:
        debug_mode_ = configuration.get_internal_options_value('guardsarm_clusterd', 'debug', 2, 0) or args.debug_level
    except Exception:
        debug_mode_ = 0
