...
#            guardsarm-states-inventory-* for each agent.name BEFORE restarting.
...
#                        guardsarm-states-inventory-* for each agent.name before
...
#                        guardsarm.agent.name — that's the actual nested path in
#                        guardsarm-states-inventory-* docs). Use guardsarm.agent.id if
#                        your VM hostnames don't match the registered name.
#
# Run this from your workstation, not the manager. Each VM only needs SSH
# access from you and to be a Debian-based GuardSarm agent.
# ---------------------------------------------------------------------------
...
AGENT_FIELD="guardsarm.agent.name"
...
do_prep() {
    ...
    for vm in "${HOSTS[@]}"; do
        ...
        echo "[$vm] stopping guardsarm-agent..."
        ssh_run "$vm" "${SP}service guardsarm-agent stop || ${SP}systemctl stop guardsarm-agent || true" \
...
do_start_parallel_ssh() {
    ...
    for vm in "${HOSTS[@]}"; do
        (
            t0=$(date +%s.%N)
            ssh_run "$vm" "${SP}service guardsarm-agent start || ${SP}systemctl start guardsarm-agent" \
...
do_start_scheduled() {
    ...
    local SP; SP=$(sudo_prefix)
    for vm in "${HOSTS[@]}"; do
        ssh_run "$vm" "echo '${SP}service guardsarm-agent start || ${SP}systemctl start guardsarm-agent' \
...
indexer_snapshot_for_agent() {
    ...
    curl -sk -u "$INDEXER_AUTH" -X POST \
        "$INDEXER_URL/guardsarm-states-inventory-*/_search?size=0" \
...
try:
    ...
    if not bs:
        ...
        parts = [b["key"].replace("guardsarm-states-inventory-", "") + "=" + str(b["doc_count"]) for b in bs]
...
indexer_delete_for_agent() {
    local agent_value="$1"
    curl -sk -u "$INDEXER_AUTH" -X POST \
        "$INDEXER_URL/guardsarm-states-inventory-*/_delete_by_query?refresh=true&conflicts=proceed" \
        -H 'Content-Type: application/json' \
        -d "{\"query\":{\"term\":{\"${AGENT_FIELD}\":\"${agent_value}\"}}}" \
        2>/dev/null \
        | python3 -c '
...
do_resync() {
    ...
    if [[ "$WIPE_INDEXER" == true ]]; then
        ...
        for vm in "${HOSTS[@]}"; do
            ...
            echo "curl -sk -u '$INDEXER_AUTH' '$INDEXER_URL/guardsarm-states-inventory-*/_search?size=0&q=${AGENT_FIELD}:$vm' | python3 -m json.tool"
        done
    fi
}
