...
This section is the per-section/per-option reference for `guardsarm-manager.conf` (`/var/guardsarm-manager/etc/guardsarm-manager.conf`, root tag `<guardsarm_config>`). Each page documents one top-level section with all recognized options, their defaults, and allowed values verified against the parser source.
...
| [indexer](indexer.md) | GuardSarm Indexer connection |
...
| [guardsarm_db](guardsarm-db.md) | Database backup |
