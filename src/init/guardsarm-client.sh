...
# machine-readable GUARDSARM_VERSION/GUARDSARM_TYPE keys emitted by info().
...
check_folders()
{
    ...
    if [ ! -d $SOCKETS_FOLDER ]
    then
        if rm -rf $SOCKETS_FOLDER && mkdir -p $SOCKETS_FOLDER && chown guardsarm:guardsarm $SOCKETS_FOLDER && chmod 770 $SOCKETS_FOLDER
        then
...
add_to_cgroup()
{
    AGENTD_PID=$(head -n 1 ${DIR}/var/run/guardsarm-agentd-*.pid 2>/dev/null)
    CGROUP_PATH="/sys/fs/cgroup/systemd/system.slice/guardsarm-agent.service/cgroup.procs"

    # Check if cgroup path exists
    if [ ! -f "$CGROUP_PATH" ]; then
        echo "Warning: cgroup path does not exist: $CGROUP_PATH" >&2
    else
        for pidfile in ${DIR}/var/run/guardsarm-*.pid; do
            [ -f "$pidfile" ] || continue
...
restart)
    ...
    ;;
reload)
    DAEMONS=$(echo $DAEMONS | sed 's/guardsarm-agentd//')
    if is_systemd; then
    ...
    pid=`cat ${DIR}/var/run/guardsarm-agentd-*.pid`
    kill -USR1 $pid
    ;;
status)
    lock
