...
curl -k -X GET "https://localhost:55000/agents?q=version!%3Dguardsarm%205.0.0" \
  -H "Authorization: Bearer $TOKEN"
```
...
All exceptions inherit from `GuardSarmException` (defined in `core/exception.py`), with a numeric error code catalog:

| Range | Category | Examples |
|-------|----------|----------|
| 900–999 | API-level errors | Child process terminated, executor failure, endpoint restricted to master |
| 999–1099 | Core GuardSarm errors | Incompatible Python, internal error, command errors, socket issues |
