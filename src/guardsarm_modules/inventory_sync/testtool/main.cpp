/*
 * GuardSarm Vulnerability scanner - InventorySync Integration Test Tool
...
struct AgentTestData
{
     ...
     *       "index": "guardsarm-states-inventory-packages", // optional, auto-detected if omitted
     ...
     *   "data_context": [
     *     {
     *       "index": "guardsarm-states-inventory-system",    // optional, auto-detected if omitted
 ...
 *
 * This test component simulates the GuardSarm manager's alert-receiver socket.
...
private:
    ...
    void handleFlatBufferMessage(const char* data, size_t size)
    {
        ...
        auto message = GuardSarm::SyncSchema::GetMessage(data);

        switch (message->content_type())
        {
            case GuardSarm::SyncSchema::MessageType_StartAck: handleStartAck(message->content_as_StartAck()); break;
            case GuardSarm::SyncSchema::MessageType_EndAck: handleEndAck(message->content_as_EndAck()); break;
            case GuardSarm::SyncSchema::MessageType_ReqRet: handleReqRet(message->content_as_ReqRet()); break;
            default:
                std::cout << "[WARN] Unknown message type: " << static_cast<int>(message->content_type()) << std::endl;
        }
    }

    void handleStartAck(const GuardSarm::SyncSchema::StartAck* startAck)
    {
    ...
    void handleEndAck(const GuardSarm::SyncSchema::EndAck* endAck)
    {
    ...
    void handleReqRet(const GuardSarm::SyncSchema::ReqRet* reqRet)
    {
...
/**
 * @brief Helper to map string mode -> GuardSarm::SyncSchema::Mode
 */
inline GuardSarm::SyncSchema::Mode parseMode(const std::string& modeStr)
{
    if (modeStr == "full" || modeStr == "ModuleFull")
    {
        return GuardSarm::SyncSchema::Mode_ModuleFull;
    }
    if (modeStr == "delta" || modeStr == "ModuleDelta")
    {
        return GuardSarm::SyncSchema::Mode_ModuleDelta;
    }
    // Default to delta
    return GuardSarm::SyncSchema::Mode_ModuleDelta;
}
...
/**
 * @brief Helper to map string option -> GuardSarm::SyncSchema::Option
 */
inline GuardSarm::SyncSchema::Option parseOption(const std::string& opt)
{
    if (opt == "VDFirst")
        return GuardSarm::SyncSchema::Option_VDFirst;
    if (opt == "VDSync")
        return GuardSarm::SyncSchema::Option_VDSync;
    if (opt == "Sync")
        return GuardSarm::SyncSchema::Option_Sync;
    return GuardSarm::SyncSchema::Option_VDSync;
}
...
public:
    ...
    buildStart(const nlohmann::json& startJson, uint64_t defaultSize, const std::vector<std::string>& defaultIndices)
    {
        ...
        auto message =
            GuardSarm::SyncSchema::CreateMessage(builder, GuardSarm::SyncSchema::MessageType_Start, startOffset.Union());
     ...
     * @param index     Index name (guardsarm-states-inventory-packages/system/hotfixes)
    ...
    static std::vector<uint8_t> buildDataValue(uint64_t session,
                                               ...
                                               const std::string& id,
                                               GuardSarm::SyncSchema::Operation operation)
    {
        flatbuffers::FlatBufferBuilder builder;
        ...
        GuardSarm::SyncSchema::DataValueBuilder dataBuilder(builder);
        ...
        auto message =
            GuardSarm::SyncSchema::CreateMessage(builder, GuardSarm::SyncSchema::MessageType_DataValue, dataOffset.Union());
        builder.Finish(message);
        ...
        uint64_t session, uint64_t seq, const nlohmann::json& payload, const std::string& index, const std::string& id)
    {
        ...
        GuardSarm::SyncSchema::DataContextBuilder dataBuilder(builder);
        dataBuilder.add_session(session);
        dataBuilder.add_seq(seq);
        dataBuilder.add_index(indexStr);
        dataBuilder.add_id(idStr);
        ...
        auto dataOffset = dataBuilder.Finish();
        auto message =
            GuardSarm::SyncSchema::CreateMessage(builder, GuardSarm::SyncSchema::MessageType_DataContext, dataOffset.Union());
        builder.Finish(message);
    ...
    static std::vector<uint8_t> buildEnd(uint64_t session)
    {
        ...
        auto message = GuardSarm::SyncSchema::CreateMessage(builder, GuardSarm::SyncSchema::MessageType_End, endOffset.Union());
        builder.Finish(message);
               ...
               std::atomic<bool>& receivedEndAck)
{
    ...
    for (const auto& dataValue : testData.dataValues)
    {
        ...
        auto op = (operation == "delete") ? GuardSarm::SyncSchema::Operation_Delete : GuardSarm::SyncSchema::Operation_Upsert;
