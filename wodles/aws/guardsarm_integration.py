...
GUARDSARM_DEFAULT_RETRY_CONFIGURATION = {aws_tools.RETRY_ATTEMPTS_KEY: 10, aws_tools.RETRY_MODE_BOTO_KEY: 'standard'}
MESSAGE_HEADER = "1:GuardSarm-AWS:"


class GuardSarmIntegration:
    """
    ...
    def __init__(self, access_key, secret_key, profile, iam_role_arn, service_name=None, region=None,
        ...
        self.guardsarm_path = utils.find_guardsarm_path()
        self.guardsarm_version = utils.get_guardsarm_version()
        self.guardsarm_queue = path.join(self.guardsarm_path, "queue", "sockets", "queue")
        self.guardsarm_wodle = path.join(self.guardsarm_path, "wodles", "aws")
    ...
    def default_config(profile: str) -> dict:
        ...
        This method is called when GuardSarm Integration is instantiated and sets a default config using .aws/config file
        ...
        if path.exists(aws_tools.DEFAULT_AWS_CONFIG_PATH):
            ...
            try:
                ...
                if aws_tools.RETRY_ATTEMPTS_KEY in profile_config \
                    ...
                    # Set retry config
                    retries = copy.deepcopy(GUARDSARM_DEFAULT_RETRY_CONFIGURATION)
                    aws_tools.debug(
            ...
            args['config'] = config.Config(retries=copy.deepcopy(GUARDSARM_DEFAULT_RETRY_CONFIGURATION))
            aws_tools.debug(
    ...
    def send_msg(self, msg, dump_json=True):
        """
        Sends an AWS event to the GuardSarm Queue
        ...
        try:
            ...
            if e.errno == 111:
                aws_tools.error("GuardSarm must be running.")
                sys.exit(11)
            elif e.errno == 90:
                aws_tools.error("Message too long to send to GuardSarm.  Skipping message...")
                aws_tools.debug('+++ ERROR: Message longer than buffer socket for GuardSarm. Consider increasing rmem_max. '
                                'Skipping message...', 1)
            else:
                aws_tools.error("Error sending message to guardsarm: {}".format(e))
                sys.exit(13)
        except Exception as e:
            aws_tools.error("Error sending message to guardsarm: {}".format(e))
            sys.exit(13)
...
class GuardSarmAWSDatabase(GuardSarmIntegration):
    """
    ...
    def __init__(self, access_key, secret_key, profile, iam_role_arn, db_name,
        ...
        GuardSarmIntegration.__init__(self, service_name=service_name,
        ...
        self.db_path = "{0}/{1}.db".format(self.guardsarm_wodle, db_name)
        ...
        try:
            if self.db_cursor.execute(self.sql_find_table, {'name': 'metadata'}).fetchone():
                ...
                try:
                    ...
                    if metadata_version != self.guardsarm_version:
                        self.db_cursor.execute(self.sql_update_version_metadata, {'guardsarm_version': self.guardsarm_version})
                ...
                try:
                    ...
                    self.db_cursor.execute(self.sql_insert_version_metadata, {'guardsarm_version': self.guardsarm_version})
