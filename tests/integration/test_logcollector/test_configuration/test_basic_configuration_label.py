...
brief: The 'guardsarm-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       the 'label' tag is set in the configuration, and the GuardSarm API returns the same values for
...
config_path = Path(CONFIGURATIONS_PATH, 'guardsarm_basic_configuration_label.yaml')
...
def test_configuration_label(test_configuration, test_metadata, configure_local_internal_options, set_guardsarm_configuration,
                             daemons_handler_module, stop_logcollector):
    '''
    description: Check if the 'guardsarm-logcollector' daemon can monitor log files configured to use labels.
                 ...
                 Finally, the test will verify that the GuardSarm API returns the same values for
    ...
    guardsarm_log_monitor.start(callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_ANALYZING_FILE, {
