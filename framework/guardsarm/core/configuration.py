...
from guardsarm.core import common, guardsarm_socket
from guardsarm.core.exception import GuardSarmError, GuardSarmInternalError, GuardSarmResourceNotFound
from guardsarm.core.utils import cut_array, load_guardsarm_xml, safe_move
...
def _replace_custom_values(opt_value: Union[list, dict, str]) -> Union[list, dict, str]:
    """Replace custom values introduced by 'load_guardsarm_xml' with their real values.
...
def _guardsarmconf2json(xml_conf: str) -> dict:
    """Return guardsarm-manager.conf in JSON from XML.
    ...
    -------
    dict
        Final JSON with the guardsarm-manager.conf content.
    """
    final_json = {}

    for root in list(xml_conf):
        if root.tag.lower() == "guardsarm_config":
...
def _rcl2json(filepath: str) -> dict:
    ...
    try:
        ...
        raise GuardSarmError(1101, str(e))
...
def get_ossec_conf(section: str = None, field: str = None, conf_file: str = common.OSSEC_CONF,
    ...
    """Return guardsarm-manager.conf (manager) as dictionary.
    ...
    -------
    dict
        guardsarm-manager.conf (manager) as dictionary.
    """
    try:
        # Read XML
        xml_data = load_guardsarm_xml(conf_file)

        # Parse XML to JSON
        data = _guardsarmconf2json(xml_data)
    except Exception as e:
        if not from_import:
            raise GuardSarmError(1101, extra_message=str(e))
        else:
            print(f"guardsarm-manager-apid: There is an error in the guardsarm-manager.conf file: {str(e)}")
            sys.exit(0)
    ...
    if section:
        try:
            ...
            if section not in CONF_SECTIONS.keys():
                raise GuardSarmError(1102, extra_message=e.args[0])
            else:
                raise GuardSarmError(1106, extra_message=e.args[0])
...
def get_agent_conf(group_id: str = None, offset: int = 0, limit: int = common.DATABASE_LIMIT,
    ...
    if not os_path.exists(os_path.join(common.SHARED_PATH, group_id)):
        raise GuardSarmResourceNotFound(1710, group_id)
    ...
    if not os_path.exists(agent_conf):
        raise GuardSarmError(1006, agent_conf)
    ...
    try:
        ...
        # Parse XML to JSON
            ...
            # Read XML
            xml_data = load_guardsarm_xml(agent_conf)
...
def get_file_conf(filename: str, group_id: str = None, type_conf: str = None, raw: bool = False) -> dict | str:
    ...
    if not os_path.exists(file_path):
        raise GuardSarmError(1006, file_path)
    ...
    if type_conf:
        if type_conf in types:
            ...
            raise GuardSarmError(1104, f'{type_conf}. Valid types: {types.keys()}')
    else:
...
def parse_internal_options(high_name: str, low_name: str) -> str:
    """Parse guardsarm-manager-internal-options.conf file.
    ...
    Raises
    ------
    GuardSarmInternalError(1107)
        Internal options file not found.
    GuardSarmInternalError(1108)
        Value not found in guardsarm-manager-internal-options.conf.

    Returns
    -------
    str
        Value of the guardsarm-manager-internal-options.conf option.
    """
...
def get_internal_options_value(high_name: str, low_name: str, max_: int, min_: int) -> int:
    ...
    if not option.isdigit():
        raise GuardSarmError(1109, f'Option: {high_name}.{low_name}. Value: {option}')

    option = int(option)
    if option < min_ or option > max_:
        raise GuardSarmError(1110, f'Max value: {max_}. Min value: {min_}. Found: {option}.')
...
def upload_group_configuration(group_id: str, file_content: str) -> str:
    ...
    GuardSarmError(1113)
        XML syntax error.
    GuardSarmError(1114)
        GuardSarm syntax error.
    GuardSarmError(1115)
        Error executing verify-agent-conf.
    GuardSarmInternalError(1743)
        Error running GuardSarm syntax validator.
    GuardSarmInternalError(1016)
        Error moving file.
    ...
    try:
        ...
        raise GuardSarmError(1113, str(e))

    try:
        # check GuardSarm xml format
        try:
            subprocess.check_output([os_path.join(common.GUARDSARM_PATH, "bin", "verify-agent-conf"), '-f', tmp_file_path],
            ...
            # Syscheck remote configuration in '/var/guardsarm-manager/tmp/api_tmp_file_2019-01-08-01-1546959069.xml' is corrupted.
            # \n\n
            ...
            if output_regex:
                raise GuardSarmError(1114, ' '.join(output_regex))
            else:
                raise GuardSarmError(1115, e.output.decode())
        except Exception as e:
            raise GuardSarmInternalError(1743, str(e))

        # move temporary file to group folder
        try:
            new_conf_path = os_path.join(common.SHARED_PATH, group_id, "agent.conf")
            safe_move(tmp_file_path, new_conf_path, ownership=(common.guardsarm_uid(), common.guardsarm_gid()),
                      permissions=0o660)
        except Exception as e:
            raise GuardSarmInternalError(1016, extra_message=str(e))
...
def get_active_configuration(component: str, configuration: str, agent_id: Optional[str] = None) -> dict:
    ...
    GuardSarmInternalError(1121)
        If the socket cant be created.
    GuardSarmInternalError(1118)
    ...
    component_socket_mapping = {'agent': 'analysis', 'analysis': 'analysis', 'auth': 'auth',
                                ...
                                'request': 'remote', 'syscheck': 'syscheck', 'guardsarm-manager-db': 'wdb', 'wmodules': 'wmodules'}
    component_socket_dir_mapping = {'agent': 'sockets', 'analysis': 'sockets',
                                    ...
                                    'request': 'sockets', 'syscheck': 'sockets', 'guardsarm-manager-db': 'db',
                                    'wmodules': 'sockets'}

    if not component or not configuration:
        raise GuardSarmError(1307)

    # Check if the component is correct
    components = component_socket_mapping.keys()
    if component not in components:
        raise GuardSarmError(1101, f'Valid components: {", ".join(components)}')
    ...
    def get_active_configuration_manager():
        ...
        dest_socket = os_path.join(common.GUARDSARM_PATH, "queue", component_socket_dir_mapping[component],
                                   component_socket_mapping[component])

        # Verify component configuration
        if not os.path.exists(dest_socket):
            raise GuardSarmError(1121,
        ...
        if component_socket_mapping[component] not in sockets_json_protocol:
            ...
            # Socket connection
            try:
                s = guardsarm_socket.GuardSarmSocket(dest_socket)
            except GuardSarmInternalError:
                raise
            except Exception as unhandled_exc:
                raise GuardSarmInternalError(1121, extra_message=str(unhandled_exc))
            ...
            try:
                ...
                raise GuardSarmInternalError(1118, extra_message="Data could not be received")
            finally:
                s.close()
        ...
        # JSON socket message
            ...
            msg = guardsarm_socket.create_guardsarm_socket_message(origin={'module': common.origin_module.get()},
                                                           command=GETCONFIG_COMMAND,
                                                           parameters={'section': configuration})

            # Socket connection
            try:
                s = guardsarm_socket.GuardSarmSocketJSON(dest_socket)
            except GuardSarmInternalError:
                raise
            except Exception as unhandled_exc:
                raise GuardSarmInternalError(1121, extra_message=str(unhandled_exc))
    ...
    if rec_error == 'ok' or rec_error == 0:
        ...
        if data.get('auth', {}).get('use_password') == 'yes':
            try:
                with open(os_path.join(common.GUARDSARM_PATH, "etc", "authd.pass"), 'r') as f:
        ...
        raise GuardSarmError(1117 if "No such file or directory" in rec_data or "Cannot send request" in rec_data else 1116,
                         extra_message=f'{component}:{configuration}')


def write_ossec_conf(new_conf: str):
    """Replace the current guardsarm configuration (guardsarm-manager.conf) with the provided configuration.
    ...
    try:
        ...
        raise GuardSarmError(1126, extra_message=str(e))
