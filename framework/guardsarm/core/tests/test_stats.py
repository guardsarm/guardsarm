...
with patch('guardsarm.core.common.guardsarm_uid'):
    with patch('guardsarm.core.common.guardsarm_gid'):
        ...
        from guardsarm.core import common, stats
...
@patch('guardsarm.core.guardsarm_socket.GuardSarmSocketJSON.close')
@patch('guardsarm.core.guardsarm_socket.GuardSarmSocketJSON.send')
@patch('guardsarm.core.guardsarm_socket.GuardSarmSocketJSON.__init__', return_value=None)
def test_get_daemons_stats_socket(mock__init__, mock_send, mock_close, agents_list, expected_socket_response,
    ...
    with patch('guardsarm.core.guardsarm_socket.GuardSarmSocketJSON.receive',
...
def test_get_daemons_stats_socket_ko(agents_list):
    ...
    socket = '/test_path/socket'
    with pytest.raises(GuardSarmInternalError, match=f".* 1121 .*: {socket}"):
...
def test_get_daemons_stats_from_socket(agent_id, daemon, response):
    ...
    with patch('guardsarm.core.guardsarm_socket.GuardSarmSocket.__init__', return_value=None) as mock_socket:
        with patch('guardsarm.core.guardsarm_socket.GuardSarmSocket.send', side_effect=None) as mock_send:
            with patch('guardsarm.core.guardsarm_socket.GuardSarmSocket.receive', return_value=response.encode()):
                with patch('guardsarm.core.guardsarm_socket.GuardSarmSocket.close', side_effect=None):
                    stats.get_daemons_stats_from_socket(agent_id, daemon)

        mock_socket.assert_called_once_with(os.path.join(common.GUARDSARM_PATH, "queue", "sockets", "remote"))
...
def test_get_daemons_stats_from_socket(agent_id, daemon, responses, expected, expected_socket_calls,
    ...
    with patch('guardsarm.core.guardsarm_socket.GuardSarmSocket.__init__', return_value=None) as mock_socket:
        with patch('guardsarm.core.guardsarm_socket.GuardSarmSocket.send', side_effect=None) as mock_send:
            with patch('guardsarm.core.guardsarm_socket.GuardSarmSocket.receive', side_effect=responses):
                with patch('guardsarm.core.guardsarm_socket.GuardSarmSocket.close', side_effect=None):
