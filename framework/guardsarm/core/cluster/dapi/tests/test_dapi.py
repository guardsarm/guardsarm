...
with patch('guardsarmsiem.common.guardsarm_uid'):
    with patch('guardsarmsiem.common.guardsarm_gid'):
        ...
        from guardsarm.core.cluster.dapi.dapi import DistributedAPI, APIRequestQueue, SendSyncRequestQueue
        from guardsarm.core.manager import get_manager_status
        from guardsarm.core.results import GuardSarmResult, AffectedItemsGuardSarmResult
        from guardsarm import agent, cluster, manager, GuardSarmError, GuardSarmInternalError
        from guardsarm.core.exception import GuardSarmClusterError, GuardSarmException
...
@pytest.mark.parametrize('kwargs', [
    {'f_kwargs': {'select': ['id']}, 'rbac_permissions': {'mode': 'black'}, 'nodes': ['worker1'],
     'basic_services': ('guardsarm-manager-modulesd', 'guardsarm-manager-db'), 'request_type': 'local_master'},
...
def test_DistributedAPI_debug_log():
    ...
    logger_ = TestingLogger(logger_name="guardsarm-api")
    ...
    logger_ = TestingLogger(logger_name="guardsarm")
...
@patch('guardsarm.core.cluster.dapi.dapi.DistributedAPI.execute_local_request',
       new=AsyncMock(return_value=GuardSarmResult({'result': 'local'})))
@patch('guardsarm.core.cluster.dapi.dapi.DistributedAPI.forward_request',
       new=AsyncMock(return_value=GuardSarmResult({'result': 'forward'})))
@patch('guardsarm.core.cluster.dapi.dapi.DistributedAPI.execute_remote_request',
       new=AsyncMock(return_value=GuardSarmResult({'result': 'remote'})))
...
def test_DistributedAPI_distribute_function(api_request, request_type, node, expected, f_kwargs):
    ...
    """
    # Mock get_node
    with patch('guardsarm.core.cluster.cluster.get_node', return_value={'type': node}):
...
@patch('guardsarm.core.cluster.dapi.dapi.DistributedAPI.execute_local_request',
       new=AsyncMock(return_value=GuardSarmResult({'result': 'local'})))
@patch('guardsarm.core.cluster.dapi.dapi.DistributedAPI.get_solver_node',
       new=AsyncMock(return_value=GuardSarmResult({'unknown': ['001', '002']})))
...
def test_DistributedAPI_distribute_function_mock_solver(api_request, request_type, node, expected):
    ...
    expected : str
        Expected result
    """
    with patch('guardsarm.core.cluster.cluster.get_node', return_value={'type': node, 'node': 'master'}):
...
def test_DistributedAPI_distribute_function_exception():
    ...
    logger_ = logging.getLogger("guardsarm")
    with patch("guardsarm.core.cluster.dapi.dapi.get_node_wrapper", side_effect=GuardSarmError(4000)):
        dapi = DistributedAPI(f=agent.get_agents_summary_status, logger=logger_)
        get_error_result = dapi.get_error_info(Exception("testing"))
        assert 'unknown-node' in get_error_result
        assert get_error_result['unknown-node']['error'] == 'GuardSarm Internal Error. See log for more detail'

    with patch("guardsarm.core.cluster.dapi.dapi.get_node_wrapper", side_effect=GuardSarmError(4001)):
        dapi = DistributedAPI(f=agent.get_agents_summary_status, logger=logger_)
        with pytest.raises(GuardSarmError, match='.* 4001 .*'):
            dapi.get_error_info(Exception("testing"))

    with patch("guardsarm.core.cluster.dapi.dapi.get_node_wrapper", return_value=NodeWrapper()):
        dapi = DistributedAPI(f=agent.get_agents_summary_status, logger=logger_)
...
def test_DistributedAPI_local_request_errors():
    ...
    with patch('guardsarm.core.cluster.dapi.dapi.DistributedAPI.execute_local_request',
               new=AsyncMock(side_effect=GuardSarmInternalError(1001))):
        dapi_kwargs = {'f': agent.get_agents_summary_status, 'logger': logger}
        ...
        try:
            raise_if_exc(loop.run_until_complete(dapi.distribute_function()))
        except GuardSarmInternalError as e:
            assert e.code == 1001
    ...
    with patch('guardsarm.cluster.get_nodes_info', side_effect=json.decoder.JSONDecodeError('test', 'test', 1)):
        with patch('guardsarm.core.cluster.dapi.dapi.DistributedAPI.check_guardsarm_status'):
...
def test_DistributedAPI_local_request(mock_local_request):
    ...
    with patch('asyncio.wait_for', new=AsyncMock(side_effect=GuardSarmError(1001))):
        dapi_kwargs = {'f': manager.status, 'logger': logger}
        raise_if_exc_routine(dapi_kwargs=dapi_kwargs, expected_error=1001)
    ...
    with patch('asyncio.wait_for', new=AsyncMock(side_effect=GuardSarmInternalError(1001))):
        dapi_kwargs = {'f': manager.status, 'logger': logger}
        raise_if_exc_routine(dapi_kwargs=dapi_kwargs, expected_error=1001)
    ...
    exc_code = 3000
    cluster_exc = GuardSarmClusterError(exc_code)
    with patch('asyncio.wait_for', new=AsyncMock(side_effect=cluster_exc)):
        with patch.object(TestingLogger, "error") as logger_error_mock:
            # Test GuardSarmClusterError caught in execute_local_request and ProblemException raised
            dapi_kwargs = {'f': manager.status, 'logger': testing_logger}
            raise_if_exc_routine(dapi_kwargs=dapi_kwargs, expected_error=exc_code)

            # Test GuardSarmClusterError is raised when using debug in execute_local_request and distribute_function
            dapi = DistributedAPI(f=manager.status, logger=testing_logger, debug=True)
            try:
                raise_if_exc(loop.run_until_complete(dapi.distribute_function()))
            except GuardSarmClusterError as e:
...
@patch('guardsarm.core.cluster.dapi.dapi.node_info', {'type': 'worker'})
@patch('guardsarm.core.cluster.local_client.LocalClient.execute', return_value='invalid_json')
...
@patch('guardsarm.core.cluster.local_client.LocalClient.execute', new=AsyncMock(return_value='{"Testing": 1}'))
...
@patch('guardsarm.core.cluster.cluster.get_node', return_value={'type': 'master', 'node': 'master-node'})
@patch('guardsarm.core.cluster.dapi.dapi.DistributedAPI.get_solver_node', return_value={'worker1': ['001', '002']})
@patch('guardsarm.core.cluster.local_client.LocalClient.execute', return_value='invalid_json')
...
@patch('guardsarm.core.cluster.local_client.LocalClient.send_file', new=AsyncMock(return_value='{"Testing": 1}'))
@patch('guardsarm.core.cluster.local_client.LocalClient.execute', new=AsyncMock(return_value='{"Testing": 1}'))
@patch('guardsarm.core.cluster.dapi.dapi.DistributedAPI.get_solver_node',
       new=AsyncMock(return_value=GuardSarmResult({'testing': ['001', '002']})))
def test_DistributedAPI_tmp_file():
    ...
    with patch('guardsarm.core.cluster.cluster.get_node', return_value={'type': 'master', 'node': 'unknown'}):
        with patch('guardsarm.core.cluster.dapi.dapi.get_node_wrapper',
                   return_value=AffectedItemsGuardSarmResult(affected_items=[{'type': 'master', 'node': 'unknown'}])):
            dapi_kwargs = {'f': manager.status, 'logger': logger, 'request_type': 'distributed_master',
                           'f_kwargs': {'tmp_file': 'dapi_file.txt'}}
            raise_if_exc_routine(dapi_kwargs=dapi_kwargs)

    open(tmp_file_path, 'a').close()
    with patch('guardsarm.core.cluster.cluster.get_node', return_value={'type': 'unk', 'node': 'master'}):
        raise_if_exc_routine(dapi_kwargs=dapi_kwargs)
...
def test_DistributedAPI_tmp_file_cluster_error():
    ...
    with patch('guardsarm.core.cluster.cluster.get_node', return_value={'type': 'master', 'node': 'unknown'}):
        with patch('guardsarm.core.cluster.dapi.dapi.get_node_wrapper',
                   return_value=AffectedItemsGuardSarmResult(affected_items=[{'type': 'master', 'node': 'unknown'}])):
            with patch('guardsarm.core.cluster.local_client.LocalClient.execute',
                       new=AsyncMock(side_effect=GuardSarmClusterError(3022))):
                dapi_kwargs = {'f': manager.status, 'logger': logger, 'request_type': 'distributed_master',
                               'f_kwargs': {'tmp_file': 'dapi_file.txt'}}
            ...
            open(tmp_file_path, 'a').close()
            with patch('guardsarm.core.cluster.local_client.LocalClient.execute',
                       new=AsyncMock(side_effect=GuardSarmClusterError(1000))):
                dapi_kwargs = {'f': manager.status, 'logger': logger, 'request_type': 'distributed_master',
                               'f_kwargs': {'tmp_file': 'dapi_file.txt'}}
                raise_if_exc_routine(dapi_kwargs=dapi_kwargs, expected_error=1000)


@patch('guardsarm.core.cluster.local_client.LocalClient.execute',
       new=AsyncMock(return_value='{"items": [{"name": "master"}], "totalItems": 1}'))
@patch('guardsarm.agent.Agent.get_agents_overview', return_value={'items': [{'id': '001', 'node_name': 'master'},
...
def test_DistributedAPI_get_solver_node(mock_agents_overview):
    ...
    nodes_info_result = AffectedItemsGuardSarmResult()
    nodes_info_result.affected_items.append({'name': 'master'})
    common.cluster_nodes.set(['master'])

    with patch('guardsarm.core.cluster.dapi.dapi.get_nodes_info', new=AsyncMock(return_value=nodes_info_result)):
        with patch('guardsarm.core.cluster.dapi.dapi.node_info', {'type': 'master', 'node': 'unknown'}):
            dapi_kwargs = {'f': manager.status, 'logger': logger, 'request_type': 'distributed_master',
            ...
            expected = AffectedItemsGuardSarmResult()
            expected.affected_items = [{'id': '001', 'node_name': 'master'}]
            with patch('guardsarm.agent.get_agents_in_group', return_value=expected):
                dapi_kwargs = {'f': manager.status, 'logger': logger, 'request_type': 'distributed_master',
...
@pytest.mark.parametrize('api_request', [
    ...
    guardsarm.core.manager.status
])
@patch('guardsarm.core.manager.get_manager_status', return_value={process: 'running' for process in get_manager_status()})
def test_DistributedAPI_check_guardsarm_status(status_mock, api_request):
    """Test `check_guardsarm_status` method from class DistributedAPI."""
    dapi = DistributedAPI(f=api_request, logger=logger)
    data = dapi.check_guardsarm_status()
    assert data is None
...
@patch('guardsarm.core.cluster.dapi.dapi.node_info', {'node': 'random_node'})
def test_DistributedAPI_check_guardsarm_status_exception(status_value):
    """Test exceptions from `check_guardsarm_status` method from class DistributedAPI."""
    statuses = {process: status_value for process in sorted(get_manager_status())}
    with patch('guardsarm.core.manager.get_manager_status',
               return_value=statuses):
        dapi = DistributedAPI(f=agent.get_agents_summary_status, logger=logger)
        try:
            dapi.check_guardsarm_status()
        except GuardSarmInternalError as e:
            assert e.code == 1017
            assert statuses
...
class TestDistributedAPIRunLocal:
    ...
    @patch("guardsarm.core.common.reset_context_cache")
    @patch("guardsarm.core.common.cluster_nodes")
    @patch("guardsarm.core.common.origin_module")
    @patch("guardsarm.core.common.current_user")
    @patch("guardsarm.core.common.broadcast")
    @patch("guardsarm.core.common.rbac")
    ...
    @patch("guardsarm.core.common.broadcast")
    @patch("guardsarm.core.common.rbac")
    def test_run_local_with_guardsarm_exception(
        self,
        mock_rbac_ctx,
        mock_broadcast_ctx,
        mock_current_user_ctx,
        ...
        """Test that GuardSarmException is re-raised properly."""
        mock_broadcast_token = "test_token"
        mock_broadcast_ctx.set.return_value = mock_broadcast_token

        guardsarm_exception = GuardSarmException("Test error")
        mock_function.side_effect = guardsarm_exception

        with pytest.raises(GuardSarmException) as exc_info:
            DistributedAPI.run_local(
                f=mock_function, f_kwargs=sample_kwargs, **sample_context_vars
            )

        # Verify it's the same exception instance
        assert exc_info.value is guardsarm_exception
    ...
    def test_run_local_with_other_exception(
        ...
        """Test that non-GuardSarmException exceptions are not caught."""
        mock_broadcast_token = "test_token"
        mock_broadcast_ctx.set.return_value = mock_broadcast_token
...
@patch("guardsarm.core.cluster.common.import_module", return_value="os.path")
@patch("asyncio.get_running_loop")
async def test_APIRequestQueue_run(loop_mock, import_module_mock):
    ...
    class RequestQueueMock:
        async def get(self):
            return 'guardsarm*request_queue*test ' \
    ...
    with patch.object(logger, "error", side_effect=Exception("break while true")) as logger_mock:
        ...
        logger_mock.assert_called_once_with("Error in DAPI request. The destination node is "
                                            "not connected or does not exist: 'guardsarm'.")

        node = NodeMock()
        with patch.object(node, "send_request", side_effect=GuardSarmClusterError(3020, extra_message="test")):
            with patch.object(node, "send_string", return_value=b"noerror"):
                with patch("guardsarm.core.cluster.dapi.dapi.DistributedAPI", return_value=DistributedAPI_mock()):
                    server.clients = {"guardsarm": node}
                    with pytest.raises(Exception):
                        await apirequest.run()

            with patch.object(node, "send_string", Exception("break while true")):
                with patch("guardsarm.core.cluster.dapi.dapi.DistributedAPI", return_value=DistributedAPI_mock()):
                    with patch("guardsarm.core.cluster.dapi.dapi.contextlib.suppress", side_effect=Exception()):
                        apirequest.logger = logging.getLogger("apirequest")
                        with pytest.raises(Exception):
                            await apirequest.run()


@patch("guardsarm.core.cluster.dapi.dapi.contextlib.suppress", side_effect=Exception())
@patch("asyncio.get_running_loop")
async def test_SendSyncRequestQueue_run(loop_mock, contexlib_mock):
    ...
    class RequestQueueMock:
        async def get(self):
            return "guardsarm*request_queue*test {\"daemon_name\": \"test\"}"
    ...
    with patch.object(logger, "error", side_effect=Exception("break while true")) as logger_mock:
        ...
        with patch.object(node, "send_request", Exception("break while true")):
            with patch("guardsarm.core.cluster.dapi.dapi.guardsarm_sendsync", side_effect=Exception("break while true")):
                server.clients = {"guardsarm": node}
                sendsync.logger = logging.getLogger("sendsync")
                with pytest.raises(Exception):
                    await sendsync.run()

            with patch("guardsarm.core.cluster.dapi.dapi.guardsarm_sendsync", side_effect="noerror"):
                with pytest.raises(Exception):
                    await sendsync.run()
...
@pytest.mark.parametrize('tmp_file, expected_error', [
    ('../../etc/ossec.conf', GuardSarmClusterError),
    ('../../../etc/passwd', GuardSarmClusterError),
    ('/etc/passwd', GuardSarmClusterError),
    ('../api/configuration/security/private_key.pem', GuardSarmClusterError),
    ('/var/ossec/api/configuration/security/private_key.pem', GuardSarmClusterError),
    ('valid_file.txt', None),
...
async def test_send_tmp_file_path_validation(tmp_file, expected_error):
    ...
    with patch('guardsarm.core.cluster.dapi.dapi.DistributedAPI.__init__', return_value=None):
        dapi = DistributedAPI()
...
async def test_send_tmp_file_empty_parameter():
    ...
    with patch('guardsarm.core.cluster.dapi.dapi.DistributedAPI.__init__', return_value=None):
        ...
        with pytest.raises(GuardSarmClusterError) as exc_info:
            await dapi.send_tmp_file()
