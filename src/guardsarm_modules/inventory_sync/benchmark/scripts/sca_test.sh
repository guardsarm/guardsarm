...
AGENT_CONTAINER="${AGENT_CONTAINER:-guardsarm-agent-5x-ubuntu}"
AGENT_NAME="${AGENT_NAME:-guardsarm-agent-5x-ubuntu}"
MANAGER_SERVICE="${MANAGER_SERVICE:-guardsarm-manager}"
API_URL="${GUARDSARM_API_URL:-https://localhost:55000}"
API_USER="${GUARDSARM_API_USER:-guardsarm}"
API_PASS="${GUARDSARM_API_PASS:-guardsarm}"
...
elif [[ -x "/workspaces/GuardSarm_5x/venv/bin/python" ]]; then
  PYTHON_BIN="/workspaces/GuardSarm_5x/venv/bin/python"
else
...
Runs:
  1. Stop guardsarm-manager so the agent cannot enroll during preparation
  2. Reset the agent as a first-run agent
  3. Start guardsarm-manager
...
Environment:
  GUARDSARM_API_USER / GUARDSARM_API_PASS / GUARDSARM_API_URL
...
wait_for_api() {
  log "Waiting for GuardSarm API at ${API_URL}..."
  local attempt token
...
main() {
  ...
  local token
  token="$(wait_for_api)" || die "GuardSarm API did not become ready"
