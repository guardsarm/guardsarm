...
The **InventorySync VD Test Tool** (`inventory_sync_testtool`) is a comprehensive testing utility designed to validate the complete end-to-end integration between the **InventorySync** module and the **Vulnerability Detection (VD)** scanner in GuardSarm.
...
It simulates agent inventory data ingestion with real **FlatBuffer** messages and triggers vulnerability scanning workflows without requiring a full GuardSarm deployment.
...
The test tool **requires** a running GuardSarm Indexer instance to validate end-to-end functionality.
...
"data_values": [
  {
    "operation": "upsert",
    "payload": {
      "_index": "guardsarm-states-inventory-packages",
      "_id": "guardsarm_001_f033cfe690b80a478fb4c832934f0fd55927c349",
      "_score": 2,
      "_source": {
        "guardsarm": {
          "agent": {
  ...
  },
  {
    ...
    "payload": {
      "_index": "guardsarm-states-inventory-packages",
      "_id": "guardsarm_001_f033cfe690b80a478fb4c832934f0fd55927c350",
      "_score": 2,
      "_source": {
        "guardsarm": {
          "agent": {
...
"data_context": [
  {
    "payload": {
      "_index": "guardsarm-states-inventory-system",
      "_id": "guardsarm_001_f033cfe690b80a478fb4c832934f0fd55927c351",
      "_score": 2,
      "_source": {
        "guardsarm": {
          "agent": {
...
Start {
  ...
  index: ["guardsarm-states-inventory-system", "guardsarm-states-inventory-packages"],
  agentname: "ubuntu22",
  agentversion: "v5.0.0",
  architecture: "aarch64",
  hostname: "ubuntu22",
...
DataValue {
  ...
  index: "guardsarm-states-inventory-packages" / "guardsarm-states-inventory-hotfixes",
...
DataContext {
  session: <StartAck.session>,
  seq: <...>,
  index: "guardsarm-states-inventory-system" / ...,
  data: <JSON payload stored as raw bytes>
}
```
  ...
  "configurations": [
    {
      ...
      "args": [
        "--input",
        "${workspaceFolder}/src/guardsarm_modules/inventory_sync/testtool/test_data/INPUT_000.json",
        "--config",
        "${workspaceFolder}/src/guardsarm_modules/inventory_sync/testtool/test_data/config.json",
        "--wait",
        "15",
        "--verbose"
      ],
...
./inventory_sync_testtool \
  --input src/guardsarm_modules/inventory_sync/testtool/test_data/INPUT_000.json \
  --config src/guardsarm_modules/inventory_sync/testtool/test_data/config.json \
  --wait 15 \
  --verbose
```
...
1. **Stateful Testing**
   ...
   ```bash
   rm -rf /tmp/guardsarm_inventorysync_test.db
   ```
