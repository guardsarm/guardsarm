...
from guardsarm.core import common, utils
from guardsarm.core import guardsarm_socket
from guardsarm.core.exception import GuardSarmError, GuardSarmInternalError, GuardSarmException

def get_daemons_stats_socket(socket: str, agents_list: Union[list[int], str] = None, last_id: int = None) -> dict:
    """Send message to GuardSarm socket to get statistical information.
    ...
    """
    # Create message
    full_message = guardsarm_socket.create_guardsarm_socket_message(
    ...
    # Connect to socket
    try:
        s = guardsarm_socket.GuardSarmSocketJSON(socket)
    except Exception:
        raise GuardSarmInternalError(1121, extra_message=socket)
...
def send_command_to_socket(dest_socket: str, command: str) -> dict:
    ...
    except GuardSarmException:
        # Error connecting to socket
        raise GuardSarmInternalError(1121)
...
def get_daemons_stats_from_socket(agent_id: str, daemon: str) -> dict:
    ...
    if socket_response.get('error', 0) != 0:
        ...
        raise GuardSarmError(1117, extra_message=rec_msg)
