#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# indexer_control.sh — Wrapper around guardsarm-indexer lifecycle for benchmarks.
#
# Subcommands:
#   start             service guardsarm-indexer start
#   stop              service guardsarm-indexer stop
#   restart           stop + start
#   status            service guardsarm-indexer status
#   init-security     /usr/share/guardsarm-indexer/bin/indexer-security-init.sh
...
INIT_SCRIPT="/usr/share/guardsarm-indexer/bin/indexer-security-init.sh"
...
Commands:
  start                Start guardsarm-indexer
  stop                 Stop guardsarm-indexer (used for indexer_down scenario)
...
cmd_start() {
    echo "[indexer] starting guardsarm-indexer..."
    service guardsarm-indexer start
}

cmd_stop() {
    echo "[indexer] stopping guardsarm-indexer..."
    service guardsarm-indexer stop
}

cmd_status() {
    service guardsarm-indexer status || true
}
