...
from guardsarm.core.task import GuardSarmDBQueryTask
...
def get_task_status(filters: dict = None, select: list = None, search: dict = None, offset: int = 0,
                    limit: int = DATABASE_LIMIT, sort: dict = None, q: str = None, ) -> AffectedItemsGuardSarmResult:
    ...
    AffectedItemsGuardSarmResult
        Tasks's status.
    """
    result = AffectedItemsGuardSarmResult(all_msg='All specified task\'s status were returned',
                                      some_msg='Some status were not returned',
                                      none_msg='No status was returned')

    with GuardSarmDBQueryTask(filters=filters, offset=offset, limit=limit, query=q, sort=sort, search=search,
