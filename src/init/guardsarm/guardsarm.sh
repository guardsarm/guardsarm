    ...
    if command -v semodule > /dev/null && command -v getenforce > /dev/null; then
        if [ -f selinux/guardsarm.pp ]; then
            if [ $(getenforce) != "Disabled" ]; then
                cp selinux/guardsarm.pp /tmp && semodule -i /tmp/guardsarm.pp
                rm -f /tmp/guardsarm.pp
...
GuardSarmUpgrade()
{
    ...
    # Remove existing SQLite databases for GuardSarm DB, only if upgrading from 3.2..3.6
    ...
    if [ -d $PREINSTALLEDDIR/logs/ossec ]; then
        if [ "$(ls -A $PREINSTALLEDDIR/logs/ossec)" ]; then
            mv -f $PREINSTALLEDDIR/logs/ossec/* $PREINSTALLEDDIR/logs/guardsarm
        fi
        rm -rf $PREINSTALLEDDIR/logs/ossec
    fi

    # Remove deprecated GuardSarm tools
    ...
    rm -f $PREINSTALLEDDIR/active-response/bin/guardsarm-slack
